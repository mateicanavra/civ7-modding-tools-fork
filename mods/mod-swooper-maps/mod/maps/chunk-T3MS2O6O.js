...
// ../../packages/mapgen-core/dist/chunk-AQM6GQTT.js
...
function runPlacement(adapter, iWidth, iHeight, options = {}) {
  ...
  try {
    if (starts?.westContinent && starts?.eastContinent) {
      ...
      const PLOT_TAG_NONE = 0;
      const PLOT_TAG_LANDMASS = 1;
      const PLOT_TAG_WEST_LANDMASS = 2;
      const PLOT_TAG_EAST_LANDMASS = 3;
      const PLOT_TAG_WATER = 4;
      const PLOT_TAG_WEST_WATER = 5;
      const PLOT_TAG_EAST_WATER = 6;
      ...
      for (let y = 0; y < iHeight; y++) {
        for (let x = 0; x < iWidth; x++) {
          adapter.setPlotTag(x, y, PLOT_TAG_NONE);
          const terrain = adapter.getTerrainType(x, y);
          if (terrain !== OCEAN_TERRAIN4 && terrain !== COAST_TERRAIN4) {
            adapter.addPlotTag(x, y, PLOT_TAG_LANDMASS);
            if (x >= iEastContinentLeftCol) {
              adapter.addPlotTag(x, y, PLOT_TAG_EAST_LANDMASS);
            } else {
              adapter.addPlotTag(x, y, PLOT_TAG_WEST_LANDMASS);
            }
            landTagged++;
          } else {
            adapter.addPlotTag(x, y, PLOT_TAG_WATER);
            if (x >= iEastContinentLeftCol - 1) {
              adapter.addPlotTag(x, y, PLOT_TAG_EAST_WATER);
            } else {
              adapter.addPlotTag(x, y, PLOT_TAG_WEST_WATER);
            }
            waterTagged++;
          }
        }
  ...
  try {
    if (starts?.westContinent && starts?.eastContinent) {
      ...
      const eastContinent = starts.eastContinent;
      const LANDMASS_REGION_WEST = 1;
      const LANDMASS_REGION_EAST = 2;
      const OCEAN_TERRAIN4 = 0;
      let westMarked = 0;
