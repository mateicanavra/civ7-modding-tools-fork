...
// ../../packages/mapgen-core/dist/chunk-PELSB7DI.js
...
function tryCrustFirstLandmask(width, height, plateIds, closeness, closenessLimit, targetLandTiles, landmassCfg, ctx) {
  ...
  const crustTypes = assignCrustTypes(graph, ctx ? () => ctxRandom(ctx, "CrustType", 1e6) / 1e6 : Math.random, {
    continentalFraction,
  ...
  const continentalPlates = crustTypes.reduce((acc, t) => acc + (t === 1 ? 1 : 0), 0);
  return {
    landMask,
    landTiles,
    seaLevel,
...
function createPlateDrivenLandmasses(width, height, ctx, options = {}) {
  ...
  const interiorNoiseWeight = clamp013(tectonicsCfg.interiorNoiseWeight, 0.3);
  const arcWeight = clampRange(tectonicsCfg.boundaryArcWeight, 0.8, 0, 2);
  const arcNoiseWeight = clamp013(tectonicsCfg.boundaryArcNoiseWeight, 0.5);
  const fractalGrain = clampInt2(
  ...
  let generationMode = "legacy";
  let seaLevel = null;
  let crustPlateCount = 0;
  let crustContinentalPlates = 0;
  let crustConfigApplied = null;
  let baseHeightRange = null;
  ...
  if (crustResult) {
    landMask.set(crustResult.landMask);
    finalLandTiles = crustResult.landTiles;
    generationMode = "crust-first";
    seaLevel = crustResult.seaLevel;
    crustPlateCount = crustResult.plateCount;
    crustContinentalPlates = crustResult.continentalPlates;
    crustConfigApplied = crustResult.crustConfigApplied;
    baseHeightRange = crustResult.baseHeightRange;
  }
  let bestThreshold = null;
  let interiorMin = 255, interiorMax = 0, arcMin = 255, arcMax = 0, interiorStretch = 1;
  if (generationMode === "legacy") {
    ...
    let { land: landCount, boundaryLand } = computeShares(candidateThreshold);
    ...
    finalLandTiles = landCount;
    for (let i = 0; i < size; i++) {
      const score = computeLandScore(i);
      const isLand = score >= candidateThreshold && closeness[i] <= closenessLimit;
      if (isLand) {
        landMask[i] = 1;
      }
    }
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (generationMode === "legacy" && isLand) {
        finalLandTiles++;
      }
  ...
  if (generationMode === "crust-first") {
    ...
    console.log(
      `${logPrefix} Crust config: plates=${crustContinentalPlates}/${crustPlateCount} continental, edgeBlend=${applied ? applied.edgeBlend.toFixed(2) : "n/a"}, noise=${applied ? applied.noiseAmplitude.toFixed(2) : "n/a"}, heights=[${applied ? applied.oceanicHeight.toFixed(2) : "n/a"},${applied ? applied.continentalHeight.toFixed(2) : "n/a"}]`
    );
    if (baseHeightRange) {
      console.log(
        `${logPrefix} Height range: [${baseHeightRange.min.toFixed(3)},${baseHeightRange.max.toFixed(3)}], targetLandTiles=${targetLandTiles}, waterPct=${waterPct.toFixed(1)}%`
      );
    } else {
      console.log(
        `${logPrefix} Target land: ${targetLandTiles}, waterPct=${waterPct.toFixed(1)}%`
      );
    }
  } else {
    console.log(
      `${logPrefix} Generation stats: threshold=${bestThreshold}, landTiles=${finalLandTiles}/${size} (${(finalLandTiles / size * 100).toFixed(1)}%)`
    );
    console.log(
      `${logPrefix} Score ranges: interior=[${interiorMin},${interiorMax}], arc=[${arcMin},${arcMax}], stretch=${interiorStretch.toFixed(2)}`
    );
    console.log(
      `${logPrefix} Config: closenessLimit=${closenessLimit}, targetLandTiles=${targetLandTiles}, waterPct=${waterPct.toFixed(1)}%`
    );
  }
  ...
  return {
    ...
    threshold: bestThreshold ?? void 0
  };
}
var _cache2 = null;
...
// ../../packages/mapgen-core/dist/chunk-B2HRK2PJ.js
