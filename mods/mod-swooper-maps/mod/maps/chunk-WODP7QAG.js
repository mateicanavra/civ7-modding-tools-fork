...
// ../../packages/mapgen-core/dist/chunk-N7YZ73GH.js
...
function computePlateBasedScores(ctx, scores, hillScores, options, isWaterCheck, adapter, foundation) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const rawMtn = adapter.getFractalHeight(MOUNTAIN_FRACTAL, x, y) | 0;
      const rawHill = adapter.getFractalHeight(HILL_FRACTAL3, x, y) | 0;
      const fractalMtn = (rawMtn > 255 ? rawMtn >>> 8 : rawMtn) / 255;
      const fractalHill = (rawHill > 255 ? rawHill >>> 8 : rawHill) / 255;
...
function computeFractalOnlyScores(ctx, scores, hillScores, adapter) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const rawMtn = adapter.getFractalHeight(MOUNTAIN_FRACTAL, x, y) | 0;
      const rawHill = adapter.getFractalHeight(HILL_FRACTAL3, x, y) | 0;
      scores[i] = (rawMtn > 255 ? rawMtn >>> 8 : rawMtn) / 255;
      hillScores[i] = (rawHill > 255 ? rawHill >>> 8 : rawHill) / 255;
    }
  }
}
...
function runPlacement(adapter, iWidth, iHeight, options = {}) {
  ...
  try {
    ...
    console.log("[Placement] AreaBuilder.recalculateAreas failed (pre-starts):", err);
  }
  try {
    adapter.recalculateFertility();
    console.log("[Placement] Fertility recalculated successfully");
  } catch (err) {
    console.log("[Placement] FertilityBuilder.recalculate failed (pre-starts):", err);
  }
  try {
