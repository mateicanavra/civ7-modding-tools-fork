...
// ../../packages/mapgen-core/dist/chunk-NTNGTQZD.js
...
function runPlacement(adapter, iWidth, iHeight, options = {}) {
  ...
  try {
    if (starts?.westContinent && starts?.eastContinent) {
      const eastContinent = starts.eastContinent;
      const iEastContinentLeftCol = eastContinent.west;
      const OCEAN_TERRAIN4 = 0;
      const COAST_TERRAIN4 = 1;
      let landTagged = 0;
      let waterTagged = 0;
      for (let y = 0; y < iHeight; y++) {
        for (let x = 0; x < iWidth; x++) {
          adapter.setPlotTag(x, y, PLOT_TAG.NONE);
          const terrain = adapter.getTerrainType(x, y);
          if (terrain !== OCEAN_TERRAIN4 && terrain !== COAST_TERRAIN4) {
            adapter.addPlotTag(x, y, PLOT_TAG.LANDMASS);
            if (x >= iEastContinentLeftCol) {
              adapter.addPlotTag(x, y, PLOT_TAG.EAST_LANDMASS);
            } else {
              adapter.addPlotTag(x, y, PLOT_TAG.WEST_LANDMASS);
            }
            landTagged++;
          } else {
            adapter.addPlotTag(x, y, PLOT_TAG.WATER);
            if (x >= iEastContinentLeftCol - 1) {
              adapter.addPlotTag(x, y, PLOT_TAG.EAST_WATER);
            } else {
              adapter.addPlotTag(x, y, PLOT_TAG.WEST_WATER);
            }
            waterTagged++;
          }
        }
      }
      console.log(`[Placement] Plot tags set: ${landTagged} land tiles, ${waterTagged} water tiles`);
    }
  } catch (err) {
    console.log("[Placement] addPlotTag failed:", err);
  }
  try {
    if (starts?.westContinent && starts?.eastContinent) {
      const westContinent = starts.westContinent;
      const eastContinent = starts.eastContinent;
      const hasLandmassRegion = typeof LandmassRegion !== "undefined";
      console.log(`[Placement] LandmassRegion available: ${hasLandmassRegion}`);
      if (hasLandmassRegion) {
        console.log(`[Placement] LandmassRegion keys: ${Object.keys(LandmassRegion).join(", ")}`);
        console.log(`[Placement] LANDMASS_REGION_WEST=${LandmassRegion.LANDMASS_REGION_WEST}, LANDMASS_REGION_EAST=${LandmassRegion.LANDMASS_REGION_EAST}`);
      }
      const LANDMASS_REGION_WEST = hasLandmassRegion ? LandmassRegion.LANDMASS_REGION_WEST : 1;
      const LANDMASS_REGION_EAST = hasLandmassRegion ? LandmassRegion.LANDMASS_REGION_EAST : 2;
      const OCEAN_TERRAIN4 = 0;
      console.log(`[Placement] Using region IDs: WEST=${LANDMASS_REGION_WEST}, EAST=${LANDMASS_REGION_EAST}`);
      let westMarked = 0;
      for (let y = westContinent.south; y <= westContinent.north; y++) {
        for (let x = westContinent.west; x <= westContinent.east; x++) {
          if (adapter.getTerrainType(x, y) !== OCEAN_TERRAIN4) {
            adapter.setLandmassRegionId(x, y, LANDMASS_REGION_WEST);
            westMarked++;
          }
        }
      }
      let eastMarked = 0;
      for (let y = eastContinent.south; y <= eastContinent.north; y++) {
        for (let x = eastContinent.west; x <= eastContinent.east; x++) {
          if (adapter.getTerrainType(x, y) !== OCEAN_TERRAIN4) {
            adapter.setLandmassRegionId(x, y, LANDMASS_REGION_EAST);
            eastMarked++;
          }
        }
      }
      console.log(`[Placement] Landmass regions marked: ${westMarked} west tiles, ${eastMarked} east tiles`);
    }
  } catch (err) {
    console.log("[Placement] setLandmassRegionId failed:", err);
  }
  try {
    if (!starts) {
...
// ../../packages/mapgen-core/dist/chunk-6DVG2JVC.js
