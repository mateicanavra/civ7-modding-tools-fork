...
import {
  MinHeap,
  computeCellResistance,
  filterByMinComponentSize,
  ...
  strategies: {
    default: {
      ...
      run: (input, config) => {
        ...
        const microplateMinAreaCells = Math.max(1, polarPolicy.microplateMinAreaCells);
        const tangentialSpeed = Math.max(0, polarPolicy.tangentialSpeed);
        const tangentialJitterDeg = Math.max(0, polarPolicy.tangentialJitterDeg);
