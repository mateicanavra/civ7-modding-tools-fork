...
export function rebind() {
    ...
    // 4) Groups
    LANDMASS_CFG = safeObj(__LANDMASS__()) as any;
    LANDMASS_GEOMETRY = safeObj((LANDMASS_CFG as any).geometry) as any;
    COASTLINES_CFG = safeObj(__COASTLINES__()) as any;
    MARGINS_CFG = safeObj(__MARGINS__()) as any;
    ISLANDS_CFG = safeObj(__ISLANDS__()) as any;
    CLIMATE_BASELINE_CFG = safeObj(__CLIMATE_BASELINE__()) as any;
    CLIMATE_REFINE_CFG = safeObj(__CLIMATE_REFINE__()) as any;
    BIOMES_CFG = safeObj(__BIOMES__()) as any;
    FEATURES_DENSITY_CFG = safeObj(__FEATURES_DENSITY__()) as any;
    CORRIDORS_CFG = safeObj(__CORRIDORS__()) as any;
    PLACEMENT_CFG = safeObj(__PLACEMENT__()) as any;
    DEV_LOG_CFG = safeObj(__DEV__()) as any;
    WORLDMODEL_CFG = safeObj(__WM__()) as any;

    // 5) Corridor sub-groups
    CORRIDOR_POLICY = safeObj((CORRIDORS_CFG as any).policy) as any;
    CORRIDOR_KINDS = safeObj((CORRIDORS_CFG as any).kinds) as any;
