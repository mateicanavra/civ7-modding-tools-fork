...
- Subtree-based CLI commands expose only relevant flags; `remoteUrl` is required only for `config`, `import`, and `setup`. Repo URL and default branch are stored during setup, and remote names are derived from existing git remotes.
