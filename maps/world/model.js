...
function computePlates(width, height) {
  ...
  for (let ry = 0; ry < rows && placed < count; ry++) {
    for (let cx = 0; cx < cols && placed < count; cx++) {
      ...
      const jx =
        (TerrainBuilder?.getRandomNumber?.(seedJitter * 2 + 1, "PlateJX") ??
          0) - seedJitter;
      const jy =
        (TerrainBuilder?.getRandomNumber?.(seedJitter * 2 + 1, "PlateJY") ??
          0) - seedJitter;

      const sx = clampInt(baseX + jx, 0, width - 1);
      const sy = clampInt(baseY + jy, 0, height - 1);

      // Movement vector from angle (small magnitude)
      const angleDeg =
        angles[
          TerrainBuilder?.getRandomNumber?.(angles.length || 1, "PlateAng") ?? 0
        ] ?? 0;
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (closeness[i] > 32 && sndIdx >= 0) {
        ...
        if (roll < Math.max(0, convBias - 5)) bt = ENUM_BOUNDARY.convergent;
        else if (roll < Math.min(100, convBias + 35)) bt = ENUM_BOUNDARY.divergent;
...
function computePressure(width, height) {
  ...
  for (let i = 0; i < bumps; i++) {
    const cx =
      TerrainBuilder?.getRandomNumber?.(width, "PressCX") ?? (i * width) / bumps;
    const cy =
      TerrainBuilder?.getRandomNumber?.(height, "PressCY") ?? (i * height) / bumps;
    const a = amp * (0.75 + (TerrainBuilder?.getRandomNumber?.(50, "PressA") ?? 0) / 100);
...
function computeWinds(width, height) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    // Per-row variance
    const varU =
      Math.round(((TerrainBuilder?.getRandomNumber?.(21, "WindUVar") ?? 0) - 10) * variance) |
      0;
    const varV =
      Math.round(((TerrainBuilder?.getRandomNumber?.(11, "WindVVar") ?? 0) - 5) * variance) |
      0;
