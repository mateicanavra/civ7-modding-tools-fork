...
export function addIslandChains(iWidth, iHeight) {
  ...
  const paradiseWeight =
    (STORY_TUNABLES?.hotspot?.paradiseBias ?? 2) | 0; // default 2
  const volcanicWeight =
    (STORY_TUNABLES?.hotspot?.volcanicBias ?? 1) | 0; // default 1
  const peakPercent = Math.max(
    0,
    Math.min(
      100,
      Math.round((STORY_TUNABLES?.hotspot?.volcanicPeakChance ?? 0.33) * 100)
    )
  );
  ...
  for (let y = 2; y < iHeight - 2; y++) {
    for (let x = 2; x < iWidth - 2; x++) {
      ...
      const v = FractalBuilder.getHeight(globals.g_MountainFractal, x, y);
      const isHotspot = StoryTags.hotspot.has(storyKey(x, y));

      // Base sparse placement vs. hotspot-biased placement
      const baseAllowed =
        v > threshold && TerrainBuilder.getRandomNumber(8, "Island Seed") === 0;
      ...
      if (isHotspot) {
        ...
        if (!classifyParadise) {
          ...
          if (TerrainBuilder.getRandomNumber(100, "HotspotPeak") < peakPercent) {
