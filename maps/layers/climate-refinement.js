...
export function refineRainfallEarthlike(iWidth, iHeight) {
  ...
  // Pass E: Hotspot island microclimates (paradise/volcanic centers)
  {
        ...
        for (let x = 0; x < iWidth; x++) {
          ...
          for (let dy = -radius; dy <= radius && (!nearParadise || !nearVolcanic); dy++) {
            for (let dx = -radius; dx <= radius; dx++) {
              ...
              const key = `${nx},${ny}`;
              if (!nearParadise && hasParadise && StoryTags.hotspotParadise.has(key)) nearParadise = true;
              if (!nearVolcanic && hasVolcanic && StoryTags.hotspotVolcanic.has(key)) nearVolcanic = true;
