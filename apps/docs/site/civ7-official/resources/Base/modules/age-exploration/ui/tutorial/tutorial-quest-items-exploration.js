                ...
                if (playerCities) {
                    for (let i = 0; i < playerCities.length; ++i) {
                        ...
                        if (city && city.Workers) {
                            let cityplots = city.getPurchasedPlots();
...
TutorialManager.add({
    ...
    quest: {
        ...
        getDescriptionLocParams: () => {
            ...
            let itreasureResourcesGoal = ECONOMIC_QUEST_4_RESOURCE_GOAL;
            let treasureFleetCreated = "[icon:QUEST_ITEM_OPEN]";
            const player = Players.get(GameContext.localPlayerID);
            if (player && player.Units) {
                let playerCities = player.Cities?.getCities();
                if (playerCities) {
    ...
    onCompleteCheck: (_item) => {
        ...
        if (player && player.Units) {
            ...
            for (let i = 0; i < playerUnits.length; ++i) {
                let thisUnit = playerUnits[i];
                const unitDefinition = GameInfo.Units.lookup(thisUnit.type); //this part seems redundant but sometimes the type isn't easy to find
...
//# sourceMappingURL=file:///age-exploration/ui/tutorial/tutorial-quest-items-exploration.js.map
