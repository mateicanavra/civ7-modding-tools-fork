...
import TutorialItem, { TutorialAnchorPosition, TutorialCalloutOptionDefinition } from '/base-standard/ui/tutorial/tutorial-item.js';
...
TutorialManager.add({
	...
	runAllTurns: true,
	onActivateCheck: (_node: TutorialItem) => {
		return Configuration.getGame().previousAgeCount > 0;
	},
});
...
TutorialManager.add({
	...
	onActivateCheck: (_item: TutorialItem) => {
		let missionaryUnlocked: boolean = false;
		if (InterfaceMode.getCurrent() == "INTERFACEMODE_CITY_PRODUCTION") {
			const player: PlayerLibrary | null = Players.get(GameContext.localPlayerID);
			if (player && player.Units) {
				const cityID: ComponentID | null = UI.Player.getHeadSelectedCity();
