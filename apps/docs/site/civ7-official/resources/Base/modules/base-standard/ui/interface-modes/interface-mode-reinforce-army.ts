...
interface ReinforceArmyInterfaceModeContext extends InterfaceMode.Context {
	UnitID: ComponentID;
	OperationArguments: any;
}
/**
 * Handler for INTERFACEMODE_REINFORCE_ARMY.
 */
class ReinforceArmyInterfaceMode extends ChoosePlotInterfaceMode {

	private _OperationResult: OperationResult | null = null;
	private validPlots = new Set<PlotIndex>();

	initialize() {
		const context = this.Context as ReinforceArmyInterfaceModeContext;
		const args: any = {};
		const result = Game.UnitOperations.canStart(context.UnitID, 'UNITOPERATION_REINFORCE_ARMY', args, false);
	...
	commitPlot(plot: PlotCoord) {
		...
		Game.UnitOperations.sendRequest(unitID, 'UNITOPERATION_REINFORCE_ARMY', args);
	}
}

InterfaceMode.addHandler('INTERFACEMODE_REINFORCE_ARMY', new ReinforceArmyInterfaceMode());