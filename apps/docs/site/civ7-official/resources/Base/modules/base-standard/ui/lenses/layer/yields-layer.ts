...
enum ConstructibleYieldState {
	NOT_PRODUCING,
	PRODUCING
}

interface PlotState {
	revealedState: RevealedStates;
	constructibleState: ConstructibleYieldState;
	yields: Map<YieldType, number>;
}
...
class YieldsLensLayer extends BaseSpriteGridLensLayer {
	...
	private yieldSpritePadding: number = 12;
	private yieldIcons: Map<YieldType, string[]> = new Map<YieldType, string[]>();
	private plotStates: PlotState[] = []
	private revealedStates: RevealedStates[] = [];
	private plotsNeedingUpdate: number[] = [];
	private constructibleStates: ConstructibleYieldState[] = [];
	...
	initLayer() {
		...
		for (let i = 0; i < revealedStatesLength; ++i) {
			...
			this.plotStates.push({
				...
				yields: new Map<YieldType, number>(),
	...
	private calculateConstructibleState(plotIndex: number) {
		...
		const constructibles = MapConstructibles
			...
			.filter(c => c && c.complete && c.owner != PlayerIds.WORLD_PLAYER) as Constructible[];
	...
	private onPlotYieldChanged(data: Plot_EventData) {
		const plotIndex = data.location.x + (GameplayMap.getGridWidth() * data.location.y);
		if (this.plotsNeedingUpdate.indexOf(plotIndex) != null) {
			this.plotsNeedingUpdate.push(plotIndex);
		}
	...
	private onConstructibleChange(data: ConstructibleAddedToMap_EventData & ConstructibleRemovedFromMap_EventData & DistrictDamageChanged_EventData) {
		const plotIndex = data.location.x + (GameplayMap.getGridWidth() * data.location.y);
	...
	private updatePlotState(plotIndex: number, revealedState?: RevealedStates, constructibleState?: ConstructibleYieldState) {
		...
		if (revealedState == RevealedStates.HIDDEN) {
			...
			if (needsRefresh) {
				...
				const groupWidth: number = (yields.length - 1) * this.yieldSpritePadding;
