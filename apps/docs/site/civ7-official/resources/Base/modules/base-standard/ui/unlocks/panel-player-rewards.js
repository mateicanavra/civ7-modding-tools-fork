...
class PanelPlayerRewards extends Panel {
    ...
    populatePlayerCurrency(root) {
        ...
        for (let category = 0; category < orderList.length; category++) {
            const instance = rewardPoints.find((target) => { return target.category == orderList[category]; });
            const instanceValue = instance?.value || 0;
            const cardCategory = Object.keys(CardCategories).find(key => CardCategories[key] === orderList[category]);
    ...
    populateRewards(root) {
        ...
        rewards.forEach(reward => {
            ...
            unlocksItem.classList.add('screen-unlocks__item', 'flow-row', 'flex-auto', 'h-32', 'w-full', 'my-1', 'bg-primary', 'justify-between', 'bg-center', 'relative', 'group');
...
//# sourceMappingURL=file:///base-standard/ui/unlocks/panel-player-rewards.js.map
