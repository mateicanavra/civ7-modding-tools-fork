...
interface TeleportToCityInterfaceModeContext extends InterfaceMode.Context {
	UnitID: ComponentID;
...
/**
 * Handler for INTERFACEMODE_TELEPORT_TO_CITY.
 */
class TeleportToCityInterfaceMode extends ChoosePlotInterfaceMode {

	private _OperationResult: OperationResult | null = null;
	private validPlots = new Set<PlotIndex>();

	initialize() {
		const context = this.Context as TeleportToCityInterfaceModeContext;
		const args: any = {};
		const result = Game.UnitOperations.canStart(context.UnitID, 'UNITOPERATION_TELEPORT_TO_CITY', args, false);
	...
	commitPlot(plot: PlotCoord) {
		...
		args.X = plot.x;
		args.Y = plot.y;
		Game.UnitOperations.sendRequest(unitID, 'UNITOPERATION_TELEPORT_TO_CITY', args);
	}
}

InterfaceMode.addHandler('INTERFACEMODE_TELEPORT_TO_CITY', new TeleportToCityInterfaceMode());