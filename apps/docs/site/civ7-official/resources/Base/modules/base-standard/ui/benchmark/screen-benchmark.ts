...
// Chart js is a library with no typings, so stub out the service as an any type.
declare const Chart: any;

let openBenchmarkScreen: ComponentRoot<ScreenBenchmark> | undefined;

class ScreenBenchmark extends Panel {
	private readonly benchUpdateListener = this.onBenchmarkUpdate.bind(this);
	private readonly benchEndedListener = this.onBenchmarkEnded.bind(this);
	private readonly navigateInputListener = this.onNavigateInput.bind(this);
	private readonly engineInputListener = this.onEngineInput.bind(this);

	private readonly isGraphicsBenchmark = Benchmark.Game.getBenchmarkType() == GameBenchmarkType.GRAPHICS;
	private readonly resultsFrame = document.createElement("div");
	private readonly counter = this.createInfoElement(this.isGraphicsBenchmark ? "LOC_UI_BENCHMARK_FRAME" : "LOC_UI_BENCHMARK_TURN");
	private readonly turnTime = this.createInfoElement("LOC_UI_BENCHMARK_TURN_TIME");
	private readonly percentile99 = this.createInfoElement("LOC_UI_BENCHMARK_99_PERCENTILE");
	private readonly average = this.createInfoElement("LOC_UI_BENCHMARK_AVERAGE");
	private readonly lightGraph = document.createElement("div");

	private readonly closeButton = document.createElement("fxs-hero-button");
	private readonly graphCanvas = document.createElement("canvas");
	private readonly filePath = document.createElement("div");

	// Any types are used to store Chart js types as the library has no typings.
	private graph: any;
	private graphConfig: any = {};
	private graphData: { x: number, y: number }[] = [];
	private frameValues: number[] = [];
	...
	private onBenchmarkEnded(endData: BenchEndedData): void {
	...
	private onBenchmarkUpdate(updateData: BenchUpdatedData): void {
	...
	private createInfoElement(label: string) {
	...
	private drawDistributionGraph(percentile99: number) {
