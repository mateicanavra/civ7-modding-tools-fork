...
import { FxsModalFrame } from '/core/ui/components/fxs-modal-frame.js';
...
class ScreenTechCivicComplete extends Panel {
	...
	private static readonly UNLOCKED_ITEM_WIDTH: number = 13;
	private static readonly UNLOCKED_ITEM_PADDING: number = 1;
	private static readonly UNLOCKED_ITEM_LEFT_OFFSET: number = ScreenTechCivicComplete.UNLOCKED_ITEM_WIDTH + ScreenTechCivicComplete.UNLOCKED_ITEM_PADDING;
	private static readonly UNLOCKED_ITEMS_CONTAINER_WIDTH: number = 60;
	private static readonly UNLOCKED_ITEMS_EXTRA_OFFSET: number = 4; //Extra offset to allow the last item to be fully displayed + some deadspace to better communicate there is nothing else in the list

	private static readonly UNLOCKED_ITEMS_VISIBLE_NUMBER: number = Math.floor(ScreenTechCivicComplete.UNLOCKED_ITEMS_CONTAINER_WIDTH / (ScreenTechCivicComplete.UNLOCKED_ITEM_WIDTH + ScreenTechCivicComplete.UNLOCKED_ITEM_PADDING)); // How many items we can see before having to scroll. Use on Gamepad mode only.

	private unlockedItems: HTMLElement[] = [];
	// TODO: To use a fxs-scrollable-horizontal instead of the following arrows and a scrolling handled locally by the screen
	private leftScrollArrow: HTMLElement | null = null;
	private rightScrollArrow: HTMLElement | null = null;

	private currentScrollOffset: number = 0;

	private unlockedItemDefinitions: ProgressionTreeNodeUnlockDefinition[] = [];
	private firstVisibleUnlockedItemIndex: number = 0; // Index of the first / left most visible unlocked item. Use on Gamepad mode only.

	private unlockedItemFocusListener = this.onUnlockedItemFocus.bind(this);

	private treeType = ProgressionTreeTypes.CULTURE;
	private readonly modalFrame: ComponentRoot<FxsModalFrame> = document.createElement('fxs-modal-frame');
	private readonly buttonSlot: ComponentRoot<FxsVSlot> = document.createElement('fxs-vslot');
	private readonly unlockedItemsContainer = document.createElement('fxs-hslot');
	private readonly unlockItemsParentWrapper = document.createElement('fxs-scrollable');

	private popupData = TechCivicPopupManager.currentTechCivicPopupData!;
	private node!: ProgressionTreeNode;
	private nodeDefinition = this.popupData?.node!;

	private tooltipsFocused: boolean = false;
	...
	private onChangePolicies = () => {
		TechCivicPopupManager.closePopup();
		ContextManager.push("screen-policies", { singleton: true, createMouseGuard: true });
	}

	private updateItemsUnlockedByNode() {
		this.unlockedItemDefinitions = [];

		const treeNodeUnlocks: ProgressionTreeNodeUnlockDefinition[] = TreeNodesSupport.getValidNodeUnlocks(this.node);
	...
	private createUnlockedItem(unlockInfo: ProgressionTreeNodeUnlockDefinition, index: number, isOverflow: boolean): HTMLElement {
	...
	private scrollUnlockedItems(direction: ScrollDirection) {
		const totalIconsWidth: number = ScreenTechCivicComplete.UNLOCKED_ITEM_LEFT_OFFSET * this.unlockedItemDefinitions.length;

		let leftOffsetDelta: number = ScreenTechCivicComplete.UNLOCKED_ITEM_LEFT_OFFSET;
		if (direction == ScrollDirection.Left) {
			...
			const currentRightBounds: number = this.currentScrollOffset + ScreenTechCivicComplete.UNLOCKED_ITEMS_CONTAINER_WIDTH + ScreenTechCivicComplete.UNLOCKED_ITEMS_EXTRA_OFFSET;
		...
		const step: number = (direction == ScrollDirection.Left) ? 1 : -1;
	...
	private onLeftScrollArrow = () => {
		this.scrollUnlockedItems(ScrollDirection.Left);
	}

	private onRightScrollArrow = () => {
		this.scrollUnlockedItems(ScrollDirection.Right);
	}

	private onUnlockedItemFocus(event: FocusEvent) {
		...
		const indexStr: string | null = target.getAttribute("tabindex");
	 ...
	 * </fxs-vslot>
	 */
	private renderUnlockedItemsSection(unlocksTitle: string) {
	...
	private render() {
		...
		const popupData: {
			[key in ProgressionTreeTypes]: PopupData
		} = {
		...
		const numUnlockedItems: number = this.unlockedItemDefinitions.length;
		...
		if (quote && Locale.keyExists(quote.Quote)) {
			...
			let quoteHTML: string = `<fxs-inner-frame class="min-h-32 mx-4 p-3">
