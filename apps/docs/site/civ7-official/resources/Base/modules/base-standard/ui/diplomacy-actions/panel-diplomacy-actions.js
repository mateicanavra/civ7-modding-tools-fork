...
class DiplomacyActionPanel extends DiplomacyInputPanel {
    constructor() {
        ...
        this.selectedPlayerChangedListener = this.onSelectedPlayerChanged.bind(this);
        this.supportChangedListener = this.onSupportChanged.bind(this);
        this.viewReceiveFocusListener = this.onViewReceiveFocus.bind(this);
        this.actionCanceledListener = this.onActionCanceled.bind(this);
        this.diplomacyQueueChangedListener = this.onDiplomacyQueueChanged.bind(this);
        this.gameCoreEventPlaybackCompleteListener = this.onGameCoreEventPlaybackCompleteListener.bind(this);
        this.populateInitialDataTimerListener = this.onPopulateInitialDataTimerFinished.bind(this);
        this.onHandleWarSupportClosedListener = this.onHandleWarSupportClosed.bind(this);
        this.panels = [];
        this.initialLoadComplete = false;
        this.firstFocusSection = null;
        this.ongoingActionPageNumber = 0;
    ...
    populateInitialData() {
        ...
        if (DiplomacyManager.selectedActionID != -1) {
            const actionData = Game.Diplomacy.getDiplomaticEventData(DiplomacyManager.selectedActionID);
            if (!actionData) {
            ...
            if (!(actionData.actionType == DiplomacyActionTypes.DIPLOMACY_ACTION_GIVE_INFLUENCE_TOKEN && DiplomacyManager.selectedPlayerID == actionData.targetPlayer)) {
                ...
                const infIndepData = Game.Diplomacy.getInfluenceIndependentData(actionData.uniqueID);
    ...
    trySupportBefriendIndependent() {
        const ongoingActions = Game.Diplomacy.getPlayerEvents(DiplomacyManager.selectedPlayerID).filter((action) => {
        ...
        const supportResult = Game.PlayerOperations.canStart(GameContext.localPlayerID, PlayerOperationTypes.SUPPORT_DIPLOMATIC_ACTION, supportArgs, false);
    ...
    populateOngoingProjects() {
        ...
        const myActions = ongoingActions.filter((action) => { return action.initialPlayer == GameContext.localPlayerID && action.actionType != DiplomacyActionTypes.DIPLOMACY_ACTION_DECLARE_WAR && action.actionType != DiplomacyActionTypes.DIPLOMACY_ACTION_FORM_ALLIANCE; });
        const theirActions = ongoingActions.filter((action) => { return action.initialPlayer != GameContext.localPlayerID && action.actionType != DiplomacyActionTypes.DIPLOMACY_ACTION_DECLARE_WAR && action.actionType != DiplomacyActionTypes.DIPLOMACY_ACTION_FORM_ALLIANCE; });
    ...
    createOngoingActionItem(action) {
        const actionData = Game.Diplomacy.getProjectDataForUI(action.initialPlayer, -1, DiplomacyActionTargetTypes.NO_DIPLOMACY_TARGET, DiplomacyActionGroups.NO_DIPLOMACY_ACTION_GROUP, -1, DiplomacyActionTargetTypes.NO_DIPLOMACY_TARGET).find(project => project.actionID == action.uniqueID);
        ...
        if (actionData?.actionType == DiplomacyActionTypes.DIPLOMACY_ACTION_GIVE_INFLUENCE_TOKEN) {
            ...
            if (stageData.stageType == stages[0].stageType) {
                const projectDefinition = GameInfo.DiplomacyActions.lookup(actionData.actionType);
    ...
    populatePlayerCivInfo() {
        ...
        const civDefinition = GameInfo.Civilizations.lookup(playerObject.civilizationType);
        if (!civDefinition) {
        ...
        const leaderBonusItems = Database.query('config', 'select * from LeaderItems order by SortIndex')?.filter(item => item.LeaderType == leaderDefinition.LeaderType);
        ...
        const civBonusItems = Database.query('config', 'select * from CivilizationItems order by SortIndex')?.filter(item => item.CivilizationType == civDefinition.CivilizationType);
        const civTrait = civBonusItems?.find(item => item.Kind == "KIND_TRAIT");
        const civAbilityName = civTrait?.Name;
        const civAbilityDescription = civTrait?.Description;
        const civUniqueItems = civBonusItems?.filter(item => item.Kind == 'KIND_BUILDING' || item.Kind == 'KIND_IMPROVEMENT' || item.Kind == 'KIND_UNIT' || item.Kind == "KIND_QUARTER");
    ...
    populateGovernmentInfo() {
        ...
        const governmentDefinition = GameInfo.Governments.lookup(playerObject.Culture.getGovernmentType());
        ...
        const governmentCelebrationTypes = Game.Culture.GetCelebrationTypesForGovernment(governmentDefinition.GovernmentType);
    ...
    findOrCreateRelationshipRow(id, titleString) {
        let relatioshipRowWrapper = this.Root.querySelector(`#panel-diplomacy-actions__relationship-row-${id}`);
    ...
    populateActionsPanel() {
        ...
        if (otherPlayer != null && otherPlayer.isMajor) {
            const theirWars = Game.Diplomacy.getPlayerEvents(DiplomacyManager.selectedPlayerID).filter((action) => { return action.actionType == DiplomacyActionTypes.DIPLOMACY_ACTION_DECLARE_WAR; });
    ...
    createStartActionListItem(projectData, recentlyCompletedData) {
        ...
        if (projectData.actionGroup == DiplomacyActionGroups.DIPLOMACY_ACTION_GROUP_ESPIONAGE && projectData.actionType != DiplomacyActionTypes.DIPLOMACY_ACTION_COUNTER_SPY) {
            const projectDefinition = GameInfo.DiplomacyActions.lookup(projectData.actionType);
            if (projectDefinition) {
        ...
        if (projectData.targetList2.length > 0 && !recentlyCompletedData) {
            ...
            if (recentlyCompletedData) {
                ...
                const results = Game.PlayerOperations.canStart(GameContext.localPlayerID, projectData.operationType, actionOperationArguments, false);
                if (!results.Success) {
    ...
    showBefriendIndependentDetails() {
        if (Players.get(DiplomacyManager.selectedPlayerID)?.isIndependent) {
            const ourActions = Game.Diplomacy.getPlayerEvents(DiplomacyManager.selectedPlayerID).filter((action) => {
                return action.initialPlayer == GameContext.localPlayerID && action.actionType == DiplomacyActionTypes.DIPLOMACY_ACTION_GIVE_INFLUENCE_TOKEN;
            });
            ...
            if (befriendIndependentActionID == -1) {
                const otherActions = Game.Diplomacy.getPlayerEvents(DiplomacyManager.selectedPlayerID).filter((action) => {
...
class PlayerDiplomacyActionPanel extends DiplomacyActionPanel {
    ...
    populateActionsPanel() {
        ...
        Players.getAliveIds().forEach(playerID => {
            ...
            if (localPlayerDiplomacy?.hasMet(playerID) && otherPlayer != null && otherPlayer.isMajor) {
                const theirWars = Game.Diplomacy.getPlayerEvents(playerID).filter((action) => { return action.actionType == DiplomacyActionTypes.DIPLOMACY_ACTION_DECLARE_WAR; });
                theirWars.forEach(war => {
    ...
    createOngoingActionListItem(action) {
        ...
        if (projectDefinition && action.actionGroup == DiplomacyActionGroups.DIPLOMACY_ACTION_GROUP_ESPIONAGE && action.actionType != DiplomacyActionTypes.DIPLOMACY_ACTION_COUNTER_SPY) {
            ...
            const stage1MinTurns = stage1MaxTurns - Game.Diplomacy.modifyByGameSpeed(projectDefinition.RandomInitialProgress);
            ongoingActionElement.setAttribute("data-tooltip-content", Locale.stylize(Locale.compose(action.description) +
                "[N]" +
...
class OtherPlayerDiplomacyActionPanel extends DiplomacyActionPanel {
    constructor() {
        super(...arguments);
        this.diplomacyWarPeaceListener = (data) => { this.onDiplomacyWarPeace(data); };
        this.relationshipChangedListener = (eventData) => { this.onRelationShipChanged(eventData); };
    }
    onAttach() {
        super.onAttach();
    ...
    onSupportChanged(data) {
        ...
        const actionData = Game.Diplomacy.getDiplomaticEventData(data.actionID);
        const isIndependent = Players.get(DiplomacyManager.selectedPlayerID)?.isIndependent ?? false;
        const isInvolved = ((actionData.initialPlayer == DiplomacyManager.selectedPlayerID) ||
...
//# sourceMappingURL=file:///base-standard/ui/diplomacy-actions/panel-diplomacy-actions.js.map
