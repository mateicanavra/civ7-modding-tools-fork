...
export const UpdateCityDetailsEventName = 'update-city-details' as const;
class UpdateCityDetailsEvent extends CustomEvent<never> {
	constructor() {
...
class CityDetailsModel {
	...
	isBeingRazed: boolean = false;
	getTurnsUntilRazed: number = -1;

	treasureFleetText: string = "";

	connectedSettlementFood: { name: string, amount: number }[] = [];

	private onUpdate?: (model: CityDetailsModel) => void;
	...
	private updateGate = new UpdateGate(() => {
		...
		for (const constructibleID of constructibles.getIds()) {
			...
			for (const index in maintenances) {
				...
				if (maintenanceValue > 0) {
					...
					const maintenanceYieldData: ConstructibleYieldData = {
		...
		if (Game.age == Game.getHash("AGE_EXPLORATION") && city.isDistantLands) {
			...
			const result: OperationResult = cityResources.canStartTreasureFleet();
			if (!result.Success) {
				if (result.FailureReasons) {
					const failureString: string = formatStringArrayAsNewLineText(result.FailureReasons);
	...
	private buildSendingFoodData(selectedSettlement: City) {
		const sendingFoodData: { city: City, data: { town: City, amount: number }[] }[] = []
	...
	private updateUniqueQuarterData(districtData: DistrictData) {
		...
		let isBuildingType1Complete: boolean = false;
		...
		let isBuildingType2Complete: boolean = false;
		for (const constructibleData of districtData.constructibleData) {
	...
	private getUniqueQuarterDefinition(): UniqueQuarterDefinition | undefined {
	...
	private addYieldSteps(baseYield: YieldData, steps: TrackedValue[], yieldDefinition: YieldDefinition, isModifier: boolean) {
	...
	private setYieldAndGetIcon(yieldData: YieldData, step: TrackedValue, yieldDefinition: YieldDefinition) {
	...
	private addYieldAndGetIconForConstructible(yieldData: YieldData, constructibleData: ConstructibleData, step: TrackedValue, yieldDefinition: YieldDefinition) {
		// Add to yield map
		...
		if (currentValue == undefined) {
			...
			const constructibleYieldData: ConstructibleYieldData = {
				name: yieldData.name,
				value: step.value,
				icon: yieldDefinition.YieldType,
				iconContext: "YIELD",
