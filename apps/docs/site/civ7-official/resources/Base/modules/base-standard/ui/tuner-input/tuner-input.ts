...
class Tuner_MapPanel {
	...
	private previewModelGroup: WorldUI.ModelGroup | null = null;
	...
	onActionA(loc: float2): boolean {
		if (g_TunerState.MapPanel) {
			if (g_TunerState.MapPanel.placementMode == 'Unit') {
				...
				args.Location = loc;
				let selectedPlayer: PlayerId = localPlayerID;
				...
				let selectedIndependent: number = -1;
		...
		if (g_TunerState.WorldUIModels) {
			...
			if (g_TunerState.WorldUIModels.placementMode == 'Model') {
				if (g_TunerState.WorldUIModels.selectedAssetName) {
					...
					if (previewGroup) {
						let offset: float3 = { x: 0, y: 0, z: 0 };
						let params: WorldUI.ModelParams = { placement: PlacementMode.TERRAIN };
...
enum ShowingOverlayType {
	NONE,
	AREAS,
	REGIONS
};

const AREA_HIGHLIGHT_COLOR = { x: 0, y: 0.671, z: 0, w: 0.75 } as const;
const REGION_HIGHLIGHT_COLOR = { x: 0, y: 0.671, z: 0, w: 0.75 } as const;

class Tuner_MapAreasPanel {
	private overlayGroup: WorldUI.OverlayGroup | null = null; // = WorldUI.createOverlayGroup("AppealOverlayGroup", OVERLAY_PRIORITY.PLOT_HIGHLIGHT);
	private overlay: WorldUI.PlotOverlay | null = null; // = this.appealOverlayGroup.addPlotOverlay();
	private showing: ShowingOverlayType = ShowingOverlayType.NONE;
	private showingId: number = -1;
	...
	showMapArea(areaId: number) {
		if (this.showing != ShowingOverlayType.AREAS || this.showingId != areaId) {
			...
			const plots: number[] = MapAreas.getAreaPlots(this.showingId);
			if (plots.length > 0) {
	...
	showMapRegion(regionId: number) {
		if (this.showing != ShowingOverlayType.REGIONS || this.showingId != regionId) {
			...
			const plots: number[] = MapRegions.getRegionPlots(this.showingId);
			if (plots.length > 0) {
...
// Declare the external tuner state that the Tuner scripts will be populating
// This will contain all the state that the Tuner panels fiddle with.
declare var g_TunerState: any;
...
class TunerInput {
	...
	onReady() {
		window.addEventListener('tuner-user-action-a', (event: CustomEvent) => {
			if (!this.onUserActionA(event)) {
				event.stopImmediatePropagation();
				event.preventDefault();
			}
		}, true);	// Action A, usually left-click
		window.addEventListener('tuner-user-action-b', (event: CustomEvent) => {
...
var g_TunerInput: TunerInput = new TunerInput();
