...
enum ConstructibleIcons {
	EMPTY = "BUILDING_OPEN",
	ADD = "BUILDING_ADD",
	WARNING = "BUILDING_WARNING"
}

type constructibleInfo = {
	name: string;
	type: string;
	shouldShow: boolean;
	details: string[];
...
class PlaceBuildingModel {
	...
	shouldShowOverbuild: boolean = false;
	...
	shouldShowUniqueQuarterText: boolean = false;
	uniqueQuarterText: string = "";
	uniqueQuarterWarning: string = "";

	shouldShowFromThisPlot: boolean = false;
	fromThisPlotYields: string[] = [];

	shouldShowAdjacencyBonuses: boolean = false;
	adjacencyBonuses: string[] = [];

	warehouseBonuses: yieldInfo[] = [];
	...
	private _OnUpdate?: (model: PlaceBuildingModel) => void;

	set updateCallback(callback: (model: PlaceBuildingModel) => void) {
	...
	updateGate = new UpdateGate(() => {
		...
		let isUniqueImprovement: boolean = false;

		const playerConfig: ConfigurationPlayerAccessor = Configuration.getPlayer(GameContext.localPlayerID);
		const civType: string | null = playerConfig.civilizationTypeName;
		if (civType) {
			const civTrait: CivilizationTraitDefinition | null = GameInfo.LegacyCivilizationTraits.lookup(civType);
		...
		const showUrbanWarning: boolean = !(isUniqueImprovement || constructibleDef.ConstructibleClass == "WONDER");
				...
				if (constructibles[1]) {
					...
					if (selectedDistrict.isUrbanCore) {
						...
						if (this.isRepairing) {
							...
							this.overbuildText = Locale.compose("LOC_UI_CITY_VIEW_PLACE_OVER_DESC", this.firstConstructibleSlot.name);
							const converWarning: string = Locale.compose("LOC_UI_CITY_VIEW_CONVERT_TO_URBAN_WARNING");
							const placeOver: string = Locale.compose("LOC_UI_CITY_VIEW_PLACE_OVER", this.overbuildConstructibleSlot.name);
	...
	private getConstructibleInfoByComponentID(constructibleID: ComponentID): constructibleInfo {
		let constructibleInfo: constructibleInfo = {
			name: "",
			type: ConstructibleIcons.EMPTY,
			shouldShow: true,
			details: [],
...
const PlaceBuilding: PlaceBuildingModel = new PlaceBuildingModel();
engine.whenReady.then(() => {
