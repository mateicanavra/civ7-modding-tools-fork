...
import { PlayerData, VictoryData } from '/base-standard/ui/victory-manager/victory-manager.js';
...
class PanelAdvisorVictory extends Panel {
	private trackVictoryActivateListener = this.onTrackVictoryActivate.bind(this);
	private engineInputListener = this.onEngineInput.bind(this);
	private navigateInputListener = this.onNavigateInput.bind(this);
	private civilopediaListener = this.onCivilopediaButtonInput.bind(this);

	private onRadioButtonListener = this.onRadioButtonInput.bind(this);

	private carousel: HTMLElement | null = null;
	private carouselIndex: number = 0;

	private selectedAgeType: string = "";
	private selectedAdvisor: AdvisorType | null = null;

	private playerData: PlayerData | null = null;
	private activeQuest: QuestItem | undefined = undefined;
	private victoryQuests: QuestItem[] = [];
	private radioButtons: HTMLElement[] = [];

	private isTopFocused: boolean = true;

	private selectedRewardIndex: number = 0;
	private rewardElemends: HTMLElement[] = [];

	private showVictoryDetailsLink: boolean = false;
	...
	private render() {
		...
		const advisor: string | null = this.Root.getAttribute('advisor-type');
		if (!advisor) {
		...
		const victoryData: VictoryData[] | undefined = AdvisorProgress.victoryData.get(this.selectedAgeType);
		...
		const chosenVictory: VictoryData | undefined = victoryData.find(victory => victory.ClassType == AdvisorProgress.getLegacyPathClassTypeByAdvisorType(advisorType));
		if (!chosenVictory) {
			console.error('panel-advisor-victory: render - Failed to get victoryData');
			return;
		}
		const advisorPanel: HTMLElement = document.createElement('div');
	...
	private createVictoryCivilopediaLink(): HTMLElement {
	...
	private createCivilopediaText(): HTMLElement {
		...
		const victoryCivilopediaLinkText: string = `<span class="font-body text-secondary">${victoryTypeTitle}</span>`
	...
	private createQuestCarousel(): HTMLElement {
		// base box
		...
		const advisorText: HTMLElement = document.createElement('div');
		...
		const bodyText: HTMLElement = document.createElement('div');
		...
		const carouselText: HTMLElement = document.createElement('div');
		carouselText.classList.add('carousel-text');

		const progressText: HTMLElement = document.createElement('p');
		...
		const currentQuest: QuestItem = this.getCurrentQuest();
		if (currentQuest) {
			...
			if (currentQuest.victory) {
				const progressDescription: string = this.getStateString(currentQuest.victory.state);
	...
	private getStateString(state: VictoryQuestState): string {
		switch (state) {
	...
	private onCarouselLeft() {
		this.carouselAction(true);
	}

	private onCarouselRight() {
		this.carouselAction();
	}

	private updateRadioButton(prevIndex: number, currentIndex: number) {
	...
	*/
	private carouselAction(moveBack?: boolean) {
	...
	private updateVictoryQuests() {
		...
		const currentQuest: QuestItem = this.victoryQuests[this.carouselIndex];
		if (!currentQuest) {
	...
	private getCurrentQuest() {
		if (this.activeQuest) {
			const activeQuestId: string = this.activeQuest.id;
			const activeIndex: number = this.victoryQuests.findIndex(item => item.id == activeQuestId);
	...
	onAttributeChanged(_name: string, _oldValue: string | null, newValue: string | null): void {
		if (!newValue) {
			return;
		}
	...
	private renderTopPanel(chosenVictory: VictoryData, chosenAdvisor: AdvisorType): HTMLElement {
		const topPanel: HTMLElement = document.createElement('div');
		...
		const victoryTextContainer: HTMLElement = document.createElement('div');
		victoryTextContainer.classList.add('advisor-panel_victory-text', 'mt-6', 'text-center')
		// victorty title
		const victoryName: HTMLElement = document.createElement('p');
		...
		// victory description
		const victoryDescription: HTMLElement = document.createElement('p');
	...
	private renderRewardPip(milestoneRewards: AgeProgressionRewardDefinition[], ageProgressRewardAmt: number): HTMLElement {
	...
	private renderBottomPanel(chosenAdvisor: AdvisorType): HTMLElement {
		const bottomPanel: HTMLElement = document.createElement('div');
	...
	private renderTrackVictoryCheckBox(): HTMLElement {
		const trackQuestWrapper: HTMLElement = document.createElement('div');
		trackQuestWrapper.classList.add('flow-row', 'text-base', 'font-title', 'uppercase', 'flex-auto', 'font-bold', 'tracking-150', 'relative');
		const isTrackerVisible: boolean = (this.activeQuest && !Online.Metaprogression.isPlayingActiveEvent()) ? true : false;
		trackQuestWrapper.classList.toggle("hidden", !isTrackerVisible);

		const checkBox: HTMLElement = document.createElement('fxs-checkbox');
		...
		const trackerText: HTMLElement = document.createElement('p');
	...
	onTrackVictoryActivate(event: CustomEvent): void {
	...
	toggleCheckBox(): void {
		const checkBox: HTMLElement | null = this.Root.querySelector<HTMLElement>('.advisor-victory_tracker');
	...
	private onRadioButtonInput(event: CustomEvent): void {
		if (event.target instanceof HTMLElement) {
			const targetActivateIndex: string | null = event.target.getAttribute('value');
	...
	private onCivilopediaButtonInput(_event: CustomEvent) {
