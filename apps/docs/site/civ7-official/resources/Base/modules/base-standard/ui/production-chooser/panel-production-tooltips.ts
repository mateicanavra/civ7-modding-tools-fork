...
type ConstructibleMaintenanceRow = Pick<Constructible_MaintenanceDefinition, 'YieldType' | 'Amount'>;

class ProductionConstructibleTooltipType implements TooltipType {
	private definition: ConstructibleDefinition | undefined;
	private maintenance: ConstructibleMaintenanceRow[] | null = null;
	private target: HTMLElement | null = null;

	// #region Element References
	private readonly container = document.createElement('fxs-tooltip');
	private readonly description = document.createElement('p');
	private readonly header = document.createElement('div');
	private readonly productionCost = document.createElement('div');
	private readonly baseYield = document.createElement('div');
	private readonly constructibleName = document.createElement('div');
	private readonly constructibleBonusContainer = document.createElement('div');
	private readonly maintenanceContainer = document.createElement('div');
	private readonly maintenanceEntriesContainer = document.createElement('div');
	private readonly gemsContainer = document.createElement('div');
	// #endregion
	...
	constructor() {
		...
		this.maintenanceContainer.append(
			maintenanceLabel,
	...
	isUpdateNeeded(target: HTMLElement): boolean {
		const targetConstructibleItem = target.closest<HTMLElement>('[data-tooltip-style="production-constructible-tooltip"]');
		...
		this.maintenance = Database.query<ConstructibleMaintenanceRow>(
			'gameplay',
	...
	update() {
		...
		const {
			...
			adjacencies,
			effects
		} = getConstructibleEffectStrings(definition.ConstructibleType, city);
		...
		if (recommendations) {
			const parsedRecommendations: Array<AdvisorUtilities.AdvisorClassObject> = JSON.parse(recommendations);
			const advisorList: Array<AdvisorRecommendations> = parsedRecommendations.map(rec => rec.class);
...
class ProductionUnitTooltipType implements TooltipType {
	private definition: UnitDefinition | undefined;
	private target: HTMLElement | null = null;
	...
	private readonly header = document.createElement('div');
	private readonly productionCost = document.createElement('div');
	private readonly itemName = document.createElement('div');
	private readonly maintenanceContainer = document.createElement('div');
	private readonly maintenanceCostText = document.createElement('div');
	private readonly gemsContainer = document.createElement('div');
	// #endregion
	...
	isUpdateNeeded(target: HTMLElement): boolean {
		const targetUnitItem = target.closest<HTMLElement>('[data-tooltip-style="production-unit-tooltip"]');
...
class ProductionProjectTooltipType implements TooltipType {
	private target: HTMLElement | null = null;

	// #region Element References
	private readonly tooltip = document.createElement('fxs-tooltip');
	private readonly icon = document.createElement('fxs-icon');
	private readonly itemName = document.createElement('div');
	private readonly description = document.createElement('p');
	private readonly requirementsContainer = document.createElement('div');
	private readonly requirementsText = document.createElement('div');
	private readonly gemsContainer = document.createElement('div');
	// #endregion
	...
	isUpdateNeeded(target: HTMLElement): boolean {
		const newTarget = target.closest<HTMLElement>('town-focus-chooser-item, production-chooser-item');
