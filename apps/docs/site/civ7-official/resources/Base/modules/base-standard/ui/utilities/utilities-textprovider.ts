...
export function getUnlockTargetIcon(targetType: string, targetKind: string): string {
	// Target is a CONSTRUCTIBLE
	if (targetKind == "KIND_CONSTRUCTIBLE") {
		const constructibleInfo = GameInfo.Constructibles.find(o => o.ConstructibleType == targetType);
		if (constructibleInfo) {
...
export function getNodeName(nodeData: ProgressionTreeNode): string {
	if (!nodeData) { return ""; }

	const nodeInfo = GameInfo.ProgressionTreeNodes.lookup(nodeData.nodeType);
	if (!nodeInfo) { return ""; }

	let nodeName: string = Locale.compose(nodeInfo.Name ?? nodeInfo.ProgressionTreeNodeType);
