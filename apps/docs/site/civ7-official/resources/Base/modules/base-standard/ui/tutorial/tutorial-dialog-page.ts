...
export default class TutorialDialogPage extends Component {
	...
	onAttach() {
		...
		const element: HTMLDivElement | null = this.Root.querySelector<HTMLDivElement>(".tutorial-dialog-backgrounds");
		if (element) {
			this.backgroundImages.forEach((imageURL, i) => {
				let img: HTMLDivElement = document.createElement("div");
	...
	private setImages() {
		const images: NodeListOf<HTMLElement> = this.Root.querySelectorAll<HTMLElement>('.tutorial-image');
		images.forEach((image: HTMLElement) => {
			...
			const imageContainer: Element | null = this.Root.querySelector('.tutorial-dialog-page-image-container');
			if (imageContainer) {
	...
	private setBackgroundImageInDiv(value: string, cssClassName: string): boolean {
		...
		const element: HTMLDivElement | null = (this.Root.querySelector<HTMLDivElement>(`.${cssClassName}`));
		if (!element) {
			return false;
		}

		const imageURL: string = `url(${value})`;
		element.style.backgroundImage = imageURL;
		return true;
	}

	private setStringInDivClass(value: string, cssClassName: string): boolean {
		if (value == undefined || value == null) {
