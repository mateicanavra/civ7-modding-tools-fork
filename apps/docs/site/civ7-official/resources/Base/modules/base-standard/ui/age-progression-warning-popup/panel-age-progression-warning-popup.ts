...
class PanelAgeProgressionWarningPopup extends Panel {
	...
	private onDetailsPressedListener = this.onDetailsPressed.bind(this);
	private onContinuePressedListener = this.onContinuePressed.bind(this);

	private description!: HTMLElement;
	private turnsRemainingText!: HTMLElement;
	private detailsButton!: HTMLElement;
	private continueButton!: HTMLElement;
	private textScrollable!: ComponentRoot<FxsScrollable>;
	...
	private onDetailsPressed() {
		this.close();
		if (Game.AgeProgressManager.isFinalAge) {
			const pediaPage: Page | null = Civilopedia.getPage("AGES", "AGES_8");
			if (!pediaPage) {
			...
			const pediaPage: Page | null = Civilopedia.getPage("AGES", "AGES_1");
			if (!pediaPage) {
