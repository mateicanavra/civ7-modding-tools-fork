/**
 * @file interface-mode-army-overrun.ts
...
interface ArmyOverrunInterfaceModeContext extends InterfaceMode.Context {
	UnitID: ComponentID;
	OperationArguments: any;
}

/**
 * Handler for INTERFACEMODE_ARMY_OVERRUN
 */
class ArmyOverrunInterfaceMode extends ChoosePlotInterfaceMode {

	private _OperationResult: OperationResult | null = null;
	private validPlots = new Set<PlotIndex>();

	initialize() {
		const context = this.Context as ArmyOverrunInterfaceModeContext;
		const args: any = {};
		const result = Game.UnitCommands.canStart(context.UnitID, 'UNITCOMMAND_ARMY_OVERRUN', args, false);
	...
	commitPlot(plot: PlotCoord) {
		...
		args.X = plot.x;
		args.Y = plot.y;
		Game.UnitCommands.sendRequest(unitID, 'UNITCOMMAND_ARMY_OVERRUN', args);
		UI.sendAudioEvent(Audio.getSoundTag('data-audio-unit-combat-confirmed', 'interact-unit'));
	}
}

InterfaceMode.addHandler('INTERFACEMODE_ARMY_OVERRUN', new ArmyOverrunInterfaceMode());