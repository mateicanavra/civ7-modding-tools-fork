...
function generateMap() {
	...
	let iRandomRiver: number = TerrainBuilder.getRandomNumber(10, "Intensity of Rivers");
	if (iRandomRiver == 0) {
...
function createLandmasses(iWidth: number, iHeight: number, continent1: ContinentBoundary, continent2: ContinentBoundary,
	iStartSectorRows: number, iStartSectorCols: number, startSectors: boolean[], iOceanWaterColumns: number) {

	let iRandomLandmassSetup: number = TerrainBuilder.getRandomNumber(4, "Shuffle Map Landmass Structure");
...
function createArchipelago(iWidth: number, iHeight: number, continent1: ContinentBoundary, continent2: ContinentBoundary,
...
function createContinents(iWidth: number, iHeight: number, continent1: ContinentBoundary, continent2: ContinentBoundary,
	iStartSectorRows: number, iStartSectorCols: number, startSectors: boolean[]) {
...
function createFractal(iWidth: number, iHeight: number, westContinent: ContinentBoundary, eastContinent: ContinentBoundary,
	iStartSectorRows: number, iStartSectorCols: number, startSectors: boolean[]) {

	FractalBuilder.create(globals.g_LandmassFractal, iWidth, iHeight, 3, 0);
	let iWaterHeight: number = FractalBuilder.getHeightFromPercent(globals.g_LandmassFractal, globals.g_WaterPercent);
	let iBuffer: number = Math.floor(iHeight / 13.5);
	let iBuffer2: number = Math.floor(iWidth / 21.0);
...
function createTerraMass(iWidth: number, iHeight: number, continent1: ContinentBoundary, continent2: ContinentBoundary,

	iStartSectorRows: number, iStartSectorCols: number, startSectors: boolean[], bFirst: boolean, iOceanWaterColumns: number) {
	let iSize: number = TerrainBuilder.getRandomNumber(6, "Random Land Size") + 2;
	//console.log("Frac Size " + iSize);
	FractalBuilder.create(globals.g_LandmassFractal, iWidth, iHeight, iSize, 0);
	let iRanHeight: number = 100 - ((iSize * 10) - 20);
	let iMaxHeight: number = FractalBuilder.getHeightFromPercent(globals.g_LandmassFractal, iRanHeight);
