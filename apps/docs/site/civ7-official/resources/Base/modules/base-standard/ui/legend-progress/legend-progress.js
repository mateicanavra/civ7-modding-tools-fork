...
class LegendProgressScreen extends Panel {
    onAttach() {
        ...
        if (isPlayingActiveEvent) {
            ...
            var leaderboardButton = document.createElement("fxs-button");
    ...
    buildEarnedProgressItem(item) {
        ...
        let existingWidth = item.previousXP < item.previousLevelXP ? 0 : levelRatio * (item.previousXP - item.previousLevelXP);
        ...
        let newWidth = item.previousXP < item.previousLevelXP ? levelRatio * (item.gainedXP + item.previousXP - item.previousLevelXP) : levelRatio * item.gainedXP;
...
//# sourceMappingURL=file:///base-standard/ui/legend-progress/legend-progress.js.map
