...
export const BuildingPlacementHoveredPlotChangedEventName = 'building-placement-hovered-plot-changed' as const;
...
export const BuildingPlacementSelectedPlotChangedEventName = 'building-placement-selected-plot-changed' as const;
...
export const BuildingPlacementConstructibleChangedEventName = 'building-placement-constructible-changed' as const;
...
// Combines YieldLike for type and main yield along with data necessary show the player the changes
...
const directionNames: Map<DirectionTypes, string> = new Map<DirectionTypes, string>([
...
class BuildingPlacementManagerClass {
	private static instance: BuildingPlacementManagerClass | null = null;
	...
	private _currentConstructible: ConstructibleDefinition | null = null;
	...
	private allPlacementData: PlacementData | undefined;

	// Placement data for the currently selected constructible
	private selectedPlacementData: PlacementBuildingData | undefined;

	//Plots that are already developed and have buildings placed on them
	private _urbanPlots: PlotIndex[] = [];
	...
	private _selectedPlotIndex: PlotIndex | null = null;
	...
	initializePlacementData(cityID: ComponentID) {
		this._cityID = cityID;
	...
	selectPlacementData(cityID: ComponentID, operationResult: OperationResult, constructible: ConstructibleDefinition) {
		...
		operationResult.ExpandUrbanPlots?.forEach(p => {
			const location: float2 = GameplayMap.getLocationFromIndex(p);
			const city: ComponentID | null = MapCities.getCity(location.x, location.y);
	...
	getTotalYieldChanges(plotIndex: number): YieldChangeInfo[] | undefined {
		...
		const yieldChangeInfo: YieldChangeInfo[] = [];
	...
	getPlotYieldChanges(plotIndex: number): YieldChangeInfo[] | undefined {
		...
		// Warehouse Bonuses
		let warehouseBonuses: Map<YieldType, number> = new Map<YieldType, number>();
		...
		warehouseBonuses.forEach((change: number, yieldType: YieldType) => {
	...
	getAdjacencyYieldChanges(plotIndex: number): YieldChangeInfo[] | undefined {
		const placementPlotData = this.getPlacementPlotData(plotIndex);
		if (!placementPlotData) {
	...
	private getDirectionString(fromPlot: PlotIndex, toPlot: PlotIndex): string {
	...
	private getPlacementPlotData(plotIndex: number): PlacementPlotData | undefined {
		if (!this.selectedPlacementData) {
	...
	getOverbuildConstructibleID(plotID: number) {
		if (!this.selectedPlacementData) {
	...
	getYieldPillIcon(yieldType: string, yieldNum: number, mainYield?: boolean) {
		let yieldIconPath = "";
	...
	isValidPlacementPlot(plotIndex: number) {
	...
	getAdjacencyBonuses(): AdjacencyData[] {

		let adjacencyData: AdjacencyData[] = [];
		...
		const yieldAdjacencies: YieldChangeData[] | undefined = this.city?.Yields?.calculateAllAdjacencyYieldsForConstructible(this.currentConstructible.ConstructibleType, this.selectedPlotIndex);
		if (!yieldAdjacencies) {
		...
		yieldAdjacencies.forEach(adjacency => {
			...
			const buildingLocation: float2 = GameplayMap.getLocationFromIndex(this.selectedPlotIndex);
			const adjacencyDirection: DirectionTypes = GameplayMap.getDirectionToPlot(buildingLocation, adjacencyLocation);
			const directionName: string | undefined = directionNames.get(adjacencyDirection);
	...
	getCumulativeAdjacencyBonuses(): AdjacencyData[] {

		let cumulativeData: AdjacencyData[] = [];
	...
	getWarehouseBonuses(): WarehouseData[] {
		...
		const allWarehouseBonuses: YieldChangeData[] | undefined = this.city?.Yields?.getAllWarehouseYieldsForConstructible(this.currentConstructible.ConstructibleType);
		if (allWarehouseBonuses) {
			allWarehouseBonuses.forEach((warehouseBonuse) => {
				const yieldDef: YieldDefinition | null = GameInfo.Yields.lookup(warehouseBonuse.yieldType);
				if (!yieldDef) {
	...
	getCumulativeWarehouseBonuses(): WarehouseData[] {

		let cumulativeData: WarehouseData[] = [];
...
const BuildingPlacementManager: BuildingPlacementManagerClass = new BuildingPlacementManagerClass();
