...
function createPrimaryLandmass(iWidth: number, iHeight: number, continent1: ContinentBoundary, continent2: ContinentBoundary,
	iStartSectorRows: number, iStartSectorCols: number, startSectors: boolean[], bWestSide: boolean, iOceanWaterColumns: number) {
...
function createSecondaryLandmassBalanced(iWidth: number, iHeight: number, continent1: ContinentBoundary, continent2: ContinentBoundary,
	iStartSectorRows: number, iStartSectorCols: number, startSectors: boolean[], bWestSide: boolean, iOceanWaterColumns: number) {
	let iSize: number = TerrainBuilder.getRandomNumber(6, "Random Land Size");
	console.log("Random Land Size " + iSize);
	FractalBuilder.create(globals.g_LandmassFractal, iWidth, iHeight, iSize, 0);
	let iRanHeight: number = 130 - (iSize * 10);
	let iWaterHeight: number = FractalBuilder.getHeightFromPercent(globals.g_LandmassFractal, (globals.g_WaterPercent * iRanHeight) / 100);
...
function createSecondaryLandmassStandard(continent1: ContinentBoundary) {
	...
	let iGrain: number = TerrainBuilder.getRandomNumber(3, "Secondary Landmass Fractal Grain") + 2;  // Min of 2
	...
	let iWaterHeightRange: number = 30;
	let iRanHeight: number = TerrainBuilder.getRandomNumber(iWaterHeightRange, "Random Water Height");
	...
	let iWaterHeight: number = FractalBuilder.getHeightFromPercent(globals.g_LandmassFractal, (globals.g_WaterPercent * globals.g_Cutoff) + iRanHeight - 5);

	for (let iY: number = continent1.south; iY <= continent1.north; iY++) {
		for (let iX: number = continent1.west; iX <= continent1.east; iX++) {
			let terrain: TerrainType = globals.g_FlatTerrain;
			let iPlotHeight: number = FractalBuilder.getHeight(globals.g_LandmassFractal, iX - continent1.west, iY - continent1.south);
