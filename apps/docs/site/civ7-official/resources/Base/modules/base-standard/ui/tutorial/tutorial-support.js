...
function accept(params) {
    ...
    const nextID = (params) ? params.nextID : undefined;
    return {
        callback: () => { },
        text: text,
 ...
 */
export function ensurePropertiesExist(obj, properties) {
 ...
 */
export function OpenCivilopediaAt(searchTerm) {
...
export function hasAnyWondersUnlocked(_TutorialItem) {
 ...
 */
export function didTechUnlock(node, techName, depth = 1) {
    ...
    let recentResearchNodeType = techs.getLastCompletedNodeType();
    ...
    if (depth != 1 && isMatch) {
        const progressionNode = Game.ProgressionTrees.getNode(event.player, recentResearchNodeType);
 ...
 */
export function didCivicUnlock(node, civicName, depth = 1) {
    const event = TutorialManager.activatingEvent;
    if (!event) {
 ...
 */
export function getUnitFromEvent(errorMsg = "none") {
    const event = TutorialManager.activatingEvent;
    if (!event) {
    ...
    const unit = Units.get(event.unit); // componentID
    if (!unit) {
...
export function getUnitFromOnly(errorMsg = "none") {
    const event = TutorialManager.activatingEvent;
    if (!event) {
        console.error("Tutorial helper failed obtaining an event to return player and unit. msg: ", errorMsg);
        return null;
...
export function hasTreeUnlocks(node) {
    ...
    let recentResearchNodeType = undefined;
 ...
 */
export function isUnitOfType(node, unitTypeNames) {
 ...
 * @returns true if match, false otherwise.
 */
export function isUnitOfDomain(node, domain) {
 ...
 * @returns true if match, false otherwise.
 */
export function isTreasureResource(node) {
    const event = TutorialManager.activatingEvent;
    if (!event) {
 ...
 * @returns The unit name, NO_UNIT otherwise.
 */
export function getNameOfFirstUnitWithTag(tag) {
 ...
 * @returns The unit name, NO_UNIT otherwise.
 */
export function getNameOfFirstUnlockedUnitWithTag(tag) {
    const player = Players.get(GameContext.localPlayerID);
    if (player && player.Units) {
 ...
 */
export function getTutorialPrompts(actionPrompts) {
 ...
 */
export function getCurrentTurnBlockingNotification(playerID) {
...
*/
export function setNextItemActivation(item, nextID) {
    if (!nextID) {
...
*/
export function canQuestActivate(prevQuestTracking, currQuestTracking) {
    ...
    const firstQuestInPath = items.find(item => item.victory?.type == currentPathType && item.victory.order == 1);
...
*/
export function activateNextTrackedQuest(item) {
    ...
    const items = Array.from(QuestTracker.getItems());
    const nextQuestItem = items.find(item => item.victory?.type == currentType && item.victory.order == nextQuestOrder);
...
export var AdvisorRecommendations;
(function (AdvisorRecommendations) {
    AdvisorRecommendations["NO_ADVISOR"] = "recommendation-none";
    AdvisorRecommendations["CULTURAL"] = "recommendation-cultural";
    AdvisorRecommendations["ECONOMIC"] = "recommendation-economic";
    AdvisorRecommendations["MILITARY"] = "recommendation-military";
    AdvisorRecommendations["SCIENTIFIC"] = "recommendation-scientific";
})(AdvisorRecommendations || (AdvisorRecommendations = {}));
export var AdvisorUtilities;
(function (AdvisorUtilities) {
    ...
    AdvisorUtilities.createAdvisorRecommendationTooltip = createAdvisorRecommendationTooltip;
    ...
    AdvisorUtilities.createAdvisorRecommendation = createAdvisorRecommendation;
    /**
    ...
    */
        ...
        for (const recommendation of recommendations) {
            ...
            if (definitionType == type) {
                ...
                const objectTypes = recommendation.whichAdvisors.map(type => {
                    let cssClass = AdvisorRecommendations.NO_ADVISOR;
                    switch (type) {
    ...
    AdvisorUtilities.getBuildRecommendationIcons = getBuildRecommendationIcons;
    /**
     * Helper function to get the icon CSS classes for advisor recommendation gems
     * @returns array with CSS classes for gems
    */
    ...
    AdvisorUtilities.getTreeRecommendationIcons = getTreeRecommendationIcons;
    AdvisorUtilities.getTreeRecommendations = (subject) => {
...
//# sourceMappingURL=file:///base-standard/ui/tutorial/tutorial-support.js.map
