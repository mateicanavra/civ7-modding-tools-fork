...
class GreatWorksModel {
    constructor() {
        ...
        this.greatWorkCreatedListener = () => { this.update(); };
        this.greatWorkArchivedListener = () => { this.update(); };
        this.greatWorkMovedListener = () => { this.update(); };
        this.cityProductionCompletedListener = () => { this.update(); };
        this.greatWorksHotkeyListener = () => { this.onGreatWorksHotkey(); };
        this.greatWorkSlotCreatedListener = () => { this.update(); };
        this.GreatWorks = [];
        this.isGreatWorksInit = false;
        this.YieldTotals = [];
        ...
        this.LatestGreatWorkDetails = null;
    ...
    handleYield(gwEntry, city, building, yieldType) {
        if (city.Constructibles) {
            ...
            // add the totals
            let yieldTotalItem = this.YieldTotals.find(item => item.type === yieldType);
            if (yieldTotalItem) {
    ...
    selectEmptySlot(cityID, buildingID) {
        if (this.hasSelectedGreatWork()) {
            if (this.selectedGreatWorkLocation != -1) {
                ...
                const result = Game.PlayerOperations.canStart(GameContext.localPlayerID, PlayerOperationTypes.MOVE_GREAT_WORK, args, false);
                if (result.Success) {
...
//# sourceMappingURL=file:///base-standard/ui/great-works/model-great-works.js.map
