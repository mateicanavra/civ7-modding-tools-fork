...
// Global variables
var g_numMajorPlayers: number = 0;
//var g_outgoingAge: number = 0;
var g_incomingAge: number = 0;


function requestInitializationParameters(initParams: AgeInitializationParameters) {
...
function generateTransition() {
	...
	var iRemovedResourcePlots: number[] = [];
	let aGeneratedResources: ResourceType[] = ResourceBuilder.getGeneratedMapResources();
	...
	for (let iPlayer: number = 0; iPlayer < g_numMajorPlayers; iPlayer++) {
		let regressedCities: ComponentID[] = regressCitiesToTowns(iPlayer);
		if (continuityMode) {
...
function addNewResources(iRemovedResourcePlots: number[], aGeneratedResources: ResourceType[]) {
...
function regressCitiesToTowns(iPlayer: number): ComponentID[] {
	const player = Players.get(iPlayer);
	const playerSettlements = player?.Cities?.getCityIds();
	let regressedCities: ComponentID[] = []
...
function changeCapitalCards(iPlayer: number) {
	const capitalOptions: number = 2;
	const player = Players.get(iPlayer);
	let playerSettlements = player?.Cities?.getCityIds();
	let currentPlayerCapitalName: string = "LOC_ERROR_NO_CAPITAL_NAME";
...
function positionUnits(iPlayer: number) {
	const player = Players.get(iPlayer);
	if (player != null) {
...
function positionArmyCommanders(iPlayer: number) {
	...
	if (player != null) {
		...
		if (playerSettlements != null && playerSettlements.length > 0) {
			...
			let cityCount: number = playerSettlements.length;
...
function positionFleetCommanders(iPlayer: number) {
...
function createUnitFromShadowAtLocation(player: PlayerLibrary, shadow: UnitShadow, location: PlotCoord): ComponentID | null {
...
function getNumDefenders(): number {
...
function capGold(iPlayer: number) {
	const player = Players.get(iPlayer);
...
function capInfluence(iPlayer: number) {
	const player = Players.get(iPlayer);
...
function generateDarkAgeCards(iPlayer: number) {
...
function generateRetainCityCards(iPlayer: number, aSettlements: ComponentID[]) {
	...
	if (player != null) {
		if (aSettlements.length > 0) {
			if (Game.age == Database.makeHash("AGE_EXPLORATION")) {
				...
				for (let i: number = 0; i < aSettlements.length; i++) {
					card.effects.push({
...
function generateDynamicVictoryCards(iPlayer: number) {
	const player = Players.get(iPlayer);
