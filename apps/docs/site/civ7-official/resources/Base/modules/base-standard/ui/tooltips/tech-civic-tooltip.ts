...
import TechTreeChooser, { TechChooserNode, TechTreeChooserModel } from '/base-standard/ui/tech-tree-chooser/model-tech-tree-chooser.js';
import CultureTreeChooser, { CultureChooserNode, CultureTreeChooserModel } from '/base-standard/ui/culture-tree-chooser/model-culture-tree-chooser.js';
import CultureTree, { CultureTreeModel } from '/base-standard/ui/culture-tree/model-culture-tree.js';
import { TreeGridCard, TreeNodesSupport } from '/base-standard/ui/tree-grid/tree-support.js';
import TechTree, { TechTreeModel } from '/base-standard/ui/tech-tree/model-tech-tree.js';
...
class TechCivicTooltipType implements TooltipType {

	private model: TechTreeChooserModel | CultureTreeChooserModel;
	private fragment = document.createDocumentFragment();
	private tooltip: ComponentRoot<Tooltip> | null = null;
	private hoveredNodeID: string | null = null;

	constructor(model: TechTreeChooserModel | CultureTreeChooserModel) {
		this.model = model;
	}
	...
	isUpdateNeeded(target: HTMLElement): boolean {
		const nodeIDString: string | null = RecursiveGetAttribute(target, "node-id") ?? "";
	...
	update() {
		...
		const node: TechChooserNode | CultureChooserNode | undefined = this.model.findNode(this.hoveredNodeID);
		if (!node) {
			console.error("tech-civic-tooltip: Attempting to update Tech/Civic info tooltip, but unable to get selected node");
			return;
		}
		...
		// TODO Add back when there an acceptable replacement for img-ornament-thin-center-hex
...
class TreeTooltipType implements TooltipType {

	private model: CultureTreeModel | TechTreeModel;
	private fragment = document.createDocumentFragment();
	private tooltip: ComponentRoot<Tooltip> | null = null;
	private hoveredNodeID: string | null = null;
	private level: number | null = null;

	constructor(model: CultureTreeModel | TechTreeModel) {
		this.model = model;
	}
	...
	isUpdateNeeded(target: HTMLElement): boolean {
		const nodeIDString: string | null = target.getAttribute("type");
		const level: string | null = target.getAttribute("level");
	...
	update() {
		...
		const node: TreeGridCard | undefined = this.model.findNode(this.hoveredNodeID);
		if (!node) {
			console.error("tech-tree-tooltip: Attempting to update Tech/Civic info tooltip, but unable to get selected node");
			return;
		}
