...
import PlotCursor, { PlotCursorUpdatedEvent, PlotCursorUpdatedEventName } from '/core/ui/input/plot-cursor.js';
...
class PanelUnitCombatPreview extends Component {
	...
	private queryCombatID: ComponentID = ComponentID.getInvalidID();
	private location: float2 | null = null;

	private selectedUnitID: ComponentID = ComponentID.getInvalidID();
	private targetID: ComponentID = ComponentID.getInvalidID();
	// If we add more target types swap this for an enum
	private isTargetDistrict: boolean = false;
	private isRangedAttacking: boolean = false;

	private attackerUnitIcon?: HTMLDivElement;
	private attackerHealth?: HTMLDivElement;
	private attackerSimulatedHealth?: HTMLDivElement;
	private attackerStatsString?: HTMLDivElement;
	private attackerStatsIcon?: HTMLImageElement;
	private attackerLeaderIcon?: HTMLElement;
	private attackerModifierContainer?: HTMLDivElement;
	private attackerModifierInfo?: HTMLDivElement;

	private targetUnitIcon?: HTMLDivElement;
	private targetHealth?: HTMLDivElement;
	private targetSimulatedHealth?: HTMLDivElement;
	private targetStatsString?: HTMLDivElement;
	private targetStatsIcon?: HTMLImageElement;
	private targetLeaderIcon?: HTMLElement;
	private targetModifierContainer?: HTMLDivElement;
	private targetModifierInfo?: HTMLDivElement;

	private outcomeLabel?: HTMLDivElement;
	private outcomeColorBottom?: HTMLDivElement;
	private outcomeColorTop?: HTMLDivElement;

	private readonly rangedAttackStartedEventListener = this.onRangedAttackStarted.bind(this);
	private readonly rangedAttackFinishedEventListener = this.onRangedAttackFinished.bind(this);

	onInitialize(): void {
		this.attackerUnitIcon = document.getElementById("attacker-unit-icon") as HTMLDivElement;
		this.attackerHealth = document.getElementById("attacker-health") as HTMLDivElement;
		this.attackerSimulatedHealth = document.getElementById("attacker-simulated-health") as HTMLDivElement;
		this.attackerStatsString = document.getElementById("attacker-stats-string") as HTMLDivElement;
		this.attackerStatsIcon = document.getElementById("attacker-stats-icon") as HTMLImageElement;
		this.attackerLeaderIcon = document.getElementById("attacker-leader-icon") as HTMLElement;
		this.attackerModifierContainer = document.getElementById("attacker-modifier-container") as HTMLDivElement;
		this.attackerModifierInfo = document.getElementById("attacker-modifier-info") as HTMLDivElement;

		this.targetUnitIcon = document.getElementById("target-unit-icon") as HTMLDivElement;
		this.targetHealth = document.getElementById("target-health") as HTMLDivElement;
		this.targetSimulatedHealth = document.getElementById("target-simulated-health") as HTMLDivElement;
		this.targetStatsString = document.getElementById("target-stats-string") as HTMLDivElement;
		this.targetStatsIcon = document.getElementById("target-stats-icon") as HTMLImageElement;
		this.targetLeaderIcon = document.getElementById("target-leader-icon") as HTMLElement;
		this.targetModifierContainer = document.getElementById("target-modifier-container") as HTMLDivElement;
		this.targetModifierInfo = document.getElementById("target-modifier-info") as HTMLDivElement;

		this.outcomeLabel = document.getElementById("outcome-label") as HTMLDivElement;
	...
	private onUpdate() {
		...
		const wasEnemyHovered: boolean = ComponentID.isValid(this.targetID);
		const previousTargetID: ComponentID = this.targetID;					// Save old value.
		this.targetID = this.getTargetAtCursor();								// Get latest target
		const isEnemyHovered: boolean = ComponentID.isValid(this.targetID);
	...
	private onPlotCursorCoordsUpdated(_event: PlotCursorUpdatedEvent) {
		this.onUpdate();
	}
	 ...
	 */
	private getTargetAtCursor(): ComponentID {
		...
		const plotLocation: float2 | null = PlotCursor.plotCursorCoords;
		if (plotLocation == null) {
			this.location = plotLocation;
			return ComponentID.getInvalidID();
		}

		const revealedState: RevealedStates = GameplayMap.getRevealedState(GameContext.localPlayerID, plotLocation.x, plotLocation.y);
		...
		if (this.location == null || this.location.x != plotLocation.x || this.location.y != plotLocation.y) {
			...
			const defendingDistrictID: ComponentID = Game.Combat.getDefensibleDistrict(this.location);
			if (ComponentID.isValid(defendingDistrictID)) {
				this.isTargetDistrict = true;
				return defendingDistrictID;
			}
			else {
				const defendingUnitID: ComponentID = Game.Combat.getBestDefender(this.location, this.selectedUnitID);
	...
	private realizeCombatPreview() {
		const attackingUnit: Unit | null = Units.get(this.selectedUnitID!);
		if (attackingUnit == null) {
			console.error("panel-unit-combat-preview: unable to get unit with selectedUnitID during realizeCombatPreview()");
			return;
		}

		const attackingUnitCombat: UnitCombat | undefined = attackingUnit.Combat;
	...
	private onSimulateCombatResult(results: SimulateAttackResults & GameQueryResults) {
		...
		const attackingUnit: Unit | null = Units.get(results.Attacker.ID);
		if (attackingUnit == null) {
		...
		let targetData: TargetPreviewData = {
			name: "",
			maxHitPoints: -1,
			currentHealth: 1,
			damage: -1,
		...
		const targetDistrict: District | null = Districts.get(results.Defender.ID);
		if (targetDistrict != null) {
			targetData.name = Locale.compose("LOC_UI_CITY_DISTRICT_NAME");
			targetData.owner = targetDistrict.owner;
			const districtHealth: number | undefined = Players.get(targetDistrict.owner)?.Districts?.getDistrictHealth(results.Location);
			if (districtHealth) {
			...
			const targetUnit: Unit | null = Units.get(results.Defender.ID);
			if (targetUnit != null) {
		...
		const combatType: CombatTypes | null = results.CombatType;

		//======================
		//Attacking Unit Stats
		//======================

		const attackerStrengthType: CombatStrengthTypes | null = results.Attacker.CombatStrengthType;
		...
		const attackingUnitHealth: UnitHealth | undefined = attackingUnit.Health;
		if (!attackingUnitHealth) {
			console.error("panel-unit-combat-preview, realizeCombatPreview(): unable to get attackingUnitHealth");
			return;
		}
		const attackerHealthPercentage: number = ((attackingUnitHealth.maxDamage - attackingUnitHealth.damage) / attackingUnitHealth.maxDamage) * 100;
		this.attackerHealth!.style.height = attackerHealthPercentage.toString() + "%";

		//Health after combat
		let attackerSimulatedHealthPercentage: number = ((attackingUnitHealth.maxDamage - attackingUnitHealth.damage - results.Attacker.DamageTo) / attackingUnitHealth.maxDamage) * 100;
		...
		if (attackingPlayer) {
			const leader: LeaderDefinition | null = GameInfo.Leaders.lookup(attackingPlayer.leaderType);
			const attackerLeaderIconSrc: string = Icon.getLeaderPortraitIcon(attackingPlayer.leaderType, 32);
			if (attackerLeaderIconSrc) {
				this.attackerLeaderIcon!.setAttribute("data-icon-id", leader!.LeaderType);
			}
		}

		const attackerPrimaryColor: string = UI.Player.getPrimaryColorValueAsString(attackingUnit.owner);
		const attackerSecondaryColor: string = UI.Player.getSecondaryColorValueAsString(attackingUnit.owner);
		...
		const defenderStrengthType: CombatStrengthTypes | null = results.Defender.CombatStrengthType;
		...
		const targetHealthPercentage: number = ((targetData.currentHealth) / targetData.maxHitPoints) * 100;
		this.targetHealth!.style.height = targetHealthPercentage.toString() + "%";

		let targetSimulatedHealthPercentage: number = ((targetData.currentHealth - targetData.damage) / targetData.maxHitPoints) * 100;
		...
		if (targetPlayer) {
			const leader: LeaderDefinition | null = GameInfo.Leaders.lookup(targetPlayer.leaderType);
			if (leader) {
				const targetLeaderIconSrc: string = UI.getIconURL(leader.LeaderType, "LEADER");
		...
		const targetPrimaryColor: string = UI.Player.getPrimaryColorValueAsString(targetData.owner);
		const targetSecondaryColor: string = UI.Player.getSecondaryColorValueAsString(targetData.owner);
		...
		let victoryType: string = "victory";
		if (combatType == CombatTypes.COMBAT_RANGED) {
			...
			if (results.Defender.DamageTo >= results.Defender.MaxHitPoints) {
				...
				const combatDifference: number = results.Defender.DamageTo - results.Attacker.DamageTo;
	...
	private updateCombatMods(combatantName: string, unitResults: SimulateAttackResults_CombatMember, modContainer?: HTMLDivElement, modInfo?: HTMLDivElement) {
		if (!modContainer) {
		...
		const unitNameText: HTMLDivElement = document.createElement("div");
		...
		//header divider
		const headerDivider: HTMLDivElement = document.createElement("div");
		...
		let baseStrengthType: string = "";
		const unitStrengthType: CombatStrengthTypes | null = unitResults.CombatStrengthType;
	...
	private addModifierText(modContainer: HTMLDivElement, modText: string) {
		const modifierText: HTMLDivElement = document.createElement("div");
	...
	private onRangedAttackStarted() {
		this.isRangedAttacking = true;
	}

	private onRangedAttackFinished() {
	...
	private onUnitSelectionChanged(data: UnitSelectionChangedData) {
		...
		let unit: Unit | null = Units.get(data.unit);
		if (unit) {
	...
	private onCombat(data: Combat_EventData) {
