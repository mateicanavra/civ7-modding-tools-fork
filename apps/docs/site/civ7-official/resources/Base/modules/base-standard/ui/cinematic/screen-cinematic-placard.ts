...
interface CinematicUIInfo {
	cinematicOverTitle: string;
	cinematicTitle: string;
	cinematicQuote: string;
	cinematicAuthor: string;
...
class ScreenCinematicPlacard extends Component {
	...
	private replayCinematicListener: EventListener = this.onReplayCinematic.bind(this);
	private closeCinematicListener: EventListener = this.onCloseCinematic.bind(this);
	...
	protected canReplay: boolean = true;
	protected hasQuote: boolean = true;
	protected canContinue: boolean = true;
	protected defaultTimeToWait: number = 30;

	// For signaling to the context manager if this is repeat request.
	private isReplaying: boolean = false;
	public setReplaying() { this.isReplaying = true; }
	public clearReplaying() { this.isReplaying = false; }
	...
	onAttach() {
		...
		const audio: string | null = CinematicManager.getCinematicAudio();
		if (audio) {
			UI.sendAudioEvent(audio);
		}

		const cinematicUIInfo: CinematicUIInfo = this.buildCinematicInfo();
	...
	private realizeCinematicUI(cinematicUIInfo: CinematicUIInfo) {

		const quoteTitleElement: HTMLElement = MustGetElement(".cinematic-moment_title-header", this.Root);
		quoteTitleElement.setAttribute("title", cinematicUIInfo.cinematicTitle);

		const quoteSubtitleElement: HTMLElement = MustGetElement(".cinematic-moment_title-subtitle", this.Root);
		quoteSubtitleElement.setAttribute("data-l10n-id", cinematicUIInfo.cinematicSubtitle);

		if (this.hasQuote) {
			const quoteTextElement: HTMLElement = MustGetElement(".cinematic-moment_quote-text", this.Root);
			quoteTextElement.setAttribute("data-l10n-id", cinematicUIInfo.cinematicQuote);

			const quoteAuthorElement: HTMLElement = MustGetElement(".cinematic-moment_quote-author-text", this.Root);
			quoteAuthorElement.setAttribute("title", cinematicUIInfo.cinematicAuthor);
		}
		else {
			const quoteContainerElement: HTMLElement = MustGetElement(".cinematic-moment_quote-container", this.Root);
			quoteContainerElement.classList.add("hidden");
		}

		const quoteOverTitleElement: HTMLElement = MustGetElement(".cinematic-moment_title-overtitle", this.Root);
		quoteOverTitleElement.setAttribute("data-l10n-id", cinematicUIInfo.cinematicOverTitle);

	}

	protected buildCinematicInfo() {
		return {
	...
	private attachFunctionButtons() {
		const cinematicMomentReplayButton: HTMLElement = MustGetElement(".cinematic-moment__replay-button", this.Root);
		cinematicMomentReplayButton.setAttribute("action-key", "inline-shell-action-1");

		const cinematicMomentCloseButton: HTMLElement = MustGetElement(".cinematic-moment__close-button", this.Root);
		...
		if (!this.canContinue) {
			...
			let timeToExit: number | null = Number(this.Root.getAttribute("autoCompleteDuration"));
			if (timeToExit == null) {
...
class ScreenWonderCompletePlacard extends ScreenCinematicPlacard {
	...
	protected buildCinematicInfo(): CinematicUIInfo {
		let quote: string = "";
		let title: string = "";
		let subtitle: string = "";
		let author: string = "";
		const overTitle: string = Locale.compose("LOC_UI_NEW_WONDER");

		const location: float2 = CinematicManager.getCinematicLocation();
		...
		for (let i: number = 0; i < constructibles.length; i++) {
			const instance: Constructible | null = Constructibles.getByComponentID(constructibles[i]);
			if (instance) {
...
class ScreenNaturalWonderRevealedPlacard extends ScreenCinematicPlacard {
	...
	protected buildCinematicInfo(): CinematicUIInfo {
		let quote: string = "";
		let title: string = "";
		const subtitle: string = Game.getTurnDate();
		let author: string = "";
		const overTitle: string = Locale.compose("LOC_UI_NEW_NATURAL_WONDER");
...
class ScreenVictoryPlacard extends ScreenCinematicPlacard {
	...
	protected buildCinematicInfo(): CinematicUIInfo {
		let title: string = "";
		let subtitle: string = Game.getTurnDate();

		const victoryType: string | null = this.Root.getAttribute("victoryType");
		if (!victoryType) {
...
class ScreenNaturalDisasterPlacard extends ScreenCinematicPlacard {
	...
	protected buildCinematicInfo(): CinematicUIInfo {
		...
		const location: float2 = CinematicManager.getCinematicLocation();
		const plotIndex: number = GameplayMap.getIndexFromLocation(location);
		const stormID: ComponentID | null = MapStorms.getActiveStormIDAtPlot(plotIndex);
		if (stormID) {
			...
			const stormDefinition: RandomEventDefinition | null = GameInfo.RandomEvents.lookup(stormInfo.type);
