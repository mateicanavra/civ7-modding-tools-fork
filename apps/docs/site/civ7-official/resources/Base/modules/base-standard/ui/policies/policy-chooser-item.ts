...
export enum PolicyChooserItemIcon {
	NONE,
	COMMUNISM,
	DEMOCRACY,
	FASCISM,
...
export interface PolicyChooserNode extends ChooserNode {
	description: string;
	isCrisis: boolean;
	isPlaceable: boolean;
	iconType: PolicyChooserItemIcon;
	isSelectable: boolean;
	traitType?: string;
}

export class PolicyChooserItem extends ChooserItem {
	public get policyChooserNode(): PolicyChooserNode | null {
		return this._chooserNode as PolicyChooserNode;
	}

	public set policyChooserNode(value: PolicyChooserNode | null) {
	...
	render() {
		...
		if (node.isPlaceable) {
			const normalClassToAdd: string = node.isCrisis ? "policy-chooser-element__crisis-placeable" : "policy-chooser-element__normal-placeable";
			...
			const glowClassToAdd: string = node.isCrisis ? "policy-chooser-element__crisis-placeable-glow" : "policy-chooser-element__normal-placeable-glow";
			...
			const normalClassToAdd: string = node.isCrisis ? "policy-chooser-element__crisis" : "policy-chooser-element__normal";
			normalState.classList.add(normalClassToAdd, "card-bg", "normal-bg", "absolute", "size-full", "pointer-events-none", "bg-center", "bg-no-repeat", "bg-cover");

			const glowClassToAdd: string = node.isCrisis ? "policy-chooser-element__crisis-glow" : "policy-chooser-element__normal-glow";
			glowState.classList.add(glowClassToAdd, "card-bg", "glow-bg", "absolute", "size-full", "pointer-events-none", "bg-center", "bg-no-repeat", "bg-cover");
		}
		...
		if (node.isCrisis) {
			...
			switch (node.iconType) {
				case PolicyChooserItemIcon.TRADITION:
					...
					const civSymbolStr: string = node.traitType.replace("TRAIT_", "civ_sym_").toLowerCase();
...
declare global {
	interface HTMLElementTagNameMap {
		'policy-chooser-item': ComponentRoot<PolicyChooserItem>
	}
}
