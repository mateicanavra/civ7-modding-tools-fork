...
const DEBUG_dummy_notifications = 0; // How many dummy notifications should we add in onAttach()?
...
(function (NotificationView) {
    class ViewItem extends NotificationModel.TypeEntry {
        constructor() {
            super(...arguments);
            this.root = null;
            this.button = { id: ComponentID.getInvalidID(), element: null };
            this.needsUpdate = false;
        }
    }
    NotificationView.ViewItem = ViewItem;
...
class PanelNotificationTrain extends Panel {
    constructor() {
        ...
        this.toRemoveIds = [];
        this.toAddIds = [];
        this.notificationList = document.createElement("fxs-vslot");
        this.viewItems = {};
        this.focusNotificationsListener = this.onFocusNotifications.bind(this);
        this.notificationEngineInputListener = this.onNotificationEngineInput.bind(this);
        this.notificationActivatedListener = this.onNotificationActivated.bind(this);
        this.actionActivateNotificationListener = this.onActionActivateNotification.bind(this);
        this.notificationAddedEventListener = this.onNotificationAdded.bind(this);
        this.notificationRemovedEventListener = this.onNotificationRemoved.bind(this);
        this.notificationHighlightEventListener = this.onNotificationHighlight.bind(this);
        this.notificationUnHighlightEventListener = this.onNotificationUnHighlight.bind(this);
        this.notificationRebuildEventListener = this.onNotificationRebuild.bind(this);
        this.notificationUpdateEventListener = this.onNotificationUpdate.bind(this);
        this.notificationHideEventListener = this.onNotificationHide.bind(this);
        this.dummyNotificationId = 0; // Internally used to avoid id conflicts on the dummy notifications
        this.navHelpContainer = null;
    ...
    createDebugNotificationItem(id, notificationContainer, viewItem) {
        ...
        return this.createNotificationItem(id, notificationContainer, (_) => {
    ...
    createViewItem(notificationType) {
        ...
        if (notificationContainerInfo.viewItem.notifications.length > 0) {
            ...
            const notificationButton = this.createNotificationItem(notificationID, notificationContainerInfo.notificationContainer, this.notificationActivatedListener, notificationContainerInfo.viewItem.notifications.length);
        ...
        if (!item.hasID(id)) {
            ...
            if (!item.button.element) {
                ...
                if (notificationContainer && notificationContainer.childElementCount == 0) {
                    // only add one button
                    const button = this.createNotificationItem(id, notificationContainer, this.actionActivateNotificationListener, instances);
                    if (button) {
    ...
    onDismissNotification(event) {
        ...
        if (event.target instanceof HTMLElement) {
            ...
            if (endTurnblockingType != EndTurnBlockingTypes.NONE) {
                const endTurnblockingNotificationId = Game.Notifications.findEndTurnBlocking(localPlayerID, endTurnblockingType);
    ...
    onClickUpArrow(item, notificationContainer) {
        ...
        for (let i = 0; i < notificationContainer.childElementCount; i++) {
            ...
            this.setButtonContents(notificationContainer.children[i], item.notifications[currentIndex], currentIndex);
        }
    }
...
//# sourceMappingURL=file:///base-standard/ui/notification-train/panel-notification-train.js.map
