...
class PanelReligionChooser extends Panel {
	...
	// header
	private religionHeader: HTMLDivElement | null = null;
	private chosenReligion: HTMLElement | null = null;
	private religionButtons: HTMLElement[] = [];
	private religionName: HTMLElement | null = null;
	private currentBeliefsContainer: HTMLElement | null = null;

	// options are the belief choices per belief class
	private beliefOptionVSlot: HTMLElement | null = null;
	private beliefOptionButtons: HTMLElement[] = [];
	private beliefOptionIndices: number[] = [];

	private enhancerTitle: HTMLElement | null = null;
	private enhancerAmount: HTMLElement | null = null;
	private enhancerButton: HTMLElement | null = null;
	private enhancerIcon: HTMLElement | null = null;
	private enhancerName: HTMLElement | null = null;
	private enhancerDescription: HTMLElement | null = null;
	private enhancerIndex: number = -1;

	private reliquaryTitle: HTMLElement | null = null;
	private reliquaryAmount: HTMLElement | null = null;
	private reliquaryButton: HTMLElement | null = null;
	private reliquaryIcon: HTMLElement | null = null;
	private reliquaryName: HTMLElement | null = null;
	private reliquaryDescription: HTMLElement | null = null;
	private reliquaryIndex: number = -1;

	private founderTitle: HTMLElement | null = null;
	private founderAmount: HTMLElement | null = null;
	private founderButtons: HTMLElement[] = [];
	private founderIcons: HTMLElement[] = [];
	private founderNames: HTMLElement[] = [];
	private founderDescriptions: HTMLElement[] = [];
	private founderIndices: number[] = [];

	// stats
	private citiesIcon: HTMLElement | null = null;
	private citiesValue: HTMLElement | null = null;
	private townsIcon: HTMLElement | null = null;
	private townsValue: HTMLElement | null = null;

	private players: PlayerId[] = [];
	private currentPlayer: PlayerLibrary | null = null;
	private currentActiveBeliefClass: string = "";
	...
	onAttach() {
		...
		this.religionHeader = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__religion-header");

		this.chosenReligion = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__chosen-religion");
		this.religionName = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__religion-title");

		const religionButtonContainer: HTMLElement = document.createElement("fxs-hslot");
		religionButtonContainer.classList.add("panel-religion-chooser__religion-buttons-container");

		// stats
		this.citiesIcon = this.Root.querySelector<HTMLDivElement>(".city-icon");
		this.citiesValue = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__cities-value");
		this.townsIcon = this.Root.querySelector<HTMLDivElement>(".town-icon");
		this.townsValue = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__towns-value");
		...
		this.enhancerTitle = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__enhancer-header-text");
		this.enhancerAmount = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__enhancer-header-assigned");
		this.enhancerButton = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__enhancer-button");
		this.enhancerName = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__enhancer-name");
		this.enhancerDescription = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__enhancer-description");
		this.enhancerIcon = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__enhancer-icon");

		this.reliquaryTitle = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__reliquary-header-text");
		this.reliquaryAmount = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__reliquary-header-assigned");
		this.reliquaryButton = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__reliquary-button");
		this.reliquaryName = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__reliquary-name");
		this.reliquaryDescription = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__reliquary-description");

		this.founderTitle = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__founder-header-text");
		this.founderAmount = this.Root.querySelector<HTMLDivElement>(".panel-religion-chooser__founder-header-assigned");

		// create founder buttons
		for (let index = 1; index <= 3; index++) {
			let founderButton: HTMLElement | null = this.Root.querySelector<HTMLElement>(`.panel-religion-chooser__founder-button-0${index}`);
			if (founderButton != null) {
				founderButton.setAttribute('tabindex', "-1");
				this.founderButtons.push(founderButton);
				founderButton?.addEventListener('action-activate', (event: CustomEvent) => { this.onBeliefButtonActivated(event) });
				founderButton?.addEventListener('focus', (event: FocusEvent) => { this.onButtonFocused(event) });
				founderButton?.addEventListener('blur', (event: FocusEvent) => { this.onButtonFocused(event) });
			}
			let founderIcon: HTMLElement | null = this.Root.querySelector<HTMLElement>(`.panel-religion-chooser__founder-icon-0${index}`);
			let founderName: HTMLElement | null = this.Root.querySelector<HTMLElement>(`.panel-religion-chooser__founder-name-0${index}`);
			let founderDescription: HTMLElement | null = this.Root.querySelector<HTMLElement>(`.panel-religion-chooser__founder-description-0${index}`);
			if (founderIcon) {
		...
		if (this.enhancerButton != null) {
			this.enhancerButton.addEventListener('action-activate', (event: CustomEvent) => { this.onBeliefButtonActivated(event) });
			this.enhancerButton.addEventListener('focus', (event: FocusEvent) => { this.onButtonFocused(event) });
			this.enhancerButton.addEventListener('blur', (event: FocusEvent) => { this.onButtonFocused(event) });
			this.enhancerButton.setAttribute('tabindex', "-1");
		}
		if (this.reliquaryButton != null) {
			this.reliquaryButton?.addEventListener('action-activate', (event: CustomEvent) => { this.onBeliefButtonActivated(event) });
			this.reliquaryButton?.addEventListener('focus', (event: FocusEvent) => { this.onButtonFocused(event) });
			this.reliquaryButton?.addEventListener('blur', (event: FocusEvent) => { this.onButtonFocused(event) });
	...
	private createReligionButton(buttonContainer: HTMLElement, religionData: ReligionDefinition | null) {
		if (religionData == null) {
			return;
		}
		//make a button
		const newbutton: HTMLElement = document.createElement("religion-button");
		...
		let buttonIconPath: string = `fs://game/unitflag_missingicon.png`;  			//`fs://game/${religionData.IconString}`;	// TODO: update when religion icons are on the cloud
		...
		newbutton.addEventListener('action-activate', (event: CustomEvent) => { this.onReligionButtonActivated(event) });
		newbutton.addEventListener('focus', (event: FocusEvent) => { this.onButtonFocused(event) });
		newbutton.addEventListener('blur', (event: FocusEvent) => { this.onButtonFocused(event) });


	}

	private updateReligionData(player: PlayerLibrary | null) {
		if (player == null) {
			return;
		}
		const currentReligion: PlayerReligion | null = Players.Religion?.get(player.id);
		...
		const chooseReligionText: string = currentReligion.getReligionName();
		if (this.religionName) {
			this.religionName.setAttribute('data-l10n-id', chooseReligionText);
		}
		if (this.chosenReligion) {
			let iconPath: string = `fs://game/${religionDef.IconString}`;
		...
		// stats
		{
			...
			let citiesInReligion: number = 0;
			let townsInReligion: number = 0;
		...
		const beliefs: BeliefType[] | null = currentReligion.getBeliefs();
		if (beliefs) {
	...
	private createBeliefOption(belief: BeliefDefinition) {
		if (belief == null) {
			return;
		}
		//make a button
		let buttonContainer = document.createElement("div");
		let newbutton: HTMLElement = document.createElement('belief-button');
		newbutton.setAttribute('tabindex', "-1");
		...
		let buttonIcon = document.createElement("fxs-icon");
		buttonIcon.classList.add("panel-religion-chooser__religion-button-icon");
		let buttonPath: string = `fs://game/unitflag_missingicon.png`;					// TODO: update when beliefs have icons
		...
		newbutton.addEventListener('action-activate', (event: CustomEvent) => { this.onBeliefOptionActivated(event) });
		newbutton.addEventListener('focus', (event: FocusEvent) => { this.onButtonFocused(event) });
		newbutton.addEventListener('blur', (event: FocusEvent) => { this.onButtonFocused(event) });
	}

	private onBeliefOptionActivated(event: ActionActivateEvent) {
	...
	private onBeliefButtonActivated(event: CustomEvent) {
		if (event.target instanceof HTMLElement) {
	...
	private onReligionButtonActivated(event: CustomEvent) {
		if (event.target instanceof HTMLElement) {
