...
class DiplomacyActionDetailsScreen extends Panel {
    constructor() {
        super(...arguments);
        this.supportChangedListener = (data) => { this.onSupportChanged(data); };
        this.engineInputListener = (inputEvent) => { this.onEngineInput(inputEvent); };
        this.yourWarContentContainer = null;
        this.supportWarContentContainer = null;
        this.actionContentColumn = null;
        this.titleProgressBarContainer = null;
        this.mainPledgeContainer = null;
        this.leftPledgeButton = null;
        this.rightPledgeButton = null;
        this.supportYourselfButton = null;
    ...
    createPledgeButton(isInitial) {
        ...
        buttonIdleOverlay.classList.add("panel-diplomacy-project-reaction__response-item-bg", "min-h-56", "pb-3", "grow", "flow-col", "justify-start", "absolute", "inset-0", "focus\\:opacity-0", "group-hover\\:opacity-0", "group-active\\:opacity-0", "opacity-100", "group", "-mt-0\\.5");
    ...
    createPledgeGroupLocalTotal(value) {
        let pledgeSlots = value;
        let remainingSlots = 20 - pledgeSlots;
    ...
    determinePledgeStatus(actionData, target) {
        ...
        if (actionData.completionScore > 0) {
            const completionData = Game.Diplomacy.getCompletionData(DiplomacyManager.selectedActionID);
            if (completionData) {
                ...
                const turnsToCompletion = Math.max(Math.ceil((actionData.completionScore - actionData.progressScore) / actionData.support), 1);
    ...
    createPledgeGroups(actionData, completionData, localPlayerDiplomacy) {
        const leftPlayer = this.determineLeftPlayer(actionData); // Pass these in?
        const rightPlayer = this.determineRightPlayer(actionData); // Pass these in?
        ...
        const rightPledgeRemainingSlotsContainer = MustGetElement(".remaining-slots-container", rightPledgeLocalTotal);
        ...
        leftPledgeTotals.forEach(total => {
            const pledgeLeaderIcon = this.createPledgeLeaderIcon(total.playerID, localPlayerDiplomacy);
            if (!pledgeLeaderIcon) {
                return;
            }
    ...
    populateActionDetails() {
        ...
        if (actionData.actionType == DiplomacyActionTypes.DIPLOMACY_ACTION_GIVE_INFLUENCE_TOKEN) {
            ...
            this.updatePlayerGroups(actionData, leftPlayer);
            this.leftPledgePlayers.forEach(playerID => {
                const pledgeLeaderIcon = this.createPledgeLeaderIcon(playerID, localPlayerDiplomacy);
                if (!pledgeLeaderIcon) {
                    return;
                }
        ...
        const projectData = Game.Diplomacy.getProjectDataForUI(actionData.initialPlayer, -1, DiplomacyActionTargetTypes.NO_DIPLOMACY_TARGET, DiplomacyActionGroups.NO_DIPLOMACY_ACTION_GROUP, -1, DiplomacyActionTargetTypes.NO_DIPLOMACY_TARGET).find(project => project.actionID == DiplomacyManager.selectedActionID);
    ...
    updateActionDetails() {
        ...
        if (this.yourWarContentContainer) {
            const oldSupportContainer = this.yourWarContentContainer.querySelector(".diplomacy-action-details__support-container");
    ...
    supportSide(initial) {
        ...
        const globalOpposeResults = Game.PlayerOperations.canStart(GameContext.localPlayerID, PlayerOperationTypes.SUPPORT_DIPLOMATIC_ACTION, globalOpposeArgs, false);
        if (Game.Diplomacy.isProjectCanceled(DiplomacyManager.selectedActionID)) {
            return false;
        }
        const actionData = Game.Diplomacy.getDiplomaticEventData(DiplomacyManager.selectedActionID);
    ...
    canSupportSide(initial) {
        ...
        const results = Game.PlayerOperations.canStart(GameContext.localPlayerID, PlayerOperationTypes.SUPPORT_DIPLOMATIC_ACTION, supportArgs, false);
    ...
    createPledgeLeaderIcon(playerID, localPlayerDiplomacy) {
    ...
    realizeFocus() {
        ...
        const availableElements = this.focusableElements.filter((element) => element?.getAttribute("has-diploaction-focus-priority") == "true");
...
//# sourceMappingURL=file:///base-standard/ui/diplomacy-action-details/screen-diplomacy-action-details.js.map
