...
import WorldAnchorTextManager, { AnchorTextInterface } from '/base-standard/ui/world-anchor-text/world-anchor-text-manager.js';
...
export class AnchorText extends Component implements AnchorTextInterface {
	...
	onAttach() {
		...
		const SECONDS_TO_DELAY_PER_MESSAGE: number = 6;
		const delay: string = (parseInt(this.Root.getAttribute('data-anchor-text-delay') ?? "0") * SECONDS_TO_DELAY_PER_MESSAGE).toString() + "s";
