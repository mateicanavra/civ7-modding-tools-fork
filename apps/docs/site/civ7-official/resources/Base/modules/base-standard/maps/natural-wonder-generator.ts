 ...
 */
export function addNaturalWonders(iWidth: number, iHeight: number, iNumNaturalWonders: number, wonderEventActive: boolean = false, requestedWonders: string[] = []) {
 ...
 * @param iNumNaturalWonders - the total number of natural wonders *requested*.  The function may not be able to place that many wonders
*/
function placeWonders(iWidth: number, iHeight: number, iNumNaturalWonders: number, wonderEventActive: boolean, requestedWonders: string[]) {
	...
	if (aPossibleWonders.length > 0) {
		...
		for (let iI = 0; iI < aPossibleWonders.length; iI++) {
			if (iPlacedWonders < iNumNaturalWonders) {
				let eFeature: FeatureType = aPossibleWonders[iI];
				...
				if (nwDef != null) {
					...
					if (nwDef.PlaceFirst == true) {
						...
						if (aPossibleLocations.length > 0) {
							...
							let placementLocation: float2 = GameplayMap.getLocationFromIndex(aPossibleLocations[randomIndex])
