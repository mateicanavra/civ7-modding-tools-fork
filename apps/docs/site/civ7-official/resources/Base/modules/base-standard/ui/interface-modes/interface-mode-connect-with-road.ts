/**
 * @file interface-mode-connect-with-road.ts
...
interface ConnectWithRoadInterfaceModeContext extends InterfaceMode.Context {
	UnitID: ComponentID;
	OperationArguments: any;
}
/**
 * Handler for INTERFACEMODE_CONNECT_WITH_ROAD.
 */
class ConnectWithRoadInterfaceMode extends ChoosePlotInterfaceMode {

	private _OperationResult: OperationResult | null = null;
	private validPlots = new Set<PlotIndex>();

	initialize() {
		const context = this.Context as ConnectWithRoadInterfaceModeContext;
		const args: any = {};
		const result = Game.UnitCommands.canStart(context.UnitID, 'UNITCOMMAND_CONNECT_WITH_ROAD', args, false);
	...
	commitPlot(plot: PlotCoord) {
		...
		args.X = plot.x;
		args.Y = plot.y;
		Game.UnitCommands.sendRequest(unitID, 'UNITCOMMAND_CONNECT_WITH_ROAD', args);
	}
}

InterfaceMode.addHandler('INTERFACEMODE_CONNECT_WITH_ROAD', new ConnectWithRoadInterfaceMode());