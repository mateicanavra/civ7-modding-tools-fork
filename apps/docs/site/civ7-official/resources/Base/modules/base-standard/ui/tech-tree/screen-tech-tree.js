...
class ScreenTechTree extends Panel {
    constructor(root) {
        ...
        this.viewTechProgressionTreeListener = this.onViewProgressionTree.bind(this);
    ...
    cleanPreviousSelectedNode() {
        if (this.previousSelectedNode != undefined) {
            const selectedElement = this.Root.querySelector(`tree-card[type="${this.previousSelectedNode}"]`);
            if (selectedElement) {
    ...
    refreshDetailsPanel(nodeId, level = "0") {
        ...
        const selectedElement = this.Root.querySelector(`tree-card[type="${this.selectedNode}"]`);
    ...
    onTargetTechlistItem() {
        if (this.selectedNode) {
            const nodeIndex = +(this.selectedNode);
            const args = { ProgressionTreeNodeType: nodeIndex };
            const result = Game.PlayerOperations.canStart(GameContext.localPlayerID, PlayerOperationTypes.SET_TECH_TREE_TARGET_NODE, args, false);
            if (result.Success) {
...
//# sourceMappingURL=file:///base-standard/ui/tech-tree/screen-tech-tree.js.map
