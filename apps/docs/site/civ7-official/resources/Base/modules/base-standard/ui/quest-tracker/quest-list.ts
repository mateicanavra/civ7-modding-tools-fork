...
import { LensActivationEvent, LensActivationEventName } from '/core/ui/lenses/lens-manager.js';
import { InputEngineEvent } from "/core/ui/input/input-support.js";

const getQuestDescription = (item: QuestItem) => {
...
export class QuestList extends Panel {
	...
	private questItemContainer!: HTMLElement;
	private questItemList!: HTMLElement;
	private questItemElements: HTMLElement[] = [];
	private questVisibilityToggle!: ComponentRoot<FxsMinusPlusButton>;
	private bgQuestext!: HTMLElement;
	private questVisibilityNavHelp!: HTMLElement;

	private drawerOut = !this.selectOneMode;
	private dirty = false;
	private listVisibilityToggleListener = this.listVisibilityToggle.bind(this);
	...
	private updateListener = this.updateQuestList.bind(this);
	private readonly activeLensChangedListener = this.onActiveLensChanged.bind(this);

	private visibleQuests: QuestItem[] = [];

	private get selectOneMode() {
	...
	private onActiveLensChanged(event: LensActivationEvent) {
	...
	private onActiveDeviceTypeChanged({ detail: { gamepadActive } }: ActiveDeviceTypeChangedEvent) {
	...
	private updateGate = new UpdateGate(() => {
		...
		this.visibleQuests = Array.from(questItems).filter(item => {
			...
			const isVisible: boolean = QuestTracker.isQuestVictoryInProgress(item.id);
			return isVisible;
		});
	...
	private onSelectQuest(event: ActionActivateEvent) {
		QuestTracker.selectedQuest = (event.target as HTMLElement)?.getAttribute("data-quest-id") ?? "";
