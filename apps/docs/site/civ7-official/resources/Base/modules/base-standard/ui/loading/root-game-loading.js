...
(function () {
    ...
    function updateCursorType(deviceType) {
    ...
    function realizeInfo() {
        ...
        const leaderName = playerConfig.leaderName ?? 'ERROR: Missing Leader Name';
        const civName = playerConfig.civilizationFullName ?? playerConfig.civilizationName ?? 'ERROR: Missing Civilizaiton Name';
        ...
        let civTrait = GameInfo.LegacyCivilizationTraits.lookup(civType);
        // Fill Data per User's Choices
        const leaderImageElement = document.querySelector(".root-game-loading__leaderPortrait");
        if (leaderImageElement) {
            const imagePath = leaderLoadingInfo?.LeaderImage;
            if (imagePath) {
                leaderImageElement.style.backgroundImage = `url(${imagePath})`;
            }
        }
        const backgroundImageElement = document.querySelector(".root-game-loading__scene-fg");
        if (backgroundImageElement) {
            const imagePath = window.innerWidth >= 1080 ? civLoadingInfo?.BackgroundImageHigh : civLoadingInfo?.BackgroundImageLow;
            if (imagePath) {
        ...
        const smallIconsBox = document.querySelector(".root-game-loading__small-screen-area");
        if (smallIconsBox) {
        ...
        if (uiFontScale >= FontScale.Medium) {
            ...
            if (uiFontScale >= FontScale.Large) {
                ...
                if (uiViewingExperience != UIViewExperience.Mobile) {
                    ...
                    const uniqueIconsArea = document.querySelector(".root-game-loading__unique-horizontal-flex-box");
                    if (uniqueIconsArea) {
        ...
        const smallLeaderIcon = document.querySelector(".root-game-loading-small-leader-icon");
        ...
        const smallCivIcon = document.querySelector(".root-game-loading-small-civ-icon");
        if (smallCivIcon && civLoadingInfo) {
            const civIconURL = "url('fs://game/civ_sym_" + civLoadingInfo.CivilizationType.slice(13).toLowerCase() + ".png')";
        ...
        GameInfo.Units.forEach((unit) => {
            ...
            if (civTrait.TraitType == unit.TraitType) {
                ...
                const baseDescription = Database.query('config', 'SELECT Description from CivilizationItems where Type=?', unit.UnitType);
                if (baseDescription && baseDescription.length > 0 && baseDescription[0].Description) {
                    unit.Description = baseDescription[0].Description;
                }
                uniqueUnits.push(unit);
            }
        });
        // Current design is to show one military and one non military unit, so lets grab the first of each
        let militaryUnitIndex = uniqueUnits.findIndex((unit => { return unit.CoreClass == "CORE_CLASS_MILITARY"; }));
        let nonmilitaryUnitIndex = uniqueUnits.findIndex((unit => { return unit.CoreClass != "CORE_CLASS_MILITARY"; }));
        ...
        if (uniquesContainer && smallScreenUniques) {
            ...
            for (const building of uniqueBuildings) {
                const constructible = GameInfo.Constructibles.lookup(building.ConstructibleType);
                if (constructible && uniqueCount < 4) {
                    addUniqueCallout(uniquesContainer, smallScreenUniques, constructible.Name, constructible.Description ? constructible.Description : "", UI.getIconCSS(constructible.ConstructibleType, "BUILDING"));
                    uniqueCount++;
                }
            }
            // improvements
            let uniqueImprovements = [];
            ...
            for (const improvement of uniqueImprovements) {
                const constructible = GameInfo.Constructibles.lookup(improvement.ConstructibleType);
                if (constructible && uniqueCount < 4) {
                    addUniqueCallout(uniquesContainer, smallScreenUniques, constructible.Name, constructible.Description ? constructible.Description : "", UI.getIconCSS(constructible.ConstructibleType, "IMPROVEMENT"));
                    uniqueCount++;
                }
            }
        }
    }
    function addUniqueCallout(parent, smallParent, name, description, iconURL) {
        const uiViewingExperience = UI.getViewExperience();
        const { uiFontScale } = Configuration.getUser();
    ...
    function showLoadingAnimation() {
        const animButtonContainer = document.getElementById('loading-screen__animation-container');
    ...
    function createStartButton(isFirstGame) {
        const startContainer = document.getElementById('start_game');
        if (!startContainer) {
            console.error("game-loading: Unable to find container for continue button.");
            return;
        }
        const fragment = document.createDocumentFragment();
        const continueButtonParent = document.createElement("fxs-activatable");
        ...
        const continueButtonOutline = document.createElement("div");
        ...
        const continueButtonHighlight = document.createElement("div");
        ...
        const continueButtonIcon = document.createElement("div");
        ...
        const loadingContainerElement = document.createElement("div");
        ...
        const loadingNavHelpIcon = document.createElement("fxs-nav-help");
        loadingNavHelpIcon.setAttribute('action-key', 'inline-accept');
        loadingContainerElement.appendChild(loadingNavHelpIcon);
        const startTextElement = document.createElement("div");
        startTextElement.classList.add("root-game-loading__loading-gif-text", "font-bold");
        const startButtonText = isFirstGame ? 'LOC_LOADING_BEGIN_GAME' : 'LOC_LOADING_CONTINUE_GAME';
    ...
    function handleUIGameLoadingProgressChanged(UIGameLoadingProgressState) {
            ...
            preloaded.then(() => {
                ...
                Loading.runWhenLoaded(() => {
                    ...
                    if (UI.getGameLoadingState() == UIGameLoadingState.WaitingForUIReady) {
                        const gameConfig = Configuration.getGame();
                        const isEraTransition = gameConfig.previousAgeCount > 0;
                        const isFirstGame = !gameConfig.isSavedGame || !isEraTransition;
...
//# sourceMappingURL=file:///base-standard/ui/loading/root-game-loading.js.map
