...
export class TreeChooserItem extends FxsChooserItem {
	protected _treeChooserNode: TreeChooserNode | null = null;

	public get treeChooserNode(): TreeChooserNode | null {
		return this._treeChooserNode;
	}

	public set treeChooserNode(value: TreeChooserNode | null) {
	...
	render() {
		...
		const primaryIcon: HTMLElement = this.createChooserIcon(node.isLocked ? node.icon : node.primaryIcon, node.isLocked);
		...
		if (node.isLocked) {
			const reqStatuses: HTMLDivElement = document.createElement("div");
			...
			for (const unlockDepth of node.unlocksByDepth) {
				const unlockEle: HTMLDivElement = document.createElement("div");
				...
				for (const unlockItem of unlockDepth.unlocks) {
					const unlockItemEle: HTMLDivElement = document.createElement("div");
					unlockItemEle.classList.add("flex", 'items-stretch', "w-9", "h-9", "mr-1\\.5", "mb-2.\\5");

					const unlockItemIconImg: HTMLImageElement = document.createElement("img");
		...
		let turns: HTMLDivElement | null = null;
		let percentBarContainer: HTMLDivElement | null = null;
		...
		if (node.isLocked) {
			...
			const percentLabel: HTMLDivElement = document.createElement("div");
			percentLabel.classList.add("font-title-xs");
			percentLabel.innerHTML = node.percentCompleteLabel;

			const percentBar: HTMLDivElement = document.createElement("div");
			percentBar.classList.add("bg-black", "relative", "w-11", "h-2");

			const percentBarFill: HTMLDivElement = document.createElement("div");
			...
			const turnsClock: HTMLDivElement = document.createElement("div");
			...
			const turnLabel: HTMLDivElement = document.createElement("div");
