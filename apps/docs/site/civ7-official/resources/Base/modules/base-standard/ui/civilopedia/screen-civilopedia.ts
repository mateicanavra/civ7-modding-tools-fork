...
import FxsTextbox from '/core/ui/components/fxs-textbox.js';
...
const PediaSearchCloseEventName: string = "pedia-search-close";
class PediaSearchCloseEvent extends CustomEvent<never> {
	constructor() {
...
class ScreenCivilopedia extends Panel {
	...
	private PediaSearchCloseListener = this.onPediaSearchClose.bind(this);
	private navComponent: PediaNavigation | null = null;
	private sideMenuComponent: PediaPageList | null = null;
	private searchBar: FxsTextbox | null = null;
	...
	onAttach() {
		...
		const mainScrollable = this.getComponentRoot<ComponentRoot<FxsScrollable>>(".content-main-scroll").component;
		mainScrollable.setEngineInputProxy(this.Root);

		// get some components 
		this.navComponent = this.getComponentRoot<ComponentRoot<PediaNavigation>>(".pedia-navigation").component;
		this.sideMenuComponent = this.getComponentRoot<ComponentRoot<PediaPageList>>(".pedia-page-list").component;
		this.searchBar = this.getComponentRoot<ComponentRoot<FxsTextbox>>(".pedia-search__textbox").component;
	...
	onPediaSearchClose(_event: PediaSearchCloseEvent) {
		this.realizeFocus();
	}
	...
	private getComponentRoot<T>(componentName: string): T {
		const componentElement: HTMLElement = MustGetElement(componentName, document);
	...
	private onEngineInput(inputEvent: InputEngineEvent) {
		...
		let handledInput: boolean = false;
...
class PediaBreadCrumbs extends Component {
	private boundOnNavigate = () => { this.onNavigate(); }
	private scrollReadyListener: EventListener = this.onScrollReady.bind(this);
	private crumbsContainer!: HTMLElement;
	private crumbClicked: boolean = false;
	private elementToScrollTo: HTMLElement | null = null;
	...
	private onScrollReady(event: Event) {
		if (this.elementToScrollTo) {
			const scrollComponentRoot = event.target as ComponentRoot<FxsScrollableHorizontal>;
	...
	private refresh() {
		...
		for (const [index, historyPage] of Civilopedia.instance.history.entries()) {
			const historyPageDetails: Civilopedia.PageDetails | null = Civilopedia.instance.getPage(historyPage.sectionID, historyPage.pageID);
			...
			const historyPageTitle: string = historyPageDetails.nameKey;
			if (!historyPageTitle) {
			...
			const indexInHistory: number = index;
...
class PediaSearch extends Component {
	private textInputListener = this.onTextInput.bind(this);
	private focusInListener = this.onTextEngineInput.bind(this);
	private validateVirtualKeyboardListener = this.onTextInput.bind(this);
	private onEngineInputListener = this.onEngineInput.bind(this);

	private textInput!: HTMLElement;
	private resultsContainer!: HTMLElement;
	private resultsScrollableContainer!: HTMLElement;
...
class PediaNavigation extends Component {
	private boundOnNavigate = (page: Civilopedia.Page) => { this.onNavigatePage(page); }
	private boundOnNavigateForward = () => { this.onNavigateForward(); }
	private boundOnNavigateHome = () => { this.onNavigateHome(); }
	private boundOnNavigateBack = () => { this.onNavigateBack(); }
	private activeDeviceChangedListener = this.onActiveDeviceChange.bind(this);

	private backContainer!: HTMLElement;
	private forwardContainer!: HTMLElement;
	private backArrow!: HTMLElement;
	private homeElement!: HTMLElement;
	private forwardArrow!: HTMLElement;
	...
	onNavigatePage(_page: Civilopedia.Page) {
		this.toggleNavArrows();
	}
	private toggleNavArrows() {
...
class PediaSectionList extends Component {
	...
	private onSectionTabSelectedListener = this.onTabSelected.bind(this);
	...
	private onTabSelected(e: TabSelectedEvent) {
		const newSectionID: string = this.allSectionIds[e.detail.index];
...
class PediaPageList extends Component {
	...
	private navigateListener = (page: Civilopedia.Page) => { this.onNavigate(page); }

	private pageTabs: Map<string, HTMLElement> = new Map<string, HTMLElement>();
	private currentSectionID: string | null = null;
	private currentPageID: string | null = null;

	private pageItemContainer!: HTMLElement;
	...
	onNavigate(page: Civilopedia.Page) {
	...
	onNavigateToElement(element: HTMLElement) {
	...
	onArticleUp() {
		...
		if (selection) {
			const divider = selection.previousElementSibling as HTMLElement | null;
			if (divider) {
				const prevSelection = divider.previousElementSibling as HTMLElement | null;
				if (prevSelection) {
	...
	onArticleDown() {
		const selection = this.Root.querySelector(".page-item-selected");
		if (selection) {
			const divider = selection.nextElementSibling as HTMLElement | null;
			if (divider) {
				const nextSelection = divider.nextElementSibling as HTMLElement | null;
				if (nextSelection) {
...
class PediaPageGroup extends ChooserItem {
	...
	private collapseIcon!: HTMLDivElement;
...
class PediaMainContent extends Component {
	...
	private mainTextContainer!: HTMLElement;
	...
	refresh() {
		...
		const currentPage: Civilopedia.Page = Civilopedia.instance.currentPage;
		const currentPageDetails: Civilopedia.PageDetails | null = Civilopedia.instance.getPage(currentPage.sectionID, currentPage.pageID);
...
function sortCivilopediaSidebarPanels(a: CivilopediaPageSidebarPanelDefinition, b: CivilopediaPageSidebarPanelDefinition): number {
	return a.SortIndex - b.SortIndex;
}

class PediaSidebarContent extends Component {

	private boundRefresh = this.doRefresh.bind(this);
	...
	onAttach(): void {
		...
		let panels: CivilopediaPageSidebarPanelDefinition[] = [];
...
class PediaChapter extends Component {
	...
	onAttach(): void {
		...
		if (sectionID == "CIVILIZATIONS") {
			const civTrait = Database.query('config', 'select * from CivilizationItems order by SortIndex')?.find(item => item.CivilizationType == pageID && item.Kind as string == "KIND_TRAIT");

			const traitHeader = document.createElement('fxs-header');
			traitHeader.classList.value = 'font-title text-lg text-secondary';
			traitHeader.setAttribute('title', civTrait?.Name as string);
			...
			traitText.innerHTML = Locale.stylize(civTrait?.Description as string);
			this.Root.appendChild(traitText);
		} else if (sectionID == "LEADERS") {
			const leaderTrait = Database.query('config', 'select * from LeaderItems order by SortIndex')?.find(item => item.LeaderType == pageID && item.Kind as string == "KIND_TRAIT");

			const traitHeader = document.createElement('fxs-header');
			traitHeader.classList.value = 'font-title text-lg text-secondary';
			traitHeader.setAttribute('title', leaderTrait?.Name as string);
			traitHeader.setAttribute('filigree-style', 'small');
			this.Root.appendChild(traitHeader);
			...
			traitText.role = "paragraph";
			traitText.classList.value = 'px-3 m-3 pb-3 font-body text-base pointer-events-auto';
			traitText.innerHTML = Locale.stylize(leaderTrait?.Description as string);
			this.Root.appendChild(traitText);
		}
