...
// Only static type imports are allowed here
import type { InputEngineEvent, IEngineInputHandler, NavigateInputEvent } from '/core/ui/input/input-support.js';
import type { DialogBoxCallbackSignature, DialogBoxID } from '/core/ui/dialog-box/manager-dialog-box.js';
 ...
 */
class LoadingInputHandler implements IEngineInputHandler {
	private loadScreen: HTMLElement | null = null;
...
const loadingInputHandler: LoadingInputHandler = new LoadingInputHandler();
...
const dialogExitId: DialogBoxID = displayRequestUniqueId();
let isClosingDialogOpen = false;

function openCivilopedia(searchTerm: string | undefined) {
	if (searchTerm) {
...
const userRequestCloseListener = () => {
	...
	const dbCallback: DialogBoxCallbackSignature = (eAction) => {
...
const rootElement: HTMLElement | null = document.querySelector<HTMLElement>(":root");
if (rootElement) {
...
if (Network.supportsSSO()) {
	const tooltips = MustGetElement<HTMLElement>("#tooltips", document.body);
...
new Promise<void>((resolve, reject) => {
	const checkDOMReady = () => {
		...
		requestAnimationFrame(() => {
			const keyElement: HTMLElement | null = document.querySelector<HTMLElement>("#world");
...
const onPullCurtain = () => {
	...
	if (curtain) {
		...
		curtain.addEventListener("animationend", (event: AnimationEvent) => {
