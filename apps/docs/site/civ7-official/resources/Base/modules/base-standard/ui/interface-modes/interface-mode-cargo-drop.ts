...
/**
 * Handler for INTERFACEMODE_CARGO_DROP.
 */
class AerialReconInterfaceMode extends ChoosePlotInterfaceMode {
	...
	initialize() {
		...
		const args: any = {};
		const result = Game.UnitOperations.canStart(context.UnitID, 'UNITOPERATION_CARGO_DROP', args, false);
	...
	commitPlot(plot: PlotCoord) {
		...
		args.X = plot.x;
		args.Y = plot.y;
		Game.UnitOperations.sendRequest(unitID, 'UNITOPERATION_CARGO_DROP', args);
	}
}

InterfaceMode.addHandler('INTERFACEMODE_CARGO_DROP', new AerialReconInterfaceMode());