...
class ScreenCultureTreeChooser extends Panel {
	...
	private cultureItemListener: EventListener = (event: CustomEvent) => { this.onActivateCultureListItem(event); }
	private treeRevealItemListener: EventListener = (event: CustomEvent) => { this.onTreeProgressListItem(event); }
	private expandCollapseListener: EventListener = this.onToggleExpandCollapse.bind(this);
	...
	private availableTreeTypes?: string;

	private cultureList: HTMLDivElement | null = null;
	private currentResearchDivider: HTMLDivElement | null = null;
	private headerAllAvailable: HTMLElement | null = null;
	private headerTreeProgressContainer: HTMLElement | null = null;
	...
	render() {
		...
		this.cultureList = MustGetElement<HTMLDivElement>('.culture-tree-list', this.Root);
	...
	onReceiveFocus() {
		...
		const cultureList: HTMLDivElement = MustGetElement('.culture-tree-list', this.Root);
	...
	private createCultureItem(cultureItem: ComponentRoot<TreeChooserItem>, node: TreeChooserNode) {
	...
	private openGridView() {
		...
		if (!this.availableTreeTypes) {
			...
			if (allTreeTypes) {
				const allTreeTypesCSV: string = allTreeTypes.join(',');
	...
	private onActivateCultureListItem(event: CustomEvent) {
	...
	private onTreeProgressListItem(event: CustomEvent) {
		if (event.target instanceof HTMLElement) {
