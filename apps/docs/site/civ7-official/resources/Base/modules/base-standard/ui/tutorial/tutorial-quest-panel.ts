...
import TutorialItem, { TutorialAnchorPosition, TutorialQuestPanelDefinition, TutorialQuestPanelOptionDefinition } from '/base-standard/ui/tutorial/tutorial-item.js';
...
import { DialogBoxAction, DialogBoxCallbackSignature, DialogBoxOption } from '/core/ui/dialog-box/manager-dialog-box.js';
...
class TutorialQuestPanel extends Component {
	...
	private data!: TutorialQuestPanelDefinition;
	private itemID: string = "";
	private isClosed: boolean = true; 		// Is this in a closed stated?
	private nextID?: string;
	private selectedAdvisorQuestPath: AdvisorType = AdvisorTypes.NO_ADVISOR;
	...
	onAttach(): void {
		...
		if (this.data.title) {
			...
			const title: string = Locale.compose(this.data.title);
	...
	onReceiveFocus() {
		...
		const advisorButton: HTMLElement | null = this.Root.querySelector('.quest-advisor-button');
		if (advisorButton) {
			Focus.setContextAwareFocus(advisorButton, this.Root);
		}
		else {
			const exitButton: HTMLElement = MustGetElement('.quest-advisor__emergency-exit', this.Root);
	...
	private getContentData() {
		const item: TutorialItem | undefined = TutorialManager.getCalloutItem(this.itemID);
		...
		const questPanelDefine: TutorialQuestPanelDefinition | undefined = item.questPanel;
		if (!questPanelDefine) {
		...
		let params: LocalizedTextArgument[] = [];
		let content: string = "";

		const hasAdvisors: boolean = this.data.advisors.length > 0;
		if (hasAdvisors) {
		...
		let prompts: LocalizedTextArgument[] = [];
	...
	private setAdvisors() {
		...
		const advisorsContainer: HTMLElement = MustGetElement('.tutorial-quest-panel-advisors', this.Root);
		...
		for (let advisor of this.data.advisors) {
			const advisorElement: HTMLElement = document.createElement('fxs-vslot');
			...
			const advisorPicContainer: HTMLElement = document.createElement('div');
			advisorPicContainer.classList.add("relative", "flex", "self-center", "pb-2");

			const advisorBg: HTMLElement = document.createElement('div');
			...
			const advisorImage: HTMLElement = document.createElement('div');
			...
			const advisorTypeIconBg: HTMLElement = document.createElement('div');
			...
			const advisorTypeIcon: HTMLElement = document.createElement('div');
			...
			const advisorQuoteContainer: HTMLElement = document.createElement('div');
			advisorQuoteContainer.classList.add("relative", "my-2", "mx-4");

			const advisorQuoteBackground: HTMLElement = document.createElement('fxs-inner-frame');
			advisorQuoteBackground.classList.add("quest-advisor-quote-background", "absolute", "size-full");

			const advisorQuote: HTMLElement = document.createElement('div');
			...
			const optionDef: TutorialQuestPanelOptionDefinition = advisor.button;
			const caption: string | undefined = Locale.compose(optionDef.text);
			if (caption == undefined || caption == null) {
				console.error("tutorial-callout: setupOption(): Missing caption");
				continue;
			}

			const advisorButton: ComponentRoot<FxsButton> = document.createElement('fxs-button');
		...
		const advisorButtons: HTMLElement = MustGetElement('.tutorial-quest-panel-advisors', this.Root);
		FocusManager.setFocus(advisorButtons);
	}

	/// Helper
	private setHTMLInDivClass(innerHTML: string, cssClassName: string) {
	...
	private onAdvisorButtonSelected = (pathName: string, pathDescription: string) => {
		...
		const actionCallback = (action: DialogBoxAction) => {
