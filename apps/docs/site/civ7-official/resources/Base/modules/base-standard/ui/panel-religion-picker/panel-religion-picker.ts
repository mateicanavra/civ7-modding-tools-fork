...
import { TextBoxTextEditStopEvent, TextBoxTextEditStopEventName } from '/core/ui/components/fxs-textbox.js';


class ScreenReligionPicker extends ScreenGeneralChooser {

	private religionConfirmButtonListener = this.onConfirm.bind(this);
	private religionNameActivatedListener = this.onReligionNameActivated.bind(this);
	private onReligionNameTextEntryStoppedListener = this.onReligionNameTextEntryStopped.bind(this);

	private religionConfirmButton!: HTMLElement;
	private religionNameRegularContainer!: HTMLElement;
	private religionInfoNameContainer!: HTMLElement;
	private religionInfoNameTextBox!: HTMLElement;
	private religionInfoNameEditButton!: HTMLElement;

	private playerObject!: PlayerLibrary;

	private selectedReligionType: string = "";
	private selectedReligionEntry: HTMLElement | null = null;
	private hasSelectedCustomReligion: boolean = false;
	...
	onAttach() {
		...
		const religionSubsystemFrame: HTMLElement = MustGetElement(".religion-picker_religion-window", this.Root);
	...
	onReceiveFocus() {
		...
		const focusElement: HTMLElement = MustGetElement('.religion-picker_religions', this.Root);
	...
	private onReligionNameActivated() {
		if (this.hasSelectedCustomReligion && this.religionInfoNameTextBox.getAttribute("enabled") == "false") {
			this.toggleEditReligionName();
		}
	}

	private toggleEditReligionName() {
		const inputEnabled: boolean = this.religionInfoNameTextBox.getAttribute("enabled") == "true";
	...
	protected createReligionEntries(entryContainer: HTMLElement) {
		...
		for (const religion of GameInfo.Religions) {
			...
			const iconLink: string = UI.getIconCSS(religion.ReligionType, "RELIGION_DECO");
			...
			const religionAlreadyTaken: boolean = Game.Religion.hasBeenFounded(religion.ReligionType);
	...
	entrySelected(entryElement: HTMLElement) {
		...
		const religionName: string | null = entryElement.getAttribute("religion-name");
		if (!religionName) {
			console.error("panel-religion-picker: entrySelected - religion attribute religion-name not found.");
			return;
		}

		const religionType: string | null = entryElement.getAttribute("religion-type");
		if (!religionType) {
			console.error("panel-religion-picker: entrySelected - religion attribute religion-type not found.");
			return;
		}

		const religionIconLink: string | null = entryElement.getAttribute("religion-icon");
		if (!religionIconLink) {
			console.error("panel-religion-picker: entrySelected - religion attribute religion-icon not found.");
			return;
		}

		const religionIsCustom: string | null = entryElement.getAttribute("religion-is-custom");
		if (!religionIsCustom) {
			console.error("panel-religion-picker: entrySelected - religion attribute religion-is-custom not found.");
			return;
		}

		const religionIcon: HTMLElement = MustGetElement('.religion-picker_religion-info-icon-container_religion', this.Root);
		...
		const religionIconGlow: HTMLElement = MustGetElement(".religion-picker_religion-info-icon-container_glow", this.Root);
	...
	private onConfirm() {
		const customReligionName: string | null = this.hasSelectedCustomReligion ? this.religionInfoNameTextBox.getAttribute("value") : null;
	...
	private buildReligionContainer() {
		...
		const religionInfoFounderName: HTMLElement = MustGetElement('.religion-picker_religion-info-founder-name', this.Root);
		...
		const religioninfoHolyCityName: HTMLElement = MustGetElement('.religion-picker_religion-info-city-name', this.Root);
		...
		const religionsContainer: HTMLElement = MustGetElement('.religion-picker_religions', this.Root);
	...
	private onReligionNameTextEntryStopped(event: TextBoxTextEditStopEvent) {
