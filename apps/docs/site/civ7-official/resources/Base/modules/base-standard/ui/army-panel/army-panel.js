...
var UnitArmyPanelState;
(function (UnitArmyPanelState) {
    UnitArmyPanelState[UnitArmyPanelState["HIDDEN"] = 0] = "HIDDEN";
    UnitArmyPanelState[UnitArmyPanelState["VISIBLE"] = 1] = "VISIBLE";
})(UnitArmyPanelState || (UnitArmyPanelState = {}));
;
;
...
class ArmyPanel extends Panel {
    ...
    constructor(root) {
        super(root);
        this.unitId = null;
        this._currentState = UnitArmyPanelState.HIDDEN;
        this.armyActions = [];
        this.armyActionContainer = null;
    ...
    setupArmy() {
        ...
        let currentArmy = CommanderInteractModel.currentArmyCommander?.packedUnits;
        ...
        const reinforcementItems = CommanderInteractModel.availableReinforcements;
    ...
    createArmyUnitButton(unitId) {
        ...
        if (ComponentID.isValid(unitId)) {
            let unitHealthbar = newUnitButton.querySelector(".army-panel__unit-health-bar-fill");
            let healthbarBg = newUnitButton.querySelector(".army-panel__unit-health-bar-bg");
            if (unit) {
    ...
    setUnitButtonData(button, unit) {
        if (button) {
            button.addEventListener('action-activate', (event) => { this.onArmyButtonActivated(event); });
            button.addEventListener('focus', (event) => { this.onButtonFocused(event); });
            button.addEventListener('blur', (event) => { this.onButtonFocused(event); });
            if (unit) {
                const reinforcementItems = CommanderInteractModel.availableReinforcements;
                reinforcementItems.forEach((item) => {
    ...
    setActionButtonData(button, action) {
        button.addEventListener('action-activate', (event) => { this.onArmyActionActivated(event); });
        button.addEventListener('focus', (event) => { this.onButtonFocused(event); });
        button.addEventListener('blur', (event) => { this.onButtonFocused(event); });
    ...
    onArmyButtonActivated(event) {
        if (event.target instanceof HTMLElement) {
            ...
            let selectedUnit = CommanderInteractModel.currentArmyCommander?.packedUnits[unitIndex];
    ...
    setupArmyActions(unit) {
        ...
        const processOperation = (operation) => {
            ...
            const enabled = Game.UnitOperations?.canStart(unit.id, operation.OperationType, parameters, false);
        ...
        const reinforceArmy = GameInfo.UnitOperations.lookup("UNITOPERATION_CALL_REINFORCEMENTS");
...
//# sourceMappingURL=file:///base-standard/ui/army-panel/army-panel.js.map
