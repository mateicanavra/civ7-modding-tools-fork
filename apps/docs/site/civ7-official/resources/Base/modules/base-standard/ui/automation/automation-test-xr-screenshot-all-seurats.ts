...
enum XRScreenshotAllSeuratsScene {
	MainMenu,
	Gameplay,
	//Diplomacy,
}

enum XRScreenshotAllSeuratsMenuState {
	Initialising,
	Teleporting,
	FreezingView,
	Screenshotting,
...
enum XRScreenshotAllSeuratsGameplayState {
	GameNotLoaded,
	GameLoaded,
	ChangingVista,
	FreezingView,
...
enum XRScreenshotAllSeuratsCivilisations {
	AKSUM,
	EGYPT,
	GREECE,
	HAN,
...
enum XRScreenshotAllSeuratsPopulations {
	Low,
	Medium,
	High,
}
...
export class AutomationTestXRScreenshotAllSeurats {
	...
	sceneIndex = 0;
	sceneState = XRScreenshotAllSeuratsScene.MainMenu;

	menuZoneIndex = 0;
	menuZones: string[] = new Array<string>;
	menuState: XRScreenshotAllSeuratsMenuState = XRScreenshotAllSeuratsMenuState.Initialising;

	gameplaySeuratIndex = 0;
	gameplaySeuratPopulation: XRScreenshotAllSeuratsPopulations = XRScreenshotAllSeuratsPopulations.Low;
	gameplayState = XRScreenshotAllSeuratsGameplayState.GameNotLoaded;

	orientationIndex = 0;
	readonly orientations = [
		{ x: 0, y: -90, z: -90 },	// Forward
		{ x: 90, y: -90, z: -90 },	// Left
		{ x: -90, y: -90, z: -90 },	// Right
		{ x: 180, y: -90, z: -90 },	// Behind
		{ x: 0, y: 0, z: -90 },		// Floor
		{ x: 0, y: -180, z: -90 }	// Ceiling
	];

	readonly pauseTimeAny = 0.5;
	readonly pauseTimeStartDelay = 1.0;
	readonly pauseTimeTeleport = 0.5;
	readonly pauseTimeFreeze = 0.15;
	readonly pauseTimeTakeScreenshot = 0.15;
	readonly pauseTimeGameplayTurnModal = 5.0;
	readonly pauseTimeChangeVista = 0.2;

	private automationTestXRScreenshotAllSeuratsListener: ArbitraryCallback = (command: string, ...args: any) => {
	...
	teleportToNextLocation() {
		...
		if (!result) AutomationSupport.FailTest("Attempted to teleport to zone that does not exist");
	}
