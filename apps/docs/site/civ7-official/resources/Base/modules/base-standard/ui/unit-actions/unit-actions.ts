...
import { UnitRenameConfirmEvent, UnitRenameConfirmEventName, UnitRenameHideStatusToggledEvent, UnitRenameHideStatusToggledEventName } from '/base-standard/ui/unit-rename/unit-rename.js';

enum UnitActionPanelState {
	HIDDEN,
	ANIMATEIN,
	ANIMATEOUT,
	VISIBLE
...
export enum UnitActionCategory {
	NONE,
	MAIN,
	COMMAND,
	HIDDEN,
...
class UnitActions extends Panel {
	...
	private isInArmy: boolean = false;
	private shelfButton!: HTMLElement;
	private animTimer: number = 0;
	public static readonly ANIM_DELAY: number = 150;

	/// Are events subscribed to?
	private isSubscribed: boolean = false;

	private _currentState: UnitActionPanelState = UnitActionPanelState.HIDDEN;
	private mainContainer: HTMLElement | null = null;
	private panelDecor: HTMLElement | null = null;

	// actions
	private actions: Array<UnitAction> = [];
	private standardContainer: HTMLElement | null = null;
	private commanderContainer: HTMLElement | null = null;
	private hiddenContainer!: HTMLElement;

	private standardActions: UnitAction[] = [];
	private commandActions: UnitAction[] = [];
	private hiddenActions: UnitAction[] = [];
	private standardActionElements: HTMLElement[] = [];
	private commandActionElements: HTMLElement[] = [];
	private hiddenActionElements: HTMLElement[] = [];

	// info
	private unitNameDiv: HTMLElement | null = null;
	private unitNameFiligreeContainer: HTMLElement | null = null;
	private commanderNameDiv: HTMLElement | null = null;
	private commanderBannerContainer: HTMLElement | null = null;
	private commanderLevelValue: HTMLElement | null = null;
	private commanderXPMeter: HTMLElement | null = null;
	private portraitContainer: HTMLElement | null = null;
	private portraitImage: HTMLElement | null = null;
	private identifierRow: HTMLElement | null = null;
	private healthValueDiv: HTMLElement | null = null;
	private unitHealthbar: HTMLElement | null = null;
	private moveValueDiv: HTMLElement | null = null;
	private unitStatsContainer: HTMLElement | null = null;
	private portraitStatsRow: HTMLElement | null = null;
	private statInstances: HTMLElement[] = [];
	private statDividers: HTMLElement[] = [];
	private commanderNameEditButton: HTMLElement | null = null;
	private commanderInfoContainer!: HTMLElement;
	private commanderRenameElement!: HTMLElement;
	...
	private unitCycleNavHelpContainer: HTMLElement | null = null;
	private unitNavHelpWorldAnchorHandle: number | null = null;
	private dialogId: DialogBoxID = displayRequestUniqueId();

	private unitReselectedListener = this.onUnitReselected.bind(this);
	private onGlobalShowListener = this.onGlobalShow.bind(this);
	private onGlobalHideListener = this.onGlobalHide.bind(this);
	private onViewReceiveFocusListener = this.onViewReceiveFocus.bind(this);
	private onUnitHotkeyListener = this.onUnitHotkey.bind(this);
	private onEngineInputListener = this.onEngineInput.bind(this);
	private onNavigateListener = this.onNavigateInput.bind(this);
	private shelfFocusoutListener = this.onShelfFocusout.bind(this);
	private shelfFocusInListener = this.onShelfFocused.bind(this);
	private shelfToggleListener = this.toggleShelfOpen.bind(this);
	private onCommanderEditPressedListener = this.onCommanderEditPressed.bind(this);
	private infoContainerEngineInputListener = this.onInfoContainerEngineInput.bind(this);
	private onCommanderNameConfirmedListener = this.onCommanderNameConfirmed.bind(this);
	private onCommanderNameHideStatusToggledListener = this.onCommanderNameHideStatusToggled.bind(this);


	private readonly raiseUnitSelectionListener = this.onRaiseUnitSelection.bind(this);
	private readonly lowerUnitSelectionListener = this.onLowerUnitSelection.bind(this);

	get isCyclable(): boolean {
		const player: PlayerLibrary | null = Players.get(GameContext.localPlayerID);
		const hasMoreThanOneUnit: boolean = player && player.Units ? player.Units.getUnits().length >= 2 : false;
	...
	private set currentState(_state: UnitActionPanelState) {
		switch (_state) {
			...
			case UnitActionPanelState.VISIBLE:
				...
				break;
			default:
				console.error(`Bad state enum of ${_state satisfies never} attempting to be set to unit-actions->currentState`);
				return;
		}
	...
	override onInitialize(): void {
		...
		const disableUnitActions: UIDebugWidgetDefinition = {
	...
	private shutdown() {
		...
		const unitComponentID: ComponentID = (UI.Player.getHeadSelectedUnit() ?? ComponentID.getInvalidID());
		this.onLowerUnitSelection(unitComponentID);
	}

	private onUnitOperationUpdated({ unit }: UnitOperation_EventData) {
	...
	private updateFocusGate = new UpdateGate(() => {
	...
	private onDebugWidgetUpdated(id: string, value: boolean) {
	...
	*/
	private onUnitOperationDeactivated({ unit, data1 }: UnitOperation_EventData) {
	...
	*/
	private onUnitOperationSegmentComplete({ unit, data1 }: UnitOperation_EventData) {
		const headUnit = UI.Player.getHeadSelectedUnit();
		if (!data1 && unit.owner == headUnit?.owner && unit.id == headUnit.id) {
			this.switchToDefault();
		}
	 ...
	 */
	private onRaiseUnitSelection(cid: ComponentID) {
	 ...
	 * Conforms to UnitSelectionListener
	 */
	private onLowerUnitSelection(cid: ComponentID) {
		if (this.isSubscribed) {
	...
	private onUnitReselected(event: CustomEvent) {
	...
	private setupUnitInfo() {
		if (this.unitId) {
			...
			if (unit) {
				...
				// Set unit name
				const unitName: string = ((unit) ? Locale.compose(unit.name) : "Unknown CID: " + ComponentID.toLogString(this.unitId));
				...
				if (!unit.isAerodromeCommander) {
					this.Root.querySelector<HTMLElement>('.unit-actions__shelf-container')?.classList.toggle("hidden", false);
					...
					this.Root.querySelector<HTMLElement>('.unit-actions__shelf-container')?.classList.toggle("hidden", true);
	...
	private addUnitCycleNavHelp() {
		...
		const unitsSamePosition: ComponentID[] = MapUnits.getUnits(unit.location.x, unit.location.y);
		const isMore2UnitsSamePositionOwned = unitsSamePosition.filter(u => u.owner == GameContext.localPlayerID).length >= 2;
		const districtIdSamePosition: ComponentID | null = MapCities.getDistrict(unit.location.x, unit.location.y);
	...
	private addStat(statValue: number, statName: string) {
	...
	private addStatDivider() {
		const statDiv: HTMLElement = document.createElement("div");
	...
	private removeAllChildren(container: HTMLElement | null) {
		if (container) {
	...
	private updateMovement(unitId: ComponentID) {
	...
	private updateHealth(unitId: ComponentID) {
		const unit: Unit | null = Units.get(unitId);
		if (unit) {
			if (!unit.isAerodromeCommander) {
				if (unit?.Health) {
	...
	private getUnitActions(unit: Unit) {
		...
		const processOperation = (operation: UnitOperationDefinition, unitAbility: UnitAbilityDefinition | null = null) => {
			...
			const exclusionResult: OperationResult = Game.UnitOperations?.canStart(unit.id, operation.OperationType, parameters, true);
			...
			if (exclusionResult.Success) {
				...
				let name: string = `[ERR] Ability (Operation) Unknown for ${operation.OperationType}`;
				...
				if (this.isTargetPlotOperation(operation.OperationType)) {
					...
					this.actions.push({
						...
						UICategory: this.getUnitActionCategory(operation.CategoryInUI),
						priority: operation.PriorityInUI ? operation.PriorityInUI : 0,
						callback: (location: PlotCoord | null) => {
							if (enabled.Success) {
								if (location) {
		...
		const processCommand = (command: UnitCommandDefinition, unitAbility: UnitAbilityDefinition | null = null) => {
			...
			let exclusionResult: OperationResult = Game.UnitCommands?.canStart(unit.id, command.CommandType, parameters, true);
			...
			if (exclusionResult.Success) {
				...
				let commandText: string = `[ERR] Command Unknown for type ${command.CommandType}`;
	...
	private setupButtonSections() {
		...
		this.Root.querySelector<HTMLElement>('.unit-actions__shelf-container')?.setAttribute('data-navrule-left', 'stop');
		this.Root.querySelector<HTMLElement>('.unit-actions__shelf-container')?.setAttribute('data-navrule-up', 'stop');
	...
	private createButtons(actions: UnitAction[]) {
		...
		for (const [index, action] of actions.entries()) {
			...
			if (actions == this.commandActions) {
				let buttonBackground: HTMLElement | null = newButton.querySelector<HTMLElement>(".unit-action-button__button-bg-container");
				let buttonHighlight: HTMLElement | null = newButton.querySelector<HTMLElement>(".unit-action-button__button-highlight");
				buttonBackground?.classList.add("isCommander");
				buttonHighlight?.classList.add("isCommander");
			}

			let newActivatable: HTMLElement | null = newButton.querySelector<HTMLElement>(".unit-actions__action-button");
	...
	private setButtonData(button: HTMLElement, action: UnitAction) {
		if (action && button) {
			...
			button.addEventListener('action-activate', (event: CustomEvent) => { this.onButtonActivated(event) });
		}
	}
	...
	private onButtonActivated(event: CustomEvent) {
		if (event.target instanceof HTMLElement) {
			...
			let selectedAction: UnitAction | null = this.findSelectedUnitAction(event.target);
	...
	private onShelfFocusout({ relatedTarget }: FocusEvent) {
	...
	private onActionChosen(action: UnitAction) {
		let doAction = () => {
		...
		if (action.confirmBody) {
			const confirmUnitActionTitleDefault: string = Locale.compose("LOC_UNIT_INTERACT_CONFIRM_ACTION_TITLE_DEFAULT");
	...
	private onCommanderEditPressed() {
		...
		const shouldHide: boolean = !(this.commanderRenameElement.getAttribute("active") == "true");
		this.commanderRenameElement.setAttribute("active", shouldHide.toString());
	}

	private onInfoContainerEngineInput(inputEvent: InputEngineEvent) {
	...
	private onCommanderNameConfirmed(event: UnitRenameConfirmEvent) {
		const newCommanderName: string = event.detail.newName;
		this.commanderRenameElement.setAttribute("active", "true");
		const args = {
	...
	private onCommanderNameHideStatusToggled(event: UnitRenameHideStatusToggledEvent) {
		if (event.detail.isHidden) {
			this.realizeFocus();
		}
	}

	private findSelectedUnitAction(selectedButton: HTMLElement): UnitAction | null {
		// figure out which button list the button is in so we can retrieve the associated action's data
		let selectedAction: UnitAction | null = null;
		let actionIndex: number = this.standardActionElements.indexOf(selectedButton);
		if (actionIndex >= 0) {
	...
	private getUnitActionCategory(category: string | undefined): UnitActionCategory {
		let mappedCategory: UnitActionCategory;
		switch (category) {
			case "MOVE":
			case "ACTIVE_PRIMARY":
...
export class UnitActionsPanelModelImpl {
	private _isShelfOpen = true;

	public get isShelfOpen() {
		return this._isShelfOpen;
	}

	public set isShelfOpen(value: boolean) {
