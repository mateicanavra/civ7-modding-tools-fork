...
import { ActionActivateEvent, FxsActivatable } from '/core/ui/components/fxs-activatable.js'
...
type YieldBarEntryType = 'YIELD_GOLD' | 'YIELD_CULTURE' | 'YIELD_SCIENCE' | 'YIELD_DIPLOMACY' | 'YIELD_HAPPINESS' | 'YIELD_CITIES';

const yieldBarEntryClassMap: {
	[key in YieldBarEntryType]: string
} = {
...
class YieldBarEntry extends FxsActivatable {
	private type: YieldBarEntryType | null = null;
	private value = 0;
	private stored: number | undefined;
	private max: number | undefined;

	private yieldIcon = document.createElement('img');
	private valueText = document.createElement('span');
	...
	onAttributeChanged(name: string, oldValue: string | null, newValue: string | null): void {
		switch (name) {
			case 'data-icon':
				this.type = newValue as YieldBarEntryType | null;
				if (this.type) {
					this.yieldIcon.style.backgroundImage = `url("${UI.getIconURL(this.type)}")`;
					if (oldValue) {
						this.Root.classList.remove(yieldBarEntryClassMap[oldValue as YieldBarEntryType]);
					}
...
export class PanelYieldBanner extends Panel {
	...
	private advancedStartEffectUsedListener = this.onAdvancedStartEffectUsed.bind(this);
	...
	private settlementCapElement = document.createElement('yield-bar-entry');
	...
	private onConstructableAddedToMap(_data: ConstructibleAddedToMap_EventData) {
		this.updateAll();
	}

	private onUnitRemovedFromMap = ({ unit }: Unit_EventData) => {
	...
	private render() {
		...
		const onYieldElementActivated = (_event: ActionActivateEvent) => {
