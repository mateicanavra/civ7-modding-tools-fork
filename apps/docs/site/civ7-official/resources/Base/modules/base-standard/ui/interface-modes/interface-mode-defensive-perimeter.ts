...
interface DefensivePerimeterInterfaceModeContext extends InterfaceMode.Context {
	UnitID: ComponentID;
...
/**
 * Handler for INTERFACEMODE_DEFENSIVE_PERIMETER.
 */
class DefensivePerimeterInterfaceMode extends ChoosePlotInterfaceMode {

	private _OperationResult: OperationResult | null = null;
	private validPlots = new Set<PlotIndex>();

	initialize() {
		const context = this.Context as DefensivePerimeterInterfaceModeContext;
		const args: any = {};
		const result = Game.UnitCommands.canStart(context.UnitID, 'UNITCOMMAND_DEFENSIVE_PERIMETER', args, false);
	...
	commitPlot(plot: PlotCoord) {
		...
		args.X = plot.x;
		args.Y = plot.y;
		Game.UnitCommands.sendRequest(unitID, 'UNITCOMMAND_DEFENSIVE_PERIMETER', args);
	}
}

InterfaceMode.addHandler('INTERFACEMODE_DEFENSIVE_PERIMETER', new DefensivePerimeterInterfaceMode());