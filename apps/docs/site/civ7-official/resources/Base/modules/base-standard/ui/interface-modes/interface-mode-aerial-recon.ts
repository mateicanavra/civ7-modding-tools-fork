...
interface AerialReconInterfaceModeContext extends InterfaceMode.Context {
	UnitID: ComponentID;
	OperationArguments: any;
}
/**
 * Handler for INTERFACEMODE_AERIAL_RECON.
 */
class AerialReconInterfaceMode extends ChoosePlotInterfaceMode {
	...
	initialize() {
		const context = this.Context as AerialReconInterfaceModeContext;
		const args: any = {};
		const result = Game.UnitOperations.canStart(context.UnitID, 'UNITOPERATION_AERIAL_RECON', args, false);
	...
	commitPlot(plot: PlotCoord) {
		...
		args.X = plot.x;
		args.Y = plot.y;
		Game.UnitOperations.sendRequest(unitID, 'UNITOPERATION_AERIAL_RECON', args);
	}
}

InterfaceMode.addHandler('INTERFACEMODE_AERIAL_RECON', new AerialReconInterfaceMode());