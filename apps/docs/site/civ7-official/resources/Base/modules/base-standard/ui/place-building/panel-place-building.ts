...
class PlaceBuildingPanel extends Panel {
	...
	private content: ComponentRoot<FxsScrollable> | null = null;
	private subsystemFrame = document.createElement("fxs-subsystem-frame");
	...
	override onReceiveFocus(): void {
		if (this.content) {
			...
			waitForLayout(() => {
				this.content = MustGetElement<ComponentRoot<FxsScrollable>>(".subsystem-frame__content", this.subsystemFrame);
				FocusManager.setFocus(this.content);
			});
		}
	}
	...
	protected requestClose() {
		const selectedCityID: ComponentID | null = InterfaceMode.getParameters().CityId != null ? InterfaceMode.getParameters().CityId : UI.Player.getHeadSelectedCity();	// May be null if placing results in deselecting city		
...
declare global {
	interface HTMLElementTagNameMap {
		'panel-place-building': ComponentRoot<PlaceBuildingPanel>;
	}
}