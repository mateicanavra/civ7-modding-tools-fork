...
import { IDisplayRequestBase, DisplayHandlerBase, DisplayHideOptions, DisplayHideReason } from '/core/ui/context-manager/display-handler.js';
...
import { RaiseDiplomacyEventName, RaiseDiplomacyEvent } from '/base-standard/ui/diplomacy/diplomacy-events.js';
import WorldInput from '/base-standard/ui/world-input/world-input.js';



export type ReactionButtonData = {
...
export type PendingStartActionData = {
	operationType: PlayerOperationType,
	operationArguments: DiplomacyActionOperationArgs
}

type WarDeclarationOperationArguments = {
	Player1: PlayerId,
	Player2: PlayerId,
	IndependentPlayer2?: IndependentTypes,
	Type: DiplomacyActionTypes
}

export type OperationSuccess = Pick<OperationResult, 'Success'>

export type DiplomacyActionOperationArgs = {
	ID?: number,
	ID2?: number,
	City?: number,
	Unit?: number,
...
export type DiplomacyActionSupportArgs = {
...
export enum TempDiplomacyActionType {
	DECLARE_WAR,
	DECLARE_PEACE,
	FORM_ALLIANCE
}
...
export class DiplomacyDialogManagerImpl extends DisplayHandlerBase<DiplomacyDialogRequest> {
	constructor() {
		super("DiplomacyDialog", 6000);
	}

	public show(request: DiplomacyDialogRequest): void {
	...
	public hide(_request: DiplomacyDialogRequest, options: DisplayHideOptions): void {
...
export class DiplomacyDealManagerImpl extends DisplayHandlerBase<DiplomacyDealRequest> {
	constructor() {
		super("DiplomacyDeal", 5000);
	}

	public show(request: DiplomacyDealRequest): void {
	...
	public hide(request: DiplomacyDealRequest, options: DisplayHideOptions): void {
		if (!request.blockClose) {
			DiplomacyManager.hide(options.reason === DisplayHideReason.Suspend);
		}
	}
...
class DiplomacyProjectManagerImpl extends DisplayHandlerBase<DiplomacyProjectRequest> {
	constructor() {
		super("DiplomaticResponseUIData", 8000);
	}

	public show(request: DiplomacyProjectRequest): void {
		DiplomacyManager.currentProjectReactionRequest = request;
		InterfaceMode.switchTo("INTERFACEMODE_DIPLOMACY_PROJECT_REACTION");
	}

	public hide(_request: DiplomacyProjectRequest, options: DisplayHideOptions): void {
...
class DiplomacyManagerImpl {
	...
	private diplomacyStatementListener = this.onDiplomacyStatement.bind(this);
	private diplomacySessionClosedListener = this.onDiplomacySessionClosed.bind(this);
	private actionDetailsClosedListener = this.onActionDetailsClosed.bind(this);
	private firstMeetReactionClosedListener = this.onFirstMeetReactionClosed.bind(this);
	private onRaiseDiplomacyHubListener = this.onRaiseDiplomacyHub.bind(this);

	private _selectedPlayerID: PlayerId = PlayerIds.NO_PLAYER;
	private _diplomacyActions: TempDiplomacyAction[] = [];

	private _availableProjects: DiplomaticProjectUIData[] = [];
	private _availableEndeavors: DiplomaticProjectUIData[] = [];
	private _availableSanctions: DiplomaticProjectUIData[] = [];
	private _availableEsionage: DiplomaticProjectUIData[] = [];
	private _availableTreaties: DiplomaticProjectUIData[] = [];
	private _selectedProjectData: DiplomaticProjectUIData | null = null;

	private isClosingActionsPanel: boolean = false;
	private _isFirstMeetDiplomacyOpen: boolean = false;
	private _isDeclareWarDiplomacyOpen: boolean = false;

	currentDiplomacyDialogData: DiplomacyDialogRequest | null = null;
	selectedActionID: DiplomacyActionEventId = -1;
	currentDiplomacyDealData: DiplomacyDealRequest | null = null;
	currentAllyWarData: DiplomaticEventHeader | null = null;
	currentProjectReactionData: DiplomaticResponseUIData | null = null;
	currentProjectReactionRequest: DiplomacyProjectRequest | null = null;
	showDiplomacyAfterFirstMeet: boolean = false;
	firstMeetPlayerID: PlayerId = PlayerIds.NO_PLAYER;
	currentEspionageData: EspionageNotificationData | null = null;
	...
	public shouldQuickClose: boolean = false;
	...
	/**
	  ...
	  */
	public hide(isSuspended: boolean): void {
	 ...
	 * @param event 
	 */
	onRaiseDiplomacyHub(event: RaiseDiplomacyEvent) {
		// Checks (if any) done in called function.
		const playerID: PlayerId = event.detail.playerID;
		this.raiseDiplomacyHub(playerID);
	}


	private raiseDiplomacyHub(playerID: PlayerId) {
	...
	closeCurrentDiplomacyDeal(closeSession: boolean, ourDiplomacySession?: number) {
	...
	closeCurrentDiplomacyProject(closeToDiploHub: boolean) {
	...
	addCurrentDiplomacyProject(request: DiplomaticResponseUIData) {
		if (request) {
	...
	private handleDealStatement(data: DiplomacyStatement_EventData): boolean {
		...
		if (data.values.DealAction && data.values.DealAction != -1) {
			...
			const newDealData: DiplomacyDealData = {
	...
	private onDiplomacyStatement(data: DiplomacyStatement_EventData) {
		...
		if (data.values && data.values.ToPlayer == GameContext.localPlayerID) {
			...
			const player: PlayerLibrary | null = Players.get(data.values.FromPlayer);
			if (player != null) {
			...
			const diplomacyDialogData: DiplomacyDialogData = this.buildDialogData(data);

			const foundDialogRequests = DisplayQueueManager.findAll((p: DiplomacyDialogRequest) => p.SessionID === data.sessionId);
	...
	private onDiplomacySessionClosed(data: DiplomacySessionClosed_EventData) {
	...
	private buildDialogData(data: DiplomacyStatement_EventData): DiplomacyDialogData {
		const statementFrame = this.getStatementFrame(data);
		const messageString: string = this.buildMessageString(data, statementFrame);
		const diplomacyMessageChoices: DiplomacyDialogChoice[] = this.getChoices(statementFrame, data.actingPlayer);
		...
		const diplomacyDialogData: DiplomacyDialogData = {
	...
	// ------------------------------------------------------------------------
	private buildMessageString(data: DiplomacyStatement_EventData, statementFrame: DiplomacyStatementFrameDefinition | undefined): string {
		...
		const otherPlayer: PlayerLibrary | null = Players.get(data.values.FromPlayer)
		if (!otherPlayer) {
		...
		const otherLeader: LeaderDefinition | null = GameInfo.Leaders.lookup(otherPlayer.leaderType);
		if (!otherLeader) {
		...
		const messageType: string = this.getResponseTypeName(data.values.ResponseType); //POSITIVE, NEGATIVE, etc
		const otherLeaderName: string = otherLeader.LeaderType;
		...
		const baseDiplomacyMessageString: string = (statementFrame && statementFrame.Text) ? statementFrame.Text : "LOC_DIPLO_" + statementType;
		...
		let diplomacyMessageString: string = baseDiplomacyMessageString + "_" + otherLeaderName + "_" + messageType;
	...
	private getResponseTypeName(responseTypeHash: number): string {
	...
	private getStatementFrame(data: DiplomacyStatement_EventData): DiplomacyStatementFrameDefinition | undefined {
	...
	private getChoices(statementFrame: DiplomacyStatementFrameDefinition | undefined, actingPlayer: PlayerId): DiplomacyDialogChoice[] {
		const choices: DiplomacyDialogChoice[] = [];
		...
		if (statementFrame) {
			if (statementFrame.Selections) {
				...
				selections.forEach(selection => {
					...
					const choice: DiplomacyDialogChoice = { ChoiceString: selection.Text, ChoiceType: selection.Key, Callback: choiceCallback };
					choices.push(choice);
				});
			}
			...
			if (statementFrame.Selections?.includes("FIRST_MEET")) {
				...
				const choice: DiplomacyDialogChoice = { ChoiceString: Locale.compose("LOC_DIPLOMACY_OPEN_DIPLOMACY"), ChoiceType: "CHOICE_ACKNOWLEDGE", Callback: choiceCallback };
				choices.push(choice);
			} else if (statementFrame.Selections == "CHOICES_DECLARE_WAR") {
				const choiceCallback = () => {
					let warID: DiplomacyActionEventId = -1;
					const jointEvents: DiplomaticEventHeader[] = Game.Diplomacy.getJointEvents(GameContext.localPlayerID, actingPlayer, false);
				...
				const choice: DiplomacyDialogChoice = { ChoiceString: Locale.compose("LOC_DIPLOMACY_OPEN_WAR_DETAILS"), ChoiceType: "CHOICE_ACKNOWLEDGE", Callback: choiceCallback };
				choices.push(choice);
			}
		}
		...
		if (choices.length <= 0) {
			...
			const defaultChoice: DiplomacyDialogChoice = { ChoiceString: Locale.compose("LOC_DIPLO_GENERIC_EXIT"), ChoiceType: "CHOICE_EXIT", Callback: defaultChoiceCallback };
	...
	populateDiplomacyActions() {
		...
		const playerDiplomacy: PlayerDiplomacy | undefined = player.Diplomacy;
		...
		const allianceArgs: object = {
			Player1: thisPlayerID,
			Player2: this._selectedPlayerID,
			Type: DiplomacyActionTypes.DIPLOMACY_ACTION_FORM_ALLIANCE
		}

		let allianceCaption: string = Locale.compose("LOC_DIPLOMACY_ACTION_FORM_ALLIANCE_NAME");
		let allianceOperationType: PlayerOperationType = PlayerOperationTypes.FORM_ALLIANCE;

		let allianceResults: OperationResult = Game.PlayerOperations.canStart(thisPlayerID, allianceOperationType, allianceArgs, false);
		...
		let peaceQueryResults: DiplomacyQueryResult = playerDiplomacy.canMakePeaceWith(this._selectedPlayerID);
		let peaceFailureTooltip: string = "";
		...
		const warQueryResults: DiplomacyQueryResult = playerDiplomacy.canDeclareWarOn(this._selectedPlayerID);
		let warFailureTooltip: string = "";
		...
		const isIndependent: boolean | undefined = Players.get(this._selectedPlayerID)?.isIndependent;
		const isMajor: boolean | undefined = Players.get(this._selectedPlayerID)?.isMajor;
		...
		const tempDeclareWarAction: TempDiplomacyAction = {
		...
		const tempMakePeaceAction: TempDiplomacyAction = {
		...
		const formAllianceAction: TempDiplomacyAction = {
	...
	confirmDeclareWar(playerID: PlayerId, warType: DiplomacyActionType) {
		const args: object = {
			Player1: GameContext.localPlayerID,
			Player2: playerID,
			Type: warType
		}
		const result: OperationResult = Game.PlayerOperations.canStart(GameContext.localPlayerID, PlayerOperationTypes.DECLARE_WAR, args, false);
		if (result.Success) {
	...
	startWarFromMap(warDeclarationTarget: WarDeclarationTarget, postDeclareWarAction: () => void): boolean {
		const args: WarDeclarationOperationArguments = {
			Player1: GameContext.localPlayerID,
		...
		const surpriseWarResults: DiplomacyQueryResult = playerDiplomacy.canDeclareWarOn(warDeclarationTarget.player);

		if (!surpriseWarResults.Success) {
			return false;
		}

		const targetPlayer: PlayerLibrary | null = Players.get(warDeclarationTarget.player);
		...
		if (targetPlayer.isIndependent) {
			const independentName: string | null = targetPlayer.civilizationFullName;
			if (!independentName) {
			...
			if (Players.get(warDeclarationTarget.player)?.isMinor) {
				...
				const cityStatePlayer: PlayerLibrary | null = Players.get(warDeclarationTarget.player);
				if (!cityStatePlayer) {
				...
				if (cityStatePlayer.Influence.getSuzerain() == GameContext.localPlayerID) {
					...
					const suzerainPlayer: PlayerLibrary | null = (Players.get(cityStatePlayer.Influence.getSuzerain()));
					if (!suzerainPlayer) {
						console.error("diplomacy-manager: Unable to get PlayerLibrary for suzerain of city-state with id " + warDeclarationTarget.player);
						return false;
					}

					return this.raiseWarTypePopup(suzerainPlayer.id, Locale.compose("LOC_DIPLOMACY_CONFIRM_DECLARE_WAR_FROM_MAP_SUZERAIN_BODY", Locale.compose(cityStatePlayer.civilizationFullName), Locale.compose(suzerainPlayer.name)), postDeclareWarAction);
				}
			}

			const otherLeaderName: string | undefined = Players.get(warDeclarationTarget.player)?.name;
			...
			return this.raiseWarTypePopup(warDeclarationTarget.player, Locale.compose("LOC_DIPLOMACY_CONFIRM_DECLARE_WAR_FROM_MAP_BODY", otherLeaderName), postDeclareWarAction);
		}
		return true; // handled
	}

	private raiseAlliancePopup(currPlayerId: PlayerId, selectedPlayerId: PlayerId, allianceOperationType: PlayerOperationType, allianceOperationArgs: object) {
		...
		const playerDiplomacy: PlayerDiplomacy | undefined = currPlayer.Diplomacy;
		if (!playerDiplomacy) {
			console.error("diplomacy-manager: Attempting to raise alliance popup, but no valid player diplomacy library!");
			return;
		}

		const acceptCallback: DialogBoxCallbackSignature = (eAction: DialogBoxAction) => {
			if (eAction == DialogBoxAction.Confirm) {
				Game.PlayerOperations.sendRequest(GameContext.localPlayerID, allianceOperationType, allianceOperationArgs);
			}
		};
	...
	private raiseWarTypePopup(targetPlayerID: PlayerId, bodyString?: string, postDeclareWarAction?: () => void): boolean {
		const playerDiplomacy: PlayerDiplomacy | undefined = Players.get(GameContext.localPlayerID)?.Diplomacy;
		if (playerDiplomacy == undefined) {
			console.error("diplomacy-manager: Attempting to raise war type popup, but no valid player diplomacy library!");
			return false;
		...
		const surpriseWarResults: DiplomacyQueryResult = playerDiplomacy.canDeclareWarOn(targetPlayerID);
		const formalWarResults: DiplomacyQueryResult = playerDiplomacy.canDeclareWarOn(targetPlayerID, WarTypes.FORMAL_WAR);

		let ourWarSupport: number = playerDiplomacy.getTotalWarSupportBonusForPlayer(targetPlayerID, formalWarResults.Success);
		let theirWarSupport: number = playerDiplomacy.getTotalWarSupportBonusForTarget(targetPlayerID, formalWarResults.Success);
		let theirInfluenceBonus: number = playerDiplomacy.getWarInfluenceBonusTarget(targetPlayerID, formalWarResults.Success);
	...
	clickStartProject(projectData: DiplomaticProjectUIData) {
	...
	getAudioIdForDiploAction(projectData: DiplomaticProjectUIData): [string, string] {
