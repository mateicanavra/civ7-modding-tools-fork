...
export namespace CityZoomer {
	export function zoomToCity(city: City) {
		Camera.saveCameraZoom();

		const region: Range2Df = { min: { x: 0.275, y: 0.025 }, max: { x: 0.975, y: 0.975 } }; // TODO use actual postitions of top and bottom bars
		const calculatedFocus: float3 | null = Camera.calculateCameraFocusAndZoom(city.getPurchasedPlots(), 30, { region: region });
		if (calculatedFocus) {
			// If zoom is greater than 1, it is not possible to fit, but center it anyway
			const cameraFrame: GameCameraKeyframe = {
				duration: 1,
				tilt: 30,
				...
				writeMask: KeyframeFlag.FLAG_ALL, // overwrite all affected camera state
