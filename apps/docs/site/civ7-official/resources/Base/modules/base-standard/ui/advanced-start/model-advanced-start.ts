...
interface CardEntry {
	...
	canBeAdded: boolean;
	cannotBeAdded: boolean; //optimization to avoid expensive javascript calls when using data-bind attributes
...
export class AdvancedStartModel {
	private onUpdate?: (model: AdvancedStartModel) => void;

	private _availableCards: CardEntry[] = [];
	private _selectedCards: CardEntry[] = [];
	private _placeableCardEffects: CardEffectEntry[] = [];
	private _filterForCards: CardCategories = CardCategories.CARD_CATEGORY_NONE;
	private _preselectIndex: number = 0;
	private _preSelectList: PreselectEntry[] = [];
	private _cardsToAddOnRemoval: string[] = [];

	private updateGate: UpdateGate = new UpdateGate(() => { this.update(); });

	private cardAddedListener = () => { this.tryAddCards(); this.updateGate.call('cardAddedListener'); };
	private cardRemovedListener = () => { this.tryAddCards(); this.updateGate.call('cardRemovedListener'); };
	private effectUsedListener = () => { this.updateGate.call('effectUsedListener'); };

	private selectedPlacementEffectID: string = '';

	// If false we have no cards that can be added so deck is probably full
	canAddCards: boolean = true;

	// True if we've confirmed the deck and want to apply effects
	deckConfirmed: boolean = false;

	// Tracks the placement complete flag from gamecore
	advancedStartClosed: boolean = false;
	...
	private update() {
		...
		const ageStarDeckCardEntries: DefinitionCollection<AdvancedStartDeckCardEntryDefinition> = GameInfo.AdvancedStartDeckCardEntries;
		...
		for (const cardInfo of playerAdvancedStart.getAvailableCards()) {
			const availableCardEntry: CardEntry = this.makeCardEntryFromAgeCardInfo(cardInfo, playerAdvancedStart);
		...
		const selectedCards: AdvancedStartCard[] = playerAdvancedStart.getCards();
	...
	private makeCardEntryFromAgeCardInfo(cardInfo: AgeCardInfo, playerAdvancedStart: PlayerAdvancedStart, selected?: boolean): CardEntry {
		...
		const canBeAdded: boolean = result.Success;
		let hasBeenAdded: boolean = false;
		let insufficientFunds: boolean = false;
		let cardTooltip = cardInfo.tooltip;

		const costs: CardCost[] = this.makeCosts(cardInfo.cost);
		// Check if they have enough funds to add a legacy
		const legacyPointsLeft: CardCategoryInstance[] = playerAdvancedStart.getLegacyPoints();
		costs.forEach(cost => {
			let pointAvailable = 0;
			const legacyPoints = legacyPointsLeft.find(legacyPoint =>
				this.getCardCategoryColor(legacyPoint.category) === cost.colorClass && legacyPoint.value > 0
			);
	...
	private parseCardText(args: string) {
	...
	private makeCosts(cost: CardCategoryInstance[]): CardCost[] {
		let output: CardCost[] = [];
		for (const instance of cost) {
			const entry: CardCost = {
	...
	getCardCategoryByColor(colorCategory: string): CardCategories {
		let output: CardCategories = CardCategories.CARD_CATEGORY_NONE;
	...
	addAvailableCard(typeID: string): boolean {
		...
		const result: OperationResult = Game.PlayerOperations.canStart(GameContext.localPlayerID, PlayerOperationTypes.ADVANCED_START_MODIFY_DECK, args, false);
		if (result.Success) {
	...
	removeAvailableCard(typeID: string) {
	...
	changePresetLegacies(indexShift: number) {
		...
		this._availableCards.forEach(card => {
			if (card.insufficientFunds == false && card.canBeAdded && this._selectedCards.findIndex(x => x.typeID == card.typeID) == -1) {
				...
				card.costs.forEach(cost => {
					const legacyPoints = availableLegacyPoints?.find(legacyPoint =>
						this.getCardCategoryColor(legacyPoint.category) === cost.colorClass && legacyPoint.value > 0
					);
					pointAvailable += legacyPoints?.value || 0;
	...
	selectPlacementEffect(typeID: string) {
	...
	placePlacementEffect(plot: PlotCoord): boolean {
		...
		const args: Object = { ID: this.selectedPlacementEffectID, X: plot.x, Y: plot.y };
		const result: OperationResult = Game.PlayerOperations.canStart(GameContext.localPlayerID, PlayerOperationTypes.ADVANCED_START_PLACE_SETTLEMENT, args, false);
		if (result.Success) {
	...
	private applyInstanceEffects() {
		...
		for (const card of selectedCards) {
			for (const effect of card.info.effects) {
				...
				for (let i = 0; i < effect.amount; i++) {
					const args: Object = { ID: effect.id };
					const result: OperationResult = Game.PlayerOperations.canStart(GameContext.localPlayerID, PlayerOperationTypes.ADVANCED_START_USE_EFFECT, args, false);
					if (result.Success) {
	...
	private isACurrency(text: string): boolean {
		return text === 'mili' || text === 'cult' || text === 'econ' || text === 'scie' || text === 'wild'
	}

	private convertCurrencyText(currencyText: string): string {
		switch (currencyText) {
			case 'mili':
				return "MILITARY"
			case 'cult':
	...
	private canNotAffordLegacy(typeID: string): boolean {
