...
class ScreenResourceAllocation extends Panel {
	...
	private cityEngineInputListener = this.onCityEngineInput.bind(this);
	private assignedResourceEngineInputListener = this.onAssignedResourceEngineInput.bind(this);

	private availableResourceFocusListener = this.onAvailableResourceFocus.bind(this);
	private cityFocusListener = this.onCityFocus.bind(this);
	private cityUnfocusListener = this.onCityUnfocus.bind(this);
	private empireResourceFocusListener = this.onEmpireResourceFocus.bind(this);
	private assignedResourceFocusListener = this.onAssignedResourceFocus.bind(this);
	private emptyResourceFocusListener = this.onEmptyResourceFocus.bind(this);
	private cityResourceContainerFocusListener = this.onCityResourceContainerFocus.bind(this);

	private availableResourceActivateListener = this.onAvailableResourceActivate.bind(this);
	private assignedResourceActivateListener = this.onAssignedResourceActivate.bind(this);
	private cityActivateListener = this.onCityActivate.bind(this);
	private resourceMovedListener = this.onResourceMoved.bind(this);
	private unassignActivateListener = this.onUnassignActivated.bind(this);

	private onResourceListFocusedListener = this.onResourceListFocused.bind(this);
	private onBonusResourceListFocusedListener = this.onBonusResourceListFocused.bind(this);
	private onFactoryResourceListFocusedListener = this.onFactoryResourceListFocused.bind(this);
	private onCityListFocusedListener = this.onCityListFocused.bind(this);
	private onfilterContainerFocusedListener = this.onfilterContainerFocused.bind(this);

	private closeListener = this.close.bind(this);

	private buttonContainer: HTMLElement | null = null;

	private focusedPanel: PanelType = PanelType.None;

	private parentSlot!: HTMLElement;
	private filterContainer!: HTMLElement;
	private cityList!: ComponentRoot<FxsSlot>;
	private availableResourceCol!: HTMLElement;
	private availableResourceList!: HTMLElement;
	private availableBonusResourceList!: HTMLElement;
	private availableFactoryResourceList!: HTMLElement;

	private cityListScrollable!: HTMLElement;
	private availableResourceListScrollable!: HTMLElement;
	private availableBonusResourceListScrollable!: HTMLElement;
	private availableFactoryResourceListScrollable!: HTMLElement;
	...
	onAttach() {
		...
		const empireResourceList: HTMLElement = MustGetElement('.empire-resource-list', this.Root);
		...
		Databind.for(cityOuterDiv, 'g_ResourceAllocationModel.availableCities', 'entry');
		{
			...
			const cityEntry: HTMLElement = document.createElement('fxs-activatable');
			...
			const cityInnerContainer: HTMLElement = document.createElement('div');
			cityInnerContainer.classList.add('city-entry-internal', 'flex', 'flex-col', 'items-start', 'relative', 'grow', 'mx-0\.5', 'm-1', 'p-1');

			const cityTopContainer: HTMLElement = document.createElement('div');
			...
			const entryName: HTMLElement = document.createElement('p');
			...
			const settlementTypeName: HTMLElement = document.createElement('p');
			...
			const inTradeNetworkWarning: HTMLElement = document.createElement('div');
			inTradeNetworkWarning.classList.add('trade-warning-text', 'font-title', 'text-xs', 'ml-1');
			inTradeNetworkWarning.innerHTML = Locale.stylize(`- [STYLE: text-negative]${Locale.compose("LOC_UI_RESOURCE_ALLOCATION_SETTLEMENT_DISCONNECTED")}[/STYLE]`);
			...
			const cityResourceContainer: HTMLElement = document.createElement('fxs-spatial-slot');
			...
			if (Game.age == Game.getHash("AGE_EXPLORATION")) {
				const cityTreasureContainerOuter: HTMLElement = document.createElement('fxs-vslot');
				...
				const cityTreasureContainer: HTMLElement = document.createElement('fxs-hslot');
			...
			if (Game.age == Game.getHash("AGE_MODERN")) {
				const cityFactoryResourceContainer: HTMLElement = document.createElement('div');
				...
				const availableFactoryResourceSlot: HTMLElement = document.createElement('fxs-activatable');
				availableFactoryResourceSlot.classList.add('img-add-slot', 'size-16', 'm-1', 'pointer-events-auto',
	...
	private onCityListFocused() {
		this.updateAllScrollbarHandleGamepad();
	}

	private onfilterContainerFocused() {
		this.updateAllScrollbarHandleGamepad();
		this.updateNavTrayEntries();
	}

	private onResourceListFocused() {
		this.updateAllScrollbarHandleGamepad();
	}

	private onBonusResourceListFocused() {
		this.updateAllScrollbarHandleGamepad();
	}

	private onFactoryResourceListFocused() {
		this.updateAllScrollbarHandleGamepad();
	}

	private buildEmpireResources(data: string, parent: HTMLElement) {
		const resourceEntry: HTMLElement = document.createElement('div');
		Databind.for(resourceEntry, data, 'resource');
		{
			resourceEntry.classList.add('resource-allocation-resource-container');

			const resourceActivatable: HTMLElement = document.createElement('fxs-activatable');
			...
			resourceActivatable.classList.add('city-resource', "mr-px",
				"hover\\:bg-secondary", "focus\\:bg-secondary"
			);
			...
			const icon: HTMLElement = document.createElement('fxs-icon');
			...
			const resourceCount: HTMLElement = document.createElement('div');
	...
	private buildTreasureResources(data: string, parent: HTMLElement) {
		const resourceEntry: HTMLElement = document.createElement('div');
		Databind.for(resourceEntry, data, 'resource');
		{
			resourceEntry.classList.add('resource-allocation-resource-container');
			...
			// Non-treasure fleet resource icon		
	...
	private buildCityResources(data: string, iconClass: string, parent: HTMLElement) {
		const resourceDiv: HTMLElement = document.createElement('div');
		Databind.for(resourceDiv, data, 'resource');
		{
			const resourceActivatable: HTMLElement = document.createElement('fxs-activatable');

			resourceActivatable.classList.add('city-resource', 'relative', 'flex', 'size-16', 'items-center', 'justify-center', 'mr-0\.5',
				"focus\\:bg-secondary"
			);
			resourceActivatable.setAttribute("tabindex", "-1");
	...
	private buildEmptySlots(data: string, parent: HTMLElement, cityIdData: string, cityNameData: string) {
		const resourceDiv: HTMLElement = document.createElement('div');
		Databind.for(resourceDiv, data, 'resource');
		{
			const resourceEntry: HTMLElement = document.createElement('fxs-activatable');
			...
			resourceEntry.classList.add('img-add-slot', 'size-16', 'mr-px', 'pointer-events-auto',
				"focus\\:bg-secondary"
			);
	...
	private onCityEngineInput(inputEvent: InputEngineEvent) {
		if (inputEvent.detail.status != InputActionStatuses.FINISH) {
			return;
		}
	}
	...
	private onShowYieldsChanged = (e: CheckboxValueChangeEvent) => {
	...
	private onShowTownsChanged = (e: CheckboxValueChangeEvent) => {
	...
	private onShowFactoriesChanged = (e: CheckboxValueChangeEvent) => {
	...
	private onUnassignActivated(event: ActionActivateEvent) {
	...
	private onAvailableResourceFocus() {
	...
	private onCityFocus(event: FocusEvent) {
		...
		const idleOverlay: HTMLElement = parent?.previousSibling as HTMLElement;
		if (idleOverlay) {
			idleOverlay.classList.add('bg-secondary');
		}

		if (ResourceAllocation.hasSelectedResource()) {
			const cityName: string | null = target.getAttribute("city-name");
			if (cityName == null) {
	...
	private onCityUnfocus(event: FocusEvent) {
		const target: HTMLElement | null = event.target as HTMLElement;
		if (target == null) {
			console.error("panel-build-queue: onCityFocus(): Invalid event target. It should be an HTMLElement");
			return;
	...
	private onEmpireResourceFocus() {
		this.focusedPanel = PanelType.EmpireResources;
	}

	private onAvailableResourceActivate(event: CustomEvent) {
		if (!(event.target instanceof HTMLElement)) {
			return;
		}

		const resourceValueAttribute: string | null = event.target.getAttribute('data-resource-value');
		const resourceClassAttribute: string | null = event.target.getAttribute('data-resource-class');
		const isInTradeNetwork: string | null = event.target.getAttribute('data-in-trade-network');
		const assignmentLocked: string | null = event.target.getAttribute('data-assignment-locked');
		...
		const resourceValue: number = parseInt(resourceValueAttribute);
	...
	private onAssignedResourceActivate(event: CustomEvent) {
		if (!(event.target instanceof HTMLElement)) {
			return;
		}
	...
	private onCityActivate(event: CustomEvent) {
		if (!(event.target instanceof HTMLElement)) {
			return;
		}
	...
	private buildAvailableResources(parent: HTMLElement, data: string, resourceClass: string) {
		...
		Databind.for(resourceDiv, data, 'entry');
		{
			...
			resourceEntry.classList.add('resource-entry', 'flex', 'relative',
				"hover\\:bg-secondary", "focus\\:bg-secondary"
			);
		...
		const unassignActivatable: HTMLElement = document.createElement('fxs-activatable');
		...
		const emptySlotHoverOverlay: HTMLElement = document.createElement('div');

		emptySlotHoverOverlay.classList.add('img-add-slot', 'size-16', 'm-1', 'pointer-events-auto', 'bg-accent-4',
			"hover\\:bg-secondary", "focus\\:bg-secondary"
		);
	...
	private setButtonContainerVisible(isVisible: boolean) {
	...
	private focusCityList() {
		const cityEntries = this.cityList.querySelectorAll<ComponentRoot<FxsSlot>>(".city-entry");

		for (const city of cityEntries) {
			if (Navigation.isFocusable(city)) {
				this.cityList.maybeComponent?.setInitialFocus(city);

				const resourceContainer = city.querySelector<ComponentRoot<FxsSlot>>(".city-resource-container");
	...
	private selectCityAndTryAllocateSelectedResource(cityEntry: HTMLElement): boolean {
		const cityIDAttribute: string | null = cityEntry.getAttribute('data-city-id');
		if (!cityIDAttribute) {
		...
		const hadSelectedResource: boolean = ResourceAllocation.hasSelectedResource(); // can be false because we also call selectCity() when toggling the More Info panel
	...
	private onAssignedResourceEngineInput(event: InputEngineEvent) {
	...
	private onAssignedResourceFocus(event: FocusEvent) {
		const target: HTMLElement | null = event.target as HTMLElement;
		if (target == null) {
	...
	private onEmptyResourceFocus(event: FocusEvent) {
		const target: HTMLElement | null = event.target as HTMLElement;
		if (target == null) {
			console.error("screen-resource-allocation: onAssignedResourceFocus(): Invalid event target. It should be an HTMLElement");
			return;
	...
	private onCityResourceContainerFocus(event: FocusEvent) {
	...
	private updateCityEntriesDisabledState() {
		const cityEntries = this.cityList.querySelectorAll<HTMLElement>(".city-entry");
	...
	private updateAvailableResourceColDisabledState() {
	...
	private updateAllUnassignActivatable() {
	...
	private updateAllScrollbarHandleGamepad() {
		const currentFocus = FocusManager.getFocus();
