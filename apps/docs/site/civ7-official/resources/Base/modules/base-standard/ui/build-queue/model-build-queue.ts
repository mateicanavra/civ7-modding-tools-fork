...
class BuildQueueModel {
	...
	private _OnUpdate?: (model: BuildQueueModel) => void;
	...
	constructor() {
		engine.whenReady.then(() => {
			...
			engine.on('CitySelectionChanged', (event: CitySelectionChangedData) => {
			...
			engine.on('CityProductionQueueChanged', (data: CityProductionChanged_EventData) => {
				if (data.cityID.owner == GameContext.localPlayerID) {
					this.updateGate.call('CityProductionQueueChanged');
				}
			});

			engine.on('CityProductionChanged', (data: CityProductionChanged_EventData) => {
				if (data.cityID.owner == GameContext.localPlayerID) {
					this.updateGate.call('CityProductionChanged');
				}
			});

			engine.on('CityYieldChanged', (data: City_EventData) => {
				if (data.cityID.owner == GameContext.localPlayerID) {
					this.updateGate.call('CityYieldChanged');
				}
			});

			engine.on('CityProductionUpdated', (data: CityProductionUpdated_EventData) => {
				if (data.cityID.owner == GameContext.localPlayerID) {
	...
	updateGate = new UpdateGate(() => {
		...
		if (player && cityID) {
			...
			if (c) {
				...
				if (queue) {
					const queueNodes: CityBuildQueueNode[] = queue.getQueue();
					queueNodes.forEach(queueData => {
						let turns: number = queue.getTurnsLeft(queueData.type);
						let progress: number = queue.getPercentComplete(queueData.type);
