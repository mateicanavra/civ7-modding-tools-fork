...
interface RemoveFromArmyInterfaceModeContext extends InterfaceMode.Context {
	UnitID: ComponentID;
	OperationArguments: any;
}
/**
 * Handler for INTERFACEMODE_REMOVE_FROM_ARMY.
 */
class RemoveFromArmyInterfaceMode extends ChoosePlotInterfaceMode {

	private _OperationResult: OperationResult | null = null;
	private validPlots = new Set<PlotIndex>();

	initialize() {
		const context = this.Context as RemoveFromArmyInterfaceModeContext;
		const args: any = {};
		const result = Game.UnitCommands.canStart(context.UnitID, 'UNITCOMMAND_REMOVE_FROM_ARMY', args, false);
	...
	commitPlot(plot: PlotCoord) {
		...
		args.X = plot.x;
		args.Y = plot.y;
		Audio.playSound("data-audio-ability-leave-army-select-plot", "interact-unit");
		Game.UnitCommands.sendRequest(unitID, 'UNITCOMMAND_REMOVE_FROM_ARMY', args);
	}
}

InterfaceMode.addHandler('INTERFACEMODE_REMOVE_FROM_ARMY', new RemoveFromArmyInterfaceMode());