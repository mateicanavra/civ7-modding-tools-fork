/**
 * @file interface-mode-air-attack.ts
...
interface AirAttackInterfaceModeContext extends InterfaceMode.Context {
	UnitID: ComponentID;
...
/**
 * Handler for INTERFACEMODE_AIR_ATTACK.
 */
class AirAttackInterfaceMode extends ChoosePlotInterfaceMode {

	private _OperationResult: OperationResult | null = null;
	private validPlots = new Set<PlotIndex>();

	initialize() {
		const context = this.Context as AirAttackInterfaceModeContext;
		const args: any = {};
		const result = Game.UnitOperations.canStart(context.UnitID, 'UNITOPERATION_AIR_ATTACK', args, false);
	...
	commitPlot(plot: PlotCoord) {
		...
		args.X = plot.x;
		args.Y = plot.y;
		Game.UnitOperations.sendRequest(unitID, 'UNITOPERATION_AIR_ATTACK', args);
		UI.sendAudioEvent(Audio.getSoundTag('data-audio-unit-combat-confirmed', 'interact-unit'));
	}
}

InterfaceMode.addHandler('INTERFACEMODE_AIR_ATTACK', new AirAttackInterfaceMode());