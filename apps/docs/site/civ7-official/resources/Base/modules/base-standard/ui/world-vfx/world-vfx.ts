...
class WorldVFXManager {
	private static instance: WorldVFXManager | null = null;
	private ambientVfxModelGroup: WorldUI.ModelGroup | null = null;
	private discoveryHashes: string[] = [];
	...
	private onConstructibleAddedToMap(data: ConstructibleAddedToMap_EventData) {
		const improvement: ConstructibleDefinition | null = GameInfo.Constructibles.lookup(data.constructibleType);
		if (improvement == null)
			return;
	...
	private onPlotEffectAddedToMap(data: PlotEffectChanged_EventData) {
		const plotEffect: PlotEffectDefinition | null = GameInfo.PlotEffects.lookup(data.effectType);
		if (plotEffect == null)
			return;
		...
		if (Players.isParticipant(GameContext.localPlayerID)
			...
			if (plotEffect.PlotEffectType == "PLOTEFFECT_FLOODED") {
				...
				var outflowEdge: number = data.floodConnectivity & 7;
	...
	private onPlotEffectRemovedFromMap(data: PlotEffectChanged_EventData) {
		const plotEffect: PlotEffectDefinition | null = GameInfo.PlotEffects.lookup(data.effectType);
		if (plotEffect == null)
			return;
	...
	private onRouteAddedToMap(data: Route_EventData) {
...
const WorldVfx: WorldVFXManager = new WorldVFXManager();
