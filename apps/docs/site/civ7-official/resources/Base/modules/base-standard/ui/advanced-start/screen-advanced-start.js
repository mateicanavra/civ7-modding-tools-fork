...
class ScreenAdvancedStart extends Panel {
    constructor() {
        ...
        this.availableCardActivateListener = this.onAvailableCardActivate.bind(this);
        this.selectedCardActivateListener = this.onSelectedCardActivate.bind(this);
        this.removeCardAnimationListener = this.onRemoveListener.bind(this);
        this.effectActivateListener = this.onEffectActivate.bind(this);
        this.confirmDeckButtonListener = this.onConfirmDeck.bind(this);
        this.forceCompleteButtonListener = this.onForceComplete.bind(this);
        this.closeButtonListener = this.close.bind(this);
        this.autoFillListener = this.autofillDeck.bind(this);
        this.cardAddedListener = this.onCardAdded.bind(this);
        this.cardRemovedListener = this.onCardRemovedListener.bind(this);
        this.effectUsedListener = this.onEffectUsed.bind(this);
        this.interfaceModeChangedListener = this.onInterfaceModeChanged.bind(this);
        this.flashCostAnimationListener = this.onFlashAnimationEnd.bind(this);
        this.filterAvailableCardsListener = this.setFilterAvailableCards.bind(this);
        this.preSelectNavListener = this.setPresetLegacies.bind(this);
        this.navigateInputListener = this.onNavigateInput.bind(this);
        this.dialogId = displayRequestUniqueId();
        this.isAdvancedStart = false;
        this.filterButtons = [];
    ...
    onAttach() {
        ...
        // Selected Cards
        const selectedCards = this.Root.querySelector('.adv-start__selected-cards-list-container');
        if (!selectedCards) {
        ...
        // Card Effects
        const cardEffects = this.Root.querySelector('.card-effects');
        if (!cardEffects) {
        ...
        this.buildButtonBar();
        const selectedCardsContainer = this.Root.querySelector('.adv-start__selected-cards-container');
    ...
    buildCardEntry(cardEntry, isSelectedCard = false) {
        ...
        if (!isSelectedCard) {
            cardEntry.setAttribute('data-bind-class-toggle', 'cannot-be-added:{{entry.cannotBeAdded}};selected:{{entry.hasBeenAdded}};short-card:{{entry.oddCard}};insufficent-funds:{{entry.insufficientFunds}}');
        }
    }
    ...
    setFilterAvailableCards(event) {
        if (event.target instanceof HTMLElement) {
            ...
            if (filterType) {
                const prevSelected = this.Root.querySelector('.adv-start__filter-button.selected');
                if (prevSelected) {
    ...
    onAvailableCardActivate(event) {
        if (event.target instanceof HTMLElement) {
            ...
            if (typeID) {
                    ...
                    if (cardInfo) {
                        if (cardInfo.id == typeID) {
                            ...
                            cardInfo.cost.forEach(cost => {
                                const legacyCost = legacyPoints.find(t => t.category == cost.category);
                                if (legacyCost) {
                ...
                const selectedCards = this.Root.querySelector('.adv-start__selected-cards-container');
                if (selectedCards) {
    ...
    onPreselectChange() {
        let preselectText = this.Root.querySelector('.adv-start__preset-text');
        if (!preselectText) {
    ...
    onConfirmDeck() {
        const playerAdvancedStart = Players.get(GameContext.localPlayerID)?.AdvancedStart;
        if (!playerAdvancedStart) {
        ...
        if (hasPointsRemaining) {
            DialogManager.createDialog_ConfirmCancel({
                ...
                callback: (eAction) => { confirmDeckCallback(eAction); }
            });
        }
        else {
    ...
    onForceComplete() {
        ...
        DialogManager.createDialog_ConfirmCancel({
            ...
            callback: (eAction) => { confirmForceCompleteCallback(eAction); }
        });
    }
    ...
    setupDeckLimitHeader() {
        const deckLimitContainer = this.Root.querySelector('.adv-start__currency-list');
        ...
        if (uiViewExperience == UIViewExperience.Mobile) {
            const deckLimitTopContainer = this.Root.querySelector('.adv-start__currency-container');
    ...
    updateDeckLimitHeader(setup = false) {
        ...
        for (let category = 0; category < orderList.length; category++) {
            const instance = legacyPoints.find((target) => { return target.category == orderList[category]; });
            const instanceValue = instance?.value || 0;
            let targetCurrency = deckLimitContainer.querySelector(`.${AdvancedStart.getCardCategoryColor(orderList[category])}`);
            if (!targetCurrency) {
            ...
            let highlightText = highlightLayer.querySelector('.adv-start__currency-text');
...
//# sourceMappingURL=file:///base-standard/ui/advanced-start/screen-advanced-start.js.map
