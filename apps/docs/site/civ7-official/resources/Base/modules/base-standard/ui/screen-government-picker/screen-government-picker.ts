...
class ScreenGovernment extends ScreenGeneralChooser {
	private confirmButtonListener: EventListener = this.confirmChooseGovernment.bind(this);
	private closeButtonEventListener: EventListener = this.onClose.bind(this);
	...
	onReceiveFocus() {
		...
		const focusElement: HTMLElement | null = MustGetElement('.government-picker__main-container', this.Root);
		if (focusElement) {
	...
	createEntries(entryContainer: HTMLElement) {
		...
		for (const startingGovernmentDef of GameInfo.StartingGovernments) {
			...
			const governmentCelebrationTypes: string[] = Game.Culture.GetCelebrationTypesForGovernment(governmentDef.GovernmentType);
	...
	private confirmChooseGovernment() {
		if (!this.currentlySelectedChoice) {
			console.error("screen-government-picker: confirmChooseGovernment() - no government choice currently selected!");
			return;
		}
		const governmentType: string | null = this.currentlySelectedChoice.getAttribute("gov-type");
		if (governmentType) {
			const governmentDef: GovernmentDefinition | null = GameInfo.Governments.lookup(governmentType);
			if (!governmentDef) {
