...
export function needHumanNearEquator() {
    ...
    let iPlayerCount = 0;
    if (mapInfo)
        iPlayerCount = mapInfo.PlayersLandmass1 + mapInfo.PlayersLandmass2;
    for (let iPlay = 0; iPlay < iPlayerCount; iPlay++) {
        if (Players.getEverAlive()[iPlay] && Players.getEverAlive()[iPlay].isHuman) {
            ...
            let uiCivType = Players.getEverAlive()[iPlay].civilizationType;
            let uiLeaderType = Players.getEverAlive()[iPlay].leaderType;
            for (let startBiomeIdx = 0; startBiomeIdx < GameInfo.StartBiasBiomes.length; startBiomeIdx++) {
                ...
                if ((civHash == uiCivType) || (ldrHash == uiLeaderType)) {
                    let szBiome = GameInfo.StartBiasBiomes[startBiomeIdx].BiomeType;
                    console.log(szBiome);
...
export function getDistanceFromContinentCenter(iX, iY, iContinentBottomRow, iContinentTopRow, iWestContinentLeftCol, iWestContinentRightCol, iEastContinentLeftCol, iEastContinentRightCol) {
    ...
    let iContinentHeight = iContinentTopRow - iContinentBottomRow;
    let iContinentWidth = iContinentRightEdge - iContinentLeftEdge;
    let iContinentCenterX = iContinentLeftEdge + (iContinentWidth / 2);
    let iContinentCenterY = iContinentBottomRow + (iContinentHeight / 2);
    let iDistance = GameplayMap.getPlotDistance(iX, iY, iContinentCenterX, iContinentCenterY);
    return iDistance;
}
export function getMaxDistanceFromContinentCenter(iX, iContinentBottomRow, iContinentTopRow, iWestContinentLeftCol, iWestContinentRightCol, iEastContinentLeftCol, iEastContinentRightCol) {
    let iContinentLeftEdge = iWestContinentLeftCol;
    let iContinentRightEdge = iWestContinentRightCol;
    if (iX >= iEastContinentLeftCol) {
        iContinentLeftEdge = iEastContinentLeftCol;
    ...
    let iContinentCenterX = iContinentLeftEdge + (iContinentWidth / 2);
    let iContinentCenterY = iContinentBottomRow + (iContinentHeight / 2);
    let iDistance = GameplayMap.getPlotDistance(iContinentLeftEdge, iContinentBottomRow, iContinentCenterX, iContinentCenterY);
    return iDistance;
}
export function getSector(iX, iY, iRows, iCols, iContinentBottomRow, iContinentTopRow, iWestContinentLeftCol, iWestContinentRightCol, iEastContinentLeftCol) {
    let iContinentBase = 0;
    if (iX >= iEastContinentLeftCol) {
        iContinentBase += (iRows * iCols);
        iX = iX - iEastContinentLeftCol + iWestContinentLeftCol;
    }
    let iXSector = Math.floor((iX - iWestContinentLeftCol) / ((iWestContinentRightCol - iWestContinentLeftCol) / iCols));
    let iYSector = Math.floor((iY - iContinentBottomRow) / ((iContinentTopRow - iContinentBottomRow) / iRows));
    let iSector = (iYSector * iCols) + iXSector;
    let iReturnValue = iContinentBase + iSector;
    return iReturnValue;
}
export function getSectorRegion(iSector, iRows, iCols, iContinentBottomRow, iContinentTopRow, iWestContinentLeftCol, iWestContinentRightCol, iEastContinentLeftCol) {
    let region = { west: 0, east: 0, south: 0, north: 0, continent: 0 };
    if (iCols == 0)
        return region; // Safety check 
    let bIsEastContinent = (iSector >= (iRows * iCols));
    let iSectorAdjust = 0;
    if (bIsEastContinent) {
        iSectorAdjust = iRows * iCols;
    }
    let row = Math.floor((iSector - iSectorAdjust) / iCols);
    let col = Math.floor((iSector - iSectorAdjust) - (row * iCols));
...
export function getHeightAdjustingForStartSector(iX, iY, iWaterHeight, iFractalWeight, iCenterWeight, iStartSectorWeight, continent1, continent2, iStartSectorRows, iStartSectorCols, startSectors) {
    ...
    let iDistanceFromCenter = getDistanceFromContinentCenter(iX, iY, continent1.south, continent1.north, continent1.west, continent1.east, continent2.west, continent2.east);
    let iMaxDistanceFromCenter = getMaxDistanceFromContinentCenter(iX, continent1.south, continent1.north, continent1.west, continent1.east, continent2.west, continent2.east);
...
export function createIslands(iWidth, iHeight, continent1, continent2, iSize) {
...
export function applyCoastalErosion(continent, strength, falloff, minRadiusFactor, verticalOnly) {
...
export function applyCoastalErosionAdjustingForStartSectors(continent1, continent2, strength, falloff, minRadiusFactor, iStartSectorRows, iStartSectorCols, startSectors) {
    ...
    function isNearContinentCorner(iX, iY, continent, bufferScale) {
        ...
        return (isNearCorner(topLeft.x, topLeft.y) ||
...
export function getContinentEdgeHeightBump(iX, iY) {
...
export function getDistanceToClosestStart(iX, iY, numFoundEarlier, startPositions) {
...
export function addLandmassPlotTags(iX, iY, iEastContinentLeftCol) {
...
export function addWaterPlotTags(iX, iY, iEastContinentLeftCol) {
...
export function adjustOceanPlotTags(bWestSide) {
...
export function adjustLakePlotTags(region, bWestSide) {
...
export function isAdjacentToNaturalWonder(iX, iY) {
...
export function isOceanAccess(iX, iY) {
    for (let iDirection = 0; iDirection < DirectionTypes.NUM_DIRECTION_TYPES; iDirection++) {
        let iIndex = GameplayMap.getIndexFromXY(iX, iY);
        let iLocation = GameplayMap.getLocationFromIndex(iIndex);
        let iAdjacentX = GameplayMap.getAdjacentPlotLocation(iLocation, iDirection).x;
...
export function removeRuralDistrict(iX, iY) {
...
export function placeRuralDistrict(iX, iY) {
    let cityID = MapCities.getCity(iX, iY);
    if (cityID != null) {
        const city = Cities.get(cityID);
        if (city != null) {
...
export function replaceIslandResources(iWidth, iHeight, zResourceClassType) {
            ...
            if (GameplayMap.hasPlotTag(iX, iY, PlotTags.PLOT_TAG_ISLAND)) {
                ...
                if (resourceAtLocation != ResourceTypes.NO_RESOURCE) {
                    if (resources.length > 0) {
                        ...
                        if (resourceChosen != ResourceTypes.NO_RESOURCE) {
                            let iResourcePlotIndex = GameplayMap.getIndexFromXY(iX, iY);
                            if (iResourcePlotIndex != -1) {
                                ...
                                let oldName = GameInfo.Resources.lookup(resourceAtLocation)?.Name;
                                let name = GameInfo.Resources.lookup(resourceChosenIndex)?.Name;
...
export function isAdjacentToLand(iX, iY) {
    if (GameplayMap.hasPlotTag(iX, iY, PlotTags.PLOT_TAG_ISLAND)) {
...
//-------------------------------------------------------------------------------------------
export function shiftTerrain(iWidth, iHeight) {
    let shift_x = 0;
    let shift_y = 0;
...
// -------------------------------------------------------------------------------------------	
export function shiftPlotTypesBy(iWidth, iHeight, xshift, yshift) {
...
// -------------------------------------------------------------------------------------------	
export function determineXShift(iWidth, iHeight) {
...
// -------------------------------------------------------------------------------------------	
export function determineYShift(iWidth, iHeight) {
...
//-- FUNCTIONS TO SHIFT LANDMASSES (i.e. to better center them)
//-------------------------------------------------------------------------------------------
export function createOrganicLandmasses(iWidth, iHeight, continent1, continent2, iFractalGrain, iWaterPercent, iLargestContinentPercent) {
    ...
    while (!bLargeEnoughFound) {
        ...
        let iWaterHeight = FractalBuilder.getHeightFromPercent(globals.g_LandmassFractal, iWaterPercent);
...
export function clearContinent(continent1) {
    for (let iY = continent1.south; iY <= continent1.north; iY++) {
        for (let iX = continent1.west; iX <= continent1.east; iX++) {
            let terrain = globals.g_OceanTerrain;
            TerrainBuilder.setTerrainType(iX, iY, terrain);
