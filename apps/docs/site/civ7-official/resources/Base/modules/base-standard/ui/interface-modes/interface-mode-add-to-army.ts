/**
 * @file interface-mode-add-to-army.ts
...
interface AddToArmyInterfaceModeContext extends InterfaceMode.Context {
	UnitID: ComponentID;
	OperationArguments: any;
}
/**
 * Handler for INTERFACEMODE_ADD_TO_ARMY.
 */
class AddToArmyInterfaceMode extends ChoosePlotInterfaceMode {

	private _OperationResult: OperationResult | null = null;
	private validPlots = new Set<PlotIndex>();

	initialize() {
		const context = this.Context as AddToArmyInterfaceModeContext;
		const args: any = {};
		const result = Game.UnitCommands.canStart(context.UnitID, 'UNITCOMMAND_ADD_TO_ARMY', args, false);
	...
	commitPlot(plot: PlotCoord) {
		...
		args.X = plot.x;
		args.Y = plot.y;
		Game.UnitCommands.sendRequest(unitID, 'UNITCOMMAND_ADD_TO_ARMY', args);
		UI.sendAudioEvent(Audio.getSoundTag('data-audio-unit-packed', 'interact-unit'));
	}
}

InterfaceMode.addHandler('INTERFACEMODE_ADD_TO_ARMY', new AddToArmyInterfaceMode());