...
class DiplomacyProjectReactionPanel extends DiplomacyInputPanel {
    constructor(root) {
        super(root);
        this.firstMeetOpenReactionsListener = this.onFirstMeetOpenReactions.bind(this);
        this.diplomacyEventResponseListener = this.onDiploEventResponse.bind(this);
        this.diplomacyDialogUpdateResponseListener = this.hideShowDialog.bind(this);
        ...
        this.containerHide = null;
        this.panelHide = null;
        this.influenceContainer = null;
        this.influenceDiplomacyBalanceContainer = null;
        this.influenceContainerImg = null;
        this.diploEventResponseTimeoutCallback = 0;
    ...
    populateProjectResponses() {
        ...
        const localPlayerInfluence = Players.get(GameContext.localPlayerID)?.DiplomacyTreasury;
        ...
        const initialPlayerName = Players.get(DiplomacyManager.currentProjectReactionData.initialPlayer)?.name;
    ...
    createResponseItem(responseData, actionID) {
        ...
        const result = Game.PlayerOperations.canStart(GameContext.localPlayerID, PlayerOperationTypes.RESPOND_DIPLOMATIC_ACTION, args, false);
        if (!result.Success) {
            ...
            responseButton.addEventListener("action-activate", () => {
                ...
                if (actionData.actionType == DiplomacyActionTypes.DIPLOMACY_ACTION_DENOUNCE_MILITARY_PRESENCE && responseData.responseType == DiplomaticResponseTypes.DIPLOMACY_RESPONSE_REJECT) {
                    ...
                    const surpriseWarResults = playerDiplomacy.canDeclareWarOn(actionData.initialPlayer);
                    const formalWarResults = playerDiplomacy.canDeclareWarOn(actionData.initialPlayer, WarTypes.FORMAL_WAR);
                    ...
                    let ourWarSupport = playerDiplomacy.getTotalWarSupportBonusForPlayer(actionData.initialPlayer, formalWarResults.Success);
                    let theirWarSupport = playerDiplomacy.getTotalWarSupportBonusForTarget(actionData.initialPlayer, formalWarResults.Success);
                    let theirInfluenceBonus = playerDiplomacy.getWarInfluenceBonusTarget(actionData.initialPlayer, formalWarResults.Success);
    ...
    onFirstMeetOpenReactions() {
        ...
        const otherPlayer = Players.get(DiplomacyManager.currentDiplomacyDialogData.OtherPlayerID);
        if (!otherPlayer) {
    ...
    createFirstMeetGreetingButton(greetingType) {
        ...
        const result = Game.PlayerOperations.canStart(GameContext.localPlayerID, PlayerOperationTypes.RESPOND_DIPLOMATIC_FIRST_MEET, args, false);
        if (greetingType == DiplomacyPlayerFirstMeets.PLAYER_REALATIONSHIP_FIRSTMEET_FRIENDLY) {
            if (DiplomacyManager.currentDiplomacyDialogData != null) {
                ...
                const capitalRevealedString = Game.Diplomacy.getFM_RevealCapitalsString(GameContext.localPlayerID, otherPlayer);
                relationshipIcon.classList.add("panel-diplomacy-project-reaction__pos-influence", "panel-diplomacy-project-reaction__influence", "-mt-3", "bg-cover");
                cardTopWrapper.appendChild(relationshipIcon);
...
//# sourceMappingURL=file:///base-standard/ui/diplomacy-project-reaction/panel-diplomacy-project-reaction.js.map
