...
// #region Types
export enum ProductionPanelCategory {
	BUILDINGS = "buildings",
	UNITS = "units",
	PROJECTS = "projects",
	WONDERS = "wonders"
...
export type ProductionChooserUnitStat = {
	name: string;
	icon: string;
	value: string;
}
...
export type ProductionChooserItemData = {
	type: string;
	...
	disabled?: boolean;
	/* an html string to display as a tooltip */
...
export type LastProductionItemData = {
	typeHash: HashId,
	name: string,
	type: string,
	isUnit: boolean,
...
let agelessItemTypes: Set<string> | null = null;
...
function cycleCity(isNext: boolean): void {
...
export const GetNextCityID = (cityID: ComponentID) => getAdjacentCityID(cityID, true)
export const GetPrevCityID = (cityID: ComponentID) => getAdjacentCityID(cityID, false)

/**
 * returns the ID of the next or previous city in the player's list of cities
 */
const getAdjacentCityID = (cityID: ComponentID, isNext: boolean): ComponentID | null => {
	let targetCityID: ComponentID | null = null;

	const player = Players.get(cityID.owner);
	if (player) {
		let playerCities = player.Cities?.getCities();
		if (playerCities?.length) {
			const currentCityIndex = playerCities.findIndex((city: City) => ComponentID.isMatch(cityID, city.id));
			let targetCityIndex;
			if (isNext) {
...
export const GetUnitStatsFromDefinition = (definition: UnitDefinition): ProductionChooserUnitStat[] => {
	let stats: ProductionChooserUnitStat[] = [];
	...
	const statsDefinition: Unit_StatDefinition | null = GameInfo.Unit_Stats.lookup(definition.UnitType);
	if (statsDefinition) {
...
export const GetCurrentBestTotalYieldForConstructible = (city: City, constructibleType: ConstructibleType): ProductionChooserYield[] => {
	const yields: ProductionChooserYield[] = [];
...
export const GetSecondaryDetailsHTML = (items: { icon: string, value: string, name: string }[]): string => {
	...
	if (operationResult.Success || insufficientFunds || !hideIfUnavailable || (operationResult.NeededUnlock != -1 && !hideIfUnavailable)) {
		...
		if (operationResult.Success || insufficientFunds || !hideIfUnavailable) {
			...
			if (possibleLocations.length > 0 && !isBuildingAlreadyQueued && !operationResult.InsufficientFunds) {
				let name: string = constructible.Name;
				...
				const item: ProductionChooserItemData = {
					name,
			...
			if (canUnlockNode) {
				...
				let item: ProductionChooserItemData = {
					turns: -1,
 ...
 */
export const CanPlayerUnlockNode = (nodeType: ProgressionTreeNodeType | undefined, playerId: PlayerId): boolean => {
...
const getProjectItems = (city: City | null, isPurchase: boolean): ProductionChooserItemData[] => {
	const projects: ProductionChooserItemData[] = [];
	if (!city) {
	...
	GameInfo.Projects.forEach(project => {
		...
		if (result.Requirements && result.Requirements?.FullFailure != true) {
			...
			if (result.Requirements.MeetsRequirements) {
				...
				const projectItem: ProductionChooserItemData = {
					name: project.Name,
...
export const GetNumUniqueQuarterBuildingsCompleted = (city: City, uq: UniqueQuarterDefinition) => {
...
export const ShouldShowUniqueQuarter = (...results: OperationResult[]) => {
...
export const GetProductionItems = (
	...
	playerGoldBalance: number,
	isPurchase: boolean,
	viewHidden: boolean,
	uqInfo: UniqueQuarterInfo | null
) => {
	const items: Record<ProductionPanelCategory, ProductionChooserItemData[]> = {
	...
	let repairItems: ProductionChooserItemData[] = [];
...
const createRepairAllProductionChooserItemData = (cost: number, turns: number): ProductionChooserItemData | null => {
	...
	const isInsufficientFunds: boolean = cost > (localPlayer.Treasury?.goldBalance || 0);
	return {
...
const getConstructibleClassPanelCategory = (constructibleClass: string): ProductionPanelCategory => {
...
const getUnits = (city: City | null, playerGoldBalance: number, isPurchase: boolean, recommendations: BuildRecommendation[], viewHidden: boolean) => {
	const units: ProductionChooserItemData[] = [];
	if (!city?.Gold) {
	...
	for (const { index, result } of results) {
		...
		const data: ProductionChooserItemData = {
...
export const Construct = (
	city: City,
	item: ProductionChooserItemData,
	isPurchase: boolean
) => {
	const typeInfo = GameInfo.Types.lookup(item.type);
	if (typeInfo) {
		// produce a union of object types where only one key is present
		type RecordOne<Keys extends string, Value> = {
			[K in Keys]: Record<K, Value>
		}[Keys];

		let args: RecordOne<'ConstructibleType' | 'UnitType' | 'ProjectType', number> & {
			X?: number;
			Y?: number;
			InsertMode?: CityOperationsParametersValues;
		};
...
export const GetCityBuildReccomendations = (city: City | null): BuildRecommendation[] => {
	if (!city) return [];

	const recommendationParams: BuildRecommendationParameters = {
		cityId: city.id,
...
export const GetTownFocusItems = (cityID: ComponentID) => {
	const projects: TownFocusItem[] = [];
...
const isSelectedFocusItem = (
	currentGrowthType: GrowthTypes | undefined,
...
export const GetCurrentTownFocus = (
	cityID: ComponentID,
	currentGrowthType: GrowthTypes | undefined,
	currentProjectType: ProjectType | undefined
) => {
...
*/
export const SetTownFocus = (cityID: ComponentID, sType: string, projectType: string) => {
	const args = {
		Type: parseInt(sType),
...
export const GetTownFocusBlp = (growthType: string | number | null, projectType: string | number | null) => {
...
export const GetLastProductionData = (cityID: ComponentID) => {
	...
	if (ids.length > 0) {
		...
		if (lastCompleted) {
			...
			if (lastDefinition) {
				...
				let yields: { icon: string, value: string }[] = []
				...
				const data: LastProductionItemData = {
