...
export class RelationshipTooltipType implements TooltipType {
	...
	private hoveredPlayerID: PlayerId | null = null;
	...
	isUpdateNeeded(target: HTMLElement): boolean {
		const playerIDString: string | null = target.getAttribute("data-player-id");
		...
		const playerID: PlayerId = playerIDString ? parseInt(playerIDString) : PlayerIds.NO_PLAYER;
	...
	update() {
		...
		const relationshipHeader: HTMLElement = document.createElement("div");
		relationshipHeader.classList.add('flex', 'flex-col', 'justify-center', 'items-center', 'w-full');

		const relationshipTitle: HTMLElement = document.createElement("div");
		...
		if (playerDiplomacy.isAtWarWith(GameContext.localPlayerID) && this.hoveredPlayerID != PlayerIds.NO_PLAYER && this.hoveredPlayerID) {
			...
			const jointEvents: DiplomaticEventHeader[] = Game.Diplomacy.getJointEvents(GameContext.localPlayerID, this.hoveredPlayerID, false);
			...
			const warInfo: HTMLElement = document.createElement("div");
			...
			if (warEventHeader.initialPlayer == GameContext.localPlayerID) {
				...
				const totalSupport: number = warData.supportingEnvoys.length - warData.opposingEnvoys.length;
		...
		const agendaNames: string[] = Game.Diplomacy.getAgendaNames(playerID);
		if (player != null && player.isAI && agendaNames.length > 0) {
			const agendaDescs: string[] = Game.Diplomacy.getAgendaDescriptions(playerID);
...
class SanctionTooltipType implements TooltipType {
