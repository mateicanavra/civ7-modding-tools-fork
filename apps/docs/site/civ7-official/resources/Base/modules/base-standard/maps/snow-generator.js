 ...
 *
*/
export function generateSnow(mapWidth, mapHeight) {
    ...
    const snowRowStarts = [globals.g_PolarWaterRows, globals.g_PolarWaterRows];
    // Rows from the northern and southern starts, to apply snow to.
    const snowRowLimits = [snowRowEnd > snowRowStarts[0] ? snowRowEnd - snowRowStarts[0] : 0, snowRowEnd > snowRowStarts[1] ? snowRowEnd - snowRowStarts[1] : 0];
    ...
    let rowStart = [(mapHeight - 1) - snowRowStarts[0], snowRowStarts[1]];
    for (let pole = 0; pole != 2; ++pole) {
        ...
        if (rowCount > 0) {
            ...
            let chanceAdjustment = Math.ceil(changeTotalAdjustment / rowCount);
            let aWeightChance = [10, 30, 60];
...
export function dumpPermanentSnow(iWidth, iHeight) {
...
//# sourceMappingURL=file:///base-standard/maps/snow-generator.js.map
