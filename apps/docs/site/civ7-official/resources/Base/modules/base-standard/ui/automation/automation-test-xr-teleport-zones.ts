...
export class AutomationTestXRTeleportZones {
	...
	testingShouldReturnHome: boolean = false;
	testingIsMovingToNewLocation: boolean = false;

	allStartingZones: string[] = new Array<string>;

	state: XRTeleportTestState = XRTeleportTestState.Teleporting;

	private automationTestXRTeleportZonesListener: ArbitraryCallback = (command: string, ...args: any) => {
	...
	onTeleportCompleted(_data: XRTeleportCompleted_EventData) {
	...
	validateTeleport() {
		switch (this.state) {
			case XRTeleportTestState.Teleporting:
				{
					// Collect All zones
					let activeZones: string[] = new Array<string>();
					let activeZoneCount: number = XR.World.getZoneCount();
