...
type PlotTooltipConstructibleInfo = {
	location: float2;
	type: string;
	age: number;
	isAgeless: boolean;
...
class PlotTooltipType implements TooltipType<PlotCoord> {

	private plotCoord: float2 | null = null;

	// TODO these variables should be removed and this functionality should be handled by the tooltip manager
	private plotCoordTimeoutId = -1
	private updateReady = false;
	private incomingPlotCoord: float2 | null = null;

	private isShowingDebug: boolean = false;

	private readonly tooltip = document.createElement('fxs-tooltip');
	private readonly container = document.createElement('div');
	private readonly yieldsFlexbox = document.createElement('div');

	private currentAge = GameInfo.Ages.lookup(Game.age)?.ChronologyIndex ?? 0;
	private agelessTypes = new Set(GameInfo.TypeTags.filter(e => e.Tag == "AGELESS").map(e => e.Type));
	...
	isUpdateNeeded(plotCoord: PlotCoord): boolean {
	...
	update() {
		...
		const terrainLabel: string = this.getTerrainLabel(plotCoord);
		const biomeLabel: string = this.getBiomeLabel(plotCoord);
		const { featureLabel, featureTooltip } = this.getFeatureLabel(plotCoord);
		const continentName = this.getContinentName(plotCoord);
		const riverLabel: string = this.getRiverLabel(plotCoord);
		const routeName: string = this.getRouteName();
		const hexResource = this.getResource();
		const playerID: PlayerId = GameplayMap.getOwner(plotCoord.x, plotCoord.y);
		...
		if (featureLabel) {
			const tooltipSecondLine: HTMLDivElement = document.createElement("div");
			...
			if (featureTooltip) {
				const tooltipSecondLineAddendum: HTMLDivElement = document.createElement("div");
		...
		if (continentName) {
			if (riverLabel) {
				const tooltipThirdLine: HTMLDivElement = document.createElement("div");
		...
		if (specialistsLabel != "") {
			const specialistText: HTMLElement = document.createElement("div");
		...
		if (hexResource) {
			...
			const tooltipIndividualYieldFlex: HTMLDivElement = document.createElement("div");
			...
			const yieldIconShadow: HTMLDivElement = document.createElement("div");
			...
			const yieldIcon: HTMLDivElement = document.createElement("div");
			...
			const toolTipIndividualYieldValues: HTMLDivElement = document.createElement("div");
		...
		if (routeName) {
			...
			const toolTipRouteInfo: HTMLDivElement = document.createElement("div");
		...
		if (!UI.isCursorLocked()) {
			...
			const topUnit: Unit | null = this.getTopUnit(this.plotCoord);
			let showHostileCursor: boolean = false;
			let owningPlayerID: PlayerId = GameplayMap.getOwner(this.plotCoord.x, this.plotCoord.y);
			...
			const revealedState: RevealedStates = GameplayMap.getRevealedState(localPlayerID, plotCoord.x, plotCoord.y);
			if (Players.isValid(localPlayerID) && Players.isValid(owningPlayerID) && (revealedState == RevealedStates.VISIBLE)) {
				const owningPlayer: PlayerLibrary | null = Players.get(owningPlayerID);
				...
				if (owningPlayer?.isIndependent) {
					...
					if (independentID != PlayerIds.NO_PLAYER) {
						const relationship: IndependentRelationship = Game.IndependentPowers.getIndependentRelationship(independentID, localPlayerID);
						if (relationship == IndependentRelationship.HOSTILE) {
							showHostileCursor = true;
						}
					}
				} else {
					var hasHiddenUnit: boolean = false;
		...
		if (this.isShowingDebug) {
			const tooltipDebugFlexbox: HTMLDivElement = document.createElement("div");
			tooltipDebugFlexbox.classList.add("plot-tooltip__debug-flexbox");
			this.container.appendChild(tooltipDebugFlexbox);

			this.appendDivider();

			const playerID: PlayerId = GameplayMap.getOwner(this.plotCoord.x, this.plotCoord.y);
			...
			const toolTipDebugTitle: HTMLDivElement = document.createElement("div");
			...
			const toolTipDebugPlotCoord: HTMLDivElement = document.createElement("div");
			...
			const toolTipDebugPlotIndex: HTMLDivElement = document.createElement("div");
	...
	isBlank(): boolean {
		...
		const localPlayerID: PlayerId = GameContext.localPlayerID;
		const revealedState: RevealedStates = GameplayMap.getRevealedState(localPlayerID, this.plotCoord.x, this.plotCoord.y);
		...
		if (selectedUnitID && ComponentID.isValid(selectedUnitID)) {
			...
			const plotUnits: ComponentID[] = MapUnits.getUnits(this.plotCoord.x, this.plotCoord.y);
			...
			let combatType: CombatTypes | null = Game.Combat.testAttackInto(selectedUnitID, args);
	...
	private appendHeaderDivider(text: string) {
		const divider = document.createElement("div");
		divider.classList.add("flex", "flex-row", "items-center", "justify-center", "my-1\\.5");

		const leftDivider: HTMLDivElement = document.createElement("div");
		leftDivider.classList.add("plot-tooltip__TitleLineleft");
		divider.appendChild(leftDivider);

		const tooltipImprovementName: HTMLDivElement = document.createElement("div");
		...
		const rightDivider: HTMLDivElement = document.createElement("div");
	...
	private appendTooltipInformation(title: string, localizedText?: string[], icon?: string, headerText?: string | false) {
		if (headerText) {
	...
	private getContinentName(location: float2) {
	...
	private getConstructibleInfo(constructible: ComponentID, plotCoordinate: float2): PlotTooltipConstructibleInfo | null {
	...
	private addConstructibleInformation(plotCoordinate: float2) {
		...
		const constructibles = plotConstructibles
			...
			.filter((c): c is PlotTooltipConstructibleInfo => c != null)
	...
	private getPlayerName(): string {
		...
		const localPlayerID: PlayerId = GameContext.localPlayerID;
		const name: string = Locale.stylize(player.name) + ((playerID == localPlayerID) ? (" (" + Locale.compose("LOC_PLOT_TOOLTIP_YOU") + ")") : "");
		return name;
	}

	private getCivName(): string {
		const playerID: PlayerId = GameplayMap.getOwner(this.plotCoord!.x, this.plotCoord!.y);
		const player: PlayerLibrary | null = Players.get(playerID);
		if (player == null) {
			return "";
		...
		const name: string = Locale.compose(GameplayMap.getOwnerName(this.plotCoord!.x, this.plotCoord!.y));
		return name;
	}

	private getTerrainLabel(location: float2): string {
	...
	private getBiomeLabel(location: float2): string {
		const biomeType: BiomeType = GameplayMap.getBiomeType(location.x, location.y);
	...
	private getResource(): ResourceDefinition | null {
		if (this.plotCoord) {
	...
	private getSpecialistDescription(): string {
		if (this.plotCoord) {
	...
	private getRouteName(): string {
		const routeType: RouteType = GameplayMap.getRouteType(this.plotCoord!.x, this.plotCoord!.y);
		const route = GameInfo.Routes.lookup(routeType);
		const isFerry: boolean = GameplayMap.isFerry(this.plotCoord!.x, this.plotCoord!.y);
	...
	private addPlotEffectNames(plotIndex: number) {
	...
	private getTopUnit(location: float2): Unit | null {
		let plotUnits: ComponentID[] = MapUnits.getUnits(location.x, location.y);
		if (plotUnits && plotUnits.length > 0) {
			const topUnit: Unit | null = Units.get(plotUnits[0]);
			return topUnit;
		}
		return null;
	}
	 ...
	 */
	private addOwnerInfo(location: float2, playerID: PlayerId) {
		...
		if (player.isIndependent) {
			...
			const plotTooltipOwnerLeader: HTMLElement = document.createElement("div");
			...
			const localPlayerID: PlayerId = GameContext.localPlayerID;
			const relationship: string = GameplayMap.getOwnerHostility(location.x, location.y, localPlayerID);
			if (relationship != null) {
				const plotTooltipOwnerRelationship: HTMLElement = document.createElement("div");
			...
			const tooltipCityBonusInfo: HTMLDivElement = document.createElement("div");
			...
			const plotTooltipOwnerCiv: HTMLElement = document.createElement("div");
			...
			const plotTooltipConqueror: HTMLElement | null = this.getConquerorInfo(districtId);
	...
	private getConquerorInfo(districtId: ComponentID | null) {
		if (!districtId) {
			return null;
		}
		...
		if (district.owner != district.controllingPlayer) {
			const conqueror: PlayerLibrary | null = Players.get(district.controllingPlayer);
			if (!conqueror) {
			...
			if (conqueror.isIndependent) {
				...
				const conquerorName: string = Locale.compose(conqueror.civilizationFullName);
				const plotTooltipConqueredCiv: HTMLElement = document.createElement("div");
	...
	private getRiverLabel(location: float2): string {
		const riverType: RiverTypes = GameplayMap.getRiverType(location.x, location.y);
		if (riverType != RiverTypes.NO_RIVER) {
			let riverNameLabel: string = GameplayMap.getRiverName(location.x, location.y);
			if (!riverNameLabel) {
				switch (riverType) {
	...
	private getFeatureLabel(location: float2) {
		let featureLabel = '';
		let featureTooltip = '';
		const featureType: FeatureType = GameplayMap.getFeatureType(location.x, location.y);
	...
	private addUnitInfo(location: float2): PlotTooltipType {
		const localPlayerID: PlayerId = GameContext.localObserverID;
		if (GameplayMap.getRevealedState(localPlayerID, location.x, location.y) != RevealedStates.VISIBLE) {
			return this;
		}
		let topUnit: Unit | null = this.getTopUnit(location);
		if (topUnit) {
		...
		const player: PlayerLibrary | null = Players.get(topUnit.owner);
		if (!player) {
			return this;
		}

		let unitName: string = Locale.compose(topUnit.name);

		this.appendDivider();

		const toolTipUnitInfo: HTMLDivElement = document.createElement("div");
		...
		const toolTipUnitCiv: HTMLDivElement = document.createElement("div");
		toolTipUnitCiv.classList.add("plot-tooltip__Civ-Info");

		if (player.isMinor || player.isIndependent) {
			const independentID: PlayerId = Game.IndependentPowers.getIndependentPlayerIDFromUnit(topUnit.id);
			if (independentID != PlayerIds.NO_PLAYER) {
				const indy: PlayerLibrary | null = Players.get(independentID);
				if (indy) {
					...
					const relationship: string = Game.IndependentPowers.getIndependentHostility(independentID, localPlayerID);

					const toolTipUnitRelationship: HTMLDivElement = document.createElement("div");
			...
			const toolTipUnitOwner: HTMLElement = document.createElement('div');
			...
			if (playerDiplomacy) {
				const relationship: string = playerDiplomacy.getRelationshipLevelName(player.id)
				if (relationship && player.id != localPlayerID) {
					const toolTipUnitRelationship: HTMLElement = document.createElement("div");
					toolTipUnitRelationship.classList.add("plot-tooltip__Unit-Relationship-Info");
	 ...
	 * @param {playerId} playerID The player associated with the request.
	 */
	private addPlotYields(location: float2, playerID: PlayerId) {

		const fragment = document.createDocumentFragment();
		let maxValueLength = 0;
		let totalYields = 0;
		for (const yieldInfo of GameInfo.Yields) {
			const yieldAmount: number = GameplayMap.getYield(location.x, location.y, yieldInfo.YieldType, playerID);
			if (yieldAmount > 0) {
				...
				const value: string = yieldAmount.toString();
	 ...
	 * @param {float2} location The X,Y plot location.
	*/
	private addPlotDistrictInformation(location: float2) {
		...
		const isUnderSiege: boolean = playerDistricts.getDistrictIsBesieged(location);

		if (!DistrictHealthManager.canShowDistrictHealth(currentHealth, maxHealth)) {
			return;
		}

		const districtContainer: HTMLDivElement = document.createElement("div");
		districtContainer.classList.add("plot-tooltip__district-container");

		const districtTitle: HTMLDivElement = document.createElement("div");
		...
		const districtHealth: HTMLDivElement = document.createElement("div");
		districtHealth.classList.add("plot-tooltip__district-health");

		const healthCaption: HTMLDivElement = document.createElement("div");
	 ...
	 * @returns 
	 */
	private addSettlingInformation(localPlayer: PlayerLibrary, location: float2) {
		...
		if (isLand && isPassable && isNotRiver) {
			const settlerTooltip: HTMLDivElement = document.createElement("div");
			settlerTooltip.classList.add("plot-tooltip__settler-tooltip");

			let addDivider = true;
			const localPlayerAdvancedStart: PlayerAdvancedStart | undefined = localPlayer?.AdvancedStart;
