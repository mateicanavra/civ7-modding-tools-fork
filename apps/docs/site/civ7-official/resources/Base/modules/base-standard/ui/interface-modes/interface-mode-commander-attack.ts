/**
 * @file interface-mode-commander-attack.ts
...
interface CommanderAttackInterfaceModeContext extends InterfaceMode.Context {
	UnitID: ComponentID;
	OperationArguments: any;
}
/**
 * Handler for INTERFACEMODE_COMMANDER_ATTACK.
 */
class CommanderAttackInterfaceMode extends ChoosePlotInterfaceMode {

	private _OperationResult: OperationResult | null = null;
	private validPlots = new Set<PlotIndex>();

	initialize() {
		const context = this.Context as CommanderAttackInterfaceModeContext;
		const args: any = {};
		const result = Game.UnitCommands.canStart(context.UnitID, 'UNITCOMMAND_COMMANDER_ATTACK', args, false);
	...
	commitPlot(plot: PlotCoord) {
		...
		args.X = plot.x;
		args.Y = plot.y;
		Game.UnitCommands.sendRequest(unitID, 'UNITCOMMAND_COMMANDER_ATTACK', args);
	}
}

InterfaceMode.addHandler('INTERFACEMODE_COMMANDER_ATTACK', new CommanderAttackInterfaceMode());