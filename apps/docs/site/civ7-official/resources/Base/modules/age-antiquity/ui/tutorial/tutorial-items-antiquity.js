...
import { NextItemStatus, TutorialAnchorPosition } from '/base-standard/ui/tutorial/tutorial-item.js';
                ...
                if (activationEventData.productionKind == ProductionKind.UNIT) {
                    ...
                    if (unitDefinition) {
                        if (unitDefinition.UnitMovementClass == "UNIT_MOVEMENT_CLASS_RECON") {
                            ...
                            const prompts = TutorialSupport.getTutorialPrompts(actionPromptsDef);
...
TutorialManager.add({
    ...
    onCompleteCheck: (_item) => {
        let mode = InterfaceMode.getCurrent();
...
TutorialManager.add({
    ...
    onActivateCheck: (_item) => {
        ...
        const blockingNotification = TutorialSupport.getCurrentTurnBlockingNotification(localPlayerID);
                        ...
                        if (city.Districts) {
                            for (const districtId of city.Districts?.getIds()) {
                                ...
                                if (district != null && district.location != null && district.location != city.location) {
                                    ...
                                    let resource = GameplayMap.getResourceType(loc.x, loc.y);
    ...
    quest: {
        ...
        getCurrentProgress: () => {
            ...
            if (player) {
                ...
                let iRevealedCount = (liveOpsStats ? liveOpsStats.numPlotsRevealed : 0);
                ...
                if (player) {
                    ...
                    if (activationEventData.viewingObjectID != null) {
                        ...
                        if (independentID != PlayerIds.NO_PLAYER) {
                            let independentName = Game.IndependentPowers.independentName(independentID);
                            let independentDiplo = Game.IndependentPowers.getIndependentHostility(independentID, GameContext.localPlayerID);
            ...
            getLocParams: (_item) => {
                ...
                if (player && player.Units) {
                    ...
                    if (unit != null && unit.owner == player?.id) {
                        let unitDef = GameInfo.Units.lookup(unit.type);
                        if (unitDef != null) {
        ...
        if (player != null) {
            ...
            if (city != null && city.owner == player?.id) {
                if (activationEventData.productionKind == ProductionKind.CONSTRUCTIBLE) {
                    let prodItem = activationEventData.productionItem;
                    let itemDef = GameInfo.Constructibles.lookup(prodItem);
                    if (itemDef != null) {
    ...
    callout: {
        ...
        body: {
            ...
            getLocParams: (_item) => {
                ...
                let strengthBonus = "";
                const activationEventData = TutorialManager.activatingEvent;
                let itemDef = GameInfo.Constructibles.lookup(activationEventData.constructibleType);
                ...
                if (player != null) {
                    ...
                    if (city != null && city.owner == player?.id) {
                        if (activationEventData.productionKind == ProductionKind.UNIT) {
                            ...
                            if (unitDef != null) {
                                let unitStats = GameInfo.Unit_Stats.lookup(unitDef.UnitType);
    ...
    callout: {
        ...
        body: {
            ...
            getLocParams: (_item) => {
                ...
                const activationEventData = TutorialManager.activatingEvent;
                if (activationEventData) {
                    let constructibleDef = GameInfo.Constructibles.lookup(activationEventData.improvementType);
    ...
    onActivateCheck: (_item) => {
        ...
        if (unitId) {
            ...
            if (player && player.Units && unit) {
                const unitList = player.Units.getUnitTypesUnlockedWithTag("UNIT_CLASS_MAKE_TRADE_ROUTE", false);
            ...
            getLocParams: (_item) => {
                ...
                if (player) {
                    ...
                    if (independentID != PlayerIds.NO_PLAYER) {
                        ...
                        let independentName = Game.IndependentPowers.independentName(independentID);
                        let isEncampment = Game.IndependentPowers.isIndependentEncampment(independentID);
    ...
    onActivateCheck: (_item) => {
        ...
        if (player) {
            ...
            if (independentID != PlayerIds.NO_PLAYER) {
                ...
                let isPlayer1Encampment = Game.IndependentPowers.isIndependentEncampment(activationEventData.player1);
                let isPlayer2Encampment = Game.IndependentPowers.isIndependentEncampment(activationEventData.player2);
                if (activationEventData.player1 == GameContext.localPlayerID || activationEventData.player2 == GameContext.localPlayerID) {
                    ...
                    for (let i = 0; i < PlayerList.length; ++i) {
                        if (PlayerList[i].isMajor && PlayerList[i].id != player.id) {
                            if (player.Diplomacy?.isAtWarWith(PlayerList[i].id)) {
                                ...
                                const peaceQueryResults = player.Diplomacy.canMakePeaceWith(PlayerList[i].id, false);
...
//# sourceMappingURL=file:///age-antiquity/ui/tutorial/tutorial-items-antiquity.js.map
