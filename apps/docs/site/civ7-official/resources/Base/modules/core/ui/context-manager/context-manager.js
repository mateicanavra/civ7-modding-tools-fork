...
export var ContextManagerEvents;
(function (ContextManagerEvents) {
    ContextManagerEvents.OnChanged = "OnContextManagerChanged";
    ContextManagerEvents.OnOpen = "OnContextManagerOpen";
    ContextManagerEvents.OnClose = "OnContextManagerClose";
...
class ContextManagerSingleton {
    constructor() {
        ...
        this.receiveFocusEvent = new CustomEvent('event-mgr-receive-focus');
        this.loseFocusEvent = new CustomEvent('event-mgr-lose-focus');
        this.popFocusEvent = new CustomEvent('event-mgr-pop');
        this.targetSlotPrefix = "target-slot-";
        this.lastActivatedComponent = null;
    ...
    get isEmpty() { return (this.screens.length == 0); }
    /** ------------------------------------------------------------------------------------------------------------------
    ...
    push(targetClassName, prop) {
        ...
        if (prop && !prop.singleton) {
            ...
            if (targetIndex == -1) {
                ...
                var possibleSpecificTarget = null;
    ...
    popUntil(targetName, prop) {
        ...
        DialogManager.clear();
        var foundIndex = this.getTargetIndex(targetName);
    ...
    popIncluding(targetClassName, prop) {
        var foundIndex = this.getTargetIndex(targetClassName);
        if (foundIndex == 0 && prop && prop.mustExist) {
    ...
    getTarget(target) {
        if (target instanceof HTMLElement) {
            return this.screens.find(function (item) { return item == target; });
        }
        else {
            const targetTagName = target.toUpperCase();
            return this.screens.find(function (item) { return item.tagName == targetTagName; });
        }
    }
    /** ------------------------------------------------------------------------------------------------------------------
    ...
    getTargetIndex(target) {
        if (target instanceof HTMLElement) {
            return this.screens.findIndex(function (item) { return item == target; });
        }
        else {
            const targetTagName = target.toUpperCase();
            return this.screens.findIndex(function (item) { return item.tagName == targetTagName; });
        }
    }
    /** ------------------------------------------------------------------------------------------------------------------
    ...
    unregisterEngineInputHandler(engineEventHandler) {
        const index = this.engineInputEventHandlers.findIndex((handler) => { return handler == engineEventHandler; });
        if (index == -1) {
...
//# sourceMappingURL=file:///core/ui/context-manager/context-manager.js.map
