...
import { SliderValueChangeEvent } from '/core/ui/components/fxs-slider.js';
...
export const EditorCalibrateHDROpenedEventName = 'editor-calibrate-hdr-opened' as const;
class EditorCalibrateHDROpenedEvent extends CustomEvent<never> {
	constructor() {
		super(EditorCalibrateHDROpenedEventName, { bubbles: false, cancelable: true });
	}
}

export const EditorCalibrateHDRClosedEventName = 'editor-calibrate-hdr-closed' as const;
class EditorCalibrateHDRClosedEvent extends CustomEvent<never> {
	constructor() {
...
class EditorCalibrateHDR extends Panel {
	...
	private confirmButtonListener = this.close.bind(this);

	private hdrSliderChangedListener = this.onHdrOptionChanged.bind(this);

	private contrastBar = document.createElement('fxs-slider');
	private brightness3dBar = document.createElement('fxs-slider');
	private uiBrightnessBar = document.createElement('fxs-slider');

	private CalibrateHDRSceneModels: WorldUI.ModelGroup | null = null;
	...
	onAttach() {
		...
		const hdrOptionPage = this.Root.querySelector<HTMLElement>('.editor-calibrate-hdr_main-content');
		if (!hdrOptionPage) {
	...
	private onHdrOptionChanged(sliderEvent: SliderValueChangeEvent) {
