...
import { InputEngineEvent, NavigateInputEvent, NavigateInputEventName } from '/core/ui/input/input-support.js';

export class FxsTextboxValidateVirtualKeyboard extends CustomEvent<{ value: string }> {
...
export class TextBoxTextChangedEvent extends CustomEvent<{ newStr: string }> {
...
export class TextBoxTextEditStopEvent extends CustomEvent<{ confirmed: boolean, inputEventName?: string }> {
...
class FxsTextbox extends ChangeNotificationComponent {
	...
	private textInputFocusListener = this.onTextInputFocus.bind(this);
	...
	private placeholder = "";
	private isPlaceholderActive = true;

	private overrideText = true;
	private showKeyboardOnActivate = true;

	private caseMode: FxsTextBoxCaseMode = FxsTextBoxCaseMode.NO_CASE;
	...
	//navigation handling.
	private onNavigateInput(inputEvent: NavigateInputEvent) {
		if (this.Root.getAttribute("enabled") == 'true') {
			inputEvent.stopPropagation();
			inputEvent.preventDefault();
		}
	...
	private onTextInputFocus(_event: FocusEvent) {
	...
	onActivate(inputEvent?: InputEngineEvent) {
	...
	private onVirtualKeyboardTextEntered(text: GenericDataString) {
		...
		if (this.Root.hasAttribute("max-length")) {
			const maxLength: number = Number.parseInt(this.Root.getAttribute("max-length") ?? "-1");
