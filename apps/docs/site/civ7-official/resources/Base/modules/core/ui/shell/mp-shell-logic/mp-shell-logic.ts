...
import DialogManager, { DialogBoxCallbackSignature, DialogBoxID, DialogBoxOption } from '/core/ui/dialog-box/manager-dialog-box.js';
...
interface MPUIEventData {
	exampleAdditionalMPData: string;
	exampleAdditionalMPDataStuff: Object;

	dialogTitle: string;
	dialogBody: string;
	dialogCallback: DialogBoxCallbackSignature;
}

export const MultiplayerMatchMakeCompleteEventName = 'mp-game-match-make-complete' as const;
...
export const MultiplayerMatchMakeFailEventName = 'mp-game-match-make-fail' as const;
...
export const MultiplayerJoinCompleteEventName = 'mp-game-join-complete' as const;
...
export const MultiplayerJoinFailEventName = 'mp-game-join-fail' as const;
export class MultiplayerJoinFailEvent extends CustomEvent<{ error: string }> {
	constructor(error: string) {
		super('mp-game-join-fail', { bubbles: false, cancelable: true, detail: { error } });
	}
}

export const MultiplayerCreateCompleteEventName = 'mp-game-create-complete' as const;
...
export const MultiplayerCreateFailEventName = 'mp-game-create-fail' as const;
export class MultiplayerCreateFailEvent extends CustomEvent<{ result: NetworkResult }> {
	constructor(result: NetworkResult) {
		super('mp-game-create-fail', { bubbles: false, cancelable: true, detail: { result } });
	}
}

export const MultiplayerCreateAttemptEventName = 'mp-game-create-attempt' as const;
...
export const MultiplayerGameAbandonedEventName = 'mp-game-abandoned' as const;
export class MultiplayerGameAbandonedEvent extends CustomEvent<{ reason: NetworkUtilities.AbandonReasonPopup }> {
...
class MultiplayerShellManagerSingleton implements IEngineInputHandler {

	private static _Instance: MultiplayerShellManagerSingleton;
	serverType: ServerType = ServerType.SERVER_TYPE_NONE;
	skipToGameCreator: boolean = false; // Landing dialog will select Internet and go straight to Host MP game (Sony activity Flow)
	private waitingForParentalPermissions: boolean = false;
	private needToDisplayExitGameErrorDialog: boolean = false;
	private canMPDialogShow: boolean = true;
	private savedErrorTitle: string = "";
	private savedErrorBody: string = "";

	private hostCreatingGameDialogBoxID?: DialogBoxID;
	private clientJoiningGameDialogBoxID?: DialogBoxID;
	private clientMatchmakingGameDialogBoxId?: DialogBoxID;
	private premiumWaitDialogBoxID?: DialogBoxID;
	private accountNotLinkedDialogBoxID?: DialogBoxID;
	private noPremiumDialogBoxID?: DialogBoxID;
	private noChildPermissionDialogBoxID?: DialogBoxID;
	private searchTimeoutDialogBoxID?: DialogBoxID;
	private exitGameErrorDialogBoxID?: DialogBoxID;
	private exitBrowserDialogBoxID?: DialogBoxID;

	private multiplayerGameAbandonedListener = (data: MultiplayerGameAbandonedData) => { this.onMultiplayerGameAbandoned(data); }
	private childNoPermissionsDialogListener = () => { this.onChildNoPermissionsDialog(); }
	private parentalPermissionListener = this.openChildMultiplayer.bind(this);

	private constructor() {

		engine.whenReady.then(() => {

			engine.on('error_unplugged_network_cable_EXAMPLE', (data: MPUIEventData) => {
			...
			engine.on('error_generic_EXAMPLE', (data: MPUIEventData) => {
				// Dialog with one option, generic confirm.
			...
			engine.on('PremiumServiceCheckComplete', (data: PremiumServiceCheckCompleteData) => { this.onPremiumServiceCheckComplete(data); });
			engine.on('MultiplayerJoinRoomAttempt', () => { this.onJoiningInProgress(); });
			engine.on('MultiplayerJoinGameComplete', () => { this.onJoinSuccess(); });
			engine.on('MultiplayerJoinRoomFailed', (data: JoinRoomFailedData) => { this.onMultiplayerJoinRoomFailed(data); });
	...
	onError_SearchIsTakingALongTime() {
		...
		const continueWaitingCallback: DialogBoxCallbackSignature = () => {
			this.searchTimeoutDialogBoxID = undefined;
		}
		let continueWaitingOption: DialogBoxOption = {
			actions: ["accept"],
			label: "LOC_MP_ERROR_CONTINUE_WAITING", //Example doesn't really exist
			callback: continueWaitingCallback,
		}

		// Option 2: return to lobby
		const returnToLobbyCallback: DialogBoxCallbackSignature = () => {
			this.searchTimeoutDialogBoxID = undefined;
		}
		let returnToLobbyOption: DialogBoxOption = {
			actions: ["cancel", "keyboard-escape"],
			label: "LOC_MP_ERROR_RETURN_TO_LOBBY", //Example doesn't really exist
			callback: returnToLobbyCallback,
		}

		// Raise dialog
		const options: DialogBoxOption[] = [continueWaitingOption, returnToLobbyOption];
	...
	onJoiningInProgress() {
		...
		// Joining Game popup
		const cancelWaitingCallback: DialogBoxCallbackSignature = () => {
	...
	onMultiplayerJoinRoomFailed(data: JoinRoomFailedData) {
		let joinErrorStr: string = "LOC_JOIN_GAME_ROOM_UNKNOWN_ERROR"
		let dataErrorStr: string | undefined = joinGameErrorTypeToErrorBody.get(data.error);
		if (dataErrorStr) {
	...
	onJoiningFail(errorMessage: string) {
		if (this.hostCreatingGameDialogBoxID != undefined) {
			DialogManager.closeDialogBox(this.hostCreatingGameDialogBoxID);
	...
	private onPremiumServiceCheckComplete(event: PremiumServiceCheckCompleteData) {
	...
	onGameBrowse(serverType: ServerType, skipToGameCreator: boolean = false) {
	...
	onAutomatch(matchAgeID: string) {
		...
		if (success) {
			...
			let matchmakeErrMsg: string = Locale.compose("LOC_UI_MP_LANDING_MATCHMAKE_START_ERROR");
	...
	*/
	exitMPGame(errorTitle: string, errorBody: string) {
	...
	browserExitError(errorTitle: string, errorBody: string) {
		this.exitToMainMenu();
	...
	hostMultiplayerGame(eServerType: ServerType) {
		if (this.hostCreatingGameDialogBoxID != undefined) {
			return;
		}
		...
		// Creating Game popup
		const cancelCreateGameCallback: DialogBoxCallbackSignature = () => {
			this.hostCreatingGameDialogBoxID = undefined;
			Network.leaveMultiplayerGame();
			window.dispatchEvent(new ResumeCloseListenerEvent());
	...
	private onLobbyError({ errorCode }: MultiplayerLobbyErrorData) {
	 ...
	 */
	private delayExecute(func: any, frames = 2) {
		if (frames == 0) {
			func();
		} else {
