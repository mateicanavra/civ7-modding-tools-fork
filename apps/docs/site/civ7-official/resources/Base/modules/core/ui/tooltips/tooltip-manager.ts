...
import { TooltipController, TooltipControllerInitializationParams } from '/core/ui/tooltips/tooltip-controller.js';
...
const DEFAULT_DELAY: number = 200; // Default delay for Gamepad and touch tooltip
...
export enum PlotTooltipPriority {
	HIGH = 0,
	LOW = 1,
}
...
export class HidePlotTooltipEvent extends CustomEvent<void> {
	constructor() {
...
export class ShowPlotTooltipEvent extends CustomEvent<void> {
	constructor() {
...
// ---------------------------------------------------------------------------
class TooltipManagerSingleton implements IEngineInputHandler {

	private static Instance: TooltipManagerSingleton;
	private timeShowStart: DOMHighResTimeStamp;
	private isShownByTimeout: boolean = true;		// Will the tooltip auto-show based on timeout?
	private isToggledOn: boolean = ActionHandler.deviceType != InputDeviceType.Touch;			// Is the tooltip forced into an on position? (via key/button press)
	private currentIsToggleOn: boolean = false; // isToggledOn after onUpdate 
	private isAnimating: boolean = false;			// Is the tooltip in the process of animating?
	private ttTypeName: string = "none";			// What is the currently showing tooltip type
	private x: number = INVALID;
	private y: number = INVALID;
	private readonly root = document.createElement('div');	// Root element for tooltips

	private _tooltip: ComponentRoot<Tooltip> | null = null;
	private get tooltip() { return this._tooltip; }
	private set tooltip(newTooltip: ComponentRoot<Tooltip> | null) {
	...
	private types: { [type: string]: TooltipType<HTMLElement> } = {};
	private plotTooltipTypes: { [type: string]: { instance: TooltipType<PlotCoord>, priority: number } } = {};
	private orderedPlotTooltipTypes: string[] = [];
	private closeOnNextMove: boolean = false;
	private plotTooltipGlobalHidden: boolean = false;
	private plotTooltipTutorialHidden: boolean = false;	// Needs to be separate from plotTooltipGlobalHidden because the view mode will change many times during tutorial and change that setting
	private touchPosition: float2 | null = null;
	private touchTarget: HTMLElement = document.body;

	private tooltipResizeObserver = new ResizeObserver(this.updateTooltipPosition.bind(this));

	// Used by the debug widget to shutdown tooltips temporarilly.
	private tooltipsDisabled: boolean = false;
	...
	private cameraWasDragging = false;

	private globalPlotTooltipHideListener = this.onGlobalPlotTooltipHide.bind(this)
	private globalPlotTooltipShowListener = this.onGlobalPlotTooltipShow.bind(this)
	private tooltipAnimationListener = this.onTooltipAnimationFinished.bind(this)
	...
	public get currentTooltip(): HTMLElement | null {
	...
	onReady() {
		...
		const tooltipsDiv: HTMLElement | null = document.getElementById("tooltips");
		if (tooltipsDiv) {
		...
		window.addEventListener(ActiveDeviceTypeChangedEventName, (event: CustomEvent) => this.onActiveDeviceTypeChanged(event));
		...
		const disablePlotTooltips: UIDebugWidgetDefinition = {
	...
	private onEngineInput = (name: string) => {
	...
	private onToggleTooltip(status: number) {
	...
	private onUpdate() {
		...
		if (this.isAnimating) {
			...
			const delta: number = (now - this.timeShowStart);
			const delay: number = (this.isShownByTimeout) ? Configuration.getUser().tooltipDelay : DEFAULT_DELAY;
			if (delta < delay) {
				// Delay to showing.
			} else {
				// Fade in
				let amount: number = (delta - delay) * 0.01;
				if (amount >= 1.0) {
					this.isAnimating = false;
				}
			}
	...
	private getTooltipTypeName(targetElement: HTMLElement): string {
	...
	private cursorTooltipCheck() {
		...
		const alternativeTargetClass: string | null = targetElement.getAttribute('data-tooltip-alternative-target');
		if (alternativeTargetClass) {
			targetElement = this.root.querySelector<HTMLElement>(`.${alternativeTargetClass}`) ?? targetElement;
		}
		...
		let ttType: TooltipType<HTMLElement | PlotCoord>;
		let isUpdateNeeded: boolean;
	 ...
	 */
	registerType(type: string, tooltipInstance: TooltipType<HTMLElement>) {
	 ...
	 * @param tooltipInstance Instance of type to use when that style if found. (Instance is recycled for each tooltip of that type.)
	 */
	registerPlotType(type: string, priority: PlotTooltipPriority, instance: TooltipType<PlotCoord>) {
	...
	private setLocation(point_x: number, point_y: number) {
		this.x = point_x;
		this.y = point_y;
	}
	...
	private onTooltipAnimationFinished(event: AnimationEvent) {
...
export class Tooltip extends Component {
	...
	private shortDelayThreshold: number = 500;
...
let ttInstance: TooltipController | null = null;
function initialize() {
	...
	const options: TooltipControllerInitializationParams = {
		containerElement: ttContainer,
		tooltipRootElement: ttRoot,
		tooltipContentElement: ttContent,
		transitionDelay: 0,
		expirationDelay: 3000,	// show tooltip for 3 seconds after expiration
		resetDelay: 800,
		pointerOffsetX: 16,
		pointerOffsetY: 16,
	}
	...
	// Register debug widget.
	const disableTooltips: UIDebugWidgetDefinition = {
		id: 'disableTooltips',
		category: 'Systems',
