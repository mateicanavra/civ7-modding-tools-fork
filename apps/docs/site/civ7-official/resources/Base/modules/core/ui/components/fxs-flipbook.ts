...
export interface FlipbookDefinition {
    // Frames per second
    fps: number;
    // 'true' to preload images
    preload: boolean;
    // The actual texture atlases
    atlas: FlipbookFrame[];
}
...
export class FxsFlipBook extends Component {
    ...
    public onAttach() {
        const attribute: string | null = this.Root.getAttribute("data-flipbook-definition");
        if (attribute) {
            const flipbookDef: FlipbookDefinition = JSON.parse(attribute);
     ...
     */
    public createFlipbook(atlas: FlipbookFrame[], fps: number, preload: boolean = false) {

        // copy and format into this.atlas
        for (let i: number = 0; i < atlas.length; i++) {
            let nFrames: number = -1;
            if (atlas[i].nFrames) {
        ...
        let f = 0;
        for (let item: number = 0; item < this.atlas.length; item++) {
            const a: AtlasItem = this.atlas[item];
    ...
    public goto(frame: number): void {
        if (frame < 0 || frame > this.nFrames) console.error(`Trying to skip to frame ${frame} but it is out of bounds (0 <= f < ${this.nFrames})`);
        else {
            this.frame = frame;
