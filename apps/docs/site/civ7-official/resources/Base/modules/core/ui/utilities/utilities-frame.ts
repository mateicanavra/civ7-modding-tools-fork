 ...
 */
export function MakeDraggable(root: HTMLElement, selector: string) {

	//setup
	let prevX: number;
	let prevY: number;
	let draggable: boolean;
	let dragArea: HTMLElement | null;
	...
	const onInputAction = (name: string, status: InputActionStatuses, x: number, y: number) => {
		if (name.substr(0, 4) == "nav-" && draggable) {
			const rect: DOMRect = root.getBoundingClientRect();
			const startLeft: number = rect.left;
			const startTop: number = rect.top;
			// How many pixels the joystick movement allows
			const PX_FACTOR: number = 120;

			if (!prevX) {
				prevX = startLeft;
				prevY = startTop;
			} else {
				const newX: number = -(x * PX_FACTOR);
				const newY: number = y * PX_FACTOR;
				...
				if (root.style.right != "initial") { root.style.right = "initial"; }
			}
		}
		...
		if (!prevX) {
			...
			const newX: number = prevX - x;
			const newY: number = prevY - y;
	...
	const onEngineInput = (inputEvent: InputEngineEvent) => {
 ...
 */
export function MakeResizeable(root: ComponentRoot) {
