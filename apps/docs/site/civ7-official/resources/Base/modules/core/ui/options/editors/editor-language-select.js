...
class EditorLanguageSelect extends Panel {
    constructor() {
         ...
         */
        this.currentAudioIdx = Locale.getCurrentAudioLanguageOption();
        /**
         * currentDisplayIdx is the index of the display language option that was selected when the screen was opened.
         */
        this.currentDisplayIdx = Locale.getCurrentDisplayLanguageOption();
        this.selectedAudioIdx = this.currentAudioIdx;
        this.selectedDisplayIdx = this.currentDisplayIdx;
        this.commitOnApply = this.Root.getAttribute('editor-commit-on-apply') === 'true';
        this.engineInputListener = this.onEngineInput.bind(this);
        this.onAudioLanguageChange = (event) => {
            if (event.detail.isChecked) {
                this.selectedAudioIdx = parseInt(event.detail.value);
            }
        };
        this.onDisplayLanguageChange = (event) => {
            if (event.detail.isChecked) {
                this.selectedDisplayIdx = parseInt(event.detail.value);
            }
        };
        this.onAcceptChanges = (_event) => {
            this.applyChanges();
        };
        this.onCancelChanges = (_event) => {
            this.close();
        };
    }
...
//# sourceMappingURL=file:///core/ui/options/editors/editor-language-select.js.map
