...
export class FxsSlot extends Component {
    constructor() {
        ...
        this.continuousCheckForInitialFocusHelpCallback = this.continuousCheckForInitialFocusHelper.bind(this);
        this.observer = new MutationObserver(this.onChildrenChanged.bind(this));
        this.isDisableFocusAllowed = false; // Can a "disabled" item still gain focus (typically a tooltip or some other way of expressing why it's disabled)
        this.initialFocus = null; // Explicitly set initial item to be focued.
        this.priorFocus = null; // Last item to be focused before a blur occurred
     ...
     */
    realizeInitialFocus() {
        const props = { isDisableFocusAllowed: this.isDisableFocusAllowed, direction: InputNavigationAction.NONE };
    ...
    onFocusOut(event) {
        ...
        const currentFocus = FocusManager.getFocus(); // WARNING: current focus should be the one referenced by event.target. it seems that many errors and code oddities arise from not using the correct element here.
    ...
    navigate(direction) {
        ...
        const props = { isDisableFocusAllowed: this.isDisableFocusAllowed, direction: direction };
...
FxsSlot.ruleDirectionCallbackMapping = new Map([
...
FxsSlot.INITIAL_THRESHOLD = 400;
FxsSlot.REPEAT_THRESHOLD = 150;
FxsSlot.NO_FOCUS_FRAMES_WARNING_THRESHOLD = 30;
...
FxsVSlot.ruleDirectionCallbackMapping = new Map([
    [InputNavigationAction.NONE, new Map([
            [NavigationRule.Escape, NavigationHandlers.handlerIgnore],
            [NavigationRule.Wrap, NavigationHandlers.handlerIgnore],
            [NavigationRule.Stop, NavigationHandlers.handlerIgnore],
...
FxsHSlot.ruleDirectionCallbackMapping = new Map([
    [InputNavigationAction.NONE, new Map([
            [NavigationRule.Escape, NavigationHandlers.handlerIgnore],
            [NavigationRule.Wrap, NavigationHandlers.handlerIgnore],
            [NavigationRule.Stop, NavigationHandlers.handlerIgnore],
...
export class FxsSidePanel extends FxsSlot {
    constructor() {
        ...
        this.onChildHiddenListener = (event) => { this.onChildHidden(event); };
        this.onChildShownListener = (event) => { this.onChildShown(event); };
    }
    onAttach() {
        super.onAttach();
...
export class FxsSpatialSlot extends FxsSlot {
    constructor() {
        super(...arguments);
        this.snUnfocusedListener = this.onElementUnfocused.bind(this);
    }
    onInitialize() {
        super.onInitialize();
...
FxsSpatialSlot.ruleDirectionCallbackMapping = new Map([
    [InputNavigationAction.NONE, new Map([
            [NavigationRule.Escape, NavigationHandlers.handlerIgnore],
            [NavigationRule.Wrap, NavigationHandlers.handlerIgnore],
            [NavigationRule.Stop, NavigationHandlers.handlerIgnore],
...
FxsSpatialSlot.sectionCount = 0;
FxsSpatialSlot.sectionIdPrefix = 'fxs-spatial-slot-';
FxsSpatialSlot.sectionIdPool = new Set();
...
//# sourceMappingURL=file:///core/ui/components/fxs-slot.js.map
