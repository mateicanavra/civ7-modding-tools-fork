...
import DialogBoxManager, { DialogBoxID } from '/core/ui/dialog-box/manager-dialog-box.js';
...
class Panel2KCodeRedemption extends Panel {
	...
	private redeemButton: HTMLElement | null = null;
	private codeTextbox: HTMLElement | null = null;
	private responseDialogBox: DialogBoxID | null = null;
	...
	private getErrorMessageLocString(errorCode: number): string {
		switch (errorCode) {
			case 400:
	...
	private executeCodeRedemption() {
		if (this.codeTextbox && !this.responseDialogBox) {
			const value: string | null | undefined = this.codeTextbox?.getAttribute('value');
			if (value) {
	...
	onAttach() {
		...
		this.codeTextbox.addEventListener('component-value-changed', (event: CustomEvent) => {
...
declare const Network: NetworkLibrary;
declare const Online: OnlineLibrary;