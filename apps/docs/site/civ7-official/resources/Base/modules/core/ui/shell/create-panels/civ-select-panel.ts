...
import { AgeData, CivData, GetAgeMap, GetCivilizationData } from '/core/ui/shell/create-panels/age-civ-select-model.js';
...
import { FxsSelectorOrnate, OrnateDropdownItem } from '/core/ui/components/fxs-selector-ornate.js';
...
class CivSelectPanel extends GameCreationPanelBase {
	private ageSelector!: ComponentRoot<FxsSelectorOrnate>;
	private civName!: HTMLElement;
	private civHistoricalChoice!: HTMLElement;
	private readonly learnMore = document.createElement('learn-more');
	private civTags!: HTMLElement;
	private civUnlocksContainer!: HTMLElement;
	private civUnlocksHeader!: HTMLElement;
	private civUnlocks: HTMLElement[] = [];
	private additionalCivInfo!: HTMLElement;
	private historicalChoiceText!: HTMLElement;
	private civAbilityTitle!: HTMLElement;
	private civAbilityText!: HTMLElement;
	private civBonusesContainer!: HTMLElement;
	private civBonuses: HTMLElement[] = [];
	private historicalCivs: { ele: HTMLElement, reason: string }[] = [];

	private tutorialPopup: HTMLElement | null = null;
	private civSelectListEle!: HTMLElement;
	private civEles: ComponentRoot<CivButton>[] = [];
	private selectedCivEle?: ComponentRoot<CivButton>;

	private civData: CivData[] = [];
	private ageMap: Map<string, AgeData>;

	private civFocusListener: EventListener = this.focusCiv.bind(this);
	private civItemListener: EventListener = this.selectCiv.bind(this);

	constructor(root: ComponentRoot<CivSelectPanel>) {
		super(root);
	...
	override onNavigateInput(event: NavigateInputEvent) {
	...
	override setPanelOptions(panelOptions: any) {
	...
	private createHistoricalChoiceIcon() {
	...
	private selectGameParamCiv(): void {
	...
	private focusCiv(event: Event) {
		if (ActionHandler.isGamepadActive) {
			this.selectCivInfo(event.target as ComponentRoot<CivButton>)
		}
	}

	private selectCiv(event: Event) {
		if (ActionHandler.isGamepadActive) {
			this.showNextPanel();
		} else {
			this.selectCivInfo(event.target as ComponentRoot<CivButton>);
	...
	private selectCivInfo(civButton: ComponentRoot<CivButton> | undefined) {
		if (civButton) {
...
declare global {
	interface HTMLElementTagNameMap {
		'civ-select-panel': ComponentRoot<CivSelectPanel>
	}
}