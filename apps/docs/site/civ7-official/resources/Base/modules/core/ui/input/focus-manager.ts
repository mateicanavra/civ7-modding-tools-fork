...
class FocusManagerSingleton {
	...
	private static Instance: FocusManagerSingleton;

	private currentFocus: HTMLElement = document.body;		// The focused HTML element (or body)
	private pendingFocus: HTMLElement | null = null;		// An item that will take focus as soon as it's connected
	private defaultFocus: HTMLElement = document.body;		// What to set as the focus if current focus becomes disconnected (should only be set ONCE per game state)
	private activeNavCallback: Function | null = null;
	private updateListener: FrameRequestCallback = this.onFrameUpdate.bind(this);
	private focusChain: WeakRef<HTMLElement>[] = [];			// Chain of focus from an element up the body (and then the default).
	private isGamepadActive: boolean = Input.getActiveDeviceType() == InputDeviceType.Controller;
	private pendingFocusFrameCount: number = 0;				// Track how many frames it took for a pending focus to be realized.
	private whoLockedFocus: string = "";

	private nonLockedFocus: HTMLElement | null = null;
	get isFocusLocked(): boolean {	// Using a property instead of a type guard function; otherwise private members would need to be public.
		return this.nonLockedFocus != null;
	}

	private constructor() {
		engine.on('input-source-changed', (deviceType: InputDeviceType, _deviceLayout: InputDeviceLayout) => { this.onActiveDeviceTypeChanged(deviceType); });
	}
	 ...
	 */
	getFocusChildOf(parent: HTMLElement): HTMLElement | null {
	...
	toLogString(target?: Element): string {
		if (target == undefined) target = this.currentFocus;
		return "name=" + + (target.nodeName ?? "") + ", tagName=" + (target.tagName ?? "") + ", className=" + (target.className ?? "");
	}

	resetFocus(target: Element) {
		const YellowOnRedANSI: string = "\x1b[41m\x1b[33m";
	 ...
	 **/
	lockFocus(target: Element, who: string, why: string): boolean {
	...
	unlockFocus(target: Element, who: string): boolean {
	...
	setFocus(target: Element) {
		...
		if (target == document.body) {	// Allowed but not great.
			...
			if ((target instanceof ComponentRoot) == false || (target as ComponentRoot).isInitialized) {
	 ...
	 */
	private readyForFocus(element: Element): boolean {
	...
	private realizeFocus() {
		...
		if (this.isGamepadActive) {
			...
			const prevCurrentFocus: HTMLElement = this.currentFocus;
			...
			if (prevCurrentFocus == this.currentFocus) {
				let audioId: string | null = this.currentFocus.getAttribute("data-audio-focus-ref");
				let audioGroup: string | null = this.currentFocus.getAttribute("data-audio-group-ref");
	 ...
	 */
	setDefaultFocus(target: HTMLElement): HTMLElement {
	 ...
	 */
	clearFocus(targetElement: HTMLElement | null = null) {
	...
	setNavCallback(callback: Function) {
		this.activeNavCallback = callback;
	}

	// DEPRECATED: Do not use, remove once screen-controller-mapping.ts is updated
	getNavCallback(): Function | null {
		return this.activeNavCallback;
	}

	private onActiveDeviceTypeChanged = (deviceType: InputDeviceType) => {
