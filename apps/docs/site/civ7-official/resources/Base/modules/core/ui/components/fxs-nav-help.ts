 ...
 */
const actionButtonMap: Map<string, string> = new Map([
 ...
 */
const getTextHelpForAction = (queryAction: string): string => {
...
export class FxsNavHelp extends Component {
	...
	private textHelp?: HTMLDivElement;
	private readonly label = document.createElement('div');
	private iconStartText!: HTMLElement;
	...
	private decorationMode = DecorationMode.NONE;
	private caption = "";
	...
	onInitialize(): void {
		...
		const hasHoverOnlyTriggerParent: boolean = this.Root.closest('[hover-only-trigger="true"]') != null;
	...
	onAttributeChanged(name: string, _oldValue: string, newValue: string) {
		if (name == "action-key") {
			...
			if (newValue != this.decorationMode && [DecorationMode.NONE, DecorationMode.BORDER].includes(newValue as DecorationMode)) {
				this.decorationMode = newValue as DecorationMode;
				this.updateGate.call('onAttributeChanged');
			}
		}
	...
	private onUpdate() {
		...
		if (this.iconElement) {
			if (imagePath != "") {
				...
				this.iconStartText.innerHTML = mapIconToText[imagePath as "ps4_icon_start" | "ps4_icon_share"] ?? "";
