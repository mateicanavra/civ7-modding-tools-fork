...
class ActionHandlerSingleton {
    ...
    onEngineInput(name, status, x, y) {
        ...
        if (isNavigation) {
            ...
            try {
                let live = Framework.ContextManager.handleInput(inputEvent);
                if (live) {
    ...
    forceCursorCheck() {
        ...
        delayByFrame(() => {
            ...
            window.dispatchEvent(new MouseEvent("mousecheck", { clientX: 0, clientY: 0, screenX: 0, screenY: 0 }));
            delayByFrame(() => {
                window.dispatchEvent(new MouseEvent("mousecheck", { clientX: x, clientY: y, screenX: x, screenY: y }));
