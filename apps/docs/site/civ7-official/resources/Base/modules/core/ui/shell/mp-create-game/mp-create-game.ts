...
class PanelMPCreateGame extends Panel {
	private wasCompact = Layout.isCompact();
	private groupNames: Map<string, string> = new Map();

	private frame!: HTMLElement;
	private hostLobbyButton!: HTMLElement;
	private optionsContainer!: HTMLElement;
	private tooltipElements: HTMLElement[] = [];

	private gameSetupRevision: number = 0;
	private ignoreParameters: number[] = [];

	private enteredAdditionalContent: boolean = false;

	// We cache the last changed parameter in case we need to refocus that parameter after rebuilding options
	private lastChangedParameter: string = '';
	private creatingOptions: boolean = false; // to avoid updating the lastChangedParameter when initializing the options and to do it only when an option is effectively changed

	private onHostLobbyListener = this.onHostLobby.bind(this);
	private onSaveConfigListener = this.onSaveConfig.bind(this);
	private onLoadConfigListener = this.onLoadConfig.bind(this);
	private onExtraContentListener = this.onExtraContent.bind(this);
	private onWindowResizeListener = this.onResize.bind(this);
	...
	onAttach() {
		...
		const leftContainer: HTMLElement = MustGetElement(".left-container", this.Root);
		const saveConfigButton: HTMLElement = MustGetElement(".save-config", this.Root);
		...
		const loadConfigButton: HTMLElement = MustGetElement(".load-config", this.Root);
		...
		if (UI.supportsDLC()) {
			const extraContentButton: HTMLElement = document.createElement('fxs-button');
		...
		let gameMode: GameModeTypes | undefined = serverTypeToGameModeType.get(MultiplayerShellManager.serverType);
		if (gameMode) {
	...
	private onHostLobby() { // TODO Investigate if we need anything from MainMenu::onNewCampaignStart()
		this.close();
	...
	private refreshGameOptions() {
		...
		let currentGroupId: GameSetupStringHandle = -1;
	...
	private refocusLastChangedParameter() {
		if (this.lastChangedParameter != '') {
			const lastChangedElement: Element | null = this.optionsContainer.querySelector(`[data-parameter-id="${this.lastChangedParameter}"]`);
	...
	private setParameterValue(parameterID: string, newValue: GameSetupParameterValue) {
	...
	private createHeader(frag: DocumentFragment, groupId: GameSetupStringHandle) {
		const groupIdString = GameSetup.resolveString(groupId)!;
		const name: string | undefined = this.groupNames.get(groupIdString);
	...
	private createOption(frag: DocumentFragment, parameter: GameSetupParameter) {
		const isCompact = Layout.isCompact();
		const parameterID: string | null = GameSetup.resolveString(parameter.ID)!;
		...
		const label: Element = document.createElement('div');
		...
		const parameterValueName: string | null | undefined = (parameter.value.name) ? GameSetup.resolveString(parameter.value.name) : parameter.value.value?.toString();
		...
		switch (parameter.domain.type) {
			case GameSetupDomainType.Select:
				const dropdown: HTMLElement = document.createElement('fxs-dropdown');
				...
				dropdown.addEventListener(DropdownSelectionChangeEventName, (event: DropdownSelectionChangeEvent) => {
					...
					const parameterID: string | null = targetElement.getAttribute('data-parameter-id');
					if (parameterID) {
						const index = event.detail.selectedIndex;
						const parameter: GameSetupParameter | null = GameSetup.findGameParameter(parameterID);
						if (parameter && parameter.domain.possibleValues && parameter.domain.possibleValues.length > index) {
							const value: GameSetupDomainValue = parameter.domain.possibleValues[index];
							this.setParameterValue(parameterID, value.value);
						}
					}
				});

				const actionsList: (DropdownItem & { sortIndex: number })[] = [];
				parameter.domain.possibleValues?.forEach((pv, index) => {
					const valueName: string | null = GameSetup.resolveString(pv.name);
					if (!valueName) {
			...
			case GameSetupDomainType.Boolean:
				if (parameter.readOnly) {
					const value: Element = document.createElement('div');
					...
					const value: Element = document.createElement('fxs-checkbox');
					value.classList.add('display-flex');
					...
					value.addEventListener("component-value-changed", (event: CustomEvent) => {
						const newValue: GameSetupParameterValue = event.detail.value;
						const parameter: GameSetupParameter | null = GameSetup.findGameParameter(parameterID);
						if (parameter) {
			...
			case GameSetupDomainType.UnsignedInteger:
				if (parameter.readOnly) {
					...
					const value: Element = document.createElement('fxs-textbox');
		...
		const description: string | null = GameSetup.resolveString(parameter.description);
		if (description) {
