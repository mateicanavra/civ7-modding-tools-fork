...
import { CategoryType, CheckboxInfo, AdvancedCheckboxInfo, DropdownInfo, OptionInfo, OptionInitHandler, OptionType, OptionUpdateHandler, SliderInfo } from '/core/ui/options/options-helpers.js';
import { FontScale } from '/core/ui/utilities/utilities-dom.js';

const FORCE_FEEDBACK_OPTION_LOW: number = 20;
const FORCE_FEEDBACK_OPTION_HIGH: number = 20;
const FORCE_FEEDBACK_OPTION_DURATION: number = 100;

// Should be a Checkbox+Slider, for now use maximum slider value to disable
const FRAMERATE_LIMIT_SLIDER_MIN: number = 30;
const FRAMERATE_LIMIT_SLIDER_INF: number = 241; // max framerate limit of 240, 241->unlimited

type UiFontScaleDropdownItem = DropdownItem & { scale: FontScale }
type UiExperienceDropdownItem = DropdownItem & { experience: UIViewExperience }
type GPUDropdownItem = DropdownItem & { deviceID: number; }
type WindowModeDropdownItem = DropdownItem & { fullscreen: boolean; }
type ResolutionDropdownItem = DropdownItem & { resolution: int2; }
type GraphicsProfileDropdownItem = DropdownItem & { profile: GraphicsProfile; }
type UpscalingAADropdownItem = DropdownItem & { mode: UpscalingAA; }
type NumberDropdownItem = DropdownItem & { value: number; }
type EnumDropdownItem<Enum> = DropdownItem & { value: Enum; }

const bPlatformSupportsXess: boolean = BuildInfo.graphics == "DX12" || BuildInfo.graphics == "Vulkan";
const bPlatformSupportsMetal: boolean = BuildInfo.graphics == "Metal";
const bPlatformSupportsMetalFX = Options.canUseMetalFx;
const upscaleItemList: UpscalingAADropdownItem[] = [
	...
	...(bPlatformSupportsXess ? [{ mode: UpscalingAA.XeSS, label: "LOC_OPTIONS_GFX_UPSCALE_AA_XESS", tooltip: "LOC_OPTIONS_GFX_UPSCALE_AA_XESS_DESCRIPTION" }] : []), // TODO _SHORT or _MINIMUM keys have more detail
...
const onAudioDynRangeInit: OptionInitHandler<DropdownInfo> = (optionInfo) => {
...
const onAudioDynRangeUpdate: OptionUpdateHandler<DropdownInfo> = (optionInfo, value) => {
	optionInfo.currentValue = value;
	Sound.setDynamicRangeOption(value);
}

const onVolumeInit: OptionInitHandler<SliderInfo> = (optionInfo) => {
...
const onMuteOnFocusLossInit: OptionInitHandler<CheckboxInfo> = (optionInfo) => {
	optionInfo.currentValue = Sound.getMuteOnFocusLoss();
}

const onMuteOnFocusLossUpdate: OptionUpdateHandler<CheckboxInfo> = (_optionInfo, value) => {
	Sound.setMuteOnFocusLoss(value);
}

const onSubtitlesInit: OptionInitHandler<CheckboxInfo> = (optionInfo) => {
	optionInfo.currentValue = Sound.getSubtitles();
}

const onSubtitlesUpdate: OptionUpdateHandler<CheckboxInfo> = (_optionInfo, value) => {
 ...
 */
const onVolumeUpdate: OptionUpdateHandler<SliderInfo> = (optionInfo, value) => {
	optionInfo.currentValue = value;
	optionInfo.formattedValue = `${Math.round(optionInfo.currentValue * 100)}%`;

	switch (optionInfo.id) {
		case "option-audio-master": Sound.volumeSetMaster(value); break;
		case "option-audio-music": Sound.volumeSetMusic(value); break;
		case "option-audio-voice": Sound.volumeSetVoice(value); break;
		case "option-audio-ui": Sound.volumeSetUI(value); break;
		case "option-audio-cinematics": Sound.volumeSetCinematics(value); break;
		case "option-audio-sfx": Sound.volumeSetSFX(value); break;
		default:
...
const advancedGraphicsOptionSetters: (() => void)[] = [];
let graphicsProfileSetter: (() => void) | undefined = undefined;
let updatingGraphicsProfile = false;

function getOptionsForProfile(profile: GraphicsProfile) {
	const profileOptions: GraphicsOptions.Data = { ...Options.graphicsOptions };
...
function addGraphicsOptionSetter(optionInfo: OptionInfo, advancedOptionSetter: () => void) {
...
const antialiasingOptions: DropdownInfo[] = [];
...
var xellOption: CheckboxInfo;
var vsyncOption: CheckboxInfo;

const onGfxCoreDropdownInit: OptionInitHandler<DropdownInfo> = (optionInfo) => {
	if (!Options.supportsGraphicOptions) {
	...
	switch (optionInfo.id) {
		case "option-gfx-profile":
			graphicsProfileSetter = () => {
				...
				for (let i = 0; i < optionInfo.dropdownItems.length; i++) {
					const item = optionInfo.dropdownItems[i] as GraphicsProfileDropdownItem;
		...
		case "option-gfx-gpu":
			optionInfo.selectedItemIndex = optionInfo.dropdownItems
				?.findIndex(({ deviceID }: GPUDropdownItem) => deviceID === Options.graphicsOptions.deviceID) ?? 0;
			break;

		case "option-gfx-window":
			optionInfo.selectedItemIndex = optionInfo.dropdownItems
				?.findIndex(({ fullscreen }: WindowModeDropdownItem) => fullscreen === Options.graphicsOptions.fullscreen) ?? 0;
			break;
		...
		case "option-gfx-upscale-aa":
			const upscaleAASetter = () => {
				...
				optionInfo.selectedItemIndex = optionInfo.dropdownItems.findIndex(({ mode }: UpscalingAADropdownItem) => {
			...
			upscaleAASetter();
			optionInfo.selectedItemIndex = optionInfo.dropdownItems
				?.findIndex(({ mode }: UpscalingAADropdownItem) => mode === Options.graphicsOptions.upscaleAA) ?? 0;
			updateAntialiasingVisiblity();
			break;
	}
}

const onGfxCoreDropdownUpdate: OptionUpdateHandler<DropdownInfo> = ({ id, dropdownItems = [] }, value) => {
	switch (id) {
		case "option-gfx-profile": {
			const { profile } = dropdownItems[value] as GraphicsProfileDropdownItem;
		...
		case "option-gfx-gpu": {
			...
			const { deviceID } = dropdownItems[value] as GPUDropdownItem;
			Options.graphicsOptions.deviceID = deviceID;
			break;
		}
		case "option-gfx-window": {
			const { fullscreen } = dropdownItems[value] as WindowModeDropdownItem;
			Options.graphicsOptions.fullscreen = fullscreen;
			break;
		}
		case "option-gfx-res": {
			const { resolution } = dropdownItems[value] as ResolutionDropdownItem;
			Options.graphicsOptions.resolution.i = resolution.i;
			Options.graphicsOptions.resolution.j = resolution.j;
			break;
		}
		case "option-gfx-upscale-aa": {
			const { mode } = dropdownItems[value] as UpscalingAADropdownItem;
...
const onGfxAntialiasingDropdownInit: OptionInitHandler<DropdownInfo> = (optionInfo) => {
	if (!Options.supportsGraphicOptions) {
	...
	switch (optionInfo.id) {
		case "option-gfx-msaa-quality":
			const msaaSelector = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems
					?.findIndex(({ value }: NumberDropdownItem) => value === Options.graphicsOptions.msaaSamples) ?? 0;
			};
		...
		case "option-gfx-xess-quality":
			const xessSelector = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems
					?.findIndex(({ value }: EnumDropdownItem<XeSSQuality>) => value === Options.graphicsOptions.xessQuality) ?? 0;
			};
		...
		case "option-gfx-fsr1-quality":
			const fsr1Selector = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems
					?.findIndex(({ value }: EnumDropdownItem<Fsr1Quality>) => value === Options.graphicsOptions.fsr1Quality) ?? 0;
			};
		...
		case "option-gfx-fsr3-quality":
			const fsr3Selector = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems
					?.findIndex(({ value }: EnumDropdownItem<Fsr3Quality>) => value === Options.graphicsOptions.fsr3Quality) ?? 0;
			};
		...
		case "option-gfx-metalfxSpatial-quality":
			const metalfxSpatialSelector = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems
					?.findIndex(({ value }: EnumDropdownItem<MetalFxSpatialQuality>) => value === Options.graphicsOptions.metalfxSpatialQuality) ?? 0;
			};
		...
		case "option-gfx-metalfxTemporal-quality":
			const metalfxTemporalSelector = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems
					?.findIndex(({ value }: EnumDropdownItem<MetalFxTemporalQuality>) => value === Options.graphicsOptions.metalfxTemporalQuality) ?? 0;
			};
...
const onGfxAntialiasingDropdownUpdate: OptionUpdateHandler<DropdownInfo> = ({ id, dropdownItems = [] }, value) => {
	switch (id) {
		case "option-gfx-msaa-quality":
			Options.graphicsOptions.msaaSamples = (dropdownItems[value] as NumberDropdownItem).value;
			break;

		case "option-gfx-xess-quality":
			Options.graphicsOptions.xessQuality = (dropdownItems[value] as EnumDropdownItem<XeSSQuality>).value;
			break;

		case "option-gfx-fsr1-quality":
			Options.graphicsOptions.fsr1Quality = (dropdownItems[value] as EnumDropdownItem<Fsr1Quality>).value;
			break;

		case "option-gfx-fsr3-quality":
			Options.graphicsOptions.fsr3Quality = (dropdownItems[value] as EnumDropdownItem<Fsr3Quality>).value;
			break;
		case "option-gfx-metalfxSpatial-quality":
			Options.graphicsOptions.metalfxSpatialQuality = (dropdownItems[value] as EnumDropdownItem<MetalFxSpatialQuality>).value;
			break;
		case "option-gfx-metalfxTemporal-quality":
			Options.graphicsOptions.metalfxTemporalQuality = (dropdownItems[value] as EnumDropdownItem<MetalFxTemporalQuality>).value;
			break;
	}
}

const onGfxCoreCheckboxInit: OptionInitHandler<CheckboxInfo> = (optionInfo) => {
	switch (optionInfo.id) {
...
const onGfxCoreCheckboxUpdate: OptionUpdateHandler<CheckboxInfo> = (optionInfo, value) => {
	switch (optionInfo.id) {
		case "option-gfx-vsync":
...
function formatFramerateLimit(value: number): string {
	if (value != 0) {
...
const onGfxCoreSliderInit: OptionInitHandler<SliderInfo> = (optionInfo) => {
	switch (optionInfo.id) {
		case "option-gfx-frame-limit-game":
			optionInfo.currentValue = Options.graphicsOptions.framerateLimit;
			if (optionInfo.currentValue == 0) { optionInfo.currentValue = FRAMERATE_LIMIT_SLIDER_INF; }
			optionInfo.formattedValue = formatFramerateLimit(Options.graphicsOptions.framerateLimit);
			break;
	}
}

const onGfxCoreSliderUpdate: OptionUpdateHandler<SliderInfo> = (optionInfo, value) => {
	switch (optionInfo.id) {
		case "option-gfx-frame-limit-game":
...
const onGfxAdvancedDropdownInit: OptionInitHandler<DropdownInfo> = (optionInfo) => {
	switch (optionInfo.id) {
		case "option-gfx-ssao-quality":
			const ssaoQualitySetter = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems?.findIndex(({ value }: EnumDropdownItem<SSAOQuality>) => {
		...
		case "option-gfx-asset-quality":
			const assetQualitySetter = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems?.findIndex(({ value }: EnumDropdownItem<AssetQuality>) => {
		...
		case "option-gfx-particle-quality":
			const particleQualitySetter = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems?.findIndex(({ value }: EnumDropdownItem<VFXQuality>) => {
		...
		case "option-gfx-texture-detail":
			const textureDetailSetter = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems?.findIndex(({ value }: EnumDropdownItem<TextureDetail>) => {
		...
		case "option-gfx-shadow-quality":
			const shadowQualitySetter = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems?.findIndex(({ value }: EnumDropdownItem<ShadowQuality>) => {
		...
		case "option-gfx-water-quality":
			const waterQualitySetter = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems?.findIndex(({ value }: EnumDropdownItem<WaterQuality>) => {
		...
		case "option-gfx-sharpening":
			const sharpeningSetter = () => {
				optionInfo.selectedItemIndex = optionInfo.dropdownItems?.findIndex(({ value }: EnumDropdownItem<SharpeningLevel>) => {
...
const onGfxAdvancedDropdownUpdate: OptionUpdateHandler<DropdownInfo> = ({ id, dropdownItems = [] }, index) => {
	switch (id) {
		case "option-gfx-ssao-quality": {
			const { value } = dropdownItems[index] as EnumDropdownItem<SSAOQuality>;
			Options.graphicsOptions.ssaoQuality = value;
			break;
		}
		case "option-gfx-asset-quality": {
			const { value } = dropdownItems[index] as EnumDropdownItem<AssetQuality>;
			Options.graphicsOptions.assetQuality = value;
			break;
		}
		case "option-gfx-particle-quality": {
			const { value } = dropdownItems[index] as EnumDropdownItem<VFXQuality>;
			Options.graphicsOptions.vfxQuality = value;
			break;
		}
		case "option-gfx-texture-detail": {
			const { value } = dropdownItems[index] as EnumDropdownItem<TextureDetail>;
			Options.graphicsOptions.textureDetail = value;
			break;
		}
		case "option-gfx-shadow-quality": {
			const { value } = dropdownItems[index] as EnumDropdownItem<ShadowQuality>;
			Options.graphicsOptions.shadowQuality = value;
			break;
		}
		case "option-gfx-water-quality": {
			const { value } = dropdownItems[index] as EnumDropdownItem<WaterQuality>;
			Options.graphicsOptions.waterQuality = value;
			break;
		}
		case "option-gfx-sharpening": {
			const { value } = dropdownItems[index] as EnumDropdownItem<SharpeningLevel>;
...
const onGfxAdvancedCheckboxInit: OptionInitHandler<CheckboxInfo> = (optionInfo) => {
	switch (optionInfo.id) {
...
const onGfxAdvancedCheckboxUpdate: OptionUpdateHandler<CheckboxInfo> = (optionInfo, value) => {
	switch (optionInfo.id) {
		case "option-gfx-intel-xefg":
...
const onTutorialLevelInit: OptionInitHandler<DropdownInfo> = (optionInfo) => {
...
const onTutorialLevelUpdate: OptionUpdateHandler<DropdownInfo> = (optionInfo, value) => {
	if (optionInfo.id != "option-tutoriallevel") {
		console.warn("The tutorial level handler was called by some other option '" + optionInfo.id + "'");
		return;
	}
...
const onCheckboxInit: OptionInitHandler<AdvancedCheckboxInfo> = (optionInfo: AdvancedCheckboxInfo) => {
	const option: unknown = UI.getOption(optionInfo.optionSet, optionInfo.optionType, optionInfo.optionName);
	if (typeof option === 'number') {
		const toggleVal: boolean = Boolean(option);
		optionInfo.currentValue = toggleVal;
	}
}

const onCheckboxUpdate: OptionUpdateHandler<AdvancedCheckboxInfo> = (optionInfo, value) => {
...
const onRemapInit: OptionInitHandler<AdvancedCheckboxInfo> = (optionInfo: AdvancedCheckboxInfo) => {
	optionInfo.currentValue = (VisualRemaps.getRemapState(optionInfo.id) == VisualRemapSelection.Disabled) ? false : true;
}

const onRemapUpdate: OptionUpdateHandler<AdvancedCheckboxInfo> = (optionInfo, value) => {
	VisualRemaps.setRemapState(optionInfo.id, value ? VisualRemapSelection.Enabled : VisualRemapSelection.Disabled);
}

const onPromotionalContentUpdate: OptionUpdateHandler<AdvancedCheckboxInfo> = (optionInfo, value) => {
	if (optionInfo.optionSet && optionInfo.optionType && optionInfo.optionName) {
		UI.setOption(optionInfo.optionSet, optionInfo.optionType, optionInfo.optionName, (value ? 1 : 0));
...
const onAdaptiveTriggersInit: OptionInitHandler<CheckboxInfo> = (optionInfo: CheckboxInfo) => {
	const value: boolean = Configuration.getUser().adaptiveTriggersEnabled;
	optionInfo.currentValue = value;
}

const onAdaptiveTriggersUpdate: OptionUpdateHandler<CheckboxInfo> = (optionInfo, value) => {
	const userConfig = Configuration.getUser();
...
const onRumbleInit: OptionInitHandler<CheckboxInfo> = (optionInfo) => {
	optionInfo.currentValue = Configuration.getUser().rumbleEnabled;
}

const onRumbleUpdate: OptionUpdateHandler<CheckboxInfo> = (optionInfo, value) => {
...
const onAutoSaveKeepCountInit: OptionInitHandler<SliderInfo> = (optionInfo) => {
	const value = Configuration.getUser().autoSaveKeepCount;
	optionInfo.currentValue = value;
	optionInfo.formattedValue = `${value}`;
}

const onAutoSaveKeepCountUpdate: OptionUpdateHandler<SliderInfo> = (optionInfo: SliderInfo, value: number) => {
	optionInfo.currentValue = value;
	optionInfo.formattedValue = `${value}`;
...
const onAutoSaveFrequencyInit: OptionInitHandler<SliderInfo> = (optionInfo) => {
	const value = Configuration.getUser().autoSaveFrequency;
	optionInfo.currentValue = value;
	optionInfo.formattedValue = `${value}`;
}

const onAutoSaveFrequencyUpdate: OptionUpdateHandler<SliderInfo> = (optionInfo: SliderInfo, value: number) => {
	optionInfo.currentValue = value;
	optionInfo.formattedValue = `${value}`;
...
const onAccessibilityInit: OptionInitHandler<DropdownInfo> = (optionInfo) => {
	switch (optionInfo.id) {
...
const onAccessibilityUpdate: OptionUpdateHandler<DropdownInfo> = (optionInfo, value) => {
	optionInfo.currentValue = value;
...
const onTooltipDelayInit: OptionInitHandler<SliderInfo> = (optionInfo) => {
...
const onTooltipDelayUpdate: OptionUpdateHandler<SliderInfo> = (optionInfo: SliderInfo, value: number) => {
...
const onCameraPanningSpeedInit: OptionInitHandler<SliderInfo> = (optionInfo) => {
	const userConfig = Configuration.getUser();
	optionInfo.currentValue = userConfig.cameraPanningSpeed;
	optionInfo.formattedValue = `${userConfig.cameraPanningSpeed}%`;
}

const onCameraPanningSpeedUpdate: OptionUpdateHandler<SliderInfo> = (optionInfo, value) => {
	const userConfig = Configuration.getUser();
...
const onUiFontScaleInit: OptionInitHandler<DropdownInfo> = (optionInfo) => {
	const { uiFontScale } = Configuration.getUser();

	if (optionInfo.dropdownItems) {
		for (let index = 0; index < optionInfo.dropdownItems.length; ++index) {
			if ((optionInfo.dropdownItems[index] as UiFontScaleDropdownItem).scale == uiFontScale) {
...
const onUiFontScaleUpdate: OptionUpdateHandler<DropdownInfo> = (optionInfo, value) => {
	const newScale = (optionInfo.dropdownItems?.[value] as UiFontScaleDropdownItem).scale;
	Configuration.getUser().setUiFontScale(newScale);
}

const onUiExperienceInit: OptionInitHandler<DropdownInfo> = (optionInfo) => {
	const experience = UI.getViewExperience();

	if (optionInfo.dropdownItems) {
		for (let index = 0; index < optionInfo.dropdownItems.length; ++index) {
			if ((optionInfo.dropdownItems[index] as UiExperienceDropdownItem).experience == experience) {
				optionInfo.selectedItemIndex = index;
				break;
			}
		}
...
const onUiExperienceUpdate: OptionUpdateHandler<DropdownInfo> = (optionInfo, value) => {
	const newExperience = (optionInfo.dropdownItems?.[value] as UiExperienceDropdownItem).experience;
...
const onUiExperienceRestore: OptionInitHandler<DropdownInfo> = (optionInfo) => {
	if (optionInfo.currentValue != optionInfo.originalValue) {
...
const onUiAutoScaleInit: OptionInitHandler<CheckboxInfo> = (optionInfo) => {
	const value = Configuration.getUser().uiAutoScale;
	optionInfo.currentValue = value;
}

const onUiAutoScaleUpdate: OptionUpdateHandler<CheckboxInfo> = (optionInfo, value: boolean) => {
	Configuration.getUser().setUiAutoScale(value);
	optionInfo.currentValue = Configuration.getUser().uiAutoScale;
}

const onShowIntroVideoInit: OptionInitHandler<CheckboxInfo> = (optionInfo) => {
	const value = UI.isShowIntroSequences();
	optionInfo.currentValue = value;
}

const onShowIntroVideoUpdate: OptionUpdateHandler<CheckboxInfo> = (optionInfo, value: boolean) => {
...
Options.addInitCallback(() => {
	...
	const colorblindAdaptationModes: DropdownItem[] = [
	...
	const uiFontScaleRanges: UiFontScaleDropdownItem[] = isConsole ? [
	...
	const expItemList: UiExperienceDropdownItem[] = [
	...
	const volumeBase = { category: CategoryType.Audio, group: 'volume', type: OptionType.Slider, initListener: onVolumeInit, updateListener: onVolumeUpdate, min: 0, max: 1, steps: 20 } satisfies Partial<SliderInfo>;
	...
	const tutorialItemList: DropdownItem[] = [
	...
	const supportedOptions: GraphicsOptions.SupportedOptions = Options.supportedOptions;

	if (supportedOptions.gpus.length > 0) {
		const ddGPUItems: GPUDropdownItem[] = [];
	...
	if (supportedOptions.canChangeScreenMode) {
		const ddWindowItems: WindowModeDropdownItem[] = [
		...
		const option: OptionInfo = { category: CategoryType.Graphics, group: 'general', type: OptionType.Dropdown, id: "option-gfx-window", initListener: onGfxCoreDropdownInit, updateListener: onGfxCoreDropdownUpdate, label: "LOC_OPTIONS_GFX_SCREEN_MODE", description: "LOC_OPTIONS_GFX_SCREEN_MODE_DESCRIPTION", dropdownItems: ddWindowItems }
		Options.addOption(option);
	}

	if (supportedOptions.resolutions.length > 0) {
		// Make the fixed "Auto" item for the resolution dropdown
		const resItemList: ResolutionDropdownItem[] = [
	...
	if (supportedOptions.canChangeVSync) {
		...
		const frameLimitSliderBase = { category: CategoryType.Graphics, group: 'general', type: OptionType.Slider, initListener: onGfxCoreSliderInit, updateListener: onGfxCoreSliderUpdate, min: FRAMERATE_LIMIT_SLIDER_MIN, max: FRAMERATE_LIMIT_SLIDER_INF } satisfies Partial<SliderInfo>;
	...
	if (supportedOptions.profiles.length > 0) {
		const ddProfileItems: GraphicsProfileDropdownItem[] = [];
	...
	if (Options.supportsGraphicOptions && Options.showAdvancedGraphicsOptions) {
		...
		const msaaSamplesItemList: NumberDropdownItem[] = [
		...
		if (supportedOptions.canChangeSSAOQuality) {
			const ssaoQualityItemList: EnumDropdownItem<SSAOQuality>[] = [
		...
		if (supportedOptions.canChangeAssetQuality) {
			...
			const assetQualityItemList: EnumDropdownItem<AssetQuality>[] = [
		...
		if (supportedOptions.canChangeTextureDetail) {
			...
			const textureDetailItemList: EnumDropdownItem<TextureDetail>[] = [
		...
		if (supportedOptions.canChangeParticleQuality) {
			const particleQualityItemList: EnumDropdownItem<VFXQuality>[] = [
		...
		if (supportedOptions.canChangeShadowQuality) {
			const shadowQualityItemList: EnumDropdownItem<ShadowQuality>[] = [
		...
		if (supportedOptions.canChangeWaterQuality) {
			const waterQualityItemList: EnumDropdownItem<WaterQuality>[] = [
		...
		if (supportedOptions.canChangeImageSharpness) {
			const sharpeningItemList: EnumDropdownItem<SharpeningLevel>[] = [
