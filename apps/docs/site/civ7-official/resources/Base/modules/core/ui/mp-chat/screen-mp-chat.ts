...
import { FxsScrollable, ScrollAtBottomEvent } from '/core/ui/components/fxs-scrollable.js';
...
enum CommonTargetImageClass {
	TEAM = "img-team-chat",
	GLOBAL = "img-global-chat",
	DIRECT = "img-direct-chat",
}
...
const mapTargetTypeToMessageHeaderPrefix = {
	[ChatTargetTypes.CHATTARGET_ALL]: (_playerFrom: ConfigurationPlayerAccessor) => "",
	[ChatTargetTypes.CHATTARGET_PLAYER]: (playerFrom: ConfigurationPlayerAccessor) => playerFrom.id == GameContext.localPlayerID ? "LOC_UI_CHAT_TO" : "LOC_UI_CHAT_FROM",
	[ChatTargetTypes.CHATTARGET_TEAM]: (_playerFrom: ConfigurationPlayerAccessor) => "LOC_UI_CHAT_TEAM",
	[ChatTargetTypes.NO_CHATTARGET]: (_playerFrom: ConfigurationPlayerAccessor) => "",
}
...
export class PrivateSelectEvent extends CustomEvent<{ currentTarget: ChatTargetEntry }> {
...
export class EmoticonSelectEvent extends CustomEvent<{ iconId: string }> {
...
export class ScreenMPChat extends Panel {
	...
	private editBoxGamepadText!: HTMLElement;
	...
	private previewTargetContainer!: HTMLElement;
	private unreadButton!: HTMLElement;
	private unreadButtonContainer!: HTMLElement;
	private inputContainer!: HTMLElement;

	private sendToButton!: HTMLElement;
	private sendToButtonIcon!: HTMLElement;
	private emoticonButton!: HTMLElement;
	private emoticonButtonIcon!: HTMLElement;
	private sendButton!: HTMLElement;
	private scrollable!: ComponentRoot<FxsScrollable>;
	private currentChatTargets: ChatTargetEntry[] = [];
	private currentTarget: ChatTargetEntry = DEFAULT_CHAT_TARGET_ENTRY;
	private stylizedMarkupMessage: string = "";
	private markupMessage: string = "";
	private editBoxObserver!: MutationObserver;
	private commandError: string = "";
	private privateToLocalPlayer: ConfigurationPlayerAccessor | null = null;
	private forcedUnread: boolean = false;

	private chatCommandManager: ChatCommandManager = new ChatCommandManager(this);

	private sendToButtonActivateListener = this.onSendToButtonActivate.bind(this);
	private sendButtonActivateListener = this.onSendButtonActivate.bind(this);
	private emoticonButtonActivateListener = this.onEmoticonButtonActivate.bind(this);
	private privateSelectListener = this.onPrivateSelect.bind(this);
	private emoticonSelectListener = this.onEmoticonSelect.bind(this);
	private editBoxAttributeMutateListener = this.onEditBoxAttributeMutate.bind(this);
	private validateVirtualKeyboardListener = this.onValidateVirtualKeyboard.bind(this);
	private editBoxValueChangeListener = this.onEditBoxValueChange.bind(this);
	private keyDownListener = this.onKeyDown.bind(this);
	private scrollAtBottomListener = this.onScrollAtBottom.bind(this);
	private unreadButtonActivateListener = this.onUnreadButtonActivate.bind(this);
	private focusListener = this.onFocus.bind(this);
	private scrollableBlurListener = this.onScrollableBlur.bind(this);
	private scrollableFocusListener = this.onScrollableFocus.bind(this);
	...
	*/
	private onEditBoxAttributeMutate(_mutationList: MutationRecord[], _observer: MutationObserver) {
		const value: string = this.editBox.getAttribute("value") ?? "";
		const newValue: string = value.slice(0, ChatGlobalTypes.MAX_CHAT_MESSAGE_SIZE);
	...
	private onPlayerConnected({ data }: GenericDataInt32) {
	...
	private onPlayerDisconnected({ playerNameT2gp, playerName1Pgp }: MultiplayerPostPlayerDisconnectedData) {
		const isScrollAtBottom = this.scrollable.component.getIsScrollAtBottom();
	...
	private onKickVoteComplete({ kickResult, kickPlayerName }: KickVoteCompleteData) {
		const isScrollAtBottom = this.scrollable.component.getIsScrollAtBottom();
	...
	private onKickDirectComplete({ kickPlayerID, kickResult, kickPlayerName }: KickDirectCompleteData) {
		const isScrollAtBottom = this.scrollable.component.getIsScrollAtBottom();
	...
	private onPlayerKicked({ kickPlayerID, kickReason, kickPlayerName }: PlayerKickedData) {
		const isScrollAtBottom = this.scrollable.component.getIsScrollAtBottom();
	...
	private onScrollableBlur(_event: FocusEvent) {
		this.updateNavHelp();
	}

	private onScrollableFocus(_event: FocusEvent) {
		this.updateNavHelp();
	}
	...
	private onScrollAtBottom(_event: ScrollAtBottomEvent) {
		Network.readChat();
		this.forcedUnread = false;
		this.updateUnreadButtonContainer();
	}

	private onUnreadButtonActivate(_event: ActionActivateEvent) {
	...
	private onSend() {
		...
		const { targetType, targetID }: ChatTargetEntry = this.currentTarget;
	...
	private onContextClose(_event: CustomEvent) {
		this.updateSendToButton();
		this.updateEmoticonButton();
	}
	...
	private updateUnreadButtonContainer(forced = false) {
		this.forcedUnread = forced;
		this.unreadButtonContainer.classList.toggle('hidden', !Network.getUnreadChatLength() && !this.forcedUnread);
	}

	private onMultiplayerChat = (data: MultiplayerChatData, playSound = true) => {
		const { fromPlayer, toPlayer, targetType } = data;
		const playerFrom: ConfigurationPlayerAccessor = Configuration.getPlayer(fromPlayer);
		const playerTo: ConfigurationPlayerAccessor = Configuration.getPlayer(toPlayer);
	...
	private onPlayerInfoChanged({ data }: GenericDataInt32) {
		...
		const teamTarget: ChatTargetEntry | undefined = currentChatTargets.find(t => t.targetType == ChatTargetTypes.CHATTARGET_TEAM);
	...
	private onPrivateSelect({ detail: { currentTarget } }: PrivateSelectEvent) {
	...
	private onEmoticonSelect({ detail: { iconId } }: EmoticonSelectEvent) {
		const oldMessage: string = this.editBox.getAttribute("value") ?? "";
		const { selectionStart = oldMessage.length } = this.editBox.querySelector<HTMLInputElement>("input") || {};
	...
	*/
	getCurrentPlayersByName(playerName: string) {
		const playersMatched: ChatTargetEntry[] = [];
	...
	setCurrentTarget(currentTarget: ChatTargetEntry) {
		this.currentTarget = currentTarget;
		this.updatePreviewTarget();
		this.updateSendToButton();
		this.updateEditBox();
	...
	setCommandError(error: string) {
		this.commandError = error;
	}

	setMarkupMessage(message: string) {
		this.markupMessage = message;
		this.stylizedMarkupMessage = Locale.stylize(message);
	}

	attachNodeToScrollable(msgNode?: HTMLElement) {
		if (!msgNode) {
			return;
		}
		...
		while (this.chatList.children.length > ChatGlobalTypes.MAX_CHAT_HISTORY) {
			this.chatList.removeChild(this.chatList.firstChild as Node);
		}
	}
	...
	private createMessage(data: MultiplayerChatData): HTMLElement {
	...
	*/
	...
	private onSendToButtonActivate(_event: ActionActivateEvent) {
		this.toggleSendToPanel();
	}

	private onSendButtonActivate(_event: ActionActivateEvent) {
		this.onSend();
		this.setCaretAtPosition();
	}

	private onEmoticonButtonActivate(_event: ActionActivateEvent) {
		this.toggleEmoticonPanel();
	}
	...
	private setCaretAtPosition(position = -1) {
		const input: HTMLInputElement | null = this.editBox.querySelector<HTMLInputElement>("input");
		if (!input) {
			return;
		}
		const elemLength: number = input.value.length;
