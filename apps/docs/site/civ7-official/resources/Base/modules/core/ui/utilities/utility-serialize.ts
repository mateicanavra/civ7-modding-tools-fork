...
abstract class SerialBase {
	...
	protected readonly hashPreamble: string;
	protected childrenIDs: Set<string> = new Set<string>();
	 ...
	 * @returns hash value
	 */
	protected makeHash(key: string): HashId {
		const hash: HashId = Database.makeHash(this.hashPreamble + key);
		return hash;
	}
	 ...
	 */
	protected readIDs(): number {
		const hash: HashId = this.makeHash("KEYS");
		const ids = GameTutorial.getProperty(hash);
		if (ids) {
			if (typeof ids === "string") {
				this.childrenIDs = new Set<string>(ids.split(","));
...
export class SerialObject extends SerialBase {
	...
	read(key: string): SerializeType {
		const hash: HashId = this.makeHash(key);
	 ...
	 */
	write(key: string, value: SerializeType) {
		const hash: HashId = this.makeHash(key);
