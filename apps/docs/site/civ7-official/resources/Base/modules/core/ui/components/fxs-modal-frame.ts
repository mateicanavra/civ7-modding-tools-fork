...
export class FxsModalFrame extends Component {
	...
	private get modalStyle(): 'generic' | 'special' | 'narrative' | 'on-map' {
	...
	onInitialize() {
		...
		this.contentAs = this.Root.dataset.contentAs as keyof HTMLElementTagNameMap || this.contentAs;
	...
	private render() {
		...
		if (modalStyle === 'generic') {
			...
			const backDropAttribute: string | null = this.Root.getAttribute("data-bg-image");
			...
			if (backDropAttribute) { bgImage.style.backgroundImage = backDropAttribute };
