...
export const FrameCloseEventName: string = 'frame-closed' as const;
export class FrameCloseEvent extends CustomEvent<{ x: number, y: number }> {
...
export class FxsFrame extends Component {
	...
	protected contentAs: keyof HTMLElementTagNameMap = 'div';
	...
	onInitialize() {
		...
		this.contentAs = contentAs as keyof HTMLElementTagNameMap | null || this.contentAs;
		...
		let closeButton = this.Root.querySelector<HTMLElement>('fxs-close-button');
		if (!closeButton && (this.Root.getAttribute('can-close') ?? 'false') == 'true') {
			closeButton = document.createElement('fxs-close-button');
			closeButton.addEventListener(ActionActivateEventName, (event: ActionActivateEvent) => {
		...
		const frameAdditionalStyling: string = this.Root.getAttribute('override-styling') ?? 'relative flex max-w-full max-h-full pt-14 px-10 pb-10';
		this.Root.classList.add('z-0', 'pointer-events-auto', ...frameAdditionalStyling.split(' '));

		const style: string = this.Root.getAttribute("frame-style") ?? "f1";
		const noFiligree: boolean = this.Root.getAttribute("no-filigree") == "true";
		if (style !== "simple" && !noFiligree) {
			const filigreeClass: string = this.Root.getAttribute('filigree-class') ?? 'mt-8';
