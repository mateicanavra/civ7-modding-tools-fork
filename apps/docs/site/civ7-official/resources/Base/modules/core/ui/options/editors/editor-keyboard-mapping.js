...
class EditorKeyboardBindingPanel extends Panel {
    ...
    updateData() {
        const { contextName = "", context = InputContext.INVALID, actionName = "", actionID = 0, } = this.editorKeybardBindingPanelNode ?? {};
...
export class EditorKeyboardButton extends FxsButton {
    ...
    updateData() {
        const { context = InputContext.INVALID, actionID = 0, gestureIndex = 0, } = this.editorControllerChooserNode ?? {};
        const actionDescription = Input.getActionDescription(actionID);
