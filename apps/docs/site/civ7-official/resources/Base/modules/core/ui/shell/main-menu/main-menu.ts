...
import DialogManager, { DialogBoxAction, DialogBoxCallbackSignature, DialogBoxID, DialogBoxOption } from '/core/ui/dialog-box/manager-dialog-box.js';
...
import { LegalDocsAcceptedEvent, LegalDocsAcceptedEventName } from '/core/ui/shell/mp-legal/mp-legal.js';
import MultiplayerShellManager from '/core/ui/shell/mp-shell-logic/mp-shell-logic.js';
import { GameCreatorOpenedEventName, GameCreatorClosedEventName, StartCampaignEventName, SendCampaignSetupTelemetryEventName } from '/core/ui/events/shell-events.js';
...
import SaveLoadData, { QueryCompleteEvent, QueryCompleteEventName, SaveGameInfo } from '/core/ui/save-load/model-save-load.js';
import { SaveLoadClosedEventName, fixupNNBSP } from '/core/ui/save-load/screen-save-load.js';
...
import { MainMenuReturnEvent, MainMenuReturnEventName, SendCampaignSetupTelemetryEvent } from '/core/ui/events/shell-events.js';
...
import { FlagsToCheckBlockedAccessReason, giftboxButtonName, ProfileHeader } from '/core/ui/profile-header/profile-header.js';
...
/**
 * Pieces needed for a button enumerated in the main menu.
 */
type MainMenuButton = {
	name: string,
	buttonListener: () => void,
 ...
 */
export let isLiveEventGame: boolean = false;
...
class MainMenu extends Component {
	...
	private profileHeader?: ComponentRoot<ProfileHeader>;
	private odrDownload?: HTMLElement;
	private slot!: HTMLElement;
	private bgContainer!: HTMLElement;
	private connStatus!: HTMLElement;
	private connIcon!: HTMLElement;
	private accountStatus!: HTMLElement;
	private accountStatusAnim!: HTMLElement;
	private accountStatusNavHelp!: HTMLElement;
	private accountIcon?: HTMLElement;
	private accountIconActivatable!: HTMLElement;
	private motdDisplay!: HTMLElement;
	private buildInfo!: HTMLElement;
	private carouselMain!: HTMLElement;
	private carouselBreadcrumbs!: HTMLElement;
	private carouselContent!: HTMLElement;
	private carouselBackButton!: HTMLElement;
	private carouselInteractButton!: HTMLElement;
	private carouselText!: HTMLElement;
	private carouselContentText!: HTMLElement;
	private carouselTextScrollable!: ComponentRoot<FxsScrollable>;
	private carouselStandardTextScrollable!: ComponentRoot<FxsScrollable>;
	private carouselImageContainer!: HTMLElement;
	private carouselBaseLayout!: HTMLElement;
	private carouselBaseLayoutImage!: HTMLImageElement;
	private carouselBaseLayoutText!: HTMLElement;
	private carouselSliderId: number = -1;
	private movieContainer!: HTMLElement;
	private shroud!: HTMLElement;
	private engineInputListener = this.onEngineInput.bind(this);
	private carouselEngineInputListener = this.onCarouselEngineInput.bind(this);
	...
	private accountIconListener = this.onClickedAccount.bind(this);
	private creditsOpenedListener = this.onCreditsOpened.bind(this);
	private creditsClosedListener = this.onCreditsClosed.bind(this);
	private returnToMainMenuListener = this.returnedToMainMenu.bind(this);
	private sendCampaignSetupTelemetryListener = this.sendCampaignSetupTelemetry.bind(this);
	private calibrateHDROpenedListener = this.onCalibrateHDROpened.bind(this);
	private calibrateHDRClosedListener = this.onCalibrateHDRClosed.bind(this);
	private eventsGoSinglePlayerListener = this.onEventsGoSP.bind(this);
	private eventsGoMultiPlayerListener = this.onEventsGoMP.bind(this);
	private eventsGoLoadListener = this.onEventsGoLoad.bind(this);
	private eventsGoContinueListener = this.onEventsGoContinue.bind(this);
	private gameCreatorOpenedListener = this.onGameCreatorOpened.bind(this);
	private gameCreatorClosedListener = this.onGameCreatorClosed.bind(this);
	private startNewCampaignListener = this.onNewCampaignStart.bind(this);
	private motdCompletedListener = this.gotMOTD.bind(this);
	private promosDataReceivedListener = this.resolvePromoDataReceived.bind(this);
	private refreshPromosListener = this.refreshPromos.bind(this);
	private startGameSectionListener = this.startSection.bind(this);
	private spoPCompleteListener = this.onSPoPComplete.bind(this);
	private spoPKickPromptCheckListener = this.onSPoPKickPromptCheck.bind(this);
	private spopHeartBeatReceivedListener = this.onSPoPHeartBeatReceived.bind(this);
	...
	private saveLoadClosedListener = this.onSaveLoadClosed.bind(this);
	private connectionStatusChangedListener = this.onConnectionStatusChanged.bind(this);
	private liveEventsSettingsChangeListener = this.onLiveEventsSettingsChanged.bind(this);
	private endStateListener = this.onAccountUpdated.bind(this);
	private rewardReceivedListener = this.onRewardRecieved.bind(this);
	private ssoRecheckTimeBufferReachedListener = this.onRecheckSSO.bind(this);
	private odrDownloadButtonActivateListener = this.onOdrButtonActivate.bind(this);

	private continueSave: SaveGameInfo | null = null;
	private MainMenuSceneModels: WorldUI.ModelGroup | null = null;
	private static readonly VO_CAMERA_POSITION: float3 = { x: -1.834, y: -23.0713, z: 15.2000 };
	private static readonly VO_CAMERA_TARGET: float3 = { x: -2.7588, y: -17.4867, z: 14.8042 };
	private currentPreloadingAsset: WorldUI.AssetID | null = null;
	private hasPreloadingBegun: boolean = false;
	private leaderIndexToPreload: number = 0;
	private campaignSetupTimestamp: DOMHighResTimeStamp = 0;
	private campaignSetupId: string | null = null;
	private carouselItems: CarouselItem[] = [];
	private selectedCarouselItem: number = 0;
	private areLegalDocsAccepted: boolean = false;
	private bootLoaded: boolean = false;
	private toggleCarouselAdded: boolean = false;
	private nextPromoAdded: boolean = false;
	private previousPromoAdded: boolean = false;
	private isUserInitiatedLogout: boolean = false;
	private firstLaunchTutorialPending: boolean = false;
	private inSubScreen: boolean = false;
	private leaderModelSetup: boolean = false;
	private mainMenuActivated = false;
	private mainMenuButtons: HTMLElement[] = [];
	private needKickDecision: boolean = false;
	// Some subscreens requires the profile header while the hdr is not one of them
	private hdrCalibrationMenuOpen: boolean = false;
	private bShowRewardsScreen: boolean = false;
	private bPendingSSOCheck: boolean = false;
	private pendingSSODialogBoxID: DialogBoxID = -1;
	private forceOfflineLegalFlow: boolean = false;
	...
	onInitialize() {
		...
		odrDownloadButton.classList.add("main-menu_odr-download-button", "img-prof-btn-bg", "pointer-events-auto", "flow-column", "justify-center", "items-center", "w-16", "h-16", "transition-transform", "hover\\:scale-110", "focus\\:scale-110");
		...
		if (Network.supportsSSO()) {
			...
			this.profileHeader.setAttribute("hide-giftbox", (Online.UserProfile.getRewardsEnabledConfiguration() ? "false" : "true"));
			...
			this.profileHeader.classList.add("main-menu__profile-header");
			this.profileHeader.setAttribute("profile-for", "main-menu");
			this.profileHeader.setAttribute("hide-progression-header", Online.Metaprogression.supportsMemento() ? "false" : "true");
	...
	onAttach() {
		...
		let mpAgeTransition: boolean = false;
		let ageTransition: boolean = false;

		let transitionState: TransitionType = Modding.getTransitionInProgress();
		...
		const buttonList: MainMenuButton[] = [];
		buttonList.push({ name: "LOC_MAIN_MENU_CONTINUE", audio: "continue", buttonListener: () => { if (this.canPerformInputs()) this.goContinue(); }, extraClass: "continue-item", disabled: true, separator: true });
		buttonList.push({ name: "LOC_MAIN_MENU_NEW_GAME", audio: "create-game", buttonListener: () => { if (this.canPerformInputs()) this.openCreateGame(); }, extraClass: "create-game-item" });
		buttonList.push({ name: "LOC_MAIN_MENU_LOAD_GAME", audio: "load-game", buttonListener: () => { if (this.canPerformInputs()) this.openLoadGame(); } });

		if (UI.isNetworkBuild() && UI.supportsMultiplayer()) {
			buttonList.push({ name: "LOC_MAIN_MENU_MULTIPLAYER", audio: "multiplayer", buttonListener: () => { if (this.canPerformInputs()) this.openMultiplayer(); }, separator: true });
		}

		if (UI.supportsDLC() && UI.isNetworkBuild()) {
			buttonList.push({ name: "LOC_UI_STORE_LAUNCHER_TITLE", audio: "store", buttonListener: () => { if (this.canPerformInputs()) this.openStore(); }, disabled: !UI.isNetworkBuild() });
		}

		if (UI.shouldShowAdditionalContent()) {
			buttonList.push({ name: "LOC_MAIN_MENU_ADDITIONAL_CONTENT", audio: "additional-content", buttonListener: () => { if (this.canPerformInputs()) this.openExtras(); } });
		} else {
			buttonList.push({ name: "LOC_MAIN_MENU_CREDITS", audio: "additional-content", buttonListener: () => { if (this.canPerformInputs()) this.onCredits(); } });
		}

		buttonList.push({ name: "LOC_MAIN_MENU_OPTIONS", audio: "options", buttonListener: () => { if (this.canPerformInputs()) this.openOptions(); } });

		// Register debug widget.
		const toggleTestScene: UIDebugWidgetDefinition = {
			id: 'toggleTestScene',
			category: 'Shell',
		...
		engine.on('DebugWidgetUpdated', (id: string, _value: boolean) => {
		...
		if (UI.canExitToDesktop()) {
			buttonList.push({ name: "LOC_MAIN_MENU_EXIT", audio: "exit", buttonListener: () => { if (this.canPerformInputs()) this.exitToDesktop(); } });
		}

		let firstButton: boolean = true;
		buttonList.forEach((button: MainMenuButton) => {
			const newButton: HTMLElement = document.createElement("fxs-text-button");
		...
		if (Network.hasProgressedPastLegalDocs()) {
			// should be the majority of the time unless experiencing network issues
			this.updateAreLegalDocsAccepted();
		}
		else {
			// the ui is ready before the network has finished fetching legal docs
			// first give some additional buffer time, the backend will inform us
			// when the timer is up and to recheck again, if it fails again, then 
			// show pending popup for network services. this keeps the timer on the backend
		...
		if (Network.supportsSSO()) {
			...
			if (!Network.isWaitingForValidHeartbeat()) {
				...
				this.accountIconActivatable.setAttribute("data-tooltip-content", Locale.compose("LOC_UI_WAITING_SPOP_HEARTBEAT_OK"));
			}
		...
		// attach listener for sending campaign setup telemetry
		...
		if (UI.shouldShowHighEndAssetsDownloadOption()) {
	...
	onReceiveFocus() {
		...
		UI.toggleGameCenterAccessPoint(true);

		if (this.isCarouselExpanded()) {
			Focus.setContextAwareFocus(this.carouselMain, this.Root);
		} else if (!this.hdrCalibrationMenuOpen) {// we dont show Online Features when the hdr calibration is open but is not the case for all subscreens
			Focus.setContextAwareFocus(this.slot, this.Root);
			this.showOnlineFeaturesUI(); // we don't show Online Features (MOTD, profile header, 2K Account when promo is expanded)
		}
	...
	checkDNAFatalError() {
		if (!Network.isConnectedToSSO() && Network.isConnectedToNetwork() && Network.isForcedOfflineFromEmptyResponse()) {
			DialogManager.createDialog_Confirm({ body: Locale.compose("LOC_UI_FORCE_OFFLINE_ACCOUNT_BODY"), title: Locale.compose("LOC_UI_OFFLINE_ACCOUNT_TITLE") });
		}
	}
	...
	private isSelectedPromoInteractable(): boolean {
	...
	private openLoadGame(isFromEvent: boolean = false) {
		if (this.checkForLegalDocs()) {
			return;
		}
	...
	private setAccountIcon(status: boolean) {
		...
		if (this.accountIcon && Network.isConnecting()) {
			...
			if (status) {
				...
				if (Network.isAccountLinked() && !Network.isAccountComplete()) {
					...
					this.accountIconActivatable.setAttribute("data-tooltip-content", "LOC_UI_ACCOUNT_LOGGEDIN_INCOMPLETE");
				} else if (Network.isLoggedIn() && !Network.isAccountLinked()) {
					this.accountIcon.style.backgroundImage = "url('fs://game/core/ui/themes/default/img/my2k_notlinked.png')";
			...
			this.accountIcon.style.backgroundImage = "url('fs://game/core/ui/themes/default/img/my2k_notloggedin.png')";
			this.accountIconActivatable.setAttribute("data-tooltip-content", "LOC_UI_CONNECTION_FAILED");
		}
	}

	private enableMainMenuButtonbyName(name: string, status: boolean, msg: string = "") {
		const resolvedName = Locale.stylize(name).toUpperCase(); // has to match stylized setup, see onAttach() buttonList
		this.mainMenuButtons.forEach((button: HTMLElement) => {
	...
	private onRewardRecieved() {
		this.bShowRewardsScreen = true;
	}

	private handleOfflineLegalDocFlow() {
		// Ui has been informed the offline legal docs are ready for parsing.
	...
	private onRecheckSSO() {
		// We have been told recheck the legal doc status
		let hasProgressedPastLegalDocs = Network.hasProgressedPastLegalDocs();
	...
	private onClickedAccount() {
		...
		if (isConnectedToNetwork && Network.isLoggedIn()) {
			if (Network.isAccountLinked() && Network.isAccountComplete()) {
				...
				const twoKPortalCallBack: DialogBoxCallbackSignature = (eAction: DialogBoxAction) => {
					if (eAction == DialogBoxAction.Confirm) {
						ContextManager.push("screen-mp-link-account", { singleton: true, createMouseGuard: true });
					}
				}

				const logoutCallback: DialogBoxCallbackSignature = (eAction: DialogBoxAction) => {
					if (eAction == DialogBoxAction.Confirm) {
				...
				const resetPrimaryCallback: DialogBoxCallbackSignature = () => {
					DialogManager.createDialog_ConfirmCancel({
						...
						canClose: true,
						callback: (eAction: DialogBoxAction) => { if (eAction == DialogBoxAction.Confirm) { Online.Metaprogression.resetPrimaryAccountSelection(); } }
					});
				}

				const logoutOption: DialogBoxOption = {
					actions: ["accept"],
					label: "LOC_GENERIC_LOGOUT",
					callback: logoutCallback,
				}

				const twoKPortalOption: DialogBoxOption = {
					actions: ["shell-action-2"],
					label: "LOC_GENERIC_TWOKPORTAL",
					callback: twoKPortalCallBack,
				}

				const resetPrimaryOption: DialogBoxOption = {
			...
			if (banInfo != "") {
				DialogManager.createDialog_Confirm({
					body: banInfo, //2K will handle localization 
	...
	private onQueryComplete(event: QueryCompleteEvent) {
		switch (event.detail.result) {
			...
			case SerializerResult.RESULT_OK:
				...
				const isDisabled: boolean = this.continueSave == null || hasMissingMods || hasUnownedMods;
				...
				if (this.continueSave) {
					...
					const tooltip = Locale.stylize("LOC_MAIN_MENU_CONTINUE_INFO",
						save.gameName,
		...
		if (!this.inSubScreen) {
			// the player already opened another screen, skip for now
			// build3DScene will be called if they return to the main menu
			this.build3DScene();
		}
	...
	private onLiveEventsSettingsChanged() {
	...
	private getAccountLinkPromptMsg() {
	...
	private goContinue() {
		if (this.continueSave && this.continueSave.missingMods.length == 0 && this.continueSave.unownedMods.length == 0) {
			UI.toggleGameCenterAccessPoint(false);
			// Loading a save currently responds to whatever the current configuration's game mode is using.
			// Reset the configuration since it may be contaminated by going into MP screens and setting the game mode as MP.
			Configuration.editGame()?.reset(GameModeTypes.SINGLEPLAYER);
			SaveLoadData.handleLoadSave(this.continueSave, ServerType.SERVER_TYPE_NONE);
		}
	}

	private showLegalDocuments() {
	...
	private gotMOTD() {
		...
		if (Online.MOTD.isMOTDReady()) {
			const titles: string[] = Online.MOTD.getAllMOTDHeaders();

			titles.forEach((title: string) => {
				const msg: string | null = Online.MOTD.getMOTD(title);
				if (msg) {
			...
			// display a msg
			const randIndex: number = Math.floor(Math.random() * titles.length);
			const msg: string | null = Online.MOTD.getMOTD(titles[randIndex]);
			if (msg) {
	...
	private onSPoPHeartBeatReceived() {
		this.onAccountUpdated();
	...
	private onSPoPKickPromptCheck() {
		if (Network.requireSPoPKickPrompt()) {
			this.getKickDecision();
		}
	}

	private getKickDecision() {
	...
	private checkForError() {
		...
		if (lastError != null) {
			...
			if (lastError == LoadErrorCause.MOD_CONTENT) {
				...
				let popupReason: number = KickReason.KICK_NONE;
				const lastReason: number | null = Modding.getLastLoadErrorReason();
				if (lastReason) {
					popupReason = lastReason;
				}
				const abandonPopup: NetworkUtilities.AbandonReasonPopup = NetworkUtilities.multiplayerAbandonReasonToPopup(popupReason);
				...
				let ownershipErrors: OwnershipEntryResult[] = Modding.getLastOwnershipCheck();
	...
	private startSection(data: string) {
		switch (data) {
			case "multiplayer":
	...
	private resolvePromoDataReceived(data: PromosDataReceivedData) {
	...
	private appendPromoToCarousel(promo: PromoData, itemIndex: number) {
		if (itemIndex < 0) {
	...
	private createCarousel(data: PromosDataReceivedData) {
		if (!Network.supportsSSO()) {
			return;
		}
		...
		this.updateCarousel(CarouselActionTypes.NO_ACTION);
	...
	private onCarouselBack = (_event: Event) => {
		this.toggleCarouselMode();
	}

	private onCarouselInteract = (_event: Event) => {
		this.interactWithSelectedPromo();
	}

	private setPromoBackButtonVisibility(isVisible: boolean) {
		if (!Network.supportsSSO()) {
			return;
		}
	...
	private setPromoInteractButtonVisibility(isVisible: boolean) {
		if (!Network.supportsSSO()) {
			return;
		}
	...
	private updatePromoButtonsVisibility() {
		if (!Network.supportsSSO()) {
			return;
		}

		let isBackVisible: boolean = false;
		let isInteractVisible: boolean = false;
	...
	private processSelectedPromo() {
		const selectedPromo: CarouselItem | undefined = this.carouselItems[this.selectedCarouselItem];
	...
	private toggleCarouselMode() {
		...
		if (this.isCarouselExpanded()) {
			...
			document.querySelectorAll(".carousel-expanded-bumper").forEach((bumper: HTMLElement) => bumper.classList.remove("hidden"));
			document.querySelectorAll(".carousel-thumbnail-bumper").forEach((bumper: HTMLElement) => bumper.classList.add("hidden"));
			document.querySelector(".carousel-close-button-div")?.classList.remove("hidden");
			this.carouselBreadcrumbs.classList.add("hidden");
			const selectedCarousel: CarouselItem | undefined = this.carouselItems[this.selectedCarouselItem];
			...
			document.querySelector(".carousel-thumb-title")!.textContent = Locale.stylize(this.carouselItems[this.selectedCarouselItem]?.carouselTitle!);
			...
			document.querySelectorAll(".carousel-expanded-bumper").forEach((bumper: HTMLElement) => bumper.classList.add("hidden"));
			document.querySelectorAll(".carousel-thumbnail-bumper").forEach((bumper: HTMLElement) => bumper.classList.remove("hidden"));
	...
	private showPromoLoadingSpinner() {
		...
		// here we need to pass in if we're in modal or carousel cause the spinner will show differently.
		const isCarouselExpanded: boolean = this.isCarouselExpanded();
	...
	private hidePromoLoadingSpinner() {
		...
		if (isCarouselExpanded) {
			...
			// hide carousel loading spinner
		}
	}

	private refreshPromos() {
		if (!Network.supportsSSO()) {
			return;
		}
	...
	private updateCarousel(action: CarouselActionTypes = CarouselActionTypes.PROCESS_PROMO) {
		...
		for (let i: number = 0; i < this.carouselBreadcrumbs.children.length; i++) {
			...
			if (i == this.selectedCarouselItem) {
				...
				if (isCarouselExpanded && action == CarouselActionTypes.PROCESS_PROMO) {
					this.processSelectedPromo();
				}

				// if carouselImageUrl (used for both carousel and modal expect when modal image is specified differently) is valid. 
				if (this.carouselItems[i].carouselImageUrl && !bForceShowPromoLoadingSpinner) {
					...
					// thumbnail
						...
						if (this.carouselItems[i].carouselTitle) {
							this.carouselText.classList.toggle("hidden", true)
							document.querySelector(".carousel-thumb-title")!.textContent = Locale.stylize(this.carouselItems[i].carouselTitle!);
						} else {
							this.carouselText.innerHTML = "";
						}
					}
				}
				// no carouselImage url found: image is not ready, still downloading... show spinner.
				else {
	...
	private isCarouselVisible(): boolean {
		if (!Network.supportsSSO()) {
			return false;
		}
	...
	private isCarouselExpanded(): boolean {
		if (!Network.supportsSSO()) {
			return false;
		}
	...
	private canPerformInputs(): boolean {
	...
	private activateMainMenu() {
		...
		this.checkDNAFatalError(); // activate after legal docs accepted to prevent possible focus issues
	}

	private trySkipMenuAnimations() {
		this.skipToMainMenu();
	}
	...
	private onCarouselEngineInput(inputEvent: InputEngineEvent) {
		if (inputEvent.detail.status != InputActionStatuses.FINISH) {
			return;
		}
	...
	private tryToHandleLegalDocs() {
		...
		if ((!this.bPendingSSOCheck && Network.hasProgressedPastLegalDocs()) || this.forceOfflineLegalFlow) {
			...
			let legalDocsCheck: boolean = false;
			...
			if (legalDocsCheck) {
				// legal docs gate functionality on main menu until online docs or offline docs are accepted
	...
	private startGame() {
		...
		let campaignSetupData: CampaignSetupData = {
	...
	private showProfilePage(profileTabToFocus: ProfileTabType) {
		Animations.cancelAllChainedAnimations();
	...
	private sendCampaignSetupTelemetry(event: SendCampaignSetupTelemetryEvent) {
		if (!this.campaignSetupId)
			return;
	...
	private onLegalDocsAccepted = (event: LegalDocsAcceptedEvent) => {
	...
	private checkForLegalDocs(): boolean {
	...
	private build3DScene() {
		...
		for (let attemptsLeft = 3; attemptsLeft >= 0; attemptsLeft--) {
			// select the last player leader (the one that's selected on the continue save)
			let leaderIndex: number = -1;
			let civIndex: number = -1;
			if (this.continueSave) {
				const lastLeader: string = this.continueSave.hostLeader;
				leaderIndex = leaderData.findIndex(l => l.leaderID == lastLeader);
				const lastCiv: string = this.continueSave.hostCivilization;

				civIndex = civData.findIndex(l => l.civID == lastCiv);
			}

			if (leaderIndex == -1) {
				// choose a random leader
				leaderIndex = leaderData.length > 0 ? UI.randomInt(0, leaderData.length - 1) : -1;
			}

			if (civIndex == -1) {
				// choose a random civ
				civIndex = civData.length > 0 ? UI.randomInt(0, civData.length - 1) : -1;
			}

			let assetName = "LEADER_FALLBACK_GAME_ASSET";
			...
			if (leaderIndex != -1) {
				...
				console.warn("main-menu build3DScene: leaderData is empty");
			}
			...
			if (leader) {
				...
				if (civIndex != -1) {
					const backgroundName: string = `bg-panel-${civData[civIndex].civID.replace("CIVILIZATION_", "").toLowerCase()}`;
					WorldUI.addMaskedBackgroundLayer(backgroundName, "mm_bg_mask", { stretch: StretchMode.UniformFill, alignY: AlignMode.Maximum }); // uniform anchored at bottom
				} else {
					console.warn("main-menu build3DScene: civData is empty");
				}
				break;
			}
		}
	...
	private beginPreloadingForNextLeader(assetToWaitFor: WorldUI.AssetID | null) {
	...
	private preloadLeaderModels(index: number) {
		const leaderData = getLeaderData();
		...
		let assetName: string = leaderData[index].leaderID + "_GAME_ASSET";
