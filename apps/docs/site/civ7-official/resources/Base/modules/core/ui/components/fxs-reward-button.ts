...
class FxsRewardButton extends FxsActivatable {
	...
	private readonly mainText = document.createElement('div');
	private readonly actionText = document.createElement('div');
	private navContainer: HTMLDivElement | null = null;
	private storyType: string = "";
	private leaderCivChoice: string = "";
	private rewardText: string = ""; //tooltip text
	...
	onInitialize() {
		...
		let icons: NarrativeRewardIconDefinition[] = [];
		if (iconsStored) {
	...
	render({ icons }: { icons: NarrativeRewardIconDefinition[] }) {
		...
		this.navContainer = this.Root.querySelector<HTMLDivElement>('.fxs-button__nav');

		this.addOrRemoveNavHelpElement(this.navContainer!, this.Root.getAttribute('action-key'));

		const rewardHolder: HTMLDivElement = document.createElement("div");
		...
		let warningBarAdded: boolean = false;
		let iconCounter = 0;
		let iconsLength = icons.length;

		const rewardIconArrange: HTMLDivElement = document.createElement("div");
		...
		if (icons && iconsLength > 0) {
			icons.forEach(icon => {
				if (icon.RewardIconType == "QUEST") {
					...
					const questIcon: HTMLDivElement = document.createElement("div");
				...
				const rewardIconContainer: HTMLDivElement = document.createElement("div");
				rewardIconContainer.classList.add("reward-icon-individual", "self-center");

				const rewardIcon: HTMLDivElement = document.createElement("div");
				rewardIcon.classList.add("fxs-reward-button__icon", "w-full", "h-full", "pointer-events-none");

				let targetIconPrefix: string = "NAR_REW_";
