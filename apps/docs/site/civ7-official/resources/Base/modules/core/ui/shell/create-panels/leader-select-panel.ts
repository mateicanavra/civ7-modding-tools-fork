...
import { getLeaderData, getMementoData, LeaderData, OwnershipAction } from '/core/ui/shell/create-panels/leader-select-model.js';
...
class LeaderSelectPanel extends GameCreationPanelBase {
	...
	private readonly leaderList = document.createElement("fxs-spatial-slot");
	private leaderButtons: ComponentRoot<LeaderButton>[] = [];

	private readonly selectedNameEle = document.createElement("p");
	private readonly selectedTagsListEle = document.createElement("p");
	private readonly selectedAbilityTitleEle = document.createElement("p");
	private readonly selectedAbilityTextEle = document.createElement("p");
	private readonly additionalLeaderInfo = document.createElement("div");
	private readonly learnMore = document.createElement('learn-more');

	private readonly mementosSection = document.createElement("div");
	private mementoSlotEles: ComponentRoot<MementoSlot>[] = [];

	private readonly unlocksHeader = document.createElement("fxs-header");
	private readonly unlockListEle = document.createElement("div");
	private unlockEles: HTMLElement[] = [];

	private readonly nextLevelUnlockEle = document.createElement("p");

	private selectedLeaderEle?: ComponentRoot<LeaderButton>;

	private isAccountLoginPending = false;

	private leaderButtonListener: EventListener = this.selectLeaderHandler.bind(this);
	private leaderFocusListener: EventListener = this.focusLeaderHandler.bind(this);
	...
	protected override onContinue(): boolean {
		super.onContinue();
	...
	private selectGameParamHero(): void {
	...
	private focusLeaderHandler(evt: Event) {
		if (ActionHandler.isGamepadActive) {
			this.selectLeader(evt.target as ComponentRoot<LeaderButton>);
		}
	}

	private selectLeaderHandler(evt: Event): void {
		if (ActionHandler.isGamepadActive) {
			this.showNextPanel();
		} else {
			this.selectLeader(evt.target as ComponentRoot<LeaderButton>);
	...
	private selectLeader(leaderButton?: ComponentRoot<LeaderButton>) {
		...
		if (leaderData) {
			...
			const gameConfig: ConfigurationGameMutator | null = Configuration.editGame();
			const playerConfig: ConfigurationPlayerMutator | null = Configuration.editPlayer(localPlayerID);
	...
	private showUnlockLeaderScreen() {
	...
	private showMementoEditor(slotIndex: number) {
