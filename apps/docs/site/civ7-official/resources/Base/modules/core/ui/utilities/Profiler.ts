...
const MAX_PROFILES: number = 40;  // Maximum number of sampled series (aka: "profiles") that can be set.
const MAX_SAMPLES: number = 1000;  // Maximum pieces of sampled data for a given profile.


export class Profiler {
  _label: string[];          // name of labels
  _start: number[][];        // starting times
  _durration: number[][];    // time from start
  _sampleIndexes: number[];  // next index for a given sample to be writen
  _labelIndex: number;       // index of the next label added
   ...
   */
  start(id: number, attributes?: any | undefined): void {
   ...
   */
  private computeAverage(id: number): number {
    let average: any = (array: number[]) => array.reduce((a: number, b: number) => a + b) / array.length;
   ...
   * @param id Identifier of the array to work on.
   */
  private findWorst(id: number): number {
    let worst: any = (array: number[]) => array.reduce(function (a, b) { return Math.max(a, b); });
   ...
   */
  toCSV(): string {
    let out: string = "#,label,average(ms),worst(ms),\n";
    for (let i: number = 0; i <= this._labelIndex; i++) {
  ...
  toChromeJSON(): string {
    ...
    for (let i: number = 0; i <= this._labelIndex; i++) {
      let completeEvent: Object = {
        name: this._label[i],
        cat: "PERF",
        ph: "X",  // "complete" event
        ts: 0,    // this._start[i][0]
