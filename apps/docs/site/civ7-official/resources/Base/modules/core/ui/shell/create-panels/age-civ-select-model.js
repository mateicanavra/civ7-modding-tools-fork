    ...
    if (playerCivilizations) {
        ...
        for (const civData of playerCivilizations.domain.possibleValues ?? []) {
            ...
            const valueUnlocks = civUnlockData
                ...
                && (unlock.AgeDomain == null || ageMap.get(unlock.AgeType)?.domain == unlock.AgeDomain));
            let civBiasForCivAndLeader = civBiasData.filter(row => row.CivilizationType == civID
            ...
            valueUnlocks.sort((a, b) => (a.AgeType == b.AgeType) ?
                Locale.compare(a.Type, b.Type) :
                Locale.compare(a.AgeType, b.AgeType));
            const unlocks = valueUnlocks.map(unlock => {
                ...
                return age
                    ...
                    : Locale.stylize('LOC_CREATE_GAME_UNLOCK_ITEM', unlock.Name);
            });
            ...
            const unlocksByCiv = civUnlockData
                .filter(unlock => unlock.Type == civID
                && (unlock.AgeDomain == null || ageMap.get(unlock.AgeType)?.domain == unlock.AgeDomain)).map(civ => {
            ...
            const unlocksByLeader = leaderUnlocks
                .filter(unlock => unlock.Type == civID
                && (unlock.AgeDomain == null || ageMap.get(unlock.AgeType)?.domain == unlock.AgeDomain)).map(unlock => {
            ...
            if (LiveEventManager.restrictToPreferredCivs()) {
                ...
                civBiasForCivAndLeader = civBiasForCivAndLeader.filter(row => row.ReasonType != 'LOC_REASON_LIVE_EVENT_DESCRIPTION');
            }
            ...
            const civHistoricalChoiceReason = civBiasForCivAndLeader.filter(row => row.ReasonType != null)
                .map(row => Locale.compose(row.ReasonType))
                .join(", ");
            const civHistoricalChoiceType = civBiasForCivAndLeader.filter(row => row.ChoiceType != null)
                .map(row => Locale.compose(row.ChoiceType))
                .join(", ");
            results.push({
                civID: civID,
                name: Locale.stylize(name),
                description: Locale.stylize(description ?? ''),
                icon: icon ?? '',
                image: image ?? '',
...
//# sourceMappingURL=file:///core/ui/shell/create-panels/age-civ-select-model.js.map
