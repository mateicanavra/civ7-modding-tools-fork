...
export namespace WorldUtil {
	...
	/**
	 * @param {boolean} revealed only get unit if on a revealed plot.
	 * @param {boolean} local only get if owned by local player
	 * @param {ComponentID} previousID an id of a unit that preceeds the unit to find (assuming multiple units on the plot)
	 */
	export type GetUnitParameters = { revealed?: boolean, local?: boolean, previousID?: ComponentID };
	 ...
	 */
	export function getUnit(location: PlotCoord, requirements?: GetUnitParameters): ComponentID {
		...
		units.some((unitID: ComponentID, index: number) => {
