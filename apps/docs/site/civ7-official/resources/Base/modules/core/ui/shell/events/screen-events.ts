...
export const EventsScreenGoSinglePlayerEventName = 'screen-events-sp' as const;
...
export const EventsScreenLoadEventName = 'screen-events-loading' as const;
...
export const EventsScreenContinueEventName = 'screen-events-continue' as const;
...
export const EventsScreenGoMultiPlayerEventName = 'screen-events-mp' as const;
...
export class ScreenEvents extends Panel {
	...
	private singlePlayerListener = () => { this.onSinglePlayer(); }
	private loadListener = () => { this.onLoadEvent(); }
	//private continueListener = () => { this.onContinueEvent(); }
	private multiPlayerListener = () => { this.onMultiPlayer(); }
	private engineInputListener = (inputEvent: InputEngineEvent) => { this.onEngineInput(inputEvent); };
	private activeLiveEventListener = this.onActiveLiveEvent.bind(this);

	private connIcon: HTMLElement | null = null;
	private connStatus: HTMLElement | null = null;
	private accountStatus: HTMLElement | null = null;
	...
	onAttach() {
		...
		const scrollable = MustGetElement<ComponentRoot<FxsScrollable>>(".events-scrollable", this.Root);
	...
	private updateText(currentEventPrefix: string, rewardData: LiveEventReward[]) {
	...
	private hideMultiplayerStatus() {
		this.connIcon = document.querySelector<HTMLElement>('.connection-icon-img');
		if (!this.connIcon) {
			console.error("screen-credits: Missing this.connIcon with '.connection-icon-img'");
			return;
		}

		this.connStatus = document.querySelector<HTMLElement>('.connection-status');
		if (!this.connStatus) {
			console.error("screen-credits: Missing this.connStatus with '.connection-status'");
			return;
		}

		this.accountStatus = document.querySelector<HTMLElement>('.account-status');
