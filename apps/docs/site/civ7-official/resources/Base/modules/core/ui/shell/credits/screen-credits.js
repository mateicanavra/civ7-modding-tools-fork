...
/*	Locales
    ...
    zh_Hant_HK
 */
export const ScreenCreditsOpenedEventName = 'screen-credits-opened';
class ScreenCreditsOpenedEvent extends CustomEvent {
    constructor() {
        super(ScreenCreditsOpenedEventName, { bubbles: false, cancelable: true });
    }
}
export const ScreenCreditsClosedEventName = 'screen-credits-closed';
...
var Speeds;
(function (Speeds) {
    Speeds[Speeds["NORMAL"] = 24] = "NORMAL";
    Speeds[Speeds["FAST"] = 64] = "FAST";
    Speeds[Speeds["VERY_FAST"] = 128] = "VERY_FAST";
    Speeds[Speeds["PLAID"] = 256] = "PLAID"; // TODO: Fix speedy items adding gaps.
})(Speeds || (Speeds = {}));
var Position;
(function (Position) {
...
var Op;
(function (Op) {
    Op[Op["no_style"] = 0] = "no_style";
    Op[Op["title_style"] = 1] = "title_style";
    Op[Op["subtitle_style"] = 2] = "subtitle_style";
    Op[Op["role_style"] = 3] = "role_style";
    Op[Op["name_style"] = 4] = "name_style";
    Op[Op["left_align"] = 5] = "left_align";
    Op[Op["center_align"] = 6] = "center_align";
    Op[Op["right_align"] = 7] = "right_align";
    Op[Op["image"] = 8] = "image";
    Op[Op["model"] = 9] = "model";
    Op[Op["remove_model"] = 10] = "remove_model";
})(Op || (Op = {}));
...
class ScreenCredits extends Panel {
    ...
    constructor(root) {
        ...
        this.fastForwardListener = this.onFastForward.bind(this);
        ...
        this.UIElements = []; // Non-screen elements to "turn off" during display.
        this.pool = document.createElement('div'); // Element to pool non-active elements off of.
        this.state = State.LOADING; // State of this object.
        this.lines = []; // Actual credits
        this.commands = new Map(); // Formatting commands (line #, command)
        this.index = 0;
    ...
    onPause() {
        const caption = (this.state == State.PAUSE) ?
    ...
    onFastForward() {
        const caption = (this.speed != Speeds.NORMAL) ?
        ...
        switch (this.speed) {
            case Speeds.NORMAL:
                this.speed = Speeds.FAST;
                break;
            case Speeds.FAST:
                this.speed = Speeds.NORMAL;
                break;
            case Speeds.VERY_FAST:
                this.speed = Speeds.NORMAL;
                break; // TODO: Fix spacing, Speeds.VERY_FAST; break;
            case Speeds.PLAID:
                this.speed = Speeds.NORMAL;
                break; // TODO: Fix spacing, Speeds.PLAID; break;
        ...
        this.lines.forEach((line, index) => {
            ...
            matches.forEach(match => {
                ...
                values.forEach(value => {
                    ...
                    switch (value.trim()) {
                        case '0':
                            command = new Command(Op.no_style, '');
                            break;
                        case '1':
                            command = new Command(Op.title_style, '');
                            break;
                        case '2':
                            command = new Command(Op.subtitle_style, '');
                            break;
                        case '3':
                            command = new Command(Op.role_style, '');
                            break;
                        case '4':
                            command = new Command(Op.name_style, '');
                            break;
                        case 'l':
                            command = new Command(Op.left_align, '');
                            break;
                        case 'c':
                            command = new Command(Op.center_align, '');
                            break;
                        case 'r':
                            command = new Command(Op.right_align, '');
                            break;
                        case 'i':
                        case 'm':
                        case 'rm':
                        case 'x':
                            command = new Command(value.trim() === 'i' ? Op.image :
                                value.trim() === 'm' ? Op.model :
                                    Op.remove_model, processedLine.replace(/\[.*?\]/g, '').trim() // Get the value outside of brackets
                            );
...
//# sourceMappingURL=file:///core/ui/shell/credits/screen-credits.js.map
