...
type SwitchValueChangeEventDetail = { value: boolean, forced: boolean };
export type SwitchValueChangeEvent = ComponentValueChangeEvent<SwitchValueChangeEventDetail>;
export class FxsSwitch extends ChangeNotificationComponent<SwitchValueChangeEventDetail> {
	private isChecked = false;

	private navHelp?: HTMLElement;
	private readonly navContainer = document.createElement('div');
	private readonly onStateElements = document.createElement('div');
	private readonly offStateElements = document.createElement('div');
	private readonly ballElement = document.createElement('div');

	private leftValue = CSS.px(0);
	private resizeObserver = new ResizeObserver(this.updateBallPosition.bind(this));
	...
	toggle(force: boolean | undefined = undefined): void {
		...
		const changeEvent: SwitchValueChangeEvent = new ComponentValueChangeEvent({ value: this.isChecked, forced: force != undefined });
	...
	private addOrRemoveNavHelpElement(parent: HTMLElement, value: string | null) {
		if (value) {
