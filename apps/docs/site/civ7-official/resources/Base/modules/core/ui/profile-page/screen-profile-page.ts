...
import { TabItem, TabSelectedEvent } from '../components/fxs-tab-bar.js';
...
import { FxsSlot } from '/core/ui/components/fxs-slot.js';
...
import { ChallengeClass, ChallengeCategory, ChallengeCategorySortIndex as ChallengeCategorySortIndex } from '/core/ui/utilities/utilities-metaprogression.js';
...
import { OrnateDropdownItem } from '/core/ui/components/fxs-selector-ornate.js';
import { CivButton } from '/core/ui/shell/civ-select/civ-button/civ-button.js';
*/

// NOTE: You MUST include the panelOptions: when launching this screen or no contents will appear!
interface ProfilePageOptions {
	onlyChallenges: boolean;			// only show (and, importantly for in-game perf, create) the Challenges tab.  Also changes the screen's title.
	onlyLeaderboards: boolean;			// only show (and, importantly for in-game perf, create) the Leaderboard tab.  Also changes the screen's title.
	noCustomize?: boolean;				// if this exists and is TRUE, the Customize tab will not be present
	focusTab: ProfileTabType;			// which tab should be focused on display the page, only usable if all tabs should be displayed (no only show)
}
...
// TODO: Server needs to provide the leader ID for leader-type challenges.  That will enable filtering the challenge list by leader (and showing the leader icons correctly)
...
*/
let legendPathItems: LegendPathItem[] = [];
...
export class ScreenProfilePage extends Panel {
	private closeButtonListener: EventListener = () => {
		/** Only populate to cloud when the player closed the "Your Progress" panel*/
		this.updateProfile();
		this.close();
	}
	private userProfileReadyListener = this.refreshLeaderboard.bind(this); // Refresh leaderboard content when the user profiles are ready 
	private leaderboardFetchedListener = this.refreshLeaderboard.bind(this); // Refresh leaderboard content when the leaderboard is fetched
	private engineInputListener = this.onEngineInput.bind(this);
	private navigationInputListener = this.onNavigateInput.bind(this);
	private progressLeaderSelectedListener = this.onClickedProgressLeaderButton.bind(this);
	private challengeTabSelectedListener = this.onChallengeTabSelected.bind(this);
	private challengeLeaderSelectedListener = this.onChallengeCategorySelected.bind(this);
	private challengeSortLeftListener = this.challengeSortLeft.bind(this);
	private challengeSortRightListener = this.challengeSortRight.bind(this);
	private badgeSelectListener = this.onClickedBadge.bind(this);
	private bannerSelectListener = this.onClickedBanner.bind(this);
	private titleSelectListener = this.onClickedTitle.bind(this);
	private borderSelectListener = this.onClickedBorder.bind(this);
	private colorSelectListener = this.onClickedColor.bind(this);
	...
	*/
	private challengeTabItems: TabItem[] = [];
	private challengeGroups: ChallengeGroup[] = [];
	// list of the current selected challenges to be shown
	private currentChallengesToShow: ChallengeItem[] = [];
	private slotGroup?: HTMLElement;
	private challengesContainerList: ComponentRoot<FxsSlot>[] = [];
	private challengesSlotGroup?: ComponentRoot<FxsSlot>;
	private onlyChallenges: boolean = false;
	private onlyLeaderboards: boolean = false;
	private focusTab: ProfileTabType = ProfileTabType.NONE;
	private noCustomize: boolean = false;
	private currentSortType = CurrentSortType.Challenge;
	private currentChallengeGroup: string = "";
	private currentHOFGroup: string = "";
	private currentlySelectedMainTab: string = "";
	private currentlySelectedCustomizeTab: string = "";
	private sortTextLocale: string[] = ["LOC_METAPROGRESSION_SORT_BY_CHALLENGES", "LOC_METAPROGRESSION_SORT_BY_COMPLETED"];
	private cardInfo = getPlayerCardInfo();
	private challengeRightScrollable?: ComponentRoot<FxsScrollable>;
	private progressRightScrollable?: ComponentRoot<FxsScrollable>;
	private readonly challengeSortHslot = document.createElement("fxs-hslot");
	private cancelRewardsUpdate: boolean = false;
	private rewardsUpdateBusy: boolean = false;

	private readonly isOfflineMemento: boolean = !Network.supportsSSO() && Online.Metaprogression.supportsMemento();
...
*/
	 ...
	 */
	readonly currentProfile: DNAUserProfile = Online.UserProfile.getUserProfileData();
	...
	populateLeaderboardDropDownList(): void {
		this.eventItems = []

		const AllLeaderboardData: DisplayableLeaderboardData[] = Online.Leaderboard.getDisplayableLeaderboardInfo();

		AllLeaderboardData.forEach(LeaderboardData => {
			let item: EventDropdownItem = {
	...
	setPanelOptions(options: object) {
		const profileOptions: ProfilePageOptions = options as ProfilePageOptions;
		...
		const titleFrame = MustGetElement<HTMLElement>(".profile-main-frame", this.Root);
		...
		if (this.onlyChallenges) {
			...
			const mainTab: HTMLElement = MustGetElement<HTMLElement>(".profile-main-tab", this.Root);
	...
	private setupProgressUI(frag: DocumentFragment) {
		const isMobileViewExperience = UI.getViewExperience() == UIViewExperience.Mobile;
		...
		if (this.isOfflineMemento) {
			...
			const progressStatsText: string = Locale.compose("LOC_PROFILE_PROGRESS_HEADER", ourPathItem.currentLevel, ourPathItem.currentXP, ourPathItem.nextLevelXP);
	...
	private onClickedProgressLeaderButton(clickEvent: ActionActivateEvent) {
	...
	private refreshProgressRewards(selectedIndex: number) {
		...
		const ourPathItem: LegendPathItem = legendPathItems[selectedIndex];
	...
	private generateRewardsList(rewardsList: HTMLElement, ourPathItem: LegendPathItem) {
	...
	private waitRewardsReady = async (rewardsList: HTMLElement, ourPathItem: LegendPathItem) => {
	...
	private populateRewards = async (rewardsList: HTMLElement, ourPathItem: LegendPathItem) => {
		let rewardNum: number = 0;
		let startingReward: number = 0;
		const itemsPerUpdate: number = 20;
	...
	private setupCustomizeUI(frag: DocumentFragment) {
		const isMobileViewExperience = UI.getViewExperience() == UIViewExperience.Mobile
		...
		for (const item of customizeItems) {
			...
			switch (item.id) {
				case "badges":
					...
					for (let badge: number = 0; badge < UnlockableRewardItems.badgeRewardItems.length; badge++) {
						const thisBadge: BadgeItem = UnlockableRewardItems.badgeRewardItems[badge];
					...
					const badgeItem: BadgeItem = UnlockableRewardItems.badgeRewardItems[0];
					...
					const isDefaultBadgeEquipped: boolean = UnlockableRewardItems.badgeRewardItems[0].url == this.cardInfo.BadgeURL;
					scrollDiv.innerHTML = `
				...
				case "banners":
					...
					for (let banner: number = 0; banner < UnlockableRewardItems.bannerRewardItems.length; banner++) {
						const thisBanner: BannerItem = UnlockableRewardItems.bannerRewardItems[banner];
					...
					// TODO: only one description is offered from the server, so either this needs to be made a single wrapping field or a second line added to the live data.
					const bannerItem: BannerItem = UnlockableRewardItems.bannerRewardItems[0];

					const isDefaultBannerEquipped: boolean = UnlockableRewardItems.bannerRewardItems[0].gameItemId == this.cardInfo.BannerId;
				...
				case "titles":
					{
						...
						for (let title: number = 0; title < UnlockableRewardItems.titleRewardItems.length; title++) {
							const thisTitle: TitleItem = UnlockableRewardItems.titleRewardItems[title];
						...
						const titleItem: TitleItem = UnlockableRewardItems.titleRewardItems[0];

						// TODO: no description is offered from the server for titles, hook it up when it exists.
						const isDefaultTitleEquipped: boolean = UnlockableRewardItems.titleRewardItems[0].locKey == this.cardInfo.TitleLocKey;
						scrollDiv.innerHTML = `
								<div role="paragraph" class="flex flex-col pointer-events-auto">
				...
				case "borders":
					...
					for (let border: number = 0; border < UnlockableRewardItems.borderRewardItems.length; border++) {
						const thisBorder: BorderItem = UnlockableRewardItems.borderRewardItems[border];
					...
					const borderItem: BorderItem = UnlockableRewardItems.borderRewardItems[0];

					// TODO: there is no server data for borders right now
					const isDefaultBorderEquipped: boolean = UnlockableRewardItems.borderRewardItems[0].url == this.cardInfo.BorderURL;
					scrollDiv.innerHTML = `
								<div role="paragraph" class="flex flex-col pointer-events-auto">
				...
				case "colors":
					...
					for (let color: number = 0; color < colorRewardItems.length; color++) {
						const thisColor: ColorItem = colorRewardItems[color];
					...
					const colorItem: ColorItem = UnlockableRewardItems.colorRewardItems[0];

					// TODO: there is no server data for colors right now
					const isDefaultColorEquipped: boolean = colorRewardItems[0].color == this.cardInfo.BackgroundColor;
					scrollDiv.innerHTML = `
							<div role="paragraph" class="flex flex-col pointer-events-auto">
		...
		customizeTabControl.addEventListener("tab-selected", (e: TabSelectedEvent) => {
	...
	private onClickedBadge(clickEvent: Event) {
		if (clickEvent.target) {
			const badge = clickEvent.target as HTMLElement;
			const isAClick: boolean = !(clickEvent.type == "mouseover" || clickEvent.type == "focus");
	...
	private updateBadgeDisplay(selected: HTMLElement, updateServer: boolean) {
		const badgeInfoJSON = selected.getAttribute("data-badge-info");
		if (badgeInfoJSON) {
			const badge: BadgeItem = JSON.parse(badgeInfoJSON);
	...
	private onClickedBanner(clickEvent: Event) {
		if (clickEvent.target) {
			const banner = clickEvent.target as HTMLElement;
			const isAClick: boolean = !(clickEvent.type == "mouseover" || clickEvent.type == "focus");
	...
	private updateBannerDisplay(selected: HTMLElement, updateServer: boolean) {
		const bannerInfoJSON = selected.getAttribute("data-banner-info");
		if (bannerInfoJSON) {
			const banner: BannerItem = JSON.parse(bannerInfoJSON);
	...
	private onClickedTitle(clickEvent: Event) {
		if (clickEvent.target) {
			const title = clickEvent.target as HTMLElement;
			const isAClick: boolean = !(clickEvent.type == "mouseover" || clickEvent.type == "focus");
	...
	private updateTitleDisplay(selected: HTMLElement, updateServer: boolean) {
		const titleInfoJSON = selected.getAttribute("data-title-info");
		if (titleInfoJSON) {
			const title: TitleItem = JSON.parse(titleInfoJSON);
	...
	private onClickedBorder(clickEvent: Event) {
		if (clickEvent.target) {
			const border = clickEvent.target as HTMLElement;
			const isAClick: boolean = !(clickEvent.type == "mouseover" || clickEvent.type == "focus");
	...
	private updateBorderDisplay(selected: HTMLElement, updateServer: boolean) {
		const borderInfoJSON = selected.getAttribute("data-border-info");
		if (borderInfoJSON) {
			const border: BorderItem = JSON.parse(borderInfoJSON);
	...
	private onClickedColor(clickEvent: Event) {
		if (clickEvent.target) {
			const color = clickEvent.target as HTMLElement;
			const isAClick: boolean = !(clickEvent.type == "mouseover" || clickEvent.type == "focus");
	...
	private updateColorDisplay(selected: HTMLElement, updateServer: boolean) {
		const colorInfoJSON = selected.getAttribute("data-color-info");
		if (colorInfoJSON) {
			const color: ColorItem = JSON.parse(colorInfoJSON);
	...
	private changeSelectionHighlight(customizeType: string, selectedElement: HTMLElement) {
		const customizedClass: string = `profile-customize-${customizeType}-selected`;
	...
	private updateLockEquipStatus(customizeType: string, isLocked: boolean, isEquipped: boolean) {
...
*/
	private setupLeaderboardUI(frag: DocumentFragment) {
		...
		const componentRoot: ComponentRoot = (eventDropdown as ComponentRoot);
	...
	private setupChallengeUI(frag: DocumentFragment) {
	...
	private setChallengeSortType(newSortType: number) {
	...
	private onChallengeTabSelected(e: TabSelectedEvent) {
	...
	private onChallengeCategorySelected(clickEvent: Event) {
		if (clickEvent.target) {
			const title = clickEvent.target as HTMLElement;
	...
	private createContents() {
		...
		Online.Metaprogression.getLegendPathsData().forEach(item => {
			const progressItem: LegendPathItem = {
		...
		if (Network.supportsSSO()) {
			...
			for (const challengeClassValue in ChallengeClass) {
				if (isNaN(Number(challengeClassValue))) { // Check if enum value is not a number (to avoid iterating over reverse mappings)
					const challengeClass = ChallengeClass[challengeClassValue as keyof typeof ChallengeClass];
					const categories: ChallengeList[] = [];
		...
		const tabContainer: HTMLElement = MustGetElement<HTMLElement>('.profile-tab-container', this.Root);
		 ...
		 */
		const setupTabItem = (selector: string, setupFunc: (frag: DocumentFragment) => void) => {
		...
		tabControl.addEventListener("tab-selected", (e: TabSelectedEvent) => {
			...
			if (this.currentlySelectedMainTab == "customize") {
				...
				const jsonString: string = JSON.stringify(playerInfo);

				for (const headerElement of headerElements) {
					headerElement.setAttribute("data-player-info", jsonString);
				}

				const currentBorder: BorderItem = UnlockableRewardItems.getBorder(playerInfo.PortraitBorder);
		...
		setTimeout(() => {
			if (this.isOfflineMemento) {
				...
				if (this.focusTab != ProfileTabType.NONE) {
					...
					const focusTab: number = this.focusTab as number;
		...
		if (!this.isOfflineMemento) {
			const progressionCard: HTMLElement = document.createElement("progression-header");
	...
	private updateLeaderboardContent(leaderboardId: string) {
		const userArea: HTMLElement = MustGetElement<HTMLElement>(".profile-leaderboards-user", this.Root);
		const scrollable: HTMLElement = MustGetElement<HTMLElement>(".profile-leaderboards-scrollable-vslot", this.Root);
		...
		const entries: DNAPlayerLeaderboardEntry[] = Online.Leaderboard.getLeaderboardEntriesByID(leaderboardId);
		const leaderboardItems: LeaderboardItem[] = [];

		// The local user, who is always displayed on top
		const playerEntry: DNAPlayerLeaderboardEntry = Online.Leaderboard.getUserLeaderboardEntryByID(leaderboardId);

		// See if leaderboard is the same as currently playing the live event 
		const isCurrentlyPlayingLiveEvent: boolean = Online.Metaprogression.isPlayingActiveEvent() && leaderboardId == Online.Leaderboard.getActiveEventLeaderboardID();
		const userItem: LeaderboardItem = { place: playerEntry.playerRanking, name: playerEntry.playerName, title: "Example Data", score: parseInt(playerEntry.playerScore), date: playerEntry.playerAchievedTime, leader: playerEntry.playerLeaderName, civilization: playerEntry.playerCivilizationName };
		...
		for (const entry of entries) {
			...
			const currentPlayerName: string = entry.playerName;
			const currentPlayerScore: string = entry.playerScore;
			const tempItem: LeaderboardItem = { place: entry.playerRanking, name: currentPlayerName, title: "Haunted Hollow", score: parseInt(currentPlayerScore), date: entry.playerAchievedTime, leader: entry.playerLeaderName, civilization: entry.playerCivilizationName };
	...
	private addChallenge(item: ChallengeItem) {
	...
	protected onNavigateInput(navEvent: NavigateInputEvent) {
...
*/
	...
	private populateChallengesTab(tab: ChallengeGroup, container: HTMLElement, sortChallenges: boolean = false) {
		...
		let buildCategoriesLength: number = tab.categories.length;
		if (isLeader) {
	...
	private buildCurrentChallengeToShowList() {
		...
		for (let challenge = 0; challenge < this.currentChallengesToShow.length; challenge++) {
			...
			const isRepeatable: boolean = challengeItem.maxCompletions > 1;
	 ...
	 */
	private buildChallengesContainer(sortChallenges: boolean = false) {
	...
	private sortChallengeList(challenges: ChallengeItem[]) {
	...
	private sortChallengeCategories(challengeGroup: ChallengeGroup) {
	...
	private createLeaderboardHeader(): DocumentFragment {
		const fragment = document.createDocumentFragment();
	...
	private createLeaderboard(item: LeaderboardItem, isCurrentlyPlaying: boolean): DocumentFragment {
		const fragment = document.createDocumentFragment();
