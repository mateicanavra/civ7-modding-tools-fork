 ...
 */
function asyncLoad(url: string): Promise<string> {
 ...
 * 
 */
const MAX_WAIT_TIMEOUT_MS: DOMHighResTimeStamp = 5000;
 ...
 */
function waitUntilValue<T>(f: () => T | null, maxFrameCount?: number): Promise<NonNullable<T>> {
	return new Promise((resolve, reject) => {
		const start = performance.now();
		let frameCount = 0;
		const updateListener: FrameRequestCallback = (timeStamp) => {
 ...
 */
function delayByFrame<T extends unknown[]>(callback: (...params: T) => void, count: number = 1, ...callbackArguments: T) {
	if (count === 0) {
...
function waitForLayout(callback: () => void): void;
 ...
 */
function waitForLayout(callback?: () => void): Promise<void> | void {
	if (callback) {
...
function handlePromiseRejection(reason: any) {
...
interface ComponentDefinition {
	 ...
	 */
	//themes?: ComponentThemeDefinition
 ...
 */
abstract class ComponentRoot<C extends Component = Component> extends HTMLElement {

	private _isInitialized: boolean = false;
	private _isMutator: boolean = false;
	private _component?: C;
	private _decorators: ComponentDecorator[] = [];
	private _typeName: string;
	private whenCreatedListeners: ((c: C) => void)[] | undefined;
	private engineListenerHandles: EventHandle[] | null = null;
	private windowListeners: { name: string, callback: (args: unknown) => void, useCapture?: boolean }[] | null = null;
	...
	listenForEngineEvent(name: string, callback: (args: unknown) => void, context?: unknown) {
	...
	listenForWindowEvent(name: string, callback: (args: unknown) => void, useCapture?: boolean) {
	...
	private initialize(): void {
		...
		if (isInStructuralMutator) {
			...
			if (definition) {
				...
				const component = new definition.createInstance(this) as C;
	 ...
	 */
	whenComponentCreated(func: (c: C) => void) {
...
type SetActivatedComponentEventDetail = { component: Component | null };
const SetActivatedComponentEventName = 'set-activated-component' as const;
class SetActivatedComponentEvent extends CustomEvent<SetActivatedComponentEventDetail> {
	constructor(component: Component | null) {
		super(SetActivatedComponentEventName, { bubbles: false, cancelable: true, detail: { component } });
	}
}

type ActivatedComponentChangeEventDetail = { component: Component | null };
const ActivatedComponentChangeEventName = 'activated-component-changed' as const;
class ActivatedComponentChangeEvent extends CustomEvent<ActivatedComponentChangeEventDetail> {
	constructor(component: Component | null) {
...
class Component {
	...
	private set audioGroup(value: string) {
		this._audioGroup = value;
	}
	private get audioGroup(): string | undefined | null {
	...
	private receiveFocusEventListener = this.onReceiveFocus.bind(this);
	private loseFocusEventListener = this.onLoseFocus.bind(this);
	private detroyEventListener = this.Destroy.bind(this);

	constructor(root: ComponentRoot) {
		this.Root = root as ComponentRoot<this>;
	}
	...
	postOnAttach(): void {
		...
		const rootDefinition: ComponentDefinition | undefined = Controls.getDefinition(root.typeName);
		if (rootDefinition && rootDefinition.attributes) {
			for (const attr of rootDefinition.attributes) {
				...
				const preAttachValue: string | null = root.getAttribute(attributeName);
	...
	Destroy(): void {
		...
		var parentNode: Node | null = this.Root.parentNode;
	 ...
	 * @summary 
	 */
	private notifyOpenAvailability(): void {
		const mapping = this.refreshCallbacksObject()

		const availableChildren: string[] = []
	...
	*/
	private processOpenCallback(name: string) {
		const mapping = this.refreshCallbacksObject()
	 ...
	 */
	private createCallbacksObject(): Record<string, OptionalOpenCallback> {
		const opens = Controls.getDefinition(this.Root.typeName)?.opens ?? [];
		const callbacks: Record<string, OptionalOpenCallback> = {};
	 ...
	 */
	private refreshCallbacksObject(): Record<string, OptionalOpenCallback> {
...
interface ComponentDecoratorProvider {
	(base: Component): ComponentDecorator;
}

class ComponentData {
	private _definition?: ComponentDefinition;
	private _whenInitialized: Promise<ComponentDefinition>;
	private _initialized_resolve: any;
	private _initialized_reject: any;

	private _decorators: ComponentDecoratorProvider[] = [];
	private _decoratorsAdded: LiteEvent<ComponentDecoratorProvider>;

	constructor() {
		this._decoratorsAdded = new LiteEvent<ComponentDecoratorProvider>();
		this._whenInitialized = new Promise<ComponentDefinition>((resolve, reject) => {
...
const ComponentValueChangeEventName = 'component-value-changed' as const;
 ...
 * ```
 */
class ComponentValueChangeEvent<T> extends CustomEvent<T> {
 ...
 */
class ChangeNotificationComponent<T = unknown> extends Component {
	private valueChangeListener: HTMLElement | null = null;
...
class ImageCache {
	private _cachedImages = new Map<string, PreloadedImage>();
...
class ComponentManager {
	...
	private _sources: Map<string, boolean> = new Map<string, boolean>();
	private _componentData: Map<string, ComponentData> = new Map<string, ComponentData>();
	private _componentInitialized: LiteEvent<string> = new LiteEvent<string>();
	...
	initializeComponents() {
		...
		const allComponentInitializedPromises: Promise<void>[] = [];
		...
		for (const [name, data] of this._componentData) {
			...
			if (definition) {
				...
				if (definition.innerHTML?.length || definition.content?.length) {
					const contentTemplates: HTMLTemplateElement[] = [];
				...
				const componentInitialized = Promise.all(promises).then(() => {
					...
					if (shouldLog) { console.log(`Defining ${name}`); }
					customElements.define(name, c);
					data.resolveInitialization();

					(definition.createInstance as typeof Component).onDefined(name);
	 ...
	 */
	loadSource(url: string, module: boolean = true): Promise<void> | boolean {
		if (!document.body) {
		...
		if (!r) {
			...
			let upResolve: any, upReject: any;
	...
	loadStyle(url: string): Promise<void> | boolean {
		...
		const el: Element | null = document.querySelector(`link[href="${url}"]`);
		if (!el) {
			let upResolve: any, upReject: any;
			let p = new Promise<void>((resolve, reject) => {
				upResolve = resolve;
			...
			style.onload = () => {
				if (Configuration.getUser().debugUILowLevelLogging > 0) { console.log(`STYLE LOADED - ${url}`); }
				upResolve();
			};
...
class LocalizationDataBoundAttributeHandler implements AttributeHandler {
...
class ComponentDataBoundAttributeHandler implements AttributeHandler {
 ...
 */
const onFxsIncludeContentLoad_Success: CustomEvent = new CustomEvent("content-load-success");
const onFxsIncludeContentLoad_Fail: CustomEvent = new CustomEvent("content-load-fail");
...
if (!USE_OLD_FOCUS_LISTENERS) {
	document.addEventListener("DOMContentLoaded", function () {
		...
		body.addEventListener('event-mgr-receive-focus', (event: Event) => {
		...
		body.addEventListener('event-mgr-lose-focus', (event: Event) => {
			if (event.target instanceof HTMLElement) {
				let target: HTMLElement | null = event.target;
				while (target) {
					if (target instanceof ComponentRoot) {
		...
		body.addEventListener('event-mgr-pop', (event: Event) => {
			if (event.target instanceof HTMLElement) {
				let target: HTMLElement | null = event.target;
				while (target) {
					if (target instanceof ComponentRoot) {
...
class Loading {
	private static processInitialScriptsRAF = 0;

	public static isInitialized = false;

	// Replaces "export const whenInitialized..."
	public static whenInitialized = new Promise<void>((resolve, _reject) => {
		engine.whenReady.then(() => {
			...
			if (isInShell || gameCoreInitialized) {
				...
				let resolveGameCoreInitialized: () => void;

				promises.push(new Promise<void>((r) => { resolveGameCoreInitialized = r; }));
	...
	public static runWhenInitialized(f: () => void) {
	...
	public static whenLoaded = new Promise<void>((resolve, _reject) => {
		Loading.runWhenInitialized(() => {
			...
			if (isInShell || gameReady) {
				...
				const eventName = 'UIGameLoadingStateChanged';
				let resolveGameReady: () => void;
				promises.push(new Promise<void>((r) => { resolveGameReady = r; }));
				...
				const callback = () => {
					...
					if (gameReady) {
						...
						engine.on('InitialScriptAdded', Loading.onInitialScriptAdded);
						const tempPromises: Promise<void>[] = [];
	...
	public static runWhenLoaded(f: () => void) {
		if (Loading.isLoaded) {
			f();
		} else {
	...
	public static isFinished = false;

	// Replaces "export const whenFinished..."
	public static whenFinished = new Promise<void>((resolve) => {
	...
	public static runWhenFinished(f: () => void) {
...
function setComponentSupportSafeMargins() {
	...
	// Setup Safe Zones 
	const safeAreaMargins: SafeAreaMargins = UI.getSafeAreaMargins();
