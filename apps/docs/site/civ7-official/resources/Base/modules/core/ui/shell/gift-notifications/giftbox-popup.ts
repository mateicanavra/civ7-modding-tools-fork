...
class GiftboxPopup extends Panel {
	...
	private continueButtonListener: EventListener = () => { this.ClosePopup(); };
	private engineInputListener = this.onEngineInput.bind(this);
	private rewardsScrollable!: ComponentRoot<FxsScrollable>;
	...
	onAttach() {
		...
		const continueButton: HTMLElement | null = this.Root.querySelector<HTMLElement>('.continue');
		if (continueButton) {
	...
	onDetach() {
		const continueButtonListener: HTMLElement | null = this.Root.querySelector<HTMLElement>('.continue');
