...
export const ActionActivateEventName: string = 'action-activate' as const;
export class ActionActivateEvent extends CustomEvent<{ x: number, y: number }> {
...
export class FxsActivatable extends Component {
	...
	private static readonly FEEDBACK_LOW = 20;
	private static readonly FEEDBACK_HIGH = 20;
	private static readonly FEEDBACK_DURATION = 100;
	private actionKey: string | null = null;
	private isFeedbackEnabled = false;
	private isSoundEnabled = true;

	private onActivatableEngineInputEventListener = this.onActivatableEngineInput.bind(this);
	private onActivatableFocusEventListener = this.onActivatableFocus.bind(this);
	private onActivatableBlurEventListener = this.onActivatableBlur.bind(this);
	private onActivatableMouseLeaveEventListener = this.onActivatableMouseLeave.bind(this);

	protected shouldPlayErrorSound = false;


	public get disabledCursorAllowed() {
		const value = this.Root.getAttribute('disabled-cursor-allowed');
		return value == null || value === 'true';
	}

	public set disabledCursorAllowed(value: boolean) {
	...
	onAttach() {
		...
		this.Root.listenForWindowEvent('engine-input', (inputEvent: InputEngineEvent) => {
 ...
 */
export const UpdateFromOperationResult = (element: HTMLElement, result: OperationResult) => {
...
export type UpdateTooltipDisabledStateParams = {
	element: HTMLElement,
	disabled: boolean,
 ...
 * 
 */
export const UpdateActivatableDisabledState = ({
	element,
	disabled,
	disabledReasons = []
}: UpdateTooltipDisabledStateParams) => {
 ...
 */
const GetFailureReasonsTooltipHTML = (reasons: LocalizedString[]) => {
	if (reasons.length == 0) {
		return '';
	}
