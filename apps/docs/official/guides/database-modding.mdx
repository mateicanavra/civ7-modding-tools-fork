...
1. Make new mod with a `.modinfo` file if you don't have one already (see the [Getting Started](/civ7-official/modding/guides/getting-started) page if you're not sure how).
...
Unfortunately no. For that we'll need Modifiers, which is *technically* just making more Gameplay Database changes, but Modifiers are also a system unto itself. Check out the [Modifier System](/civ7-official/modding/guides/the-modifier-system) article for more information.
...
The simplest way is probably browsing the game files. If you installed the game via Steam, you can find where the game is installed via the game's properties (as detailed in the [Getting Started](/civ7-official/modding/guides/getting-started) document).
...
If you have `CopyDatabasesToDisk` set to 1 (as detailed in the [Getting Started](/civ7-official/modding/guides/getting-started) document), the game will output `.sqlite` files in the `debug` folder after you have started a game.
