...
- **Canonical surfaces only:** The refactor will **express all new functionality through official, canonical interfaces** (contracts, schemas, ops). Avoid ad-hoc or hidden logic. Any transitional support for old logic must be clearly marked as temporary (e.g. `DEPRECATED`) and eliminated by refactor completion.
- **Gameplay owns its outputs:** The new Gameplay domain will **own all gameplay-specific surfaces** in map generation (start positions, resource placements, gameplay overlays, etc.). We will not leave behind “shadow” implementations in other domains. If the refactor requires interim compatibility, those shims live outside Gameplay and are explicitly deprecated.
