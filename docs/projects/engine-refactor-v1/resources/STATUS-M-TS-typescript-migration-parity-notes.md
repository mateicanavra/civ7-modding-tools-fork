...
- **Map-size presets and classic/temperate presets**
  ...
  - **Update (2025-12-21, M4 planning):** Presets are removed; entry is explicit recipe + settings selection. Any named presets are treated as named recipes (if used). See `../milestones/M4-tests-validation-cleanup.md`.
...
- **Step 1: Explicit parity matrix (JS → TS)**
  ...
  - For example:
    ...
    - ~~`bootstrap/presets/*.js` → `—` — `pending` or `intentionally dropped`, depending on decision.~~  
      **Update (2025-12-21, M4 planning):** Presets are removed; treat any named variants as named recipes. See `../milestones/M4-tests-validation-cleanup.md`.
  ...
  - **Presets decision (CIV‑10/11/12/13)**
    - Either:
      ...
      - ~~Mark these presets in the parity matrix as “intentionally dropped” with a short rationale (e.g. “we only support the modern desert-mountains entry; older presets are archived for reference”).~~  
        **Update (2025-12-21, M4 planning):** Presets are removed; do not reintroduce preset resolution. Treat named variants as named recipes. See `../milestones/M4-tests-validation-cleanup.md`.
    - ~~Update `M-TS` milestone and CIV issues to reflect the decision so future work doesn’t assume these presets must come back.~~  
      **Update (2025-12-21, M4 planning):** Presets are removed; update docs to recipe+settings selection instead. See `../milestones/M4-tests-validation-cleanup.md`.
...
- **Step 4: Behavioral acceptance checklist (TS parity checkpoint)**
  ...
  - [ ] ~~At least one preset-based config is ported or explicitly dropped.~~  
    **Update (2025-12-21, M4 planning):** Presets are removed; ensure recipe+settings entries cover desired variants. See `../milestones/M4-tests-validation-cleanup.md`.
