...
- **WorldModel status:** Resolved in Decision Frame 2 (fully excised); remaining references in docs/deferrals still need cleanup. (`docs/projects/engine-refactor-v1/deferrals.md`, `docs/projects/engine-refactor-v1/resources/*`, `docs/system/libs/mapgen/_archive/*`)
...
- **Adapter boundary (engine globals):**
  ...
  - **Risks / Provisional Notes:** Current code still reads engine globals in multiple modules (e.g., `core/terrain-constants.ts`, `core/plot-tags.ts`, `foundation/plate-seed.ts`, `foundation/plates.ts`, `domain/narrative/utils/*`). M4 scope must include enforcing the adapter boundary and removing these direct global reads.
- **WorldModel status:**
  ...
  - **Risks / Provisional Notes:** WorldModel has been removed from runtime code on this checkpoint branch; remaining references persist in docs/deferrals/slides and need cleanup/alignment.
