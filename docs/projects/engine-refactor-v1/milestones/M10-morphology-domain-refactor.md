...
This doc is the milestone-shaped wrapper around the Phase 3 implementation plan (template sections `0)`–`9)`) for the Morphology vertical domain refactor.
...
- [ ] [`M10-U01`](#m10-u01) Slice 1 — Remove overlays from Morphology contracts/ops/steps
- [ ] [`M10-U02`](#m10-u02) Slice 2 — Delete overlay system (`narrative-pre`/`narrative-mid` removed)
- [ ] [`M10-U03`](#m10-u03) Slice 3 — Introduce `map-morphology` (Gameplay stamping) + re-home coasts/continents/effects
- [ ] [`M10-U04`](#m10-u04) Slice 4 — Move remaining Morphology stamping + downstream Hydrology/Ecology stamping behind Gameplay; delete `syncHeightfield`; re-home `buildElevation()`
- [ ] [`M10-U05`](#m10-u05) Slice 5 — Publish Phase 2 `artifact:map.*` landmass region projection surfaces
- [ ] [`M10-U06`](#m10-u06) Tracing pass — Observability hardening (cross-slice; required)

### Sequencing & Parallelization Plan

**Stacks Overview**
- Stack A (sequential): `M10-U01` → `M10-U02` → `M10-U03` → `M10-U04` → `M10-U05`
- Stack B (parallel): `M10-U06` (starts in Slice 3; must land before completion)

**Notes**
- Reference `docs/process/GRAPHITE.md#stacks--milestones` for how to translate this milestone into Graphite stacks.

### Acceptance Criteria / Verification

- See `## 6) Acceptance criteria` and `## 7) Verification commands` (including Phase 5 ruthlessness + traceability gate).

---

## 0) Header (required)

- Title: `M10: Morphology Domain Refactor — Phase 3 Implementation Plan`
...
- Morphology Physics (truth-only):
  ...
  - `morphology-mid/rugged-coasts`:
    ...
    - Emit `type: "morphology.coastlines.ascii.coastMask"`: ASCII visualization of coasts vs land/water (no adapter).
  - `morphology-post/islands`:
    - Emit `type: "morphology.islands.summary"`: edit count by kind + top-N indices (deterministic ordering).
    - Emit `type: "morphology.islands.ascii.edits"`: ASCII visualization for island edits (no adapter).
  - `morphology-post/mountains`:
    - Emit `type: "morphology.mountains.summary"`: mountain/hill mask counts + top-N indices.
    - Emit `type: "morphology.mountains.ascii.reliefMask"`: ASCII visualization from mountain/hill masks (no adapter).
  - `morphology-post/volcanoes`:
    - Emit `type: "morphology.volcanoes.summary"`: volcano count + top-N indices (deterministic ordering).
    - Emit `type: "morphology.volcanoes.ascii.indices"`: ASCII visualization of volcano indices (no adapter).
- Gameplay stamping / materialization (adapter-owned):
  - `map-morphology/*`:
    - Each step emits at least one engine-surface dump at step end:
      ...
      - ASCII: land/water and relief/feature visualizations where useful (adapter reads).
...
- Prework Prompt (Agent Brief): Island chains without overlays (Physics truth only)
  ...
  - **Expected Output:**
    ...
    - Propose the minimal replacement inputs derived from Foundation/Morphology truth (e.g., plate boundary context, shelf/roughness, sea-level/topography) plus a pure deterministic noise source.
      - Noise must be derived internally from `context.env.seed` + a stable label (e.g. `morphology/plan-island-chains`); it must not become a `requires` contract input mask and must not be exposed as an optional “overlay-like” toggle.
...
- Prework Prompt (Agent Brief): Foundation volcanism + tectonics drivers for Morphology (no overlays)
  ...
  - **Expected Output:**
    ...
    - Decide how tile-space Morphology consumes volcanism deterministically (avoid “two projections” drift):
      - Preferred/expected: extend the Foundation projection (`foundation/steps/projection` + `ops.computePlates`) so `artifact:foundation.plates` publishes the Phase 2-required tile-space fields Morphology needs.
      - Morphology must not introduce a second mesh→tile projection implementation. If additional projection logic is unavoidable later, it must reuse Foundation’s canonical projection helper and remain internal-only staging (not a new cross-domain truth surface).
    ...
    - Required adapter ordering (Phase 2 stamping spec):
      - `plot-coasts` (engine fixup; must not backfeed into Physics truth):
        ...
        - Immediately assert **no land/water drift** against Morphology truth landMask:
          ...
          - If this assertion fails, fail fast and open a Phase 2 canon correction follow-up (out of scope for M10). Do not implement alternate “fallback” implementations in M10 (no shims/dual paths).
        - After `adapter.expandCoasts(...)`, perform any required runtime-local sync for this recipe (e.g., Gameplay-owned `heightfield.terrain` / `heightfield.landMask` caches if they still exist) into Gameplay-owned runtime buffers only.
          - Do not mutate `artifact:morphology.topography` (or any arrays/payloads published as Morphology truth).
          - Any divergence between the engine surface (as read by adapter) and `artifact:morphology.topography.landMask` is an integration failure.
...
- Scope:
  ...
  - Add Gameplay-owned `build-elevation` step in `map-morphology`:
    ...
    - Note: legacy orchestration evidence differs on whether a post-`buildElevation()` `stampContinents()` is necessary. M10 must implement Phase 2 ordering as-written; this is only a parity check to decide whether a *future* Phase 2 canon correction is needed.
...
- Guardrails introduced in-slice:
  ...
  - Ban `writeHeightfield` usage:
    - Gameplay plot steps must call explicit adapter setters (`setTerrainType`, `setFeatureType`) and keep any runtime-local caches separate from Physics truth buffers.
