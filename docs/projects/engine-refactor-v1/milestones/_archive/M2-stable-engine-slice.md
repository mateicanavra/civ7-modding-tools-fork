...
- Later phases from these PRDs (e.g., config shape evolution, task-graph plumbing, downstream cluster migration into the pipeline) are intentionally out of scope here and are owned by later milestones (see `../../../_archive/projects/engine-refactor-v1/milestones/M3-core-engine-refactor-config-evolution.md` and `M4-target-architecture-cutover-legacy-cleanup.md`).
