...
| --- | --- | --- | --- |
| M4-PIPELINE-CUTOVER — [M4] Pipeline cutover: RunRequest + Recipe -> ExecutionPlan (remove stageManifest/STAGE_ORDER inputs) | `docs/projects/engine-refactor-v1/issues/M4-PIPELINE-CUTOVER.md` | Readiness checklist that points to child prework artifacts and flags gaps across PIPELINE-1..6. | Low complexity; coordination only. |
...
| Issue | File | Prework | Complexity / Parallelism |
| --- | --- | --- | --- |
| M4-TAG-REGISTRY-CUTOVER — [M4] Tag registry cutover: registry-instantiated catalog + validation (effect:* schedulable) | `docs/projects/engine-refactor-v1/issues/M4-TAG-REGISTRY-CUTOVER.md` | Inventory of tag validation/verification surfaces and a proposed registry-instantiated catalog (including demo payload validation). | High complexity; low parallelism (contract design). |
...
| Issue | File | Prework | Complexity / Parallelism |
| --- | --- | --- | --- |
| M4-ENGINE-BOUNDARY-CLEANUP — [M4] Engine boundary cleanup: remove engine-global dependency surfaces | `docs/projects/engine-refactor-v1/issues/M4-ENGINE-BOUNDARY-CLEANUP.md` | Inventory of engine-global reads (GameplayMap, GameInfo, PlotTags/LandmassRegion) with replacement mapping. | Medium complexity; high parallelism (mechanical inventory). |
