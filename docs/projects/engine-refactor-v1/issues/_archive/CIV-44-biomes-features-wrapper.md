...
- [x] Consume narrative signals via canonical overlays (`StoryOverlays`); if any legacy reads require `StoryTags`, keep them strictly derived from overlays (tracked in `docs/projects/engine-refactor-v1/deferrals.md` DEF-002).
...
- [x] Steps fail fast if required dependency tags are missing (e.g., `artifact:climateField`, `artifact:storyOverlays`)
...
- `packages/mapgen-core/src/layers/biomes.ts` (modify: read rainfall from `artifact:climateField` and river adjacency from `artifact:riverAdjacency`)
- `packages/mapgen-core/src/layers/features.ts` (modify: read rainfall from `artifact:climateField`)
- `packages/mapgen-core/src/story/overlays.ts` (modify: hydrate StoryTags from overlay snapshots for M3 compatibility)
- `packages/mapgen-core/src/pipeline/standard.ts` (modify: tighten `requires` for ecology stages)
