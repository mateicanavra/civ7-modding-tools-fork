...
1) **Effect verification via adapter call-evidence.**  
   **Context:** M4 needs verifiable `effect:*` tags without expanding engine read-back APIs.  
   **Options:** (a) call-evidence via adapter hooks, (b) sampled read-back queries, (c) defer until reification/placement artifacts.  
   **Choice:** Use adapter call-evidence hooks (`verifyEffect`) for biomes/features/placement; keep stronger verification deferred.  
