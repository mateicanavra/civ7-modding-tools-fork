...
<head>
  ...
  <meta name="generator" content="civ7-modding-tools cli" />
  ...
  (function() {
    ...
    onReady(function initPanZoom() {
      ...
      function setViewBox(x, y, w, h) {
        curVB.x = x; curVB.y = y; curVB.width = w; curVB.height = h;
      ...
      function flushZoomAnimation() {
        ...
        if (t >= 1) {
          setViewBox(zoomAnim.to.x, zoomAnim.to.y, zoomAnim.to.w, zoomAnim.to.h);
          zoomAnim = null;
          return;
        }
      ...
      function scheduleZoomRect(nx, ny, nw, nh, durMs) {
        flushZoomAnimation();
        zoomAnim = { from: { x: curVB.x, y: curVB.y, w: curVB.width, h: curVB.height }, to: { x: nx, y: ny, w: nw, h: nh }, start: performance.now(), dur: durMs };
      }
      function scheduleZoomToAtAnchor(anchorRx, anchorRy, factor, durMs) {
        flushZoomAnimation();
        var currentScale = baseVB.width / curVB.width;
        var newScale = Math.min(MAX_SCALE, Math.max(MIN_SCALE, currentScale * factor));
      ...
      container.addEventListener('mousedown', function(e) {
        ...
        startVBx = curVB.x; startVBy = curVB.y; startVBw = curVB.width; startVBh = curVB.height;
        e.preventDefault();
      });
      ...
      window.addEventListener('keydown', function(e) {
        ...
        else if (e.key === 'ArrowLeft' || e.key === 'a' || e.key === 'A') { pressed.left = true; e.preventDefault(); }
