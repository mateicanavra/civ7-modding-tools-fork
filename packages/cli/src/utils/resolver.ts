...
export async function resolveRootFromConfigOrFlag(opts: ResolvedRootOptions): Promise<string> {
    ...
    const cfg = (await loadConfig(projectRoot, flagsConfig)).raw;
    // We resolve the unzip directory path and treat that as the root for crawling
    return resolveUnzipDir({ projectRoot, profile }, cfg);
}
