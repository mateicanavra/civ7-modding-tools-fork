...
export function bootstrap(options: any = {}) {
    const presets =
        Array.isArray((options as any).presets) &&
        (options as any).presets.length > 0
            ? (options as any).presets.filter(
                  (n: unknown) => typeof n === "string",
              )
            : undefined;

    const overrides =
        options && typeof options === "object" && (options as any).overrides
            ? clone((options as any).overrides)
            : undefined;

    const cfg: any = {};
    if (presets) (cfg as any).presets = presets;
    if (overrides) {
