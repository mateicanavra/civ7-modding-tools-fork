...
import { assignCorridorMetadata } from "../../narrative/corridors/style-cache.js";
...
export interface MountainPassConfig {
  maxPasses: number;
  minRangeSize: number;
  minRangeSpan: number;
  passSpacing: number;
...
function getDims(ctx: ExtendedMapContext): { width: number; height: number } {
  return {
  ...
  for (const perim of range.perimeter) {
    for (let dy = -2; dy <= 2; dy++) {
      for (let dx = -2; dx <= 2; dx++) {
        ...
        const hasNorth = inBounds(nx, ny - 1, width, height) && rangeSet.has(storyKey(nx, ny - 1));
        const hasSouth = inBounds(nx, ny + 1, width, height) && rangeSet.has(storyKey(nx, ny + 1));
        const hasEast = inBounds(nx + 1, ny, width, height) && rangeSet.has(storyKey(nx + 1, ny));
        const hasWest = inBounds(nx - 1, ny, width, height) && rangeSet.has(storyKey(nx - 1, ny));
...
function findNotchPasses(
  ...
  for (const tile of range.tiles) {
    ...
    const terrain = [
      { dx: 0, dy: -1, opposite: { dx: 0, dy: 1 } },
      { dx: -1, dy: 0, opposite: { dx: 1, dy: 0 } },
      { dx: -1, dy: -1, opposite: { dx: 1, dy: 1 } },
      { dx: -1, dy: 1, opposite: { dx: 1, dy: -1 } },
    ];
    ...
    for (const axis of terrain) {
      ...
      if (n1IsMtn && n2IsMtn) {
        ...
        if (p1Clear || p2Clear) {
          ...
          const score =
            30 +
            (p1Clear && p2Clear ? 40 : 0) +
            (1000 - Math.min(1000, elevation)) * 0.02;
  ...
  for (const perim of range.perimeter) {
    for (let dy = -2; dy <= 2; dy++) {
      for (let dx = -2; dx <= 2; dx++) {
        ...
        try {
          // isRiverAdjacent may not exist on all adapters
          const adapterAny = ctx.adapter as unknown as Record<string, unknown>;
          const isRiverAdj =
            typeof adapterAny.isRiverAdjacent === "function"
              ? (adapterAny.isRiverAdjacent as (x: number, y: number) => boolean)(nx, ny)
              : false;
...
function applyPass(
  ...
  // Store in mountain cache
  const cache = getMountainCache(ctx);
  cache.passes.add(key);
...
export function detectMountainPasses(
  ...
  // Clear existing passes in cache
  const cache = getMountainCache(ctx);
  cache.passes.clear();
  ...
  const selectedPasses = selectBestPasses(allCandidates, config);
...
export const tagMountainPasses = detectMountainPasses;

export default detectMountainPasses;
